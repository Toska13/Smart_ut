...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	ВыполнитьКонтрольКорректностиЗаполненияПрослеживаемыхТоваров(Отказ);
	
КонецПроцедуры

Процедура ВыполнитьКонтрольКорректностиЗаполненияПрослеживаемыхТоваров(Отказ)
	...
	Если Не ВыбраныТоварыВПути Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ВЫРАЗИТЬ(Таблица.АналитикаУчетаНоменклатуры КАК Справочник.КлючиАналитикиУчетаНоменклатуры) КАК АналитикаУчетаНоменклатуры,
	|	Таблица.КоличествоПоРНПТ КАК КоличествоПоРНПТ,
	|	&ДатаДокумента КАК ДатаДокумента
	|ПОМЕСТИТЬ ВТВидыЗапасов
	|ИЗ
	|	&ВидыЗапасов КАК Таблица
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|ГДЕ
	|	ЕСТЬNULL(Таблица.АналитикаУчетаНоменклатуры.Номенклатура.КодТНВЭД.ПрослеживаемыйТовар, ЛОЖЬ)
	|	И Таблица.ДатаДокумента >= &ДатаНачалаУчетаПрослеживаемыхИмпортныхТоваров
	|	И Таблица.КоличествоПоРНПТ = 0";
	
	Запрос.УстановитьПараметр("ДатаДокумента", Дата);
	Запрос.УстановитьПараметр("ДатаНачалаУчетаПрослеживаемыхИмпортныхТоваров", Константы.ДатаНачалаУчетаПрослеживаемыхИмпортныхТоваров.Получить());
	Запрос.УстановитьПараметр("ВидыЗапасов", ВидыЗапасов.Выгрузить(,"АналитикаУчетаНоменклатуры, КоличествоПоРНПТ"));
	
	ШаблонТекстаОшибки =
		НСтр("ru = 'По регистру Товары организаций не обнаружен остаток по количеству РНПТ по товару %1. 
		|Требуется ввести документ Уведомление о ввозе прослеживаемых товаров.'");
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Отказ = Истина;
		ОбщегоНазначения.СообщитьПользователю(
			СтрШаблон(ШаблонТекстаОшибки, Выборка.Номенклатура),
			,
			,
			,
			Отказ);

...
// Заполняет табличную часть УпаковкиВЕТИС документа базовыми упаковками продукции. Если их нет и заполнена
//   запись складского журнала - упаковками из записи журнала (пропорционально остатку).
//
// Параметры:
//   Объект - ДокументОбъект.ИсходящаяТранспортнаяОперацияВЕТИС - заполняемый документ
//   СписокСтрок - Массив - строки табличной части "Товары", для которых требуется заполнение
...
Процедура ЗаполнитьПотребительскиеУпаковки(Объект, СписокСтрок = Неопределено) Экспорт
	...
	|	Товары.ИдентификаторСтроки КАК ИдентификаторСтрокиТовары,
	|	Товары.КоличествоВЕТИС КАК КоличествоСтрокиТовары,
	|	УпаковкиЗаписиЖурнала.УпаковкаВЕТИС КАК УпаковкаВЕТИС,
	|	УпаковкиЗаписиЖурнала.УровеньУпаковки КАК УровеньУпаковки,
	|	УпаковкиЗаписиЖурнала.Ссылка.ЕдиницаИзмеренияВЕТИС.Коэффициент КАК КоэффициентЕдиницыУпаковки,
	|	ВЫБОР 

...
Функция ПолучитьВыданныеАвансы(Организация, НачалоПериода, КонецПериода, РасчетныйДокумент = Неопределено, ВРазрезеСтавокНДС = Ложь) Экспорт
	...
	Пока ВыборкаАванс.Следующий() Цикл
		...
		Для каждого Ставка Из МассивСтавок Цикл
			...
			Если ОблагаетсяНДСУПокупателя Тогда
				СтавкаЧислом = УчетНДСУПКлиентСервер.ПолучитьСтавкуНДС(Ставка);
				ВалютнаяСумма = ВалютнаяСумма + УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(ВалютнаяСумма, СтавкаЧислом, Ложь);
				Сумма = Сумма + УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(Сумма, СтавкаЧислом, Ложь);
			КонецЕсли;

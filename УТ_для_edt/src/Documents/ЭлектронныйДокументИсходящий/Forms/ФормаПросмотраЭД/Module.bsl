...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Объект.ДатаДокументаОтправителя = Объект.ДатаИзмененияСостоянияЭДО;
	...
	ИначеЕсли ИмяСобытия = "ПроведенаПроверкаЭП" Тогда
		Для Каждого ЭД Из Параметр.МассивЭД Цикл
			Если ЭД = ПрисоединенныйФайлСсылка Тогда
				...
				ЗаполнитьТаблицуЭП(Параметр.Отпечатки);
		...
		Если Параметр.КонтекстОперации <> Неопределено
			И Параметр.КонтекстОперации.Диагностика.Ошибки.Количество() > 0 Тогда
			ЕстьОшибкиПоДоступнымОтпечаткам = Ложь;
			ЕстьОшибкиПоНеДоступнымОтпечаткам = Ложь;
			Для каждого Ошибка Из Параметр.КонтекстОперации.Диагностика.Ошибки Цикл
				Если ДоступныеОтпечатки.НайтиПоЗначению(Ошибка.Сертификат) <> Неопределено Тогда
					ЕстьОшибкиПоДоступнымОтпечаткам = Истина;
				Иначе 
					ЕстьОшибкиПоНеДоступнымОтпечаткам = Истина;
				КонецЕсли;
			КонецЦикла;
			Если ЕстьОшибкиПоДоступнымОтпечаткам Тогда
			...
			КонецЕсли;
			Если ЕстьОшибкиПоНеДоступнымОтпечаткам Тогда
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ЗаполнитьДополнительныеПараметрыВнутреннегоДокумента(ДополнительныеПараметры, МассивДокументовУчета);
...
Процедура Переформировать(Команда)
	Оповещение = Новый ОписаниеОповещения("НачатьПереформированиеПослеВопроса", ЭтотОбъект);
	ТекстВопроса = НСтр("ru = 'Будет сформирована новая версия электронного документа. Продолжить?'");
...
&НаСервере
Процедура ОтправитьВАрхивНаСервере() 
	
	ОбменСКонтрагентамиСлужебный.УстановитьНеобходимостьОзнакомленияСЭД(ОБъект.Ссылка, Ложь);
...
Процедура ИзменитьВидимостьДоступностьНаСервере()
	...
	Элементы.Переформировать.Видимость = Объект.ВидЭД <> Перечисления.ВидыЭД.ПроизвольныйЭД
		И (Объект.СостояниеЭДО = Перечисления.СостоянияВерсийЭД.НаПодписи
	...
	Элементы.КомандаОтправитьВАрхив.Доступность = ОбменСКонтрагентамиСлужебный.ИспользоватьОзнакомлениеСЭД() И Объект.НаОзнакомлении;	
	Элементы.КомандаОтправитьВАрхив.Видимость = Элементы.КомандаОтправитьВАрхив.Доступность;
	...
	Иначе
		...
		Элементы.КомандаОтправитьПовторно.Видимость         = Ложь;
		Элементы.КомандаОтправитьВАрхив.Видимость           = Ложь;  
...
&НаСервере
Процедура ЗаполнитьТаблицуЭП(Отпечатки = Неопределено)
	
	Если Отпечатки <> Неопределено Тогда
		ДоступныеСертификаты = ОбменСКонтрагентамиСлужебный.ТаблицаДоступныхДляПодписиСертификатов(Отпечатки);
		Для каждого Сертификат Из ДоступныеСертификаты Цикл
			Если ДоступныеОтпечатки.НайтиПоЗначению(Сертификат.Отпечаток) = Неопределено Тогда
				ДоступныеОтпечатки.Добавить(Сертификат.Отпечаток);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ЗаполнитьДокументыОснования(МассивОснований)
	...
	Для Каждого Основание Из МассивОснований Цикл
		Если СтрНайти(Основание.Метаданные().ПолноеИмя(), "ПрисоединенныеФайлы") > 0 Тогда
			ТекущееОснование = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Основание, "ВладелецФайла");
		Иначе
			ТекущееОснование = Основание;
		КонецЕсли;
		
		Если Не Метаданные.ОпределяемыеТипы.ОснованияЭлектронныхДокументов.Тип.СодержитТип(ТипЗнч(ТекущееОснование)) Тогда
			Продолжить;
		КонецЕсли;
		
		НоваяСтрока = Объект.ДокументыОснования.Добавить();
		НоваяСтрока.ДокументОснование = ТекущееОснование;
...
Процедура ДобавитьВложениеИзДвоичныхДанных(ДвоичныеДанные, НаименованиеФайла, Расширение)
	...
	СтруктураФайла = Новый Структура;
	СтруктураФайла.Вставить("ИмяФайла",                           НаименованиеФайла + "." + Расширение);
	СтруктураФайла.Вставить("Наименование",                       НаименованиеФайла);
	СтруктураФайла.Вставить("Расширение",                         Расширение);
	СтруктураФайла.Вставить("Редактирует",                        Неопределено);
	СтруктураФайла.Вставить("ПодписанЭП",                         Ложь);
	СтруктураФайла.Вставить("Зашифрован",                         Ложь);
...
Процедура ПослеНастройкиОтправкиДокумента(Результат, ДополнительныеПараметры) Экспорт
	...
	НачатьПереформирование(Результат.ДополнительныеДанные);
...
&НаСервере
Процедура ЗаполнитьДополнительныеПараметрыВнутреннегоДокумента(ДополнительныеПараметры, МассивДокументовУчета)
	
	Если Не ЗначениеЗаполнено(Объект.ВидВнутреннегоДокумента) Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура НачатьПереформированиеПослеВопроса(Ответ, ДополнительныеПараметры) Экспорт
	Если Ответ = КодВозвратаДиалога.Да Тогда
		НачатьПереформирование(ДополнительныеПараметры);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура НачатьПереформирование(ДополнительныеПараметры = Неопределено)
	...
	ОбменСКонтрагентамиСлужебныйВызовСервера.ЗакрытьДокументыПринудительно(ДокументыУчета,
		...
		Объект.ВидВнутреннегоДокумента);
	...
	ЗаполнитьДополнительныеПараметрыВнутреннегоДокумента(ДополнительныеПараметры, ДокументыУчета);
	
	Оповещение = Новый ОписаниеОповещения("ЗавершитьПереформирование", ЭтотОбъект);
	ОбменСКонтрагентамиСлужебныйКлиент.ОбработатьЭД(ДокументыУчета, "Сформировать",
		ДополнительныеПараметры, Неопределено, Оповещение);
	
КонецПроцедуры

&НаКлиенте
Процедура ЗавершитьПереформирование(Результат, Контекст) Экспорт

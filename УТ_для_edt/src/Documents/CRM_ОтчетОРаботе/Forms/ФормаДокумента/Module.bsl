...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	CRM_ЛицензированиеЭкспортныеМетоды.ПолучитьЗащищеннуюОбработку().Документ_CRM_ОтчетОРаботе_ФормаДокумента_ПриСозданииНаСервере(ЭтотОбъект, Отказ, СтандартнаяОбработка);
...
Процедура КомандаЗаполнитьПоСуществующемуОтчету(Команда)
	Если Объект.СтрокиОтчета.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьПоСуществующемуОтчетуЗавершение", ЭтотОбъект), НСтр("ru='Заполнить с очисткой табличной части?';en='Fill with cleaning of tabular section?'"), РежимДиалогаВопрос.ДаНетОтмена);
        Возврат;
	КонецЕсли;
...
Процедура КомандаЗаполнитьПоРабочемуВремени(Команда)
	Если Объект.СтрокиОтчета.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьПоРабочемуВремениЗавершение", ЭтотОбъект), НСтр("ru='Заполнить с очисткой табличной части?';en='Fill with cleaning of tabular section?'"), РежимДиалогаВопрос.ДаНетОтмена);
        Возврат;
	КонецЕсли;
...
Функция ПолучитьДеньНедели(ДатаРабот)
	Возврат Сред(НСтр("ru=' ПнВтСрЧтПтСбВс';en='MonTuesWedThurFriSatSun'"), ДеньНедели(ДатаРабот) * 2, 2);
КонецФункции
...
Функция ПроверкаКорректностиЗаполненияНаКлиенте()
	...
	Для Каждого СтрокаТаблицы Из Объект.СтрокиОтчета Цикл
		Если СтрокаТаблицы.ДатаР < Объект.НачалоПериода Или СтрокаТаблицы.ДатаР > Объект.КонецПериода Тогда
			Ошибки = Ошибки
				+ ?(Ошибки = "", "", Символы.ПС)
				+ НСтр("ru='Строка';en='String'") + " " + СтрокаТаблицы.НомерСтроки + ": "

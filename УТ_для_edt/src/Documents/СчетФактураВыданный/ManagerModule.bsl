...
Функция ПолучитьПредставленияСтроки5а(Знач Таблица) Экспорт
	...
	Для Каждого СФ Из СчетаФактуры Цикл
		Если ТипЗнч(СФ) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
			РеквизитыСФ = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СФ, "Исправление, Корректировочный, РучнаяКорректировкаСуммДокумента");
			Если РеквизитыСФ.Исправление И Не РеквизитыСФ.Корректировочный Или РеквизитыСФ.РучнаяКорректировкаСуммДокумента Тогда
...
Функция ПолучитьТаблицуТоваровИсправление(МенеджерВременныхТаблиц, ДополнительныеПараметры)
	...
	|	ИЛИ НЕ ТаблицаТоваровСФ.КоличествоПоРНПТ + ТаблицаТоваровСФ.КоличествоПоРНПТУменьшение + ЕСТЬNULL(ТаблицаТоваровДО.КоличествоПоРНПТ, 0) = 0
...
Функция ПолучитьТаблицуТоваровКорректировка(МенеджерВременныхТаблиц)
	...
	|
	|	ТаблицаТоваровДО.ИсходныйСчетФактура КАК ИсходныйСчетФактура,
	|	ЕСТЬNULL(ТаблицаТоваровСФ.НомерСтроки, 0) КАК НомерСтрокиИсходногоСФ,
	|	ТаблицаТоваровДО.Номенклатура КАК Номенклатура,
	|	ТаблицаТоваровДО.Характеристика КАК Характеристика,
	|	ТаблицаТоваровДО.НоменклатураНабора КАК НоменклатураНабора,
	|	ТаблицаТоваровДО.ХарактеристикаНабора КАК ХарактеристикаНабора,

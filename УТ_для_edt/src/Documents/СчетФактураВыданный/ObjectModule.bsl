...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Товары.Количество() Тогда
		ЗаполнитьНомераСтрокПриПечати(РезультатЗаполнения);
	КонецЕсли;
...
Процедура ЗаполнитьНомераСтрокПриПечати(РезультатЗаполнения)
	...
	// Для перевыставленных СФ может быть только один документ-основание, поэтому все строки относятся к нему.
	// Также мы не пытаемся угадать соответствие строк документам отгрузки после ручной корректировки.
	...
	ИначеЕсли Исправление И Не Корректировочный И Не РучнаяКорректировкаСуммДокумента 
		И РезультатЗаполнения <> Неопределено И РезультатЗаполнения.Свойство("ТаблицаТоваровДокументовОснований") Тогда
		
		ГруппировкиОснований = СтрЗаменить(ГруппировкиПечати,"НомерСтрокиИсходногоСФ","ТипЗапасов");
		ГруппировкиОснований = СтрЗаменить(ГруппировкиОснований,",Цена","");

		ТаблицаТоваровДокументовОснований = РезультатЗаполнения.ТаблицаТоваровДокументовОснований; // - ТаблицаЗначений - 
		ТаблицаТоваровДокументовОснований.Индексы.Добавить(ГруппировкиОснований);

		СтруктураОтбора.Удалить("НомерСтрокиИсходногоСФ");
		СтруктураОтбора.Удалить("Цена");
		СтруктураОтбора.Вставить("ТипЗапасов");
		...
		Для Каждого СтрокаТоваров Из Товары Цикл
			...
			СтрокиОснований = ТаблицаТоваровДокументовОснований.НайтиСтроки(СтруктураОтбора);
			Для Каждого СтрокаОснований Из СтрокиОснований Цикл
				...
				СтрокаНомера.ДокументОснование = СтрокаОснований.ДокументРеализации;
		...
		Для Каждого СтрокаИнтервала Из ТаблицаИнтервалов Цикл
			Для Каждого СтрокаТЧ Из ДокументыОснования Цикл
				Если ТипЗнч(СтрокаТЧ.ДокументОснование) = Тип("ДокументСсылка.КорректировкаРеализации") И
					СтрокаТЧ.ДокументОснование.ДокументОснование = СтрокаИнтервала.ДокументОснование Тогда
						СтрокаТЧ.ПорядковыеНомераСтрок = СтрокаИнтервала.НомерСтрокиСФ;
				ИначеЕсли СтрокаТЧ.ДокументОснование = СтрокаИнтервала.ДокументОснование Тогда
					СтрокаТЧ.ПорядковыеНомераСтрок = СтрокаИнтервала.НомерСтрокиСФ;
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;

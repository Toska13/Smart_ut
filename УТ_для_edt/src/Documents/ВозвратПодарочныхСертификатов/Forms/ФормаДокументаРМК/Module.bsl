...
Функция ПараметрыОперацииФискализацииЧека()
	...
	Для Каждого СтрокаТЧ Из Объект.ПодарочныеСертификаты Цикл
		...
		Если ВерсияФФД = "1.2" Тогда
			СтрокаПозицииЧека.КодЕдиницыИзмерения = КодЕдиницыИзмеренияПараметраЧека("шт.");
		Иначе
			СтрокаПозицииЧека.ЕдиницаИзмерения = "шт.";
		КонецЕсли;
	...
	ПараметрыОперацииФискализацииЧека.ТорговыйОбъект = ТорговыйОбъектНаименование;
...
&НаСервере
Функция КодЕдиницыИзмеренияПараметраЧека(ЕдиницаИзмерения)
	
	Возврат РозничныеПродажи.КодЕдиницыИзмеренияПараметраЧека(ЕдиницаИзмерения);

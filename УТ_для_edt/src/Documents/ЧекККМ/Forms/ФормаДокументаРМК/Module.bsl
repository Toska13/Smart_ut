	...
	Для каждого СтрокаТЧ из ПредметыРасчетовПоДокументу.Товары Цикл
		...
		Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаТЧ, "Штрихкод")
			...
			Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаТЧ, "РезультатРаспределения")
				...
				Если МенеджерОборудованияВызовСервера.ФискальноеУстройствоПоддерживаетПроверкуКодовМаркировки(ПараметрыКассыККМ.ИдентификаторУстройства) Тогда
...
Функция ПараметрыОперацииФискализацииЧека()
	...
	ТорговыйОбъектНаименование = ТорговыйОбъектККТ();
	ПараметрыОперацииФискализацииЧека.МестоРасчетов = ТорговыйОбъектНаименование;
	
	ТорговыйОбъектМестоРасчетов = МестоРасчетовККТ();
	...
	ПараметрыОперацииФискализацииЧека.АдресРасчетов = АдресРасчетовККТ(ПараметрыОперацииФискализацииЧека);
...
Функция ПараметрыЗавершенияВводаШтрихкода(ИсходныеДанные, РезультатОбработки, ПараметрыСканирования)
	...
	ПараметрыЗавершенияВводаШтрихкода = ШтрихкодированиеИСКлиент.ПараметрыЗавершенияОбработкиШтрихкода("ОбработкаКодаМаркировкиВыполнитьДействие","ПоискПоШтрихкодуЗавершение","");
...
Процедура ПриСозданииЧека()
	...
	ШтрихкодированиеИС.ОчиститьКэшМаркируемойПродукции(ЭтотОбъект);
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтотОбъект, "ДанныеШтрихкодовУпаковокГосИС") Тогда
		ЭтотОбъект["ДанныеШтрихкодовУпаковокГосИС"].Очистить();
	КонецЕсли;
...
&НаСервере
Функция АдресРасчетовККТ(Знач ПараметрыОперацииФискализацииЧека)
	
	Возврат РозничныеПродажи.АдресРасчетовККТ(ПараметрыКассыККМ.ИдентификаторУстройства, ПараметрыОперацииФискализацииЧека.Организация);

КонецФункции

&НаСервере
Функция МестоРасчетовККТ()
	
	Возврат РозничныеПродажи.МестоРасчетовККТ(ПараметрыКассыККМ.ИдентификаторУстройства);

КонецФункции

&НаСервере
Функция ТорговыйОбъектККТ()
	
	Возврат РозничныеПродажи.ТорговыйОбъектККТ(СтруктураСостояниеКассовойСмены.Организация, СтруктураСостояниеКассовойСмены.КассаККМ);

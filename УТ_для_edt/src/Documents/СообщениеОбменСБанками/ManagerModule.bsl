...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Документ.СообщениеОбменСБанками КАК СообщениеОбменСБанками
	|ГДЕ
	|	СообщениеОбменСБанками.ПрограммаБанка = ЗНАЧЕНИЕ(Перечисление.ПрограммыБанка.ПустаяСсылка)
	|УПОРЯДОЧИТЬ ПО
	|	Дата УБЫВ";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ЗависимыеСправочники = Новый Массив;
	ЗависимыеСправочники.Добавить("Справочник.НастройкиОбменСБанками");
	ЗависимыеСправочники.Добавить("Справочник.СообщениеОбменСБанкамиПрисоединенныеФайлы");
	ЗависимыеСправочники.Добавить("Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования");
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСКонтрагентами") Тогда
		ЗависимыеСправочники.Добавить("Справочник.ЭДПрисоединенныеФайлы");
	КонецЕсли;
	...
	Если ОбновлениеИнформационнойБазы.ЕстьЗаблокированныеПредыдущимиОчередямиДанные(Параметры.Очередь, "РегистрСведений.СостояниеПодписанияЭД")
		...
		Или ОбновлениеИнформационнойБазы.ЕстьЗаблокированныеПредыдущимиОчередямиДанные(Параметры.Очередь, ЗависимыеСправочники) Тогда
...
Функция СформироватьПечатнуюФормуЭД(СообщениеОбмена, СсылкаНаДвоичныеДанныеФайла, ИдентификаторПечати, ДляПечати)
	...
	Если ОбменСБанкамиСлужебный.ЭтоЗарплатныйПроект(РеквизитыСообщенияОбмена.ВидЭД) Тогда
		Возврат ТабличныйДокументЗапрлатногоПроекта(СсылкаНаДвоичныеДанныеФайла);
	КонецЕсли;
...
Функция ТабличныйДокументЗапрлатногоПроекта(СсылкаНаДвоичныеДанныеФайла)
...
Функция ДанныеДляПечатиПлатежногоПорученияИзJSON(ДанныеДокумента)
	...
	ДанныеСтрокой = ЭлектронноеВзаимодействиеСлужебныйВызовСервера.СтрокаИзДвоичныхДанных(ДанныеДокумента);
...
Функция РеквизитыБанка(БИК)
	...
	СтруктураВозврата = Новый Структура("Наименование, Город");
	...
	Оператор1.КоличествоПолучаемыхЗаписей = 1;
	// добавим источник в запрос
	Оператор1.Источники.Добавить("Справочник." + СправочникБанки.Имя, "Банки");
	// добавим поля в запрос
	Оператор1.ВыбираемыеПоля.Добавить(РеквизитНаименование, 0);
	Оператор1.ВыбираемыеПоля.Добавить(РеквизитГород, 1);
	ПолеБИК = Оператор1.ВыбираемыеПоля.Добавить(РеквизитБИК, 2);
	// добавим условие
	Оператор1.Отбор.Добавить(Строка(ПолеБИК) + " = &БИК");

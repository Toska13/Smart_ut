...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	//Иначе
		...
		Если ТаблицаРабочихДней.Количество() = 0 Тогда
			Отказ = Истина;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнен производственный календарь для подразделения ""';en='Production schedule not fill for department ""'")
				+ Строка(Исполнитель.Подразделение) + НСтр("ru='"" на дату: ';en='"" on date: '") + Формат(НачалоПериодаПланирования, "ДФ=dd.MM.yyyy"));		
			Возврат;
		КонецЕсли;	
	//КонецЕсли;	

...
Функция ЕстьПодходящиеКонтакты(ТекущийДокумент)
	...
	ИначеЕсли ТипЗнч(ПараметрыДиалога.ПараметрОткрытия) = Тип("СправочникСсылка.КонтактныеЛицаПартнеров") Тогда
		...
		СписокКонтактов.Добавить(ПараметрыДиалога.ПараметрОткрытия);
	ИначеЕсли ТипЗнч(ПараметрыДиалога.ПараметрОткрытия) = Тип("СправочникСсылка.CRM_ПотенциальныеКлиенты") Тогда
		СписокКонтактов.Добавить(ПараметрыДиалога.ПараметрОткрытия);	
	КонецЕсли;
...
Функция ПолучитьПараметрыОткрытияДляМессенджера(ОбъектСсылка)
	...
	Если ТипЗНЧ(ОбъектСсылка) = Тип("ДокументСсылка.CRM_Интерес") Тогда
		...
		ИначеЕсли НЕ ОбъектСсылка.ПотенциальныйКлиент.Пустая() Тогда	
			ПараметрыДиалога.Вставить("ПараметрОткрытия", ОбъектСсылка.ПотенциальныйКлиент);	
		КонецЕсли;
	КонецЕсли;

...
&НаКлиентеНаСервереБезКонтекста 
Процедура СФормироватьТекстСчетаФактурыПолученные(Форма)
	...
	Если ТекстыДокументыНаОсновании.Количество() > 0 Тогда
		Форма.Элементы.ТекстСчетаФактурыПолученные.Видимость = Истина;
		Форма.ТекстСчетаФактурыПолученные = Новый ФорматированнаяСтрока(ТекстыДокументыНаОсновании);
	Иначе
		Форма.Элементы.ТекстСчетаФактурыПолученные.Видимость = Ложь;
	КонецЕсли;
...
Процедура ОбновитьТекстСчетаФактурыПолученные()
	...
	ТекстыПоляДокументыНаОсновании.Добавить(СчетаФактурыПолученныеНаОсновании.Представление);
	...
	СФормироватьТекстСчетаФактурыПолученные(ЭтотОбъект);

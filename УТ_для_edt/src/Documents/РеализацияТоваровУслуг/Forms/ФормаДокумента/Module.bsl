...
&НаКлиентеНаСервереБезКонтекста 
Процедура СФормироватьТекстДокументыНаОсновании(Форма)
	...
	Если ТекстыДокументыНаОсновании.Количество() > 0 Тогда
		Форма.Элементы.ТекстДокументыНаОснованииПодвал.Видимость = Истина;
		Форма.ТекстДокументыНаОснованииПодвал = Новый ФорматированнаяСтрока(ТекстыДокументыНаОсновании);
	Иначе
		Форма.Элементы.ТекстДокументыНаОснованииПодвал.Видимость = Ложь;
	КонецЕсли;
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ПоСоглашениюИспользуютсяДоговорыКонтрагентов = ЗначениеЗаполнено(Соглашение) И ОбщегоНазначенияУТ.ЗначениеРеквизитаОбъектаТипаБулево(Соглашение, "ИспользуютсяДоговорыКонтрагентов");
...
Процедура ПолучитьОбновитьИнформациюТранспортныхНакладных()
	...
	ДополнитьТекстДокументыНаОснованииПодвал(ЭтаФорма);
...
&НаСервереБезКонтекста
Процедура ДополнитьТекстДокументыНаОснованииПодвал(Форма)
	
	//++ Локализация
	Форма.ТекстыПоляДокументыНаОсновании.Очистить();
	Форма.ТекстыПоляДокументыНаОсновании.Добавить(Форма.ТекстДокументыНаОснованииПодвал);
	
	ПараметрыРегистрацииУведомленийОПеремещении = Документы.РеализацияТоваровУслуг.ПараметрыРегистрацииУведомленийОПеремещенииПрослеживаемыхТоваров(Форма.Объект);
	...
	Форма.ТекстыПоляДокументыНаОсновании.Добавить(УведомленияОПеремещенииНаОсновании.ПредставлениеРаспоряженияКОформлению);
	Форма.ТекстыПоляДокументыНаОсновании.Добавить(УведомленияОПеремещенииНаОсновании.ПредставлениеУведомленияОПеремещении);	
	
	СформироватьТекстДокументыНаОсновании(Форма);
	//-- Локализация
...
Функция ВозможноПробитиеЧекаПоДокументу()
	...
	Возврат ПродажиСервер.ВозможноПробитиеЧекаПоДокументу(Объект.Ссылка, Объект.Контрагент);

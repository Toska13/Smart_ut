...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если ЗначениеЗаполнено(Объект.Партнер)
			...
			// Фролов+ При копировании данные по доставе берем из документа копирования
	...
	//++ смарт:КМД 22.12.2021
	//изменил добавление реквизита формы смартСоглашениеПередИзменением на программное
	ДобавляемыеРеквизиты = Новый Массив;
	НовыйРеквизит_смартСоглашениеПередИзменением = Новый РеквизитФормы("смартСоглашениеПередИзменением", Новый ОписаниеТипов("СправочникСсылка.СоглашенияСКлиентами"));
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит_смартСоглашениеПередИзменением);
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	КонецЕсли;
	//++ #1315 смарт: Фролов 11.09.2023 //Ошибка заполнения р/с организации при копировании Заказа клиента  
	//Если Объект.ЭтапыГрафикаОплаты.Итог("СуммаПлатежа")<>Объект.Товары.Итог("СуммаСНДС") Тогда  
	Если Объект.ЭтапыГрафикаОплаты.Итог("СуммаПлатежа")<>Объект.СуммаДокумента Тогда
		ЗаполнитьЭтапыОплатыСервер(Истина,Ложь);
	КонецЕсли;	
	//-- #1315 смарт: Фролов 11.09.2023  
	
	//++ #Б-1234 смарт: Мосолов 29.10.2024 //Уведомление об автоматических скидках выводить только один раз
	смартПроверитьИзменениеСуммы();
...
Процедура СоглашениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	//Смарт {[+](фрагмент ДОБАВЛЕН), Подкин Алексей Иванович 01.07.2021 14:29:26 Задача 357
	смартСоглашениеПередИзменением = Объект.Соглашение;
	//Смарт }Подкин Алексей Иванович 01.07.2021 14:29:26
...
Процедура СоглашениеПриИзменении(Элемент)
	...
	//Смарт {[-](фрагмент УДАЛЕН)Подкин Алексей Иванович 30.09.2020 14:54:18 Задача 357: вынесено в Расчет соглашения
	//СкидкиНаценкиКлиент.СброситьФлагСкидкиРассчитаны(ЭтаФорма);
	//
	//СоглашениеПриИзмененииСервер();
	//ПродажиКлиент.ОповеститьОбОкончанииЗаполненияУсловийПродаж();
	//Смарт }Подкин Алексей Иванович 30.09.2020 14:54:18
	
	//Смарт {[+](фрагмент ДОБАВЛЕН), Подкин Алексей Иванович 30.09.2020 14:55:27 Задача 357 вопрос пользователю
	ПроверкаРеквизитов = СравнитьВалютыСоглашений();
	Если ПроверкаРеквизитов.ИзменяетсяВалютаВзаиморасчетов ИЛИ ПроверкаРеквизитов.ИзменяетсяГруппаФин ИЛИ ПроверкаРеквизитов.ИзменяетсяВалюта Тогда
		РасчетСоглашения();
	Иначе
		Оповещение = Новый ОписаниеОповещения("ПересчетЦенПосле", ЭтотОбъект);
		ПоказатьВопрос(Оповещение, "Пересчитать цены?",  
		РежимДиалогаВопрос.ДаНет,	0,	КодВозвратаДиалога.Да,		
		///	"Пересчитать цены?" 		
		);
	КонецЕсли;
	//Смарт }Подкин Алексей Иванович 30.09.2020 14:55:27	
...
//Смарт {[+](фрагмент ДОБАВЛЕН), Подкин Алексей Иванович 30.09.2020 14:56:32 Задача 357
&НаСервере
Функция СравнитьВалютыСоглашений()
	ПроверкаРеквизитов=Новый Структура("ИзменяетсяВалюта,ИзменяетсяВалютаВзаиморасчетов,ИзменяетсяГруппаФин");
	Если ЗначениеЗаполнено(ЭтаФорма["смартСоглашениеПередИзменением"]) Тогда
		ПроверкаРеквизитов.ИзменяетсяВалютаВзаиморасчетов= Объект.Соглашение.ИспользуютсяДоговорыКонтрагентов И
		ЭтаФорма["смартСоглашениеПередИзменением"].ИспользуютсяДоговорыКонтрагентов И
		Объект.Соглашение.ВалютаВзаиморасчетов<>ЭтаФорма["смартСоглашениеПередИзменением"].ВалютаВзаиморасчетов;
		
		ПроверкаРеквизитов.ИзменяетсяВалюта= Объект.Соглашение.Валюта<>ЭтаФорма["смартСоглашениеПередИзменением"].Валюта;
		ПроверкаРеквизитов.ИзменяетсяГруппаФин= Объект.Соглашение.ГруппаФинансовогоУчета<>ЭтаФорма["смартСоглашениеПередИзменением"].ГруппаФинансовогоУчета;
	иначе
		ПроверкаРеквизитов.ИзменяетсяВалютаВзаиморасчетов= Истина;
		ПроверкаРеквизитов.ИзменяетсяВалюта= Истина;
		ПроверкаРеквизитов.ИзменяетсяГруппаФин= Истина;
	КонецЕсли; 
...
//Смарт {[+](фрагмент ДОБАВЛЕН), Подкин Алексей Иванович 30.09.2020 14:56:32 Задача 357
&НаКлиенте
Процедура ПересчетЦенПосле(Результат, Параметры) Экспорт 
	
	Если Результат = КодВозвратаДиалога.Да Тогда
		РасчетСоглашения();
	иначе
		//++ #1334 смарт: Фролов 10.04.2023 //Заявка на формирование и подписание договора из 1С  
		//++ #1315 смарт: Фролов 17.03.2023 //Ошибка заполнения р/с организации при копировании Заказа клиента  
		//СоглашениеПриИзмененииСервер();
		//ХозяйственнаяОперацияПриИзмененииСервер(Ложь);
		//-- #1315 смарт: Фролов 17.03.2023 
		смартСоглашениеПриИзмененииСервер();
		//-- #1334 смарт: Фролов 10.04.2023
	КонецЕсли;

КонецПроцедуры //Смарт }Подкин Алексей Иванович 30.09.2020 14:56:32

//++ #1334 смарт: Фролов 10.04.2023 //Заявка на формирование и подписание договора из 1С  
&НаСервере
Процедура смартСоглашениеПриИзмененииСервер()
...
КонецПроцедуры //-- #1334 смарт: Фролов 10.04.2023

//Смарт {[+](фрагмент ДОБАВЛЕН), Подкин Алексей Иванович 30.09.2020 14:56:32 Задача 357
&НаКлиенте
Процедура РасчетСоглашения()
	СкидкиНаценкиКлиент.СброситьФлагСкидкиРассчитаны(ЭтаФорма);
	СоглашениеПриИзмененииСервер();
	ПродажиКлиент.ОповеститьОбОкончанииЗаполненияУсловийПродаж();
КонецПроцедуры //Смарт }Подкин Алексей Иванович 30.09.2020 14:56:32

//Смарт {[+](фрагмент ДОБАВЛЕН), Подкин Алексей Иванович 03.06.2021 15:29:49 задача 539 перенес из расширения
//&НаСервере
//Процедура смартРассчитатьОтклонениеЦеныОтРекомендованного(ТекущаяСтрока, РазницаЦен)

//	Если ЗначениеЗаполнено(РазницаЦен.смартЦенаМин)
//		//Смарт {[+](фрагмент Удален), Подкин Алексей Иванович 24.06.2021 16:15:46 задача 539 обсуждение с Фанисом
//		//И ЗначениеЗаполнено(РазницаЦен.смартЦенаМакс)
//		//Смарт }Подкин Алексей Иванович 24.06.2021 16:15:46
//		И ЗначениеЗаполнено(ТекущаяСтрока.Цена)Тогда
//		
//		ОтклонениеМин = Число(РазницаЦен.смартЦенаМин + (РазницаЦен.смартЦенаМакс - РазницаЦен.смартЦенаМин) * Число(0.3));
//		ОтклонениеМакс = Число(РазницаЦен.смартЦенаМакс - (РазницаЦен.смартЦенаМакс - РазницаЦен.смартЦенаМин) * Число(0.3));
//		
//		Если ТекущаяСтрока.ЦенаСоСкидкой < ОтклонениеМин Тогда
//			ТекущаяСтрока.Вставить("ЦветВыделения", 1);
//		ИначеЕсли ТекущаяСтрока.ЦенаСоСкидкой > ОтклонениеМакс Тогда
//			ТекущаяСтрока.Вставить("ЦветВыделения", 3);
//		Иначе
//			ТекущаяСтрока.Вставить("ЦветВыделения", 2);
//		КонецЕсли;
//	ИначеЕсли Не ЗначениеЗаполнено(РазницаЦен.смартЦенаМин)ИЛИ РазницаЦен.смартЦенаМин = 0 Тогда
//		ТекущаяСтрока.Вставить("ЦветВыделения", 0);
//	Иначе
//		ТекущаяСтрока.Вставить("ЦветВыделения", 4);	
//	КонецЕсли;
//	
//	смартУсловноеОформлениеПоМинимальнойЦене(ТекущаяСтрока);
//	
//КонецПроцедуры // смартРассчитатьОтклонениеЦеныОтРекомендованного()

//Смарт {[+](фрагмент ДОБАВЛЕН), Подкин Алексей Иванович 03.06.2021 15:29:49 задача 539 перенес из расширения
&НаСервере
Процедура смартУсловноеОформлениеПоМинимальнойЦене(ТекущаяСтрока)
	
	ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
	
    //ПолеЭлемента = ЭлементУсловногоОформления.Поля.Элементы.Добавить();
    //ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыСмартЦенаМин.Имя);
	...
	ОтборЭлемента.ПравоеЗначение = ТекущаяСтрока.НомерСтроки; 
	Если ТекущаяСтрока.ЦветВыделения = 0 Тогда
		//ЭлементУсловногоОформления.Поля.Элементы.Удалить(Элементы.ТоварыЦенаСоСкидкой);
		ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ТекстЗапрещеннойЯчейкиЦвет);
		ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Шрифт", ШрифтыСтиля.ШрифтТекста.Жирный);
		ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ОтметкаНезаполненного", Ложь);
		ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
		ЭлементУсловногоОформления.Использование = Истина;
	ИначеЕсли ТекущаяСтрока.ЦветВыделения = 1 Тогда
		ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ЦветТекста", WebЦвета.ОранжевоКрасный);
		ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Шрифт", ШрифтыСтиля.ШрифтТекста.Жирный);
		ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
		ЭлементУсловногоОформления.Использование = Истина;
	ИначеЕсли ТекущаяСтрока.ЦветВыделения = 2 Тогда
		ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ЦветТекста", WebЦвета.ТемноОранжевый);
		ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Шрифт", ШрифтыСтиля.ШрифтТекста.Жирный);
		ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
		ЭлементУсловногоОформления.Использование = Истина;
	ИначеЕсли ТекущаяСтрока.ЦветВыделения = 3 Тогда
...
КонецПроцедуры // смартУсловноеОформлениеПоМинимальнойЦене()
// Конец ЭлектронноеВзаимодействие.СервисEDI
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	//Смарт {[+](фрагмент ДОБАВЛЕН), Подкин Алексей Иванович 26.05.2021 13:24:06 задача 539
    //СтруктураДействий.Вставить("смартЗаполнитьМинМаксЦену", Новый Структура("Объект, НомерСтроки", Объект,ТекущаяСтрока.НомерСтроки));
	//Смарт }Подкин Алексей Иванович 26.05.2021 13:24:06
	
	ОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	
	//Смарт {[+](фрагмент ДОБАВЛЕН), Подкин Алексей Иванович 26.05.2021 18:57:14 задача 539
    //Если ТекущаяСтрока.ЦенаСоСкидкой <> 0 ИЛИ ТекущаяСтрока.ЦенаСоСкидкой = Неопределено Тогда
    //	СтруктураЦен = Новый Структура;
    //	СтруктураЦен.Вставить("НомерСтроки", ТекущаяСтрока.НомерСтроки);
    //	СтруктураЦен.Вставить("смартЦенаМин", ТекущаяСтрока.смартЦенаМин);
    //	СтруктураЦен.Вставить("смартЦенаМакс", ТекущаяСтрока.смартЦенаМакс);
    //	
    ...
    //	ТекущаяСтрокаСтруктура.Вставить("НомерСтроки", ТекущаяСтрока.НомерСтроки);
    //	ЗаполнитьЗначенияСвойств(ТекущаяСтрокаСтруктура, ТекущаяСтрока);
    //	смартРассчитатьОтклонениеЦеныОтРекомендованного(ТекущаяСтрокаСтруктура, СтруктураЦен);
    //КонецЕсли;
	//Смарт }Подкин Алексей Иванович 26.05.2021 18:57:14
...
Процедура ТоварыВидЦеныПриИзменении(Элемент)
	...
	СтруктураЗаполненияЦены.Вставить("ОкруглятьПриПересчетеВРубли", Истина);
	...
	СтруктураДействий.Вставить("ПересчитатьСуммуСУчетомАвтоматическойСкидки", Новый Структура("Очищать", Истина));
	СтруктураДействий.Вставить("ЗаполнитьДубликатыЗависимыхРеквизитов", ЗависимыеРеквизиты());
	//Смарт {[+](фрагмент ДОБАВЛЕН), Подкин Алексей Иванович 24.06.2021 14:00:34 задача 539
	//СтруктураДействий.Вставить("смартЗаполнитьМинМаксЦену", Новый Структура("Объект, НомерСтроки", Объект,ТекущаяСтрока.НомерСтроки));
	//Смарт }Подкин Алексей Иванович 24.06.2021 14:00:34
	ОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	//Смарт {[+](фрагмент ДОБАВЛЕН), Подкин Алексей Иванович 24.06.2021 14:01:12 задача 539
    //Если ТекущаяСтрока.ЦенаСоСкидкой <> 0 ИЛИ ТекущаяСтрока.ЦенаСоСкидкой = Неопределено Тогда
    //	СтруктураЦен = Новый Структура;
    //	СтруктураЦен.Вставить("НомерСтроки", ТекущаяСтрока.НомерСтроки);
    //	СтруктураЦен.Вставить("смартЦенаМин", ТекущаяСтрока.смартЦенаМин);
	...
	//Смарт }Подкин Алексей Иванович 24.06.2021 14:01:12
	РассчитатьИтоговыеПоказателиЗаказа(ЭтаФорма);
...
Процедура ТоварыСуммаРучнойСкидкиПриИзменении(Элемент) 
	...
	Если НЕ смартДоступностьРоли("смартРазрешитьСкидкуБольшеМаксимальной,ПолныеПрава") Тогда
		...
		Если НЕ СтруктураКонтроля=Неопределено И СтруктураКонтроля.МаксимальнаяСуммаСкидкиДляКонтроля<ТекущаяСтрока.СуммаРучнойСкидки Тогда

			ТекстСообщения = НСтр("ru = 'Сумма скидки превышает максимальное возможное значение:'") + " " + СтруктураКонтроля.МаксимальнаяСуммаСкидкиДляКонтроля + " " + Объект.Валюта;
...
//++ смарт:Подкин #914 01.03.2022 15:16:46  ограничения применения скидки больше максимальной, указанной в соглашении
...
КонецПроцедуры //-- смарт:Подкин #914 01.03.2022 15:51:40 
...
Процедура ТоварыСуммаПриИзменении(Элемент)       
	...
	Если НЕ смартДоступностьРоли("смартРазрешитьСкидкуБольшеМаксимальной,ПолныеПрава") Тогда
		...
		//-- #1319 смарт: Фролов 13.03.2023
        Если НЕ СтруктураКонтроля=Неопределено И СтруктураКонтроля.МаксимальнаяСуммаСкидкиДляКонтроля<ТекущаяСтрока.СуммаРучнойСкидки Тогда
			//++ #1319 смарт: Фролов 13.03.2023 //Ограничение количества продаж и отмена скидок
			//ТекстСообщения = НСтр("ru = 'Сумма скидки превышает максимальное возможное значение:'") + " " + СтруктураКонтроля.МаксимальнаяСуммаСкидкиДляКонтроля*ТекущаяСтрока.Количество + " " + Объект.Валюта;
			ТекстСообщения = НСтр("ru = 'Сумма скидки превышает максимальное возможное значение:'") + " " + СтруктураКонтроля.МаксимальнаяСуммаСкидкиДляКонтроля + " " + Объект.Валюта;
			//-- #1319 смарт: Фролов 13.03.2023
...
Процедура КонтрагентПриИзмененииСервер()
	...
	Если ЗначениеЗаполнено(Объект.Контрагент) Тогда
		...
		//++ #1334 смарт: Фролов 24.04.2023 //Заявка на формирование и подписание договора из 1С  
		//ЗаполнитьДоговорПоУмолчанию();
		//-- #1334 смарт: Фролов 24.04.2023
		ПродажиСервер.ПартнерПриИзменении(Объект);
		...
		ИначеЕсли ТаблицаИндивидуальныеСкидкиКлиентов <> Неопределено Тогда
			...
			Если СтрокиИндивидуальныеСкидкиКлиентов.Количество() > 0 Тогда
				...
				Если ЗначениеЗаполнено(СтрокаИндивидуальнойСкидки.ПроцентСкидки) Тогда 
					...
					СтруктураДействий = смартСтруктураДействийПересчетаСуммПослеИзмененияСкидок(Объект, Ложь); //?(Стр.ПроцентРучнойСкидки > 0, Истина, Ложь)); //++ #Б4250 смарт: Шляпников 02.09.2025 / Заказ клиента. Автоматические скидки / наценки.
					ОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(Стр, СтруктураДействий, Неопределено);
...
Процедура НазначитьРучнуюСкидкуНаСервере(СуммаСкидкиНаценки, Знач ВыделенныеСтроки, АдресВоВременномХранилище)
	...
	////++ смарт:Подкин #978 15.04.2022 11:57:36
	//Если НЕ смартДоступностьРоли("смартРазрешитьСкидкуБольшеМаксимальной,ПолныеПрава") Тогда
	//    Если Объект.Товары.Количество() Тогда   
	//        //ИзменялисьСкидки=Ложь;
	//        Для каждого Стр Из Объект.Товары Цикл
	//            Если Стр.ПроцентАвтоматическойСкидки>0 Тогда
	//                Стр.ПроцентАвтоматическойСкидки=0;    
	//                Стр.СуммаАвтоматическойСкидки=0; 
	//                // ИзменялисьСкидки=Истина;
	//			КонецЕсли;
	//        КонецЦикла;	    
	//    КонецЕсли;      
	//КонецЕсли;
	////-- смарт:Подкин #978 15.04.2022 11:57:36 
	//--#1323 смарт: Лопато 15.06.2023
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение)
	...
	Для каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		//++#1868 смарт: Лопато 19.06.2024 //Доработать механизм индивидуальных скидок (прод 1323)  
		смартПроверитьОтменитьСкидкуНаТоварСервер(Объект, ТекущаяСтрока);
		//--#1868 смарт: Лопато 19.06.2024
	...
	Для Каждого Идентификатор Из КэшированныеЗначения.ОбработанныеСтроки Цикл		
		СтрокаТовары = Объект.Товары.НайтиПоИдентификатору(Идентификатор);
		Если СтрокаТовары.ВариантОбеспечения = ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.Требуется") Тогда
...
Процедура смартПроверитьОтменитьСкидкуНаТоварСерверОбработкаСтрокиКлиент(Объект, Стр, СегментОтменаСкидкиНаТовар, КэшированныеЗначения)
	...
	Если Стр.ПроцентРучнойСкидки > 0 
		...
		СтрокиАкции = Объект.смартАкции.НайтиСтроки(Новый Структура("КодСтроки", Стр.КодСтроки));
		...
		Если НЕ смартДоступностьРоли("смартИсключениеОграниченияУстанавливатьРучныеСкидкиНаТоварыИзСегмента,ПолныеПрава")
			...
			СтруктураДействий = смартСтруктураДействийПересчетаСуммПослеИзмененияСкидок(Объект, СтрокиАкции.Количество() = 0, Истина);
		...
		Иначе
			...
			Если (Стр.ПроцентАвтоматическойСкидки > 0 И Стр.ПроцентРучнойСкидки > 0) 
				...
				СтруктураДействий = смартСтруктураДействийПересчетаСуммПослеИзмененияСкидок(Объект, СтрокиАкции.Количество() = 0, Ложь);
				ОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(Стр, СтруктураДействий, КэшированныеЗначения);
...
//++ #Б-1234 смарт: Мосолов 29.10.2024 //Уведомление об автоматических скидках выводить только один раз
&НаСервере
Процедура смартПроверитьИзменениеСуммы()

	Если ЭтаФорма.смартВопросПроСкидкиЗадан Тогда
		
		ЭтаФорма.смартСуммаДокументаИзменилась = Ложь;
		
	Иначе
		
		ЭтаФорма.смартСуммаДокументаИзменилась = ЭтаФорма.смартСуммаДокументаИзменилась 
			ИЛИ НЕ ЗначениеЗаполнено(Объект.Ссылка) 
			ИЛИ	Объект.Ссылка.Товары.Итог("СуммаСНДС") <> Объект.Товары.Итог("СуммаСНДС");
		
	КонецЕсли;
		
КонецПроцедуры //-- #Б-1234 смарт: Мосолов 29.10.2024

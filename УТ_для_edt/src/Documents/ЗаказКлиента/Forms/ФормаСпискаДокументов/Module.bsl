...
Процедура УстановитьСтатусЗакрыт(Команда)
	...
	//++#1457 смарт: Ермаков 14.09.2023 // Отключение кода. 
	//++ смарт:ММ #042 222 27.03.2020 Рефакторинг. Перенесено из расширения (отмена &Вместо) 
	// Фролов+
	//ПроверитьВозможностьЗакрытияЗаказов(СписокЗаказов);
...
//++#1457 смарт: Ермаков 14.09.2023 // Отключение процедуры. 
//++ смарт:ММ #042 222 27.03.2020 Рефакторинг. Перенесено из расширения (отмена &Вместо) 
// Фролов+
//&НаСервереБезКонтекста
//Процедура ПроверитьВозможностьЗакрытияЗаказов(СписокЗаказов)
//	ПроверитьВсеОткруженоПоЗаказам(СписокЗаказов); 
//    //++ смарт:ПА #862 13.01.2022 15:23:03
//    МассивКУдалению=Новый Массив;
//    //-- смарт:ПА #862 13.01.2022 15:23:03
//    Для Каждого Заказ Из СписокЗаказов Цикл
//        НевозможноЗакрыть=Ложь;
//		
//		смартСервер.ПроверкаНаличияГалочекФИН(Заказ.Значение,НевозможноЗакрыть);
//        Если НевозможноЗакрыть Тогда
//            //++ смарт:ПА #862 13.01.2022 14:52:29 
//            МассивКУдалению.Добавить(Заказ);
//            // СписокЗаказов.Удалить(Заказ);   
//            // 	Продолжить;
//            //-- смарт:ПА #862 13.01.2022 14:52:29
//        КонецЕсли;
//		//закомментировано 20032017 для теста производительности
//		//Модуль_Б1Сервер.ПроверитьЗаполнениеРеквизитовВСвязанныхДокументах(Заказ.Значение,НевозможноЗакрыть);
//		//Если НевозможноЗакрыть Тогда
//		//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю("В подчененных документа заказ "+Заказ.Значение+" есть документы с отличающимися реквизитами.");
//		//	СписокЗаказов.Удалить(Заказ);
//		//КонецЕсли;
//    КонецЦикла;	
//     
//    //++ смарт:ПА #862 13.01.2022 15:05:27
//    Для Каждого ЗаказУдалить Из МассивКУдалению Цикл
//        СписокЗаказов.Удалить(ЗаказУдалить); 
//    КонецЦикла;
//    //-- смарт:ПА #862 13.01.2022 15:05:27 
...
//++ смарт:ММ #042 222 27.03.2020 Рефакторинг. Перенесено из расширения (отмена &Вместо) 
//&НаСервереБезКонтекста
//Процедура ПроверитьВсеОткруженоПоЗаказам(СписокЗаказов)
//	Запрос=Новый Запрос;
//	Запрос.УстановитьПараметр("ЗаказыКлиента",СписокЗаказов);
//	Запрос.Текст="ВЫБРАТЬ РАЗРЕШЕННЫЕ
//	             |	ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Заказ
//	             |ИЗ
//	             |	РегистрНакопления.ТоварыКОтгрузке.Остатки(, ДокументОтгрузки В (&ЗаказыКлиента)) КАК ТоварыКОтгрузкеОстатки
//	             |ГДЕ
//	             |	ТоварыКОтгрузкеОстатки.КОтгрузкеОстаток > 0
//	             |	И ТоварыКОтгрузкеОстатки.Склад.ИспользоватьОрдернуюСхемуПриОтгрузке";
//	Рез=Запрос.Выполнить().Выбрать();
//	Пока Рез.Следующий() Цикл
//		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("По заказу "+Рез.Заказ+" не все отгружено");
//		СписокЗаказов.Удалить(СписокЗаказов.НайтиПоЗначению(Рез.Заказ));
//	КонецЦикла;	
//КонецПроцедуры // Фролов-
//--#1457 смарт: Ермаков 14.09.2023 // Отключение процедуры. 
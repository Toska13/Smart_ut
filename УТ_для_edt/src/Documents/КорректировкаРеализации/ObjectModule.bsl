...
Процедура ЗаполнитьДопКолонкиВидовЗапасов(ВидыЗапасов, МассивВариантовОтражения, ЗаполнятьВидЗапасовОтгрузки = Ложь) Экспорт
	...
	ТекущаяСтрока			= Новый Структура("СтавкаНДС, Сумма, СуммаНДС");
	КэшированныеЗначения	= Новый Структура;
	
	ПараметрыПересчетаСуммыНДС = ОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(ЭтотОбъект);
	ПараметрыПересчетаСуммыНДС.ЦенаВключаетНДС = Истина;
	...
	Для Каждого СтрокаТоваров Из ТаблицаТовары Цикл
		...
		Для Каждого СтрокаЗапасов Из ВидыЗапасов.НайтиСтроки(СтруктураПоиска) Цикл
			...
			Если КоличествоТоваров <> 0 Тогда
				...
				ТекущаяСтрока.Сумма = НоваяСтрока.СуммаСНДС;
				
				УчетНДСУП.ПересчитатьСуммуНДСВСтрокеТЧ(ТекущаяСтрока, ПараметрыПересчетаСуммыНДС, КэшированныеЗначения);
				ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекущаяСтрока, "СуммаНДС");
			Иначе

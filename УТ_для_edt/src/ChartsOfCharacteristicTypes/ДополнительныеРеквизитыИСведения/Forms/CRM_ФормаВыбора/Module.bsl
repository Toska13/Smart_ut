...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ВыборОбщегоСвойства Тогда
		...
		Если ЭтоДополнительноеСведение = Истина Тогда
			АвтоЗаголовок = Ложь;
			Заголовок = НСтр("ru='Выбор общего дополнительного сведения';en='Select common additional information'");
		ИначеЕсли ЭтоДополнительноеСведение = Ложь Тогда
			АвтоЗаголовок = Ложь;
			Заголовок = НСтр("ru='Выбор общего дополнительного реквизита';en='Select common additional attribute'");
		КонецЕсли;
	...
	ИначеЕсли Параметры.ВыборВладельцаДополнительныхЗначений Тогда
		...
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ru='Выбор образца';en='Select sample'");
...
Процедура ДобавитьФильтрПоНаборамСвойств()
	Если ТипЗнч(Параметры.ОтображаемыеНаборыСвойств) = Тип("Массив")
		...
		Запрос.УстановитьПараметр("Наборы", Параметры.ОтображаемыеНаборыСвойств);
		СвойстваНаборов = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Свойство");
		
		УсловиеЗапроса =
			"
			|ГДЕ
			|	СвойстваПереопределяемый.Ссылка В (&СвойстваНаборов)";
		...
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
			Список, "СвойстваНаборов", СвойстваНаборов, Истина);
	КонецЕсли;
КонецПроцедуры
...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование)
	...
	Если ЗначениеЗаполнено(НаборСвойствОтбор) Тогда

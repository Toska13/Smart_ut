...
//
Процедура ОбновитьТаблицуНапоминаний(СтруктураНовыеНапоминания, ОткрыватьАктивизироватьФорму = Истина) Экспорт // АПК:78 - исключить из проверки.
...
Процедура ОткрытьМенеджерПочты(УчетнаяЗапись,ПерейтиКЧерновикам)
	...
	Если ФормаПочтовогоМенеджера = Неопределено Тогда Возврат; КонецЕсли;

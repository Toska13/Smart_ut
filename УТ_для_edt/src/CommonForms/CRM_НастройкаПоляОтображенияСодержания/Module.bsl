...
Процедура КомандаСохранитьВФайл(Команда)
	...
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Данные должны быть записаны. Записать данные?';en='Data should be written down. Write down the data?'"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да, НСтр("ru='Сохранение настроек в файл'"));
	Иначе
...
Процедура КомандаЗаполнитьПоУмолчанию(Команда)
	...
	Если ДеревоОтображаемыеРеквизиты.ПолучитьЭлементы().Количество() > 0 Тогда
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Текущие настройки будут заменены настройками по умолчанию."
"Продолжить?';en='Current settings will be change with default settings."
"Continue?'"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Нет);
	Иначе

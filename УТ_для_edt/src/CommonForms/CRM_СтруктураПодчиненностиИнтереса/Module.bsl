...
&НаСервере
Функция ДокументОснованиеОбъекта(СсылкаНаОбъект, НаименованиеРеквизита)
	
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СсылкаНаОбъект, НаименованиеРеквизита);
...
Процедура ТаблицаОтчетаОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка, ДополнительныеПараметры)
	...
	Если ТипЗнч(Расшифровка) = Тип("ДокументСсылка.CRM_Взаимодействие") Тогда
		
		ДокументОснование = ДокументОснованиеОбъекта(Расшифровка, "ДокументОснование");
		Если ВладелецФормы <> Неопределено И ВладелецФормы.Объект.Ссылка = ДокументОснование Тогда
			ВладелецФормы.КлючОткрываемойАктивности = Расшифровка;
		КонецЕсли;
		
	ИначеЕсли ТипЗнч(Расшифровка) = Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда
		
		ДокументОснование = ДокументОснованиеОбъекта(Расшифровка, "Предмет");
		Если ВладелецФормы <> Неопределено И ВладелецФормы.Объект.Ссылка = ДокументОснование Тогда
			ВладелецФормы.КлючОткрываемойАктивности = Расшифровка;
		КонецЕсли;
		...
		ПараметрыОткрытия = Новый Структура("Ключ, ПозиционироватьНаВзаимодействие", ДокументОснование, Расшифровка);

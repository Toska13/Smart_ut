...
Процедура ПриОткрытии(Отказ)
	Если ПоказатьОповещениеОПодсказках И ПоказыватьПодсказки Тогда
		ПоказатьОповещениеПользователя(
			...
			НСтр("ru='Вывод описаний в панелях отчетов';en='Display descriptions in report panels'"),
...
Функция ВведенаСлишкомКороткаяСтрокаПоиска(Текст)
	...
	Если Не ЕстьНормальноеСлово Тогда
		ПоказатьПредупреждение(, НСтр("ru='Введены слишком короткие слова для поиска.';en='Words for search are too short.'"));
		Возврат Истина;
	КонецЕсли;
...
Процедура СброситьНастройки(Команда)
	ТекстВопроса = НСтр("ru='Сбросить настройки расположения отчетов?';en='Reset report placement settings?'");
...
Функция НачатьЗамер(Событие, Комментарий = Неопределено)
	...
	Если Событие = "СписокОтчетов.Открытие" Или Событие = "ПанельОтчетов.Открытие" Тогда
		Замер.Имя = Событие;
		Комментарий = Комментарий + "; " + НСтр("ru='Из панели отчетов:';en='From report panel:'") + " " + ПараметрыКлиента.ПутьКПодсистеме;
	Иначе
		...
		Комментарий = Комментарий + "; " + НСтр("ru='Подсказки:';en='Tooltips:'") + " " + Строка(ПоказыватьПодсказки);
	КонецЕсли;
	...
	Иначе
		Комментарий = Комментарий + "; " + НСтр("ru='Без поиска';en='Without searching'");
	КонецЕсли;
	
	Если Событие = "ВыходИзРежимаНастройки" Тогда
		Комментарий = Комментарий + "; " + НСтр("ru='Выход из режима настройки';en='Exit setup mode'");
	КонецЕсли;
...
Процедура ОпределитьПоведениеВМобильномКлиенте()
	...
	ПодстрокаПоиска = НСтр("ru='нажмите на отчете правой кнопкой мыши и';en='right-click the report and'");
	ПодстрокаЗамены = НСтр("ru='в контекстном меню';en='in the context menu'");
...
Процедура ОпределитьПодсистемыИЗаголовок(Параметры)
	...
	ЗаголовокПанели = ?(ЗаданЗаголовок, Параметры.Заголовок, НСтр("ru='Отчеты';en='Reports'"));
	...
	Если ТекущийРазделСсылка = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для панели отчетов указан несуществующий раздел ""%1"" (см. ВариантыОтчетовПереопределяемый.ОпределитьРазделыСВариантамиОтчетов).';en='A non-existent ""%1"" section is specified for the report panel (see ВариантыОтчетовПереопределяемый.ОпределитьРазделыСВариантамиОтчетов).'"),
...
Процедура ЗаполнитьПанельОтчетов(ПараметрыЗаполненияВременноеХранилище)
	...
	Иначе
		...
		Если ПараметрыЗаполнения.ДругиеРазделы.Количество() = 0 Тогда
			...
			Надпись.Заголовок = НСтр("ru='Отчеты в других разделах не найдены.';en='Reports are not found in other sections.'") + Символы.ПС;
			Надпись.Высота = 2;
		КонецЕсли;
		...
		Если ПараметрыЗаполнения.НеВыведено > 0 Тогда // Вывод информационной надписи.
			ЗаголовокНадписи = НСтр("ru='Выведены первые %1 отчетов из других разделов, уточните поисковый запрос.';en='The first %1 reports from other sections are displayed, specify a search query. '");
	...
	Если ПараметрыЗаполнения.ВариантыКоличество = 0 Тогда
		...
		Если ПараметрыЗаполнения.ВыводятсяВариантыТекущегоРаздела Тогда
			...
			Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
				...
				Иначе
					Надпись.Заголовок = НСтр("ru='Отчеты в текущем разделе не найдены.';en='Reports are not found in the current section.'");
					Надпись.Высота = 2;
				КонецЕсли;
			Иначе
				Надпись.Заголовок = НСтр("ru='В панели отчетов этого раздела не размещено ни одного отчета.';en='Report panel of this section does not contain any reports.'");
			КонецЕсли;

...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("АдресФайлаМакета") Тогда
		АдресФайлаМакета = ПоместитьВоВременноеХранилище(ПолучитьИзВременногоХранилища(Параметры.АдресФайлаМакета), УникальныйИдентификатор);
	Иначе	
		АдресФайлаМакета = ПоместитьВоВременноеХранилище(ПолучитьОбщийМакет("смартПустойМакетDOCX"), УникальныйИдентификатор);
	КонецЕсли;
...
Функция СодержитПараметры(АдресМакета)
	...
	ДеревоМакета = смартУправлениеПечатью.ИнициализироватьМакетОфисногоДокументаСКД(Макет);
	СтруктураДокумента = ДеревоМакета.СтруктураДокумента;
	ДеревоДокумента = СтруктураДокумента.ДеревоДокумента;
	УзелПараметра = смартУправлениеПечатьюСлужебный.НайтиУзелПоСодержимому(ДеревоДокумента, "w:textInput");
...
Процедура ОтобразитьИнструкцию()
	...
	Макет = ПолучитьОбщийМакет("смартИнструкцияКакВыполнитьНастройкуМакетаOfficeOpen");
...
Процедура ЗаписатьМакетыНаДополнительныхЯзыках()
	...
	смартУправлениеПечатью.ЗаписатьМакетыНаДополнительныхЯзыках(ПараметрыМакета);
...
Процедура ПодготовитьМакетКСохранению(Макет)
	...
	КонецЦикла;
	ПараметрыЗаполненияОбласти = смартУправлениеПечатью.ПараметрыЗаполненияОбласти();
	ПараметрыЗаполненияОбласти.ДобавлятьСвязи = Ложь;
	смартУправлениеПечатью.УстановитьПараметры(ДеревоМакета, СоответствиеЗамен, ПараметрыЗаполненияОбласти);
	Адрес = смартУправлениеПечатьюСлужебный.ПолучитьПечатнуюФорму(ДеревоМакета);
...
Процедура ПодготовитьМакетКОткрытию(Макет, ЗаполнитьАдресФайлаМакета = Ложь)
	...
	СтруктураДокумента = ДеревоМакета.СтруктураДокумента;
	ДеревоДокумента = СтруктураДокумента.ДеревоДокумента;
	смартУправлениеПечатью.КонвертироватьПараметры(ДеревоДокумента);
	Для Каждого Колонтитул Из СтруктураДокумента.Колонтитулы Цикл
		смартУправлениеПечатью.КонвертироватьПараметры(Колонтитул.Значение);
	КонецЦикла;
	...
	Если ЗаполнитьАдресФайлаМакета Тогда
		Адрес = смартУправлениеПечатьюСлужебный.ПолучитьПечатнуюФорму(ДеревоМакета, АдресФайлаМакета);
	Иначе
		Адрес = смартУправлениеПечатьюСлужебный.ПолучитьПечатнуюФорму(ДеревоМакета);
	КонецЕсли;
...
Процедура ЗаменитьПредставленияПараметров(Строка)
	...
	Для Каждого Параметр Из ПараметрыЗамены Цикл
		...
		ТекстОшибки = смартКонструкторФормулСлужебный.ПроверитьФормулу(ЭтотОбъект, Формула);
...
Функция ПолучитьПечатнуюФорму()
	...
	ОфисныеДокументы = смартУправлениеПечатью.СформироватьОфисныйДокумент(Макет, Ссылки, ОбъектыПечати, КодЯзыка, ПараметрыПечати);

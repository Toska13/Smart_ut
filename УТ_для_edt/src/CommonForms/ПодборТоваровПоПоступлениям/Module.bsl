...
Процедура ЗаполнитьТаблицуТоваров()
	...
	Если ЗакупкаПоИмпорту Тогда
		...
		//++ Смарт:  Демичев №1022 09.08.2022 ((Если вызвали из Таможенной декларации, то строки с товарами не сворачиваем
		//Запрос.Текст = ТекстЗапросаЗакупкаПоИмпорту();
		Если Параметры.Свойство("Смарт_ТаможеннаяДекларация") и Параметры.Смарт_ТаможеннаяДекларация Тогда
			
			Запрос.Текст = ТекстЗапросаЗакупкаПоИмпорту_СмартБезГруппировкиСтрок();
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтаФорма, "смартОтборГарантийныеЗапчасти") Тогда
			Запрос.УстановитьПараметр("смартОтборГарантия", ЭтаФорма.смартОтборГарантийныеЗапчасти);
		Иначе
			Запрос.УстановитьПараметр("смартОтборГарантия", 0);
		КонецЕсли;
		//-- #Б1598 смарт: Шляпников 09.12.2024
        
		// Фролов+
		//++#1350 смарт: Лопато 03.05.2023 //Исправление ошибки
		Если Параметры.Свойство("Груз") И ЗначениеЗаполнено(Параметры.Груз) Тогда 
		//--#1350 смарт: Лопато 03.05.2023
			Схема=Новый СхемаЗапроса;
			Схема.УстановитьТекстЗапроса(Запрос.Текст);
			Схема.ПакетЗапросов[Схема.ПакетЗапросов.Количество()-1].Операторы[0].Отбор.Добавить("ВтПоступления.ДокументПоступления.Груз=&Груз");
			
			Запрос.Текст=Схема.ПолучитьТекстЗапроса();
			Запрос.УстановитьПараметр("Груз",Параметры.Груз);
		КонецЕсли;	
		// Фролов-
...
//++ Смарт:  Демичев №1022 09.08.2022
&НаСервереБезКонтекста
Функция ТекстЗапросаЗакупкаПоИмпорту_СмартБезГруппировкиСтрок()
	...
	Возврат "ВЫБРАТЬ
			...
			|						ИНАЧЕ ИСТИНА КОНЕЦ
			//-- #Б1598 смарт: Шляпников 09.12.2024
			|				) КАК ТоварыКОформлениюДокументовИмпортаОстатки
	        |
	        |ИНДЕКСИРОВАТЬ ПО
	        |	ДокументПоступления,
	        |	ВидЗапасов,
	        ...
	        |	ПриобретениеТоваровУслугТовары.ВидЗапасов КАК ВидЗапасов,
	        |	ПриобретениеТоваровУслугТовары.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
	        |	ПриобретениеТоваровУслугТовары.Количество КАК КоличествоПоступления,
	        |	ПриобретениеТоваровУслугТовары.СтавкаНДС КАК СтавкаНДС,
	        ...
	        |	ПриобретениеТоваровУслугТовары.СтавкаНДС,
	        |	ПриобретениеТоваровУслугТовары.Цена * ПриобретениеТоваровУслугТовары.Количество,
	        ...
	        |			И ВТОстатки.ВидЗапасов = ВтПоступления.ВидЗапасов
	        |ГДЕ
	        |	(ВтПоступления.Дата >= &ДатаПоступления
	        |			ИЛИ &ДатаПоступления ЕСТЬ NULL
	        |			ИЛИ ВтПоступления.Дата ЕСТЬ NULL)
	        |	И (НЕ &ОтборПоСкладу
	        |			ИЛИ ЕСТЬNULL(ВтПоступления.Склад, ВТОстатки.АналитикаУчетаНоменклатуры.МестоХранения) = &Склад)
	        |
	        |УПОРЯДОЧИТЬ ПО

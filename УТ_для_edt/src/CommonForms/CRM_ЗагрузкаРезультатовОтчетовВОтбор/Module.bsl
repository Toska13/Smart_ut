...
Процедура НастроитьВидимостьИДоступностьФормы(Форма)
	Если Форма.Режим = "ВыборПоляОтбора" Тогда
		...
		Форма.Заголовок = НСтр("ru='Выбор поля отбора';en='Choice of filter field'");
	Иначе
		...
		Форма.Заголовок = НСтр("ru='Загрузка результата отчета в отбор';en='Uploading Report Result to Filter'");
	КонецЕсли;
КонецПроцедуры
...
Функция ПроверитьНаКорректность()
	...
	Если Не ЗначениеЗаполнено(ПолеОтбора) Тогда
		СообщитьПользователю(НСтр("ru='Поле отбора должно быть заполнено!';en='Field of filter should be fill!'"), "ПредставлениеПоляОтбора");
		Возврат Ложь;
	КонецЕсли;

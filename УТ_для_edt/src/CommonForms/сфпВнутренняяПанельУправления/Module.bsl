...
Процедура ПриОткрытии(Отказ)
	...
	Элементы.НадписьСостояниеПодключения.Заголовок			= НСтр("ru='Не подключен';en='Not connected'");
	...
	ВнутреннийНомер = сфпСофтФонПроСервер.сфпТекущийВнутреннийНомер();
	Если ЗначениеЗаполнено(ВнутреннийНомер) Тогда
		Элементы.НадписьЛинияПодключения.Заголовок				= НСтр("ru='Внутренний номер: ';en='Internal number: '") + ВнутреннийНомер;
		...
		Попытка
			СтатусЛинии = сфпСоответствиеЛинийИСтатусов.Получить(ВнутреннийНомер);
			СтатусПользователя = Лев(СтатусЛинии, 1) + НРЕГ(Сред(СтатусЛинии, 2));
			СтатусПользователяКартинка = КартинкаСтатуса(СтатусЛинии);
		Исключение КонецПопытки;	
	КонецЕсли;

	ОбновитьСтатусыЛиний(сфпСоответствиеЛинийИСтатусов);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "СофтФон_OnLinesStatus" ИЛИ ИмяСобытия = "СофтФон_OnAllLinesInfo" Тогда
		Если ЗначениеЗаполнено(ВнутреннийНомер) Тогда
			СтатусЛинии = сфпСоответствиеЛинийИСтатусов.Получить(ВнутреннийНомер);
			СтатусПользователя = Лев(СтатусЛинии, 1) + НРЕГ(Сред(СтатусЛинии, 2));
			СтатусПользователяКартинка = КартинкаСтатуса(СтатусЛинии);
		...
		ИначеЕсли ИмяСобытия = "СофтФон_OnLinesStatus" Тогда
			ОбновитьСтатусыТекущихЛиний(Параметр);
		КонецЕсли;	
	КонецЕсли;
...
Процедура ДобавитьОбновитьЗвонок(СтруктураЗвонка)
	...
	Если СтрокаЗвонка = Неопределено Тогда
		...
		СтрокаЗвонка.ЗаголовокКлиент			= НСтр("ru='Клиент:';en='Customer:'");	
		СтрокаЗвонка.ЗаголовокКонтактноеЛицо	= НСтр("ru='Контактное лицо:';en='Contact Person:'");
	КонецЕсли;
	...
	Если СтруктураЗвонка.ВходящийЗвонок Тогда
		...
		Если СтрокаПропущенногоЗвонка = Неопределено Тогда
			...
			СтрокаПропущенногоЗвонка.ЗаголовокКлиент			= НСтр("ru='Клиент:';en='Customer:'");	
			СтрокаПропущенногоЗвонка.ЗаголовокКонтактноеЛицо	= НСтр("ru='Контактное лицо:';en='Contact Person:'");
		КонецЕсли;
...
//
Процедура ВыбратьАдреснаяКнига(РезультатВыбора, ДополнительныеПараметры) Экспорт // АПК:78 - исключить из проверки.
...
//	ДополнительныеПараметры	- Структура	- Структура дополнительных параметров
//
Процедура ВыполнитьПереключитьЗвонок(СтруктураКонтакта, ДополнительныеПараметры) Экспорт // АПК:78 - исключить из проверки.
	Если СтруктураКонтакта = Неопределено Тогда Возврат; КонецЕсли;
	ТекЗвонок = ДополнительныеПараметры.ТекущийЗвонок;
	ПолеВводаНомера = СтруктураКонтакта.НомерТелефона;
...
&НаСервере
Функция ПредставлениеСтатусаЛинии(Статус)
	
	Возврат Лев(Статус, 1) + НРЕГ(Сред(Статус, 2));
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция КартинкаСтатуса(ПредставлениеСтатуса)
...
&НаСервере
Процедура ОбновитьСтатусыЛиний(СоответствиеСтатусов)
	
	ЛинииСтатусы.Очистить();
	
	Если СоответствиеСтатусов <> Неопределено Тогда
		Для Каждого ТекСоответствие Из СоответствиеСтатусов Цикл
			НоваяСтрока = ЛинииСтатусы.Добавить();
			НоваяСтрока.Линия = ТекСоответствие.Ключ;
			НоваяСтрока.Статус = ПредставлениеСтатусаЛинии(ТекСоответствие.Значение);
			НоваяСтрока.КартинкаСтатуса = КартинкаСтатуса(НоваяСтрока.Статус);
		КонецЦикла;
	КонецЕсли;
		
	Запрос = Новый Запрос("
	|ВЫБРАТЬ Пользователь, Значение
	|ИЗ РегистрСведений.CRM_НастройкиПользователей
	|ГДЕ Настройка = &Настройка И Значение <> """"
	...
	Пока Выборка.Следующий() Цикл
		СтруктураПоиска = Новый Структура("Линия", Выборка.Значение);
		НайденныеСтроки = ЛинииСтатусы.НайтиСтроки(СтруктураПоиска);
		Если НайденныеСтроки.Количество() = 0 Тогда
			СтрокаСтатуса = ЛинииСтатусы.Добавить();
			СтрокаСтатуса.Линия = Выборка.Значение;
			
		Иначе
			СтрокаСтатуса = НайденныеСтроки[0];
		КонецЕсли;
				
		СтрокаСтатуса.Пользователь = Выборка.Пользователь;
	КонецЦикла;
	
	ЛинииСтатусы.Сортировать("Линия");
	
КонецПроцедуры

&НаСервере
Процедура ОбновитьСтатусыТекущихЛиний(Линии)
	
	Для Каждого ТекЛиния Из Линии Цикл
		СтруктураПоиска = Новый Структура("Линия", ТекЛиния.Ключ);
		НайденныеСтроки = ЛинииСтатусы.НайтиСтроки(СтруктураПоиска);
		Если НайденныеСтроки.Количество() = 0 Тогда
			СтрокаСтатуса = ЛинииСтатусы.Добавить();
			СтрокаСтатуса.Линия = ТекЛиния.Ключ;
			СтрокаСтатуса.Статус = ПредставлениеСтатусаЛинии(ТекЛиния.Значение);
			СтрокаСтатуса.КартинкаСтатуса = КартинкаСтатуса(СтрокаСтатуса.Статус);
			
			ЛинииСтатусы.Сортировать("Линия");
			
		Иначе
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				НайденнаяСтрока.Статус = ПредставлениеСтатусаЛинии(ТекЛиния.Значение);
				НайденнаяСтрока.КартинкаСтатуса = КартинкаСтатуса(НайденнаяСтрока.Статус);
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;	
...
&НаКлиенте
Процедура СтатусПользователяНажатие(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	Если сфпПанельУправления <> Неопределено Тогда
		Если ВРЕГ(СтатусПользователя) = "ЛИНИЯ ЗАНЯТА" ИЛИ ВРЕГ(СтатусПользователя) = "НЕ В СЕТИ" Тогда
			Возврат;
		КонецЕсли;	
		
		СписокВыбора = Новый СписокЗначений();
		Если СтатусПользователя <> "В сети" Тогда
			СписокВыбора.Добавить(0, "В сети");
		КонецЕсли;
		Если СтатусПользователя <> "Нет на месте" Тогда	
			СписокВыбора.Добавить(1, "Нет на месте");
		КонецЕсли;
		Если СтатусПользователя <> "Не беспокоить" Тогда
			СписокВыбора.Добавить(2, "Не беспокоить");
		КонецЕсли;

		Оповещение = Новый ОписаниеОповещения("ПриВыбореСтатусаПользователя", ЭтаФорма);
...
&НаКлиенте
Процедура ПриВыбореСтатусаПользователя(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	
	Если ВыбранныйЭлемент = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если СтатусПользователя <> ВыбранныйЭлемент.Представление Тогда
		Если сфпПанельУправления <> Неопределено Тогда
			сфпПанельУправления.SetStatus(ВыбранныйЭлемент.Значение, "", 0, "", 0);
		КонецЕсли;
	
		//СтатусПользователя = ВыбранныйЭлемент.Значение;
		//СтатусПользователяКартинка = КартинкаСтатуса(СтатусПользователя);
		//СтатусПользователя = Лев(СтатусЛинии, 1) + НРЕГ(Сред(СтатусЛинии, 2));
	КонецЕсли;

...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Пользователи") И ТипЗнч(Параметры.Пользователи) = Тип("Массив") Тогда
		...
		Элементы.ИнтервалШкалыВремени.Видимость	= Ложь;
	...
	ИначеЕсли Параметры.Свойство("Место") Тогда
		Заголовок = НСтр("ru='Подбор помещения';en='Selection of locations'");
		Элементы.ИнтервалШкалыВремени.Видимость	= Ложь;
	...
	Если Параметры.Свойство("ЗакрыватьФормуПослеВыбораПериодов") Тогда
		...
		Элементы.КалендарьТабличныйДокументКонтекстноеМенюКомандаДобавитьПользователяСПериодом.Заголовок = НСтр("ru='Выбрать период';en='Select period'");
	КонецЕсли;

...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//		МодульУправлениеПечатью = ОбщегоНазначения.ОбщийМодуль("смартУправлениеПечатью");
	...
	ДоступенАвтоматическийПеревод = Ложь;
	//Если ЭтоМакет Тогда
	...
	Если ЭтоПечатнаяФорма И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.смартКонструкторФормул") Тогда
		МодульКонструкторФормул = ОбщегоНазначения.ОбщийМодуль("смартКонструкторФормул");
		...
		ПараметрыДобавления.ПриОпределенииИсточниковДоступныхПолей = "смартУправлениеПечатью";
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСФайлами") Тогда
			...
			ТипыПрисоединенныхФайлов = Метаданные.ОпределяемыеТипы.ПрисоединенныйФайл.Тип;
		КонецЕсли;
	КонецЕсли;
...
Процедура РедактироватьВоВнешнейПрограмме(Команда)
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
		...
		МодульУправлениеПечатьюКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("смартУправлениеПечатьюКлиент");
...
Процедура ИзменитьШрифт(Команда)
	...
	ОткрытьФорму("ОбщаяФорма.смартФормаВыбораШрифта",, ЭтотОбъект,,,, ОписаниеОповещения);
...
Процедура ЗаписатьТабличныйДокумент(ОбработчикЗавершения = Неопределено, ОсвободитьФайл = Ложь)
	...
	ЗаписатьТабличныйДокументИмяФайлаВыбрано(Параметры.Ссылка, ОбработчикЗавершения, ОсвободитьФайл);
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаписатьТабличныйДокументИмяФайлаВыбрано(Ссылка, Знач ОбработчикЗавершения, ОсвободитьФайл)
	...
	Если ПустаяСтрока(Параметры.ПутьКФайлу) Тогда
		...
		АдресМакета = ПолучитьАдресМакета(Истина, ДополнительныеПараметры.АдресМакетаВоВременномХранилище);
		
		ДополнительныеПараметры.Вставить("АдресМакетаВоВременномХранилище", АдресМакета);

		//Макет = ПолучитьИзВременногоХранилища(АдресМакета);
		//Макет.Показать();
		
		//Макет = ЗаписатьМакет(ДополнительныеПараметры);
		//Макет.Показать();
			ПослеЗаписиТабличногоДокумента(ДополнительныеПараметры, ДополнительныеПараметры.ОбработчикЗавершения, ОсвободитьФайл);
		//Иначе
...
Процедура ПродолжитьЗаписьТабличногоДокумента(РезультатДиалога, ДополнительныеПараметры) Экспорт
	...
	Если РезультатДиалога = КодВозвратаДиалога.Да Тогда
		...
		ЗаписатьМакет(ДополнительныеПараметры);
		ПослеЗаписиТабличногоДокумента(ДополнительныеПараметры, ДополнительныеПараметры.ОбработчикЗавершения, ДополнительныеПараметры.ОсвободитьФайл);
	КонецЕсли;
...
&НаКлиенте
Процедура ПослеЗаписиТабличногоДокумента(ДополнительныеПараметры, ОбработчикЗавершения, ОсвободитьФайл)
	...
	//ВыполнитьОбработкуОповещения(Новый ОписаниеОповещения("ОбработатьРезультатЗаписиТабличногоДокумента", ЭтотОбъект, ДополнительныеПараметры), Истина);
...
Процедура ПриЗавершенииДиалогаВыбораФайла(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	ЗаписатьТабличныйДокументИмяФайлаВыбрано(ДополнительныеПараметры, ДополнительныеПараметры.ОбработчикЗавершения, ДополнительныеПараметры.ОсвободитьФайл);
...
Процедура ЗаполнитьТабличныйДокумент(ТабличныйДокумент, Знач ЗагруженныйТабличныйДокумент)
	...
	смартШаблоныПечатныхФормСервер.СкопироватьПараметрыТабличногоДокумента(ТабличныйДокумент, ЗагруженныйТабличныйДокумент);
...
Процедура ТабличныйДокументПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Область)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.смартКонструкторФормул") Тогда
		МодульКонструкторФормулКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("смартКонструкторФормулКлиент");
...
Процедура ЗаполнитьСписокДоступныхПолей(ПараметрыЗаполнения)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.смартКонструкторФормул") Тогда
		МодульКонструкторФормул = ОбщегоНазначения.ОбщийМодуль("смартКонструкторФормул");
...
Функция ПодготовитьМакетДляЗаписи(УстановитьКодЯзыка = Истина, Отказ = Ложь)
	...
	Для НомерСтроки = 1 По Макет.ВысотаТаблицы Цикл
		Для НомерКолонки = 1 По Макет.ШиринаТаблицы Цикл
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
				МодульУправлениеПечатью = ОбщегоНазначения.ОбщийМодуль("смартУправлениеПечатьюКлиентСервер");
...
Процедура ЗаменитьПредставленияПараметров(Строка, Поле = Неопределено, Отказ = Ложь, ПредставлениеПоля = "")
	...
	Для Каждого Параметр Из ПараметрыЗамены Цикл
		...
		ТекстОшибки = "";
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.смартКонструкторФормул") Тогда
			МодульКонструкторФормулСлужебный = ОбщегоНазначения.ОбщийМодуль("смартКонструкторФормулСлужебный");
...
Функция СкопироватьТабличныйДокумент(ТабличныйДокумент, КодЯзыка)
	...
	смартШаблоныПечатныхФормСервер.СкопироватьПараметрыТабличногоДокумента(Результат, ТабличныйДокумент);
...
&НаСервере
Функция ЗаписатьМакет(ДополнительныеПараметры)
	...
	//
	//Возврат Истина;
	Макет = ПолучитьИзВременногоХранилища(ДополнительныеПараметры.АдресМакетаВоВременномХранилище);
	СтрМакет = ЗначениеВСтрокуВнутр(Макет);
	
	смартСерверПривилегированный.ЗаписатьДополнительныеДанныеОбъекта(ДополнительныеПараметры.Ссылка, СтрМакет); 

	ОбщегоНазначения.СообщитьПользователю(СтрМакет);
...
&НаСервере
Функция ПолучитьАдресМакета(ПрерватьЗаписьПриНаличииОшибокВМакете = Ложь, АдресМакетаВоВременномХранилище = "")
	...
	//Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
		...
		Если Не ЗначениеЗаполнено(АдресМакетаВоВременномХранилище) Тогда
			Если ЭтоПечатнаяФорма Тогда
				Макет = ПодготовитьМакетДляЗаписи(Ложь, Отказ);
				смартСерверПривилегированный.ЗаписатьДополнительныеДанныеОбъекта(СсылкаМакета, Макет); 
			Иначе
				Макет = ТабличныйДокумент;
				смартСерверПривилегированный.ЗаписатьДополнительныеДанныеОбъекта(СсылкаМакета, Макет); 
			КонецЕсли;
		...
		Возврат АдресМакетаВоВременномХранилище

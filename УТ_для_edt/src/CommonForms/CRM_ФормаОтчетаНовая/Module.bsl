...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если МодульУправлениеДоступом <> Неопределено Тогда
		МодульУправлениеДоступом.ОграничитьВыводКлиентскойБазы(ЭтотОбъект, "ОтчетТабличныйДокумент");
	КонецЕсли;
	
	ПравоВывода = ПравоДоступа("Вывод", Метаданные) И Элементы.ОтчетТабличныйДокумент.Вывод <> ИспользованиеВывода.Запретить;
	Элементы.ГруппаОтправить.Доступность = ПравоВывода;
	...
	Если Параметры.Свойство("ЗагружатьПользовательскиеНастройки") Тогда
		ЗагружатьПользовательскиеНастройки = Параметры.ЗагружатьПользовательскиеНастройки;
	КонецЕсли;
	...
	// элемент формы добавлен для совместимости с типовой формой
...
Процедура ПредставлениеПериодаНажатие(Элемент)
	...
	ИначеЕсли НастройкиОтчета.РежимПериода="НаДату" Тогда
		ПоказатьВводДаты(Оповещение, Период.ДатаОкончания, НСтр("ru='На дату';en='As of'"), ЧастиДаты.Дата);
	КонецЕсли; 
...
Процедура СброситьНастройкиНажатие(Элемент)
	ЗагружатьПользовательскиеНастройки = Ложь;
...
Процедура ОтчетТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	Если ЗапретитьРасшифровкуОтчета() Тогда
		СтандартнаяОбработка = Ложь;
		Возврат;
	КонецЕсли;	
...
Процедура ОтчетТабличныйДокументОбработкаДополнительнойРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Данные.Поле) И НЕ ДоступноеПоле=Неопределено И ДоступноеПоле.ТипЗначения.Типы().Количество()>0 Тогда
		ДополнительныеДействия.Добавить(СформироватьПодменюОформить(Данные.Поле, Данные.Значение), НСтр("ru='Оформить';en='ApplyAppearance'"));
		...
		ПодменюСортировка.Добавить(НаправлениеСортировкиКомпоновкиДанных.Убыв, Данные.Поле+НСтр("ru=' по убыванию';en=' descending'"),, БиблиотекаКартинок.СортироватьСписокПоУбыванию); 
...
Функция СформироватьПодменюОформить(Поле, Значение)
	...
	НСтр("ru='Редактировать список быстрых настроек';en='Edit list of quick settings'"),, 
	БиблиотекаКартинок.УсловноеОформлениеКомпоновкиДанных);
	СписокКоманд.Добавить(
	"РучнаяНастройкаОформления", 
	НСтр("ru='Ручная настройка оформления';en='Manual setting of design'"),, 
...
Функция ПолучитьДанныеРасшифровки(Расшифровка)
	...
	Попытка                       
		Если Вариант.Отчет.Имя = "CRM_АнализПервичныхИнтересов" Тогда Возврат Неопределено КонецЕсли;
	Исключение
	КонецПопытки;
...
Процедура СвернутьРазвернутьПанельНастроек(Элемент)
	...
	Элементы.СкрытьПоказатьНастройки.Заголовок = ?(НовоеЗначениеВидимость, НСтр("ru='Скрыть настройки отчета';en='Hide Report Settings'"), НСтр("ru='Показать настройки отчета'"));
...
Процедура ОбновитьПредставлениеПериода()
	...
	ИначеЕсли НастройкиОтчета.РежимПериода="НаДату" Тогда
		Если НЕ ЗначениеЗаполнено(Период.ДатаОкончания) Тогда
			ПредставлениеПериода = НСтр("ru='актуальные данные';en='actual data'");
		Иначе
...
Процедура УстановитьПараметрыИОтборыРасшифровки(ОтчетОбъект)
	...
	Для Каждого КлючИЗначение Из ФормаПараметры.Отбор Цикл
		...
		ТекстОшибки = СтрШаблон(НСтр("ru='Не удалось установить фиксированный отбор ""%1"".';en='Failed to install fixed filter ""%1"".'"), Имя);
...
Процедура ФоновоеЗаданиеПроверитьНаКлиенте()
	...
	Если ФоновоеЗаданиеВыполнено() Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru='Отчет сформирован';en='Report generated'"), , Заголовок);
...
Функция ФоновоеЗаданиеЗапустить()
	...
	ПараметрыЗапуска.НаименованиеФоновогоЗадания = НСтр("ru='Варианты отчетов: Формирование отчета';en='Report options: Report generation'");
	...
	Иначе
		...
		ОтображениеСостояния.Текст                          = НСтр("ru='Отчет формируется...';en='Processing...'");
...
Процедура ОтметитьНеактуальным()
	...
	ОтображениеСостояния = Элементы.ОтчетТабличныйДокумент.ОтображениеСостояния;
	ОтображениеСостояния.Видимость = Истина;
	ОтображениеСостояния.Текст = НСтр("ru='Отчет не сформирован. Нажмите ""Сформировать"" (F5) для получения отчета.';en='Report is not generate. Click ""Generate"" (F5) to receive report.'");
	ОтображениеСостояния.ДополнительныйРежимОтображения = ДополнительныйРежимОтображения.Неактуальность;
...
Процедура Сформировать()
	...
	Если НастройкиОтчета.РежимПериода="ЗаПериод" Тогда
		Если ЗначениеЗаполнено(Период.ДатаНачала) Тогда
			ПредставлениеДляЗаголовка = НСтр("ru='за ';en='for '")+ПредставлениеПериода;
		Иначе
	...
	ИначеЕсли НастройкиОтчета.РежимПериода="НаДату" Тогда
		Если НЕ ЗначениеЗаполнено(Период.ДатаОкончания) Тогда
			ПредставлениеДляЗаголовка = НСтр("ru='текущий момент';en='current situation'");
		Иначе
			ПредставлениеДляЗаголовка = ПредставлениеПериода;
		КонецЕсли;
		Отчет.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("Заголовок", НастройкиОтчета.НаименованиеВарианта+НСтр("ru=' на ';en=' on '")+ПредставлениеДляЗаголовка);
	Иначе
		Отчет.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("Заголовок", НастройкиОтчета.НаименованиеВарианта);
	КонецЕсли;
	
	Если НЕ ЗагружатьПользовательскиеНастройки Тогда
...
Процедура ОбновитьПараметрыПериода(Интерактивно = Ложь)
	...
	Если НЕ ПараметрКонецПериода=Неопределено Тогда
		...
		ПараметрКонецПериода.Использование = ЗначениеЗаполнено(Период.ДатаОкончания);
	КонецЕсли; 
...
Процедура ЗаполнитьВариантыСерийТочекЗначений()
	...
	ДоступныеСерии.Добавить(Неопределено, НСтр("ru='Не использовать';en='Do not use'"));
	ДоступныеТочки.Добавить(Неопределено, НСтр("ru='Не использовать';en='Do not use'"));
...
Функция ДобавитьОписаниеГруппировки(Путь, НестандартныйОбработчик = Ложь, ТипГруппировки = Неопределено)
	...
	Поле = Отчет.КомпоновщикНастроек.Настройки.ДоступныеПоляГруппировок.НайтиПоле(Новый ПолеКомпоновкиДанных(Путь));
	Если Поле=Неопределено Тогда
		Возврат "";
	КонецЕсли; 
		...
		Если (ГруппировкаКолонокПериод И НЕ ИмяПоля=ПолеГруппировкиКолонок) ИЛИ (СравнениеПериод И НЕ ИмяПоля=ИмяСравнения) Тогда
			Если НЕ ПустаяСтрока(ИмяПоляПроверки) Тогда
				Если ГруппировкаКолонокПериод И НЕ ИмяПоля=ПолеГруппировкиКолонок Тогда
					...
					ОписаниеПоля = ОписаниеПоля+НСтр("ru=' в колонках отчета';en=' in report columns'");
			...
			Иначе
				...
				ОписаниеПоля = ОписаниеПоля+НСтр("ru=' в строках отчета';en=' in report rows'");
				Если ГруппировкаКолонокПериод И НЕ ИмяПоля=ПолеГруппировкиКолонок Тогда
			...
			НСтр("ru='Запрещено использование в строках и колонках различных полей-периодов. Предварительно отключите поле %1';en='Prohibited uses in strings and columns of different fields-periods. Disconnect field %1'"),
			ОписаниеПоля));
			Результат = Ложь;
		КонецЕсли; 
	КонецЦикла; 
		...
		Если (ТочкаПериод И НЕ ИмяПоля=ДиаграммаТочки) ИЛИ (СерияПериод И НЕ ИмяПоля=ДиаграммаСерии) Тогда
			Если НЕ ПустаяСтрока(ИмяПоляПроверки) Тогда
				Если ТочкаПериод И НЕ ИмяПоля=ДиаграммаТочки Тогда
					...
					ОписаниеПоля = ОписаниеПоля+НСтр("ru=' в диаграмме';en=' in chart'");
			...
			Иначе
				ОписаниеПоля = """"+?(ПустаяСтрока(Поле.Заголовок), Поле.Поле, Поле.Заголовок)+"""";
				ОписаниеПоля = ОписаниеПоля+НСтр("ru=' в структуре отчета';en=' in report structure'");
				Если ТочкаПериод И НЕ ИмяПоля=ДиаграммаТочки Тогда
					ДиаграммаТочки = Неопределено;
				КонецЕсли; 
			...
			КонецЕсли; 
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Запрещено использование в отчете и диаграмме различных полей-периодов. Предварительно отключите поле %1';en='It is prohibit to use in report and diagram different field periods. Disconnect field"
"%1'"),
			ОписаниеПоля));
			Результат = Ложь;
		КонецЕсли; 
...
&НаСервере
Функция ЗапретитьРасшифровкуОтчета()
	ОтчетОбъект = РеквизитФормыВЗначение("Отчет");
	Возврат ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ОтчетОбъект, "ЗапретитьРасшифровкуОтчета");
КонецФункции	
...
Процедура CRM_СохранитьРезультат(Команда)
	...
	КонтекстФормы.Вставить("Отчет", Отчет);
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтотОбъект, "ОтчетДанныеРасшифровки") Тогда
		КонтекстФормы.Вставить("ОтчетДанныеРасшифровки", ОтчетДанныеРасшифровки);
	КонецЕсли;
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтотОбъект, "ПредставлениеТекущегоВарианта") Тогда
		КонтекстФормы.Вставить("ПредставлениеТекущегоВарианта", ПредставлениеТекущегоВарианта);
	КонецЕсли;
	ОткрытьФорму("ОбщаяФорма.CRM_СохранениеЗагрузкаРезультатовОтчетов", КонтекстФормы, ЭтотОбъект,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры

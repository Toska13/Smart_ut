...
Функция СхемаКомпоновкиДанныхИзТаблицыЗначений(ТаблицаЗначений) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
		...
		Поле.ПараметрыРедактирования.УстановитьЗначениеПараметра("Маска", 
			смартОбщегоНазначения.ЗначениеВJSON(ДополнительныеПараметры));
	КонецЦикла;
...
Функция ЗапуститьФоновыйПоискВСпискеПолей(Форма)
	...
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, "смартКонструкторФормулСлужебный.ЗапуститьПоискВСпискеПолей", СтруктураФормы);
...
Процедура ОбновитьКоллекцииПолей(Форма, КоллекцииПолей, ИмяСпискаПолей = "ДоступныеПоля") Экспорт
	...
	НастройкиСписка = смартКонструкторФормулКлиентСервер.НастройкиСпискаПолей(Форма, ИмяСпискаПолей);
	...
	Для Каждого КоллекцияДоступныхПолей Из КоллекцииДоступныхПолей Цикл
		Для Каждого ОписаниеПоля Из КоллекцияДоступныхПолей.Элементы Цикл
			...
			Если ТипЗнч(ОписаниеПоля) = Тип("ДоступноеПолеОтбораКомпоновкиДанных") И ЗначениеЗаполнено(ОписаниеПоля.Маска)
				...
				ДополнительныеПараметры = смартОбщегоНазначения.JSONВЗначение(ОписаниеПоля.Маска);
			КонецЕсли;
			...
			Если Не ЗначениеЗаполнено(Реквизит.Картинка) Тогда
				...
				//	Реквизит.Картинка = БиблиотекаКартинок.смартШтампЭлектроннойПодписи;
				КонецЕсли;
			КонецЕсли;
...
Функция ИсточникиДоступныхПолей(ИсточникиДоступныхПолей, НастройкиСписка, Реквизит = Неопределено, УникальныйИдентификаторФормы = Неопределено)
	...
	МассивМетаданныхПеречислений = смартКонструкторФормулПовтИсп.МетаданныеПеречислений();
...
Функция СписокИсточниковДоступныхПолей(Форма, ИмяСпискаПолей)
	...
	ИмяСпискаИсточников = смартКонструкторФормулКлиентСервер.НастройкиСпискаПолей(Форма, ИмяСпискаПолей).ИмяСпискаИсточников;
...
Процедура РазвернутьРеквизит(ИдентификаторСтроки, ИмяСписка, Форма)
	...
	НастройкиСписка = смартКонструкторФормулКлиентСервер.НастройкиСпискаПолей(Форма, ИмяСписка);
	...
	Если ТипЗнч(ИсточникиДоступныхПолей) = Тип("Строка") Тогда
		...
		ИмяСпискаИсточников = смартКонструкторФормулКлиентСервер.НастройкиСпискаПолей(Форма, ИмяСписка).ИмяСпискаИсточников;
...
Функция КартинкаТипа(ОписаниеТипов)
	...
	Картинка = смартКонструкторФормулПовтИсп.КартинкаПоИмени("ТипНеопределено");
	Если ОписаниеТипов.Типы().Количество() = 1 Тогда
		Тип = ОписаниеТипов.Типы()[0];
		Если Тип = Тип("Число") Тогда
			Картинка = смартКонструкторФормулПовтИсп.КартинкаПоИмени("ТипЧисло");
		ИначеЕсли Тип = Тип("Дата") Тогда
			Картинка = смартКонструкторФормулПовтИсп.КартинкаПоИмени("ТипДата");
		ИначеЕсли Тип = Тип("Булево") Тогда
			Картинка = смартКонструкторФормулПовтИсп.КартинкаПоИмени("ТипБулево");
		ИначеЕсли Тип = Тип("Строка") Тогда
			Картинка = смартКонструкторФормулПовтИсп.КартинкаПоИмени("ТипСтрока");
		ИначеЕсли ОбщегоНазначения.ЭтоСсылка(Тип) Тогда
			Картинка = смартКонструкторФормулПовтИсп.КартинкаПоИмени("ТипСсылка");
		ИначеЕсли Тип = Тип("УникальныйИдентификатор") Тогда
			Картинка = смартКонструкторФормулПовтИсп.КартинкаПоИмени("ТипИдентификатор");
		КонецЕсли;
	ИначеЕсли ОписаниеТипов.Типы().Количество() > 1 Тогда
		Картинка = смартКонструкторФормулПовтИсп.КартинкаПоИмени("ТипСоставнойОсновной");
	КонецЕсли;

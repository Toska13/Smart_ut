...
// * ТаблицаРеквизитовВидовЗапасовПоУмолчанию - ТаблицаЗначений - см. ЗапасыСервер.ПустаяТаблицаРеквизитовВидовЗапасовПоУмолчанию.
//
...
//
Процедура ПроверитьЗаполнениеНомеровГТД(ДокументОбъект, Отказ, ИмяТЧ = "Товары", ХозяйственнаяОперация = Неопределено) Экспорт
	...
	|			И НЕ ЕСТЬNULL(ТаблицаТовары.Номенклатура.КодТНВЭД.ПрослеживаемыйТовар, ЛОЖЬ))
	|		ИЛИ (ЕСТЬNULL(ТаблицаТовары.Номенклатура.КодТНВЭД.ПрослеживаемыйТовар, ЛОЖЬ)
...
Процедура ОтразитьТоварыКОформлениюДокументовИмпорта(ДополнительныеСвойства, Движения, Отказ) Экспорт

	Таблица = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаТоварыКОформлениюДокументовИмпорта;
	
	Если Отказ ИЛИ Таблица.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;

	Движения.ТоварыКОформлениюДокументовИмпорта.Записывать = Истина;
	Движения.ТоварыКОформлениюДокументовИмпорта.Загрузить(Таблица);
...
// Возвращает пустую типизированную таблицу значений, колонки которой описывают значения реквизитов видов запасов
// по умолчанию.
//
// Возвращаемое значение:
//	ТаблицаЗначений - таблица, которая содержит следующие колонки:
...
//		* ВидЗапасов - СправочникСсылка.ВидыЗапасов - вид запасов номенклатуры.
//		* НомерГТД - СправочникСсылка.НомераГТД - номер ГТД номенклатуры.
//		* ВладелецТовара - СправочникСсылка.Организации, СправочникСсылка.Партнеры - владелец запаса номенклатуры.
//		* ТипЗапасов - ПеречислениеСсылка.ТипыЗапасов - тип запаса номенклатуры.
//		* ТипЗапасовРезервы - ПеречислениеСсылка.ТипыЗапасов - тип запаса номенклатуры резерва.
//		* Контрагент - СправочникСсылка.Организации, СправочникСсылка.Контрагенты - контрагент организации.
//		* Договор - СправочникСсылка.ДоговорыМеждуОрганизациями, СправочникСсылка.ДоговорыКонтрагентов - договор документа.
//
...
Функция ЗапросРаспределенияРезервовПоТаблицеОстатков(МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов)
	...
	|		ПО НЕ СпрВидыЗапасов.ЭтоДубль
	|		И НЕ СпрВидыЗапасов.Устаревший
	|		И СпрВидыЗапасов.Организация = Остатки.Организация
	|		И СпрВидыЗапасов.ГруппаФинансовогоУчета = Остатки.ВидЗапасов.ГруппаФинансовогоУчета
	|	ГДЕ
	|		Остатки.КоличествоОстаток < 0
	|		И ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	|			ИЗ
	|				ТаблицаРеквизитовВидовЗапасовПоУмолчанию КАК ТаблицаРеквизитовВидовЗапасовПоУмолчанию
	|			ГДЕ
	|				ТаблицаРеквизитовВидовЗапасовПоУмолчанию.АналитикаУчетаНоменклатуры = Остатки.АналитикаУчетаНоменклатуры
	|				И ТаблицаРеквизитовВидовЗапасовПоУмолчанию.ВладелецТовара = СпрВидыЗапасов.ВладелецТовара
	|				И ТаблицаРеквизитовВидовЗапасовПоУмолчанию.ТипЗапасовРезервы = СпрВидыЗапасов.ТипЗапасов
	|				И ТаблицаРеквизитовВидовЗапасовПоУмолчанию.Контрагент = СпрВидыЗапасов.Контрагент
	|				И ТаблицаРеквизитовВидовЗапасовПоУмолчанию.Договор = СпрВидыЗапасов.Договор)
	|		) КАК Остатки
	|		
...
Процедура ДозаполнитьВидыЗапасовВОтчетахПоКомиссииИнтеркампани(ДокументОбъект,
	...
	|			ТОГДА -Остатки.КоличествоПоРНПТОстаток
	|		ИНАЧЕ Остатки.КоличествоПоРНПТОстаток
	|	КОНЕЦ                               КАК КоличествоПоРНПТ
	|ИЗ
	|	ТаблицаОстатков КАК Остатки
	...
	Для Каждого СтрокаОстатка Из ТаблицаОстатков Цикл
		...
		Для Каждого СтрокаЗапасов Из ДокументОбъект.ВидыЗапасов.НайтиСтроки(СтруктураПоиска) Цикл
			...
			Если СтрокаОстатка.Знак <> Знак
				Или СтрокаОстатка.Количество >= Количество
				Или (СтрокаОстатка.Количество < Количество
					И Знак = -1) Тогда
			...
			Если СтрокаТаблицы.Количество <> 0 Тогда
				...
				ТекстСообщения = СтрЗаменить(ТекстСообщения,
											...
											НоменклатураКлиентСервер.ПредставлениеНоменклатуры(
												СтрокаТаблицы.АналитикаУчетаНоменклатуры.Номенклатура,
												СтрокаТаблицы.АналитикаУчетаНоменклатуры.Характеристика));
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Количество%", СтрокаТаблицы.Количество);
				ТекстСообщения = СтрЗаменить(ТекстСообщения,
											"%ЕдиницаИзмерения%",
											СтрокаТаблицы.АналитикаУчетаНоменклатуры.Номенклатура.ЕдиницаИзмерения);
...
Процедура ЗаполнитьВидыЗапасовПоТаблицеОстатковКомиссионныхТоваровИнтеркампани(ДокументОбъект,
	...
	СтруктураПоиска = Новый Структура("АналитикаУчетаНоменклатуры, НомерГТД, Покупатель, Знак,
	...
	Пока ВыборкаОстаткиПоСтрокам.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			ИначеЕсли (КоличествоТовара > 0 И КоличествоОстаток > 0)
				...
				СтруктураДанныхСтроки.СуммаНДС					= ?(СуммаТовараСНДС <> 0,
																	?(СуммаНДСОстаток <> 0
																		И КоличествоОстатокВсего - Количество = 0,
																		СуммаНДСОстаток,
																		Окр(СуммаСНДС * СуммаНДСТовара / СуммаТовараСНДС, 2, 1)),
																	0);
...
Процедура ТаблицаОстатковКомиссионныхТоваровИнтеркампани(ДокументОбъект, МенеджерВременныхТаблиц)
	...
	|				ТОГДА ВЫБОР
	|						КОГДА &ЭтоОтчетОСписании
	|							ТОГДА ТоварыКОформлению.КоличествоСписаноКОформлениюПоРНПТ
	|						ИНАЧЕ ТоварыКОформлению.КоличествоКОформлениюПоРНПТ
	|					КОНЕЦ
	|			ИНАЧЕ ВЫБОР
	|					КОГДА ТоварыКОформлению.Регистратор = &ТекущийОтчет
	|						ТОГДА 0
	...
	Запрос.УстановитьПараметр("ЭтоОтчетОСписании",
							ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.ОтчетПоКомиссииМеждуОрганизациямиОСписании"));
...
Процедура ТаблицаОстатковТоваровПереданныхНаКомиссию(ДокументОбъект, ПараметрыЗаполненияВидовЗапасов, МенеджерВременныхТаблиц, ДатаОстатков)
	...
	|				ТОГДА Товары.КоличествоПоРНПТОстаток
	|			ИНАЧЕ 0
	|		КОНЕЦ КАК КоличествоПоРНПТ,
	|		ВЫБОР
...
Процедура ТаблицаОстатковКОформлениюОтчетаКомитенту(ДокументОбъект, МенеджерВременныхТаблиц, ДатаОстатков)
	...
	|				ТОГДА 0
	|			ИНАЧЕ ТоварыКОформлению.КоличествоКОформлениюПоРНПТОстаток
	|		КОНЕЦ											КАК КоличествоПоРНПТ,
	...
	|		Запасы.КоличествоПоРНПТ				КАК КоличествоПоРНПТ,
...
Процедура ТаблицаОстатковКОформлениюОтчетаКомитентуОСписании(ДокументОбъект, МенеджерВременныхТаблиц)
	...
	|					ИЛИ НАЧАЛОПЕРИОДА(ТоварыКОформлению.Период, МЕСЯЦ) < &ДатаНачалаУчетаПрослеживаемыхИмпортныхТоваров
	|				ТОГДА 0
	|			ИНАЧЕ ТоварыКОформлению.КоличествоСписаноКОформлениюПоРНПТ
	|		КОНЕЦ КАК КоличествоПоРНПТ,
	|		0 КАК СуммаВыручки,
	|		ДатыПередачиТоваровНаКомиссию.ДатаПередачи КАК ДатаПоступления
	|	ИЗ
	...
	|		ТоварыКОформлению.КоличествоСписаноКОформлениюПоРНПТПриход
	|			- ТоварыКОформлению.КоличествоСписаноКОформлениюПоРНПТРасход КАК КоличествоПоРНПТ,
	|		0 КАК СуммаВыручки,
	|		ДатыПередачиТоваровНаКомиссию.ДатаПередачи КАК ДатаПоступления
	|	ИЗ

...
//	ЭтоРасчетыСКлиентами - Булево - Переоценку необходимо выполнить только по расчетам с клиентами.
//
Процедура ВыполнитьПереоценкуЗаПериод(МассивОрганизаций, Знач НачалоПериода, Знач КонецПериода = Неопределено, ЭтоРасчетыСКлиентами = Неопределено) Экспорт
	...
	ДниПереоценки = ДниПереоценки(МассивОрганизаций, НачалоПериода, КонецПериода, ЭтоРасчетыСКлиентами);
	Для Каждого День Из ДниПереоценки Цикл
		ВыполнитьПереоценкуНаДату(МассивОрганизаций, День.Дата, ЭтоРасчетыСКлиентами);
		Если КонецДня(День.Дата) = КонецМесяца(День.Дата) Тогда
			ВыполнитьПереоценкуНаДату(МассивОрганизаций, КонецДня(День.Дата), ЭтоРасчетыСКлиентами);
		КонецЕсли;
	КонецЦикла;
...
Функция ДниПереоценки(МассивОрганизаций, НачалоПериода, КонецПериода, ЭтоРасчетыСКлиентами = Неопределено)
	...
	Запрос.УстановитьПараметр("ВалютаУправленческогоУчета",  Константы.ВалютаУправленческогоУчета.Получить());
	Запрос.УстановитьПараметр("ВалютаРегламентированногоУчета",  Константы.ВалютаРегламентированногоУчета.Получить());
	Запрос.УстановитьПараметр("ПереоценкаПоДням",  Константы.ПереоцениватьВалютныеСредстваПоДням.Получить());
	Запрос.УстановитьПараметр("РасчетыСКлиентами", ЭтоРасчетыСКлиентами = Неопределено ИЛИ ЭтоРасчетыСКлиентами);
	Запрос.УстановитьПараметр("РасчетыСПоставщиками", ЭтоРасчетыСКлиентами = Неопределено ИЛИ НЕ ЭтоРасчетыСКлиентами);
...
Функция СуммаСписания(Курс, СуммаВВалюте, СуммаВзаиморасчетов, СуммаКСписанию)
	...
	Иначе
		Если Курс = 0 Тогда
			Возврат Окр(СуммаВВалюте/СуммаВзаиморасчетов,4) * СуммаКСписанию;
		Иначе

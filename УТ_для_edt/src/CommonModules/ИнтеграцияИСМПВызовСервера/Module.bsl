...
Функция НастройкиОтветственногоЗаАктуализациюТокеновАвторизации() Экспорт
	...
	Возврат Справочники.ОтветственныеЗаАктуализациюТокеновАвторизацииИСМП.НастройкиОтветственногоЗаАктуализациюТокеновАвторизации();
...
// 
// Возвращаемое значение:
//  Массив - Массив Из - Структура со свойствами:
//   * Организация - ОпределяемыйТип.Организация - организация.
...
Функция ПолучитьНапоминанияОтветственномуЗаАктуализациюТокеновАвторизации(Настройки) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ИмяПараметраСеанса", "ДанныеКлючаСессииИСМП");
	Запрос.УстановитьПараметр("Пользователь", Пользователи.ТекущийПользователь());
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	Данные КАК ДанныеКлючаСессии
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	НапоминанияПользователяИСМП.СрокНапоминания
	|ИЗ
	|	РегистрСведений.НапоминанияПользователяИСМП КАК НапоминанияПользователяИСМП
	|ГДЕ
	|	НапоминанияПользователяИСМП.Пользователь = &Пользователь";
	...
	Если Выборка.Следующий() Тогда
		ДанныеКлючаСессии = Выборка.ДанныеКлючаСессии.Получить();
	Иначе
	...
	Для Каждого Настройка Из Настройки Цикл
		...
		Если ДанныеКлючаСессии = Неопределено
			Или ДанныеКлючаСессии[Организация] = Неопределено Тогда
			ДатаДействияТокена = ПустаяДата;
		Иначе
			ДатаДействияТокена = ДанныеКлючаСессии[Организация].ДействуетДо;
		КонецЕсли;
...
Функция КонтролироватьСтатусыКодовМаркировки(ВидПродукции = Неопределено, ВидОперации = Неопределено) Экспорт
	...
	Если Не ИнтеграцияИСМПКлиентСерверПовтИсп.ЗапрашиватьДанныеСервиса() Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	ОперацияИсключена = ОперацияИсключена(ОперацииКонтроля, ВидПродукции);
...
Функция КонтролироватьСтатусыКодовМаркировкиВРознице(ВидПродукции = Неопределено, ВидОперации = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ВидПродукции) Тогда
		ВидыПродукцииКонтроля = Новый Массив();
		ВидыПродукцииКонтроля.Добавить(ВидПродукции);
	Иначе
		ВидыПродукцииКонтроля = ИнтеграцияИСМПКлиентСерверПовтИсп.УчитываемыеВидыМаркируемойПродукции();
	КонецЕсли;
...
Функция НастройкиСканированияКодовМаркировки() Экспорт
	...
	Результат.Вставить("ПараметрыКонтроляСредствамиККТ",  НастройкаПараметровСканированияСлужебныйКлиентСервер.НоваяГруппаПараметровНастройки());
	...
	Результат.Вставить("УпрощеннаяОбработкаКодовМаркировки", Ложь);
...
Функция ОперацияИсключена(ВидыОпераций, ВидПродукции = Неопределено)
	...
	Для Каждого ВидПродукции Из ВидыПродукции Цикл
		...
		Для Каждого Операция Из ВидыОпераций Цикл
			...
			КонтрольСтатусовОтключен = НастройкаПараметровСканированияСлужебныйКлиентСерверПовтИсп.ОперацияИсключенаВНастройкахСканирования(
				ВидПродукции,
				Операция,
				ПараметрыКонтроля.ПараметрыКонтроляСтатусов);
			
			КонтрольВладельцевОтключен = НастройкаПараметровСканированияСлужебныйКлиентСерверПовтИсп.ОперацияИсключенаВНастройкахСканирования(
				ВидПродукции,
				Операция,
				ПараметрыКонтроля.ПараметрыКонтроляВладельцев);
			
			Если Не КонтрольСтатусовОтключен
				Или Не КонтрольВладельцевОтключен Тогда

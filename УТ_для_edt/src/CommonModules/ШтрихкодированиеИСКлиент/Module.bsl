...
Процедура ЗавершитьОбработкуШтрихкода(ПараметрыЗавершенияОбработкиШтрихкода) Экспорт
	...
	Если ПрисутствуетАлкогольнаяПродукция(ПараметрыСканирования.ДопустимыеВидыПродукции)
		И ПрисутствуетАлкогольнаяПродукция(РезультатОбработки.ВидыПродукции) Тогда
...
Процедура УточнитьДанныеУПользователя(ФормаВладелец, ПараметрыОткрытияФормы, ОповещениеПоЗавершениюУточненияДанных) Экспорт
	...
	ИначеЕсли ПараметрыОткрытияФормы.Операция = "ОткрытьФормуУточненияДанных" Тогда
		...
		Если ЗначениеЗаполнено(ПараметрыОткрытияФормы.КодМаркировкиДляУточнения) Тогда
			...
			КонецЕсли;
		Иначе //из формы ввода кода маркировки
			ДополнитьПараметрыУточненияИзФормыИсточника(ФормаВладелец, ПараметрыОткрытияФормы.ДанныеДляУточненияСведенийПользователя);
		КонецЕсли;
...
Процедура УточненияДанныхЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если ПараметрыСканирования.Свойство("ККТФФД12ИСМП")
		И Не РезультатОбработкиШтрихкода.ПроверкаСредствамиККТЗавершена
		И Не РезультатОбработкиШтрихкода.ДанныеШтрихкода.ЭтоШтрихкодНоменклатуры Тогда
		
		Если РезультатОбработкиШтрихкода.ТребуетсяПроверкаСредствамиККТ
...
Процедура ПодготовитьДанныеДляЗагрузкиОтчетПроизводственнойЛинии(Штрихкоды) Экспорт
	
	Для Каждого ЭлементДанных Из Штрихкоды Цикл
		
		Если ЭлементДанных.Свойство("ФорматBase64") Тогда
			ФорматBase64 = ЭлементДанных.ФорматBase64;
		Иначе
			ЭлементДанных.Вставить("ФорматBase64", Ложь);
			ФорматBase64 = Ложь;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ЭлементДанных.Штрихкод)
			И РазборКодаМаркировкиИССлужебныйКлиентСервер.НайденНедопустимыйСимволXML(ЭлементДанных.Штрихкод) Тогда
			
			ЭлементДанных.ФорматBase64 = Истина;
			ЭлементДанных.Штрихкод = ШтрихкодированиеИСКлиентСервер.ШтрихкодВBase64(ЭлементДанных.Штрихкод);
			
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ЭлементДанных.ШтрихкодУпаковки)
			И РазборКодаМаркировкиИССлужебныйКлиентСервер.НайденНедопустимыйСимволXML(ЭлементДанных.ШтрихкодУпаковки) Тогда
			
			ЭлементДанных.ФорматBase64 = Истина;
			ЭлементДанных.ШтрихкодУпаковки = ШтрихкодированиеИСКлиентСервер.ШтрихкодВBase64(ЭлементДанных.ШтрихкодУпаковки);
...
Функция ПреобразоватьВПараметрыОткрытияФормыУточнения(КодМаркировкиДляУточнения)
	...
	ДанныеДляУточнения.СоставКодаМаркировки = ДанныеШтрихкода.СоставКодаМаркировки;

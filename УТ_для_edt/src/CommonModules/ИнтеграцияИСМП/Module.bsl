...
Функция МассивЗначенийGTINДляВыбора(ТекущиеДанные, Объект, ВключатьGTINНаОстатки = Ложь) Экспорт
	...
	Пока ВыборкаВидУпаковки.Следующий() Цикл
		ДанныеВидаУпаковки = Новый Структура();
		ДанныеВидаУпаковки.Вставить("ВидУпаковки", ВыборкаВидУпаковки.ВидУпаковки);
		ДанныеВидаУпаковки.Вставить("Коэффициент", ВыборкаВидУпаковки.Коэффициент);
		СоответствиеВидуУпаковки.Вставить(ВыборкаВидУпаковки.GTIN, ДанныеВидаУпаковки);
	КонецЦикла;
	...
	Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
		...
		ДанныеПоGTIN = СоответствиеВидуУпаковки.Получить(GTIN);
		
		Если ДанныеПоGTIN <> Неопределено И ЗначениеЗаполнено(ДанныеПоGTIN.ВидУпаковки) Тогда
			Если ДанныеПоGTIN.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая
				Или ЗначениеЗаполнено(ВидУпаковки)
				И ДанныеПоGTIN.ВидУпаковки <> ВидУпаковки Тогда
				Продолжить;
			КонецЕсли;
			Если ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая
				И ДанныеПоGTIN.Коэффициент = 1 Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
...
//
//Возвращаемое значение:
//   СписокЗначений - ссылки на организации:
...
Функция ПолучитьДанныеСопоставленногоКлассификатораТНВЭД(КодТНВЭД) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("НаименованиеПолное");
	Результат.Вставить("ЭлементСправочника");
	ИнтеграцияИСМППереопределяемый.ПриОпределенииСопоставленногоКлассификатораТНВЭД(КодТНВЭД, Результат);
	Возврат Результат;

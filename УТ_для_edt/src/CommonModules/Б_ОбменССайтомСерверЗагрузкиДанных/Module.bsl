...
Функция ОбработатьКонтейнеры(ДанныеЗаказов, ПараметрыОбмена) Экспорт
...
Функция ПолучитьКонтрагентаПоКритерию(ПараметрыОбмена, Критерий, ИнформацияОКонтрагенте)
	...
	Если Критерий = "Email" ИЛИ Критерий = "Телефон" тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровИКонтрагентов") тогда
			Запрос.Текст =
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	СправочникКонтактнаяИнформация.Ссылка КАК Контрагент
			|ИЗ
			|	Справочник.Контрагенты.КонтактнаяИнформация КАК СправочникКонтактнаяИнформация
			|ГДЕ
			|	СправочникКонтактнаяИнформация.Тип = &ТипКИ
			|	И СправочникКонтактнаяИнформация.Представление = &ЗначениеКИ"; 
		Иначе		
			Запрос.Текст =
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			...
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	СправочникКонтактнаяИнформация.Ссылка КАК Партнер
			|ПОМЕСТИТЬ ВТ_Партнеры
			|ИЗ
			|	Справочник.Партнеры.КонтактнаяИнформация КАК СправочникКонтактнаяИнформация
			|ГДЕ
			|	СправочникКонтактнаяИнформация.Тип = &ТипКИ
			|	И СправочникКонтактнаяИнформация.Представление = &ЗначениеКИ
			|;
			...
			|		ПО ВТ_Контрагенты.Партнер = ВТ_Партнеры.Партнер"
		КонецЕсли;
		...
		Если Критерий = "Email" тогда
			...
			Запрос.УстановитьПараметр("ЗначениеКИ", лЗначениеПоиска);
	...
	Иначе
		...
		Если Критерий = "ИНН" ИЛИ Критерий = "ИНН+КПП" тогда

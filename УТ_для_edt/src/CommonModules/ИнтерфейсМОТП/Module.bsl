...
Функция ДеревоУпаковок(Значение, Детализация = Неопределено, ПараметрыСканирования = Неопределено) Экспорт
	...
	Для Каждого НаборПакета Из ПараметрыНабораПакетов.НаборыПакетов Цикл
		...
		Для Каждого ПакетКодовМаркировки Из НаборПакета.ПакетыКодовМаркировки Цикл
			...
			Если НаборПакета.ПрисутствуетТабачнаяПродукция
				Или ИнтеграцияИСПовтИсп.ЭтоПродукцияМОТП(НаборПакета.ВидПродукцииПодсистемы) Тогда
				РезультатЗапросаСтатусовКодовМаркировок = ЗапроситьСтатусыКодовМаркировкиПакетно(
					ПакетКодовМаркировки,
					СтатусыКодовМаркировки,
					ПараметрыДерева.Организация,, НастройкиРазбораКодаМаркировки);
			Иначе
				РезультатЗапросаСтатусовКодовМаркировок = ИнтерфейсИСМП.СтатусыКодовМаркировкиПакетно(
					ПакетКодовМаркировки,
					СтатусыКодовМаркировки,
					ПараметрыДерева.Организация,
					Истина, НастройкиРазбораКодаМаркировки); // Включать статусы вложенных кодов
			КонецЕсли;
...
Функция ДанныеПродукцииПоШтрихкодуEAN(Значение, ВидПродукции = Неопределено, Организация = Неопределено) Экспорт
	...
	РезультатЗапросаСпискаПродукции  = ЗапроситьКарточкуПродукции(МассивGTIN, ВидПродукции, Организация);
...
//		* ИННВладельца - Строка                                        - ИНН владельца кода маркировки.
// * ТекстОшибки - Строка - Текст сообщения об ошибке.
Функция ЗапроситьСтатусыКодовМаркировкиПакетно(МассивИсходныхСтрок, СтатусыКодовМаркировкиКеш = Неопределено, Организация = Неопределено, ПолеИсточник = Неопределено, НастройкиРазбора = Неопределено) Экспорт
...
//		** ВыполненаСинхронизацияGS1 - Булево - Признак выполненной синхронизации с GS1.
//  * ТекстОшибки - Строка - Текст сообщения об ошибке.
Функция ЗапроситьКарточкуПродукции(МассивGTIN, ВидПродукции, Организация = Неопределено) Экспорт

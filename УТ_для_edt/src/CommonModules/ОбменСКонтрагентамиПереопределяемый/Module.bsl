...
// Определяет имя реквизита владельца справочника НоменклатураПартнера.
//
// Параметры:
	...
	Если ВидДокумента <> "ДоговорыКонтрагентов" Тогда
		...
		Иначе
			...
			Если ЗначениеЗаполнено(ТаблицаТоваров.Итог("СуммаНДС")) Тогда
				СуммаВсегоНДС = ТаблицаТоваров.Итог("СуммаНДС") /100 * РеквизитыШапки.ПроцентОплаты;
			КонецЕсли;
...
Процедура ЗаполнитьДанныеПоОтчетуОПродажахКомиссионногоТовара(СсылкаНаОбъект, СтруктураЭД, ДеревоДокумента, Отказ) Экспорт
	...
	ТаблицаСДаннымиПрослеживаемости = ЭлектронноеВзаимодействиеУТ.ПолучитьДанныеПоПрослеживаемостиДляКомиссии(СсылкаНаОбъект, ТипЗнч(СсылкаНаОбъект));
...
Процедура ПолучитьРеквизитыТовара(РеквизитыНоменклатуры, СтруктураВозврата, ИД = Неопределено) Экспорт
	...
	//++ НЕ ГОСИС
	НоменклатураПартнера = Неопределено;
	Если РеквизитыНоменклатуры.Свойство("НоменклатураПартнера",НоменклатураПартнера)Тогда
		СтруктураВозврата.Номенклатура = НоменклатураПартнера.Номенклатура;
		СтруктураВозврата.Характеристика = НоменклатураПартнера.Характеристика;
		СтруктураВозврата.Упаковка = НоменклатураПартнера.Упаковка;
	КонецЕсли;
	//-- НЕ ГОСИС
...
Процедура СоответствиеИсходящихВидовЭДДокументамИБ(СоответствиеВидовЭДДокументамИБ) Экспорт 
	...
	СоответствиеВидовЭДДокументамИБ.Вставить(Перечисления.ВидыЭД.КаталогТоваров,         					НСтр("ru = 'Каталог товаров (формируется из Настройки отправки)'"));
	//-- НЕ ГОСИС
...
//                   нужно сформировать сообщения пользователю при необходимости самостоятельно.
//
Процедура ЗаполнитьДанныеУКД0ЗакупкаИнформацияПродавцаФНС_2020(СсылкаНаОбъект, СтруктураЭД, ДеревоДанных, Отказ) Экспорт
...
//                   нужно сформировать сообщения пользователю при необходимости самостоятельно.
//
Процедура ЗаполнитьДанныеУКД0ИнформацияПродавцаФНС_2020(СсылкаНаОбъект, СтруктураЭД, ДеревоДанных, Отказ) Экспорт

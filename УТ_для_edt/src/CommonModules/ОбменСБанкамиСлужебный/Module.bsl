...
Функция НастройкаОбмена(Организация, Банк, ТолькоДействительная = Истина) Экспорт
	...
	Если Выборка.Следующий()
		И (Не ТолькоДействительная
			Или (ТолькоДействительная И (Не Выборка.Недействительна И Не Выборка.ПометкаУдаления))) Тогда
...
// Параметры:
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - текущая настройка обмена.
//  Идентификаторы - Массив из Строка - уникальные идентификаторы сообщений обмена.
// 
// Возвращаемое значение:
//  ТаблицаЗначений - найденные сообщения обмена.
//
Функция СообщенияОбменаПоИдентификаторам(НастройкаОбмена, Идентификаторы)
	...
	|ГДЕ
	|	СообщениеОбменСБанками.НастройкаОбмена = &НастройкаОбмена
	|	И СообщениеОбменСБанками.Идентификатор В (&Идентификаторы)";
	Запрос.УстановитьПараметр("НастройкаОбмена", НастройкаОбмена);
...
Процедура СохранитьШтампыБанка(НастройкаОбмена, Выписка)
	...
	Для Каждого СтрокаТЧ Из Выписка.Операции Цикл
		Если СтрокаТЧ.НаправлениеПлатежа = "1" И ЗначениеЗаполнено(СтрокаТЧ.ИдентификаторДокумента) Тогда
			ИдентификаторыДокументов.Добавить(СтрокаТЧ.ИдентификаторДокумента);
		КонецЕсли;
	КонецЦикла;
	
	ПлатежныеДокументы = СообщенияОбменаПоИдентификаторам(НастройкаОбмена, ИдентификаторыДокументов);
	
	Для Каждого Элемент Из ПлатежныеДокументы Цикл
		СтрокаТЧ = Выписка.Операции.Найти(Элемент.Идентификатор, "ИдентификаторДокумента");
		...
		ОбменСБанкамиСлужебныйВызовСервера.ИзменитьСообщениеОбмена(Элемент.СообщениеОбмена, СтруктураИзменения);
	КонецЦикла;
...
Процедура ПрочитатьВыпискуСбербанк(ЭД, ТаблицаВыписок)
	...
	Если ЗначениеСвойстваXDTO(ЭД, "Docs") <> Неопределено Тогда
		...
		Для Каждого Операция Из Операции Цикл
			...
			НовСтрока.ДатаДокумента = XMLЗначение(Тип("Дата"), Сред(Операция.docDate, 1, 19)); // без часового пояса

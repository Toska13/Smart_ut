...
Процедура ВыполнитьУдалениеЧековККМ(ПараметрыЗадания, АдресХранилища = "") Экспорт
	...
	Запрос = Новый Запрос(
		...
		|	ВозвратПодарочныхСертификатов.КассоваяСмена КАК КассоваяСмена
		|ИЗ
		|	Документ.ВозвратПодарочныхСертификатов КАК ВозвратПодарочныхСертификатов
		|ГДЕ
		|	ВозвратПодарочныхСертификатов.КассоваяСмена В(&КассоваяСмена)
		|	И ВозвратПодарочныхСертификатов.КассоваяСмена.СтатусРегламентныхОпераций = 
		|		ЗНАЧЕНИЕ(Перечисление.СтатусыРегламентныхОперацийКассовойСмены.СозданОтчетЧекиЗаархивированы)
		|	
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|ГДЕ
		|	РеализацияПодарочныхСертификатов.КассоваяСмена В(&КассоваяСмена)
		|	И РеализацияПодарочныхСертификатов.КассоваяСмена.СтатусРегламентныхОпераций = 
		|		ЗНАЧЕНИЕ(Перечисление.СтатусыРегламентныхОперацийКассовойСмены.СозданОтчетЧекиЗаархивированы)
		|	
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		Попытка
			...
			Пока ВыборкаДокументов.Следующий() Цикл
				...
				ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ОтчетОРозничныхПродажах")
					ИЛИ ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ВозвратПодарочныхСертификатов")
					ИЛИ ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.РеализацияПодарочныхСертификатов") Тогда
...
Функция ПараметрыРегистрацииУстройства(Знач ИдентификаторУстройства)
	...
	Возврат Справочники.ПодключаемоеОборудование.ПолучитьПараметрыРегистрацииУстройства(ИдентификаторУстройства);

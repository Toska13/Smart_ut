...
Процедура ОбработкаПроведения(Объект, Отказ, РежимПроведения) Экспорт
	...
	УчетПрослеживаемыхТоваровЛокализация.ОтразитьПрослеживаемыеТоварыОтгруженныеВЕАЭС(ДополнительныеСвойства, Движения, Отказ);
	//-- Локализация
...
Функция ПолучитьДанныеДляПечатнойФормыУПД(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	УчетНДСРФ.ДополнитьЗапросПолученияДанныхДляПечатиУПД(Запрос, "Дата,Контрагент");
	...
	ПродажиСервер.ПоместитьВременнуюТаблицуДанныхПокупателя(МенеджерВременныхТаблиц);
	ОтветственныеЛицаСервер.СформироватьВременнуюТаблицуОтветственныхЛицДокументов(МассивОбъектов, МенеджерВременныхТаблиц);
	...
	|
	|	ДанныеПокупателя.КПППокупателя КАК КПППокупателя,
	|
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеПокупателя КАК ДанныеПокупателя
	|		ПО ДанныеДокумента.Ссылка = ДанныеПокупателя.Ссылка
	...
	|	ВЫБОР
	|		КОГДА &ВыводитьБазовыеЕдиницыИзмерения
	|			ТОГДА ВЫБОР КОГДА НЕ ТаблицаДокумента.Количество = 0 ТОГДА ТаблицаДокумента.СуммаБезНДС / ТаблицаДокумента.Количество ИНАЧЕ 0 КОНЕЦ
	|		ИНАЧЕ ВЫБОР КОГДА НЕ ТаблицаДокумента.КоличествоУпаковок = 0 ТОГДА ТаблицаДокумента.СуммаБезНДС / ТаблицаДокумента.КоличествоУпаковок ИНАЧЕ 0 КОНЕЦ
	|	КОНЕЦ КАК Цена,
...
Процедура ДополнитьТекстыЗапросовПроведения(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	ТекстЗапросаТаблицаПрослеживаемыеТоварыОтгруженныеВЕАЭС(Запрос, ТекстыЗапроса, Регистры);		
...
Функция ТекстЗапросаТаблицаПрослеживаемыеТоварыОтгруженныеВЕАЭС(Запрос, ТекстыЗапроса, Регистры)
	...
	Если НЕ ПроведениеСерверУТ.ЕстьТаблицаЗапроса("ВтТаблицаВидыЗапасов", ТекстыЗапроса) Тогда
		Документы.РеализацияТоваровУслуг.ТекстЗапросаВтТаблицаВидыЗапасовДекоратор(Запрос, ТекстыЗапроса);
	КонецЕсли; 
	
	Если Не ПроведениеСерверУТ.ЕстьТаблицаЗапроса("ВтКурсыВалют", ТекстыЗапроса) Тогда
		Документы.РеализацияТоваровУслуг.ТекстЗапросаВтКурсыВалютДекоратор(Запрос, ТекстыЗапроса);
	КонецЕсли;
	...
	|	ТаблицаВидыЗапасов.СуммаСНДС КАК Сумма
	|ИЗ

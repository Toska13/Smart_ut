...
//
Функция ЗаменитьИдентификаторыКартинокНаПутьКФайлам(ТекстHTML,ТаблицаФайлов,Кодировка = Неопределено, ОбработатьКартинки = Ложь, ИдентификаторФормы = Неопределено) Экспорт
...
Функция ЗаполнитьПредставлениеРешенияДляСписка(Решение, УникальныйИдентификатор, ОткрытаИзДругойФормы = Ложь) Экспорт
	...
	ТекстПоляHTML	= ТекстНачала + "<p><b><FONT size=4>"
		+ Нстр("ru='Вопрос: ';en='Question: '") + "</b>" + бзВыделитьВхожденияСтрокиПоиска(Решение.Наименование, СтрокаПоискаТМП) + "</FONT> "; 
	
	Если Решение.СтатусРешения = Перечисления.CRM_СтатусыРешений.Устарело Тогда
		ТекстПоляHTML = ТекстПоляHTML + "<FONT color=""Red"">" +  Нстр("ru='[РЕШЕНИЕ УСТАРЕНО]';en='[SOLUTION OUTDATED]'") + "</FONT> ";
	ИначеЕсли Решение.СтатусРешения = Перечисления.CRM_СтатусыРешений.НаРассмотрении Тогда
		ТекстПоляHTML = ТекстПоляHTML + "<FONT color=""Red"">" + Нстр("ru='[РЕШЕНИЕ НЕ УТВЕРЖДЕНО]';en='[SOLUTION NOT APPROVED]'") + "</FONT> ";
	КонецЕсли;
	
	Если ОткрытаИзДругойФормы Тогда
		ТекстПоляHTML = ТекстПоляHTML + "<p>" 
		+ "<FONT size=3><A href=""ДобавитьТекстВопросаВТекстПисьма"">[Добавить вопрос в текст письма]</A></FONT>" 
		+ "</p><p>";
	КонецЕсли;
	
	ТекстПоляHTML = ТекстПоляHTML + "</p><p><br>";
	
	
	Если ЗначениеЗаполнено(Решение.ОсновнаяКатегория) Тогда
		ТекстПоляHTML = ТекстПоляHTML + "<FONT size=3 color=""Gray"">" + Нстр("ru='Основная категория: ';en='Primary category: '") +  
			 бзВыделитьВхожденияСтрокиПоиска(Решение.ОсновнаяКатегория.ПредставлениеНаименования, СтрокаПоискаТМП) + "</FONT></p><p>";
	Иначе
		ТекстПоляHTML = ТекстПоляHTML + "<FONT size=3 color=""Gray"">" +  Нстр("ru='Основная категория: ';en='Primary category: '")  +  НСтр("ru='[не задана]';en='[not specified]'") + "</FONT></p>";
	КонецЕсли;
	Если ЗначениеЗаполнено(Решение.Проект) Тогда
		ТекстПоляHTML = ТекстПоляHTML + "<FONT size=3 color=""Gray"">" + Нстр("ru='Проект: ';en='Project: '") 
			+  бзВыделитьВхожденияСтрокиПоиска(Решение.Проект.Наименование, СтрокаПоискаТМП) + "</FONT></p><p>"; 
	Иначе
		ТекстПоляHTML = ТекстПоляHTML + "<FONT size=3 color=""Gray"">" + Нстр("ru='Проект: ';en='Project: '") +  НСтр("ru='[не задан]';en='[not specified]'") + "</FONT></p><p>"; 
	КонецЕсли;
	Если ЗначениеЗаполнено(Решение.Автор) Тогда
		ТекстПоляHTML = ТекстПоляHTML + "<FONT size=3 color=""Gray"">" + Нстр("ru='Автор: ';en='Author: '") +  
			 бзВыделитьВхожденияСтрокиПоиска(Решение.Автор.Наименование, СтрокаПоискаТМП) + "</FONT></p><br>";
	Иначе
		ТекстПоляHTML = ТекстПоляHTML + "<FONT size=3 color=""Gray"">" + Нстр("ru='Автор: ';en='Author: '") + НСтр("ru='[не задан]';en='[not specified]'") + "</FONT></p><br>";
	КонецЕсли;
	ТекстПоляHTML = ТекстПоляHTML + бзВыделитьВхожденияСтрокиПоиска(ТекстВопроса, СтрокаПоискаТМП);
	...
	Если (Решение.СтатусРешения = Перечисления.CRM_СтатусыРешений.Личное) И НЕ (Решение.Автор = Пользователи.АвторизованныйПользователь()) Тогда
		...
		СтруктураПоляHTML = СформироватьОписаниеТекущегоРешения(ТекстПоляHTML, Решение, 0, УникальныйИдентификатор);
		СодержимоеРезультата = СтруктураПоляHTML.Тело;
	//ИначеЕсли Решение.СтатусРешения = Перечисления.CRM_СтатусыРешений.НаРассмотрении Тогда
	//	ТекстПоляHTML = ТекстПоляHTML + ТекстКонца;
	//	СтруктураПоляHTML = СформироватьОписаниеТекущегоРешения(ТекстПоляHTML, Решение, 0, УникальныйИдентификатор);
	//	СодержимоеРезультата = СтруктураПоляHTML.Тело;
	Иначе
		Если ОткрытаИзДругойФормы И Решение.Ответы.Количество() > 1 Тогда
			ТекстПоляHTML = ТекстПоляHTML + " " 
			+ "<FONT size=3><A href=""ДобавитьВсеОтветыВТекстПисьма"">[Добавить ВСЕ ответы в текст письма]</A></FONT><BR>" 
			+ "</p><p>";
		КонецЕсли;
		ТекстПоляHTML = ТекстПоляHTML + ТекстКонца;
		СтруктураПоляHTML = СформироватьОписаниеТекущегоРешения(ТекстПоляHTML, Решение, 0, УникальныйИдентификатор);
		...
		Для Каждого ТекОтвет Из Решение.Ответы Цикл
			...
			Если Решение.Ответы.Количество() = 1 ТОгда
				ТекстПоляОтветаHTML = "<p></p><p><b>" + Нстр("ru='Ответ:';en='Answer:'") + " "+ТекОтвет.НаименованиеОтвета + " ";
			Иначе	
				ТекстПоляОтветаHTML = "<p></p><p><b>" + Нстр("ru='Ответ №';en='Answer №'") + Строка(НомерОтвета) + ": "+ТекОтвет.НаименованиеОтвета + " ";
			КонецЕсли;
			
			Если ТекОтвет.СтатусОтвета = Перечисления.CRM_СтатусыРешений.Устарело Тогда
				ТекстПоляОтветаHTML = ТекстПоляОтветаHTML + "<FONT color=""Red"">" +  Нстр("ru='[ОТВЕТ УСТАРЕН]';en='[ANSWER OUTDATED]'") + "</FONT> ";
			ИначеЕсли ТекОтвет.СтатусОтвета = Перечисления.CRM_СтатусыРешений.НаРассмотрении Тогда
				ТекстПоляОтветаHTML = ТекстПоляОтветаHTML + "<FONT color=""Red"">" + Нстр("ru='[ОТВЕТ НЕ УТВЕРЖДЕН]';en='[ANSWER NOT APPROVED]'") + "</FONT> ";
			КонецЕсли;
			
			Если ОткрытаИзДругойФормы Тогда
				ТекстСсылки = "ДобавитьТекстОтветаВТекстПисьма" + НомерОтвета;
				ТекстПоляОтветаHTML = ТекстПоляОтветаHTML + "</b>" + " " 
				+ "<FONT size=3><A href=" + ТекстСсылки +">[Добавить ответ в текст письма]</A></FONT><BR>" 
				+ "</p><p>";
			КонецЕсли;
			ТекстПоляОтветаHTML = ТекстПоляОтветаHTML + бзВыделитьВхожденияСтрокиПоиска(ТекстОтвета, СтрокаПоискаТМП);
			//
			СтруктураПоляHTML = СформироватьОписаниеТекущегоРешения(ТекстПоляОтветаHTML, Решение, ТекОтвет.ИДСтроки, УникальныйИдентификатор);
			СодержимоеРезультата = СодержимоеРезультата + "<hr><br>" + СтруктураПоляHTML.Тело;
		КонецЦикла;
		
	КонецЕсли;
	
	Возврат СодержимоеРезультата;
	
КонецФункции

Функция СформироватьОписаниеТекущегоРешения(ТекстПоляHTML, РешениеСсылка, ИДСтроки, УникальныйИдентификатор)
	...
	Если Не ПустаяСтрока(ТекстПоляHTML) Тогда
		...
		Если ТаблицаФайлов.Количество() Тогда
			...
			ДокументHTML = ЗаменитьИдентификаторыКартинокНаПутьКФайлам(ТекстПоляHTML, ТаблицаФайлов, Кодировка,,УникальныйИдентификатор);

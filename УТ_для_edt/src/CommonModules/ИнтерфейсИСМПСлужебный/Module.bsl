...
Функция ИнициализироватьПараметрыКодаМаркировки(ЭлементДанных = Неопределено, ВидПродукции = Неопределено, ВыполнятьРазбор = Истина, Настройки = Неопределено) Экспорт
	...
	Если ВыполнятьРазбор Тогда
		ПараметрыКодаМаркировки.РезультатРазбора = ШтрихкодированиеИС.НоваяСтруктураОбработкиШтрихкода(
			ИсточникДанных["cis"], ВидПродукции, Ложь, Настройки);
	КонецЕсли;
	...
	Иначе
		...
		Если ТипЗнч(ДанныеОбработки) = Тип("Структура") Тогда
			...
			Если ДанныеОбработки.Свойство("tnVedCode10") Тогда
				ДанныеПродукции.КодТНВЭД = ДанныеОбработки.tnVedCode10;
			ИначеЕсли ДанныеОбработки.Свойство("tnVedCode") Тогда
...
Функция ИнициализироватьДанныеКодовТНВЭД(ДанныеОбработки = Неопределено) Экспорт
	...
	Иначе
		...
		ДанныеОбработки.Свойство("code",        ДанныеКодов.КодТНВЭД);

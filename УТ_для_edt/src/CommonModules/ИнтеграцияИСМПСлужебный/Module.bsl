...
Процедура ПодготовитьКПередачеСообщениеАбстрактнаяАгрегацияИзменениеРасчетСтатуса(СообщенияJSON, ОбщийЭлементОчереди, ПакетСообщений)
	
	ПредыдущийПакетСообщений = Неопределено;
	Для Уровень = 1 По ПакетСообщений.Количество() Цикл
		...
		Для Каждого СообщениеJSON Из ПакетСообщений[Уровень] Цикл
			СообщениеJSON.ЗагружатьДо = АбстрактноеСообщениеJSON.Идентификатор;
			СообщенияJSON.Добавить(СообщениеJSON);
		КонецЦикла;
		
		ПредыдущийПакетСообщений = ПакетСообщений[Уровень];
...
Функция ПодготовитьКПередачеСообщенияДляАгрегацииКодовМаркировкиИСМППоДеревуУпаковок(
	...
	ПараметрыОбмена, УпаковкиДокумента);
	...
	ПараметрыАгрегации.Вставить("КоличествоКодов",               0);
	ПараметрыАгрегации.Вставить("ТелоЗапросаСоздание",           Неопределено);
	...
	Если ПараметрыАгрегации.СообщенияJSONОперацияУдаление.Количество() Тогда
		СообщенияАгрегации.Вставить(СообщенияАгрегации.Количество() + 1, ПараметрыАгрегации.СообщенияJSONОперацияУдаление);
	КонецЕсли;
...
Функция ПодготовитьКПередачеСообщенияДляАгрегацииКодовМаркировкиИСМП(
	...
	ПараметрыОбмена, ПараметрыАгрегации) Экспорт
	...
	Если СтрокаДереваВерхнегоУровня.Статус = Перечисления.СтатусыКодовМаркировкиИСМП.Неопределен Тогда
		...
		ДанныеПакетаАгрегации = ПараметрыАгрегации.УровниАгрегации[УровеньАгрегации + 1];
		
		Если ДанныеПакетаАгрегации = Неопределено Тогда
			
			ТелоЗапросаСоздание = ИнициализироватьТелоЗапросаАгрегацияСоздание(ОбщийЭлементОчереди);
			
			ДанныеПакетаАгрегации = Новый Структура;
			ДанныеПакетаАгрегации.Вставить("ТелоЗапросаСоздание",             ТелоЗапросаСоздание);
			ДанныеПакетаАгрегации.Вставить("СообщенияJSONОперацияДобавление", Новый Массив);
			ДанныеПакетаАгрегации.Вставить("КоличествоКодов",                 0);
			
			ПараметрыАгрегации.УровниАгрегации[УровеньАгрегации + 1] = ДанныеПакетаАгрегации; 
			
		КонецЕсли;
		
		Если ДанныеПакетаАгрегации.КоличествоКодов + ТекущееКоличествоКодов >= КоличествоКодовВДокументе
			И ДанныеПакетаАгрегации.КоличествоКодов > 0 Тогда
			
			// Сохранить пакет кодов в сообщение.
			СообщениеJSON = ЗаполнитьСообщениеJSONКПередаче(Новый Структура, Операция, ОбщийЭлементОчереди);
			СообщениеJSON.ЗагружатьДо = ЗагружатьДо; //ПараметрыАгрегации.ЗагружатьДо;
			СообщениеJSON.ТекстСообщенияJSON = ИнтерфейсМОТПСлужебный.ОбъектВТекстJSON(ДанныеПакетаАгрегации.ТелоЗапросаСоздание, Истина);
			ДанныеПакетаАгрегации.СообщенияJSONОперацияДобавление.Добавить(СообщениеJSON);
			
			// Подготовка нового пакета к заполнению.
			ДанныеПакетаАгрегации.ТелоЗапросаСоздание = ИнициализироватьТелоЗапросаАгрегацияСоздание(ОбщийЭлементОчереди);
			ДанныеПакетаАгрегации.КоличествоКодов = 0;
			
		КонецЕсли;
		
		// Дополнить текущий пакет кодов.
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ДанныеПакетаАгрегации.ТелоЗапросаСоздание.aggregationUnits, aggregationUnits);
		ДанныеПакетаАгрегации.КоличествоКодов = ДанныеПакетаАгрегации.КоличествоКодов + ТекущееКоличествоКодов;
	...
	Для Каждого СтрокаДерева Из СтрокаДереваВерхнегоУровня.Строки Цикл
		Результат = ПодготовитьКПередачеСообщенияДляАгрегацииКодовМаркировкиИСМП(
			...
			ЗагружатьДо, СообщенияJSON, ПараметрыОбмена, ПараметрыАгрегации);
		Если Результат.ЕстьОшибки Или Результат.ТребуетсяОбновлениеКлючаСессии Тогда
			Возврат Результат;
		КонецЕсли;
	КонецЦикла;
		...
		Если ТипЗнч(СтрокаДереваВерхнегоУровня) = Тип("ДеревоЗначений") Тогда
			...
			Если КоличествоВТекущейГруппе + ПараметрыОбработки.КоличествоВТекущейГруппе > КоличествоКодовВДокументе Тогда
				...
				Если ПакетКодовВерхнегоУровня.Количество() Тогда 
					ЗаполнитьДанныеОГрупповыхУпаковкахXDTO(ПакетКодовВерхнегоУровня, ПараметрыОбработки);
					ПакетКодовВерхнегоУровня = Новый Массив;
				КонецЕсли;
...
Процедура ЗаполнитьДанныеОГрупповыхУпаковкахXDTO(ПакетКодовВерхнегоУровня, ПараметрыОбработки)
	...
	Для Каждого ГрупповаяУпаковкаXDTO Из ПакетКодовВерхнегоУровня Цикл
		ИнтеграцияИСМПСлужебный.ЗаполнитьСвойствоXDTO(
...
//  * КонтролироватьСоставБлока - Булево.
Функция ИнициализироватьПараметрыПроверкиАгрегатов() Экспорт
	...
	Результат.Вставить("КонтролироватьСоставБлока",                Ложь);
...
Функция ВыполнитьКонтрольДопустимойИерархииУпаковки(СтрокаДерева, ПодчиненнаяСтрокаДерева, ПараметрыПроверки) Экспорт
	...
	ИначеЕсли ДопустимыеВложенияПоВидуУпаковки.Найти(ПодчиненнаяСтрокаДерева.ВидУпаковки) = Неопределено
	...
	ИначеЕсли ПараметрыПроверки.Свойство("КонтролироватьСоставБлока")
		И ПараметрыПроверки.КонтролироватьСоставБлока
...
Функция ПроверитьСоставДопустимуюИерархиюУпаковки(СтрокаДерева, ОбщийЭлементОчереди, ПараметрыОбмена, ПараметрыПроверки)
	...
	Для Каждого ПодчиненнаяСтрокаДерева Из СтрокаДерева.Строки Цикл
		...
		Если ПараметрыПроверки.КонтролироватьДопустимуюИерархиюУпаковок Тогда
			РезультатПроверки = ВыполнитьКонтрольДопустимойИерархииУпаковки(СтрокаДерева, ПодчиненнаяСтрокаДерева, ПараметрыПроверки);
...
Процедура ЗагрузитьСодержаниеРезультатаОбработкиДокумента(ЭлементОчереди, ПараметрыОбмена)
	...
	Если ЭлементОчереди.ВидПродукции = Перечисления.ВидыПродукцииИС.Духи Тогда
		ПараметрыОбработки.Вставить("ИмяПоляСотировки", "name");
	Иначе
...
Процедура ЗагрузитьРезультатОбработкиАгрегацияОтчетОбИспользованииКМРасчетСтатуса(ЭлементОчереди, ПараметрыОбмена)
	...
	Если ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ОтчетОбИспользованииРасчетСтатуса Тогда
		Операция = ЭлементОчереди.РеквизитыИсходящегоСообщения.ПараметрыЗапроса.ОперацияДокумента;
	Иначе
	...
	Иначе
		Если ЕстьОшибки И ЕстьУспешно Тогда
			...
			СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийИСМП.ЗаявкаВыполнена;
		ИначеЕсли Не ЕстьОшибки Тогда
			ОперацияДокумента = ЭлементОчереди.РеквизитыИсходящегоСообщения.ПараметрыЗапроса.ОперацияДокумента;
	...
	Если Не ЕстьОшибки 
		И ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ОтчетОбИспользованииРасчетСтатуса Тогда
		
		РезультатОбработкиЗаявки = Новый Структура;
		РезультатОбработкиЗаявки.Вставить("СтатусОбработки", Перечисления.СтатусыОбработкиСообщенийИСМП.ЗаявкаВыполнена);
...
Функция ЗапроситьКодыМаркировкиИзБизнесЗаказа(ПараметрыЗапроса, ДанныеПула, ЭлементОчереди, ПараметрыОбмена, НастройкаОбменаСУЗ)
	...
	Если РезультатЗапросаКодовМаркировки.ДанныеБлока <> Неопределено Тогда
		...
		Для Каждого ЗначениеШтрихкода Из РезультатЗапросаКодовМаркировки.ДанныеБлока.КодыМаркировки Цикл
			...
			ВыбратьСтрокуДляРаспределенияКодовМаркировки(ПараметрыСтрокЗаказа);
...
Процедура ВыбратьСтрокуДляРаспределенияКодовМаркировки(ПараметрыСтрокЗаказа)
...
Функция ВходящиеДокументы(ЭлементОчереди, ПараметрыЗагрузки)
	...
	ПараметрыОтбора.Вставить(
		"ТипДокумента",
		"LP_SHIP_GOODS_CROSSBORDER,LP_SHIP_GOODS,LP_SHIP_GOODS_CSV,LP_SHIP_GOODS_XML");
...
Процедура ПодготовитьСообщенияПоАбстрактнойОперацииВводВОборотИмпортСФТС(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей)
	...
	Если Не ДанныеШапки.Следующий() Или ШтрихкодыУпаковок.Количество() = 0 Тогда
		...
		ЗавершитьПодготовкуСообщенийЗаписатьОшибкуПротоколОбмена(ЭлементОчереди, ПараметрыОбмена);
...
Процедура ПодготовитьСообщенияПоАбстрактнойАгрегации(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей)
	...
	Если Не ДанныеШапки.ОтчетПроизводственнойЛинии Тогда
		...
		Если РезультатПолученияДанных.ЕстьОшибки Тогда
			ЗавершитьПодготовкуСообщенийЗаписатьОшибкуПротоколОбмена(ЭлементОчереди, ПараметрыОбмена, СтатусОшибкаПередачи, РезультатПолученияДанных.ТекстОшибки);
			Возврат;
		КонецЕсли;
	...
	Иначе
		...
		Если РезультатОбработкиДанныхОтчета.ЕстьОшибки Тогда
			...
			ТекстОшибки = НСтр("ru = 'Некорректный формат данных отчета производственной линии'");
			Для Каждого СтрокаДанных Из ДанныеОтчета Цикл
				Если ЗначениеЗаполнено(СтрокаДанных.ТекстОшибки) Тогда
					ТекстОшибки = ТекстОшибки + Символы.ПС + СтрокаДанных.ТекстОшибки;
				КонецЕсли;
			КонецЦикла;
			ЗавершитьПодготовкуСообщенийЗаписатьОшибкуПротоколОбмена(ЭлементОчереди, ПараметрыОбмена, СтатусОшибкаПередачи, ТекстОшибки);
			Возврат;
		...
		Для Каждого КлючИЗначение Из РезультатОбработкиШтрихкодов.РезультатыОбработки Цикл
			...
			Если РезультатОбработки.Свойство("ЕстьОшибки") И РезультатОбработки.ЕстьОшибки Тогда
				ЗавершитьПодготовкуСообщенийЗаписатьОшибкуПротоколОбмена(ЭлементОчереди, ПараметрыОбмена, СтатусОшибкаПередачи, РезультатОбработки.ТекстОшибки);
				Возврат;
			КонецЕсли;
		КонецЦикла;
	...
	Если ИнтеграцияИСКлиентСервер.ЭтоПродукцияМОТП(ЭлементОчереди.ВидПродукции) Тогда
		...
		Если ЭлементОчереди.ВидПродукции = Перечисления.ВидыПродукцииИС.АльтернативныйТабак Тогда
		...
		Если РезультатПроверки.ЕстьОшибки Тогда
			ЗавершитьПодготовкуСообщенийЗаписатьОшибкуПротоколОбмена(ЭлементОчереди, ПараметрыОбмена, СтатусОшибкаПередачи, РезультатПроверки.ТекстОшибки);
			Возврат;
		КонецЕсли;
		...
		Если Результат.ЕстьОшибки Тогда
			ЗавершитьПодготовкуСообщенийЗаписатьОшибкуПротоколОбмена(ЭлементОчереди, ПараметрыОбмена, СтатусОшибкаПередачи, Результат.ТекстОшибки);
			Возврат;
		КонецЕсли;
	...
	Иначе
		...
		Если ИнтеграцияИСКлиентСервер.ВидПродукцииИспользуетОтчетыОНанесенииКодовМаркировки(ЭлементОчереди.ВидПродукции) Тогда
			ПараметрыПроверки = ИнициализироватьПараметрыПроверкиАгрегатов();
...
Процедура ЗавершитьПодготовкуСообщенийЗаписатьОшибкуПротоколОбмена(ЭлементОчереди, ПараметрыОбмена, Статус = Неопределено, ТекстОшибки = "") Экспорт
	...
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		ДобавитьВПротоколОбменаОписаниеОшибки(ЭлементОчереди, ТекстОшибки, Статус);
					...
					ИначеЕсли ЭлементОчереди.Назначение = Перечисления.НазначениеСообщенийИСМП.СтанцияУправленияЗаказами Тогда
						...
						ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ЗакрытиеПодзаказаНаЭмиссиюКодовМаркировкиСУЗ Тогда
							ПередатьСообщениеОЗакрытииПодзаказаНаЭмиссиюКодовМаркировкиСУЗ(
								ЭлементОчереди, ПараметрыОбмена, НастройкаОбменаСУЗ);
						ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ОтчетОбИспользованииРасчетСтатуса
							Или ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.АгрегацияРасчетСтатуса Тогда
							//Фиктивная операция для расчета статуса по нескольким сообщениями.
							ЗагрузитьРезультатОбработкиАгрегацияОтчетОбИспользованииКМРасчетСтатуса(
								ЭлементОчереди, ПараметрыОбмена);
						ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ВводВОборот Тогда
...
Функция РазобратьКодМаркировкиДляБизнесЗаказа(КодМаркировки, СтрокаЗаказаНаЭмиссию, КэшРазбораКодаМаркировки)
	...
	Если КэшРазбораКодаМаркировки = Неопределено Тогда
		...
		ПримечаниеКРезультатуРазбора = Неопределено;
		ДанныеРазбора = РазборКодаМаркировкиИССлужебный.РазобратьКодМаркировки(
			КодМаркировки, ВидПродукции, ПримечаниеКРезультатуРазбора, Настройки,, Истина);

	...
	Пока ОбходТаблицы Цикл
		...
		Пока СоставУпаковки.Следующий() Цикл
			...
			Если СтрокиКеша.Количество() Тогда
				...
				Штрихкоды = Новый Соответствие(СтрокиКеша[0].ШтрихкодыУпаковок);
				Штрихкоды.Вставить(ИсходныйШтрихкод, Истина);
				СтрокиКеша[0].ШтрихкодыУпаковок = Новый ФиксированноеСоответствие(Штрихкоды);
			Иначе
				...
				Штрихкоды = Новый Соответствие;
				Штрихкоды.Вставить(ИсходныйШтрихкод, Истина);
				НоваяСтрока.ШтрихкодыУпаковок = Новый ФиксированноеСоответствие(Штрихкоды);
				Если ЭтоМаркировкаСоСпецификой Тогда
		...
		Если ЗначениеЗаполнено(ПараметрыСканирования.ПараметрыУказанияСерий) Тогда
			...
			Для Каждого СтрокаКеша Из ДанныеШтрихкодовУпаковокГосИС Цикл
				...
				Если СтрокиБезСерий.Количество() Тогда
					...
					Штрихкоды = Новый Соответствие(СтрокаПриемник.ШтрихкодыУпаковок);
					Для Каждого ШтрихкодУпаковки Из СтрокаКеша.ШтрихкодыУпаковок Цикл
						Штрихкоды.Вставить(ШтрихкодУпаковки.Ключ, Истина);
					КонецЦикла;
					СтрокаПриемник.ШтрихкодыУпаковок = Новый ФиксированноеСоответствие(Штрихкоды);
...
Функция ДанныеШтрихкодаДляДобавленияВКеш(ДанныеШтрихкода, Специфика = Неопределено) Экспорт
	...
	Если Специфика = "СрокГодностиВЕТИС" Тогда
	...
	ИначеЕсли Специфика = "СрокГодности" Тогда
		Результат.Вставить("СрокГодности");
		Результат.Вставить("ШтрихкодОднороднойУпаковки");
	ИначеЕсли Специфика = "ВЕТИС" Тогда
		Результат.Вставить("ИдентификаторПроисхожденияВЕТИС");
		Результат.Вставить("ШтрихкодОднороднойУпаковки");
	...
	ДанныеСпецифики = Новый Структура("СрокГодности,СкоропортящаясяПродукцияВЕТИС");
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеШтрихкода, "Скоропортящаяся") Тогда
		ДанныеСпецифики.СкоропортящаясяПродукцияВЕТИС = ДанныеШтрихкода.Скоропортящаяся;
	КонецЕсли;
	ЗаполнитьЗначенияСвойств(Результат, ДанныеСпецифики);
	
	//Взаимоисключающие поля
	Если Специфика = "ДоступныКодыМаркировкиОстатков" И ЗначениеЗаполнено(Результат.Номенклатура) Тогда
		Результат.GTIN = "";
	КонецЕсли;
...
Процедура ДополнитьКешИТаблицуШтрихкодовУпаковок(Форма, ОбновляемаяСтрока, ДобавленныеШтрихкодыУпаковок = Неопределено) Экспорт
	...
	Иначе
		СтрокаКеша = Форма.ДанныеШтрихкодовУпаковокГосИС.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаКеша, ОбновляемаяСтрока);
...
//   ОбновляемаяСтрока - См. ДанныеШтрихкодаДляДобавленияВКеш - результат разбора добавленного штрихкода.
//   Настройки - см. НастройкиРаспределенияНезаполненныхСтрок - поля ключей, поиска и заполнения
// 
// Возвращаемое значение:
...
Процедура ДополнитьКешШтрихкодамиБезИдентификаторов(ШтрихкодУпаковкиБезИдентификатора, ВложенныеШтрихкоды, ДанныеШтрихкодовУпаковокГосИС, ШтрихкодыУпаковокИсходнаяТаблица)
	...
	Для Каждого СтрокаТаблицы Из ВложенныеШтрихкоды Цикл
		...
		Иначе
			Для Каждого СтрокаКеша Из ДанныеШтрихкодовУпаковокГосИС Цикл
				Штрихкоды = Новый Соответствие(СтрокаКеша.ШтрихкодыУпаковок);
				Штрихкоды.Вставить(ШтрихкодУпаковкиБезИдентификатора, Истина);
				СтрокаКеша.ШтрихкодыУпаковок = Новый ФиксированноеСоответствие(Штрихкоды);
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;

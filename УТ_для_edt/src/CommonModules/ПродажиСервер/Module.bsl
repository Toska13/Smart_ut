...
Функция ДополнитьПредставлениеПредставлениемФискальнойОперацииВДокументеПродажи(Основание, СчетаФактурыВыданныеНаОсновании = Неопределено, СчетаФактурыПолученныеНаОсновании = Неопределено, Контрагент = Неопределено) Экспорт
	...
	Если ВозможноПробитиеЧека Тогда
		...
		Если ФискальнаяОперацияДанныеЖурнала = Неопределено Тогда
			...
			ЗначенияРеквизитовОснования = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Основание, "Проведен,Организация");			
			ЕстьПравоИнтерактивноПровестиЧек = (ПравоДоступа("ИнтерактивноеПроведение", Основание.Метаданные())
					ИЛИ ЗначенияРеквизитовОснования.Проведен);
					
			ЕстьПодключенноеОборудование = Ложь;
			ПодключенноеОборудованиеПоОрганизации = ПодключаемоеОборудованиеУТВызовСервера.ОборудованиеПодключенноеККассеПоОрганизации(ЗначенияРеквизитовОснования.Организация);
			...
			Если ЕстьПравоИнтерактивноПровестиЧек 
				И ЗначениеЗаполнено(ЗначенияРеквизитовОснования.Организация) Тогда
				
				Если ЕстьПодключенноеОборудование Тогда
					МассивПредставлений.Добавить(
...
// Формирует временную таблицу, содержащую данные документа по покупателю: КПП покупателя.
//
// Параметры:
//	МенеджерВременныхТаблиц - МенеджерВременныхТаблиц - Менеджер временных таблиц, содержащий таблицу ТаблицаДанныхДокументов с полями:
//		Ссылка,
...
//		Контрагент
//
Процедура ПоместитьВременнуюТаблицуДанныхПокупателя(МенеджерВременныхТаблиц) Экспорт
	...
	|ПОМЕСТИТЬ ЗначенияКППКонтрагента
	|ИЗ
	|	ТаблицаДанныхДокументов КАК ДанныеДокументов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Контрагенты.ИсторияКПП КАК ИсторияКППКонтрагентов
	|		ПО ДанныеДокументов.Контрагент = ИсторияКППКонтрагентов.Ссылка
	|			И ДанныеДокументов.Дата >= ИсторияКППКонтрагентов.Период
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ЗначенияКПП.Ссылка КАК Ссылка
	|ПОМЕСТИТЬ ИсторическоеЗначениеКППКонтрагента
	|ИЗ
	|	ЗначенияКППКонтрагента КАК ЗначенияКПП
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Контрагенты.ИсторияКПП КАК ИсторияКППКонтрагентов
	|		ПО ЗначенияКПП.Контрагент = ИсторияКППКонтрагентов.Ссылка
	...
	|			ТОГДА ЕСТЬNULL(ИсторическоеЗначениеКППКонтрагента.КПП, ЕСТЬNULL(Контрагенты.КПП, """"))
	|		ИНАЧЕ """"
	|	КОНЕЦ КАК КПППокупателя
	|ПОМЕСТИТЬ ДанныеПокупателя
	|ИЗ
	|	ТаблицаДанныхДокументов КАК ДанныеДокументов
	|		ЛЕВОЕ СОЕДИНЕНИЕ ИсторическоеЗначениеКППКонтрагента КАК ИсторическоеЗначениеКППКонтрагента
	|		ПО ДанныеДокументов.Ссылка = ИсторическоеЗначениеКППКонтрагента.Ссылка
...
Процедура СформироватьЗапросШапкаДокументаПродажи(ТекстЗапроса, ИмяТаблицы)
	...
	|	ВЫБОР
	|		КОГДА СоглашениеСКлиентом.ИспользуютсяДоговорыКонтрагентов 
	|			ТОГДА ДокументПродажи.Договор.ПорядокОплаты
	|		ИНАЧЕ СоглашениеСКлиентом.ПорядокОплаты

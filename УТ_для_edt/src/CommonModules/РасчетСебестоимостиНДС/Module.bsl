...
Функция ОписаниеЦепочекПартийНДС(ПараметрыРасчета) Экспорт
	...
	// КорректировкаПриобретенияПрошлогоПериода, Перемещение, ПеремещениеОбособленно, Сторно, ВозвратПрошлыхПериодов,
...
Функция ТекстИнициализацияПартийНДС2_4()
	Возврат
		...
		|ВЫБРАТЬ
		|	СУММА((ВЫБОР
		|		КОГДА ДД.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		...
		|	ДД.КорВидДеятельностиНДС КАК ВидДеятельностиНДС
		|
		|ПОМЕСТИТЬ ЗнаменателиПрошлыхРеализацийПартийНДС2_4
		|ИЗ
		|	РегистрНакопления.ДетализацияПартийТоваровДляНДСиУСН2_4 КАК ДД
		...
		|		ИНАЧЕ ДД.ТипЗаписи КОНЕЦ) КАК ТипЗаписи,
		|	ЛОЖЬ КАК Сторно,
		|	ВЫБОР КОГДА ЕСТЬNULL(Знаменатели.Знаменатель, 0) <> 0
		|		ТОГДА МАКСИМУМ(Знаменатели.Знаменатель)
		|		ИНАЧЕ
		|			СУММА(ВЫБОР КОГДА ДД.СтоимостьРегл <> 0
		|				ТОГДА ДД.СтоимостьРегл
		|				ИНАЧЕ ВЫРАЗИТЬ(ДД.Количество * ЕСТЬNULL(Цены.СтоимостьРегл, ЕСТЬNULL(ПлановыеЦены.Цена, 0)) КАК ЧИСЛО(31,2))
		|			КОНЕЦ)
		|	  КОНЕЦ КАК Знаменатель,
		...
		|			ТОГДА ЛОЖЬ
		|
		|		КОГДА ДД.Регистратор ССЫЛКА Документ.ПересортицаТоваров
		|		  И ДД.Партия = ДД.КорПартия
		|			ТОГДА ИСТИНА
		|		КОГДА ДД.Регистратор ССЫЛКА Документ.ПересортицаТоваров
		|			ТОГДА НЕ ДД.СлужебноеВидДвиженияПриход
		|
		|		КОГДА ДД.ТипЗаписи В (
		|				ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Потребление),
...
Функция ТекстДвиженияСебестоимостиТоваровСборка2_4()
	Возврат
		...
		|	КОНЕЦ КАК Сторно,
		|	0 КАК Приоритет,
		|	СУММА(ЕСТЬNULL(Знаменатели.Знаменатель, 0)) КАК Знаменатель,
		|	СУММА(ЕСТЬNULL(Знаменатели.ЗнаменательДляИсточников, 0)) КАК ЗнаменательДляИсточников,
		|	ЛОЖЬ КАК РасчетЗавершен,
		|	НЕОПРЕДЕЛЕНО КАК Номенклатура,
		|	НЕОПРЕДЕЛЕНО КАК Характеристика,
...
Функция ТекстРеализацииПрошлыхМесяцевПартииНДС2_4() // использует ПрошлыеРеализации 
	Возврат
		...
		|	ЛОЖЬ КАК Сторно,
		|	0 КАК Приоритет,
		|	МАКСИМУМ(Знаменатели.Знаменатель)			КАК Знаменатель,
		|	0											КАК ЗнаменательДляИсточников,
		|	ИСТИНА КАК 									РасчетЗавершен,
		|	ДД.Номенклатура   							КАК Номенклатура,
		|	ДД.Характеристика 							КАК Характеристика,
		...
		|		И ДД.КорВидДеятельностиНДС = Прошлые.ВидДеятельностиНДС
		|
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ЗнаменателиПрошлыхРеализацийПартийНДС2_4 КАК Знаменатели
		|		ПО ДД.Регистратор = Знаменатели.ДокументИсточник
		|		И ДД.Организация = Знаменатели.Организация
		|		И ДД.Партия = Знаменатели.Партия
		|		И ДД.АналитикаУчетаПартий = Знаменатели.АналитикаУчетаВнешнихПартий
		|		И ДД.КорВидДеятельностиНДС = Знаменатели.ВидДеятельностиНДС
		|ГДЕ
		|	ДД.Организация В (&ОрганизацииСУчетомПартийНДСВерсии24)
		|	И ДД.Активность
		|	И НЕ (ДД.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Перемещение)
...
Процедура ЗаполнитьРасчетнуюПартиюНДС2_4(ПараметрыРасчета, РасчетнаяПартия, Расход, Приход)
	...
	Если Приход.Регистратор = Расход.Регистратор
	 И Приход.Партия = Расход.Партия
	 И Значениезаполнено(Приход.Номенклатура)
	 И ТипЗнч(Приход.Регистратор) = Тип("ДокументСсылка.ПересортицаТоваров") Тогда
		ЗаполнитьЗначенияСвойств(РасчетнаяПартия, Приход.КорАналитикаУчетаНоменклатуры, "Номенклатура, Характеристика");
	КонецЕсли;
	
	Если ТипЗнч(РасчетнаяПартия.Регистратор) = Тип("ДокументСсылка.ПересортицаТоваров")
	 И РасчетнаяПартия.Партия = РасчетнаяПартия.КорПартия
	 И Приход.ВидДеятельностиНДС = Расход.ВидДеятельностиНДС
	 И Приход.НаправлениеДеятельности = Расход.НаправлениеДеятельности
	 И НЕ ЗначениеЗаполнено(РасчетнаяПартия.Номенклатура) Тогда
		РасчетнаяПартия.ТипЗаписи = Перечисления.ТипыЗаписейПартий.Прошлое; // не записываем движения без изменения измерений
	КонецЕсли;
	...
	// Заполняем показатели расчетной партии
	Если Приход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.КорректировкаПриобретенияПрошлогоПериода
	 И ТипЗнч(Приход.Регистратор) = Тип("ДокументСсылка.КорректировкаПриобретения")
	 И Приход.ВидДвижения = ВидДвиженияНакопления.Расход И НЕ Приход.Сторно
	 И (Приход.СтоимостьБезНДС < 0 ИЛИ Приход.СтоимостьБезНДСУпр < 0) Тогда
		ЗнакСумм = -1;
	Иначе
		ЗнакСумм = 1;
	КонецЕсли;
	...
	Для Каждого ИмяРесурса Из СуммовыеРесурсы Цикл
		РасчетнаяПартия[ИмяРесурса.Ключ] = ЗнакСумм * Окр(ПриходБазис * Приход[ИмяРесурса.Ключ], 2);
	КонецЦикла;
	...
	И НЕ (Приход.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РаспределениеРасходовНаПартииПроизводства
		...
		Для Каждого ИмяРесурса Из СуммовыеРесурсы Цикл
			Приход[ИмяРесурса.Ключ] = Приход[ИмяРесурса.Ключ] - Знак * ЗнакСумм * РасчетнаяПартия[ИмяРесурса.Ключ];
		КонецЦикла;
...
Процедура ВключитьИсключитьНДСВСтоимость(ПараметрыРасчета) Экспорт
	...
	|КорАналитикаУчетаНоменклатуры, КорАналитикаФинансовогоУчета, ВидДеятельностиНДС,
	|Количество, Стоимость, СтоимостьБезНДС, СтоимостьЗабалансовая, СтоимостьРегл, СтоимостьУпр, СтоимостьЗабалансоваяРегл, НДСРегл, НДСУпр,
	|ДопРасходы, ДопРасходыБезНДС, ДопРасходыРегл, ДопРасходыУпр, ПостояннаяРазница, ВременнаяРазница
	|");
...
Процедура ВключитьИсключитьНДСВСтоимость24(ПараметрыРасчета) Экспорт
	...
	|			 И Партии.ВидДеятельностиНДСОтгрузки <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)
	|				ТОГДА Партии.ВидДеятельностиНДСОтгрузки
	|			ИНАЧЕ Партии.КорВидДеятельностиНДС КОНЕЦ) КАК КорВидДеятельностиНДС,
	...
	|		ИНАЧЕ УчетСебестоимости.КорАналитикаФинансовогоУчета КОНЕЦ) КАК КорАналитикаФинансовогоУчета,
	|	(ВЫБОР
	|		КОГДА УчетСебестоимости.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратТоваровОтКлиентаПрошлыхПериодов)
	|			ТОГДА УчетСебестоимости.ВидДеятельностиНДС
	|		ИНАЧЕ УчетСебестоимости.КорВидДеятельностиНДС КОНЕЦ) КАК КорВидДеятельностиНДС,
	...
	|		КОГДА УчетСебестоимости.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратТоваровОтКлиентаПрошлыхПериодов)
	|			ТОГДА УчетСебестоимости.АналитикаФинансовогоУчета
	|		ИНАЧЕ УчетСебестоимости.КорАналитикаФинансовогоУчета КОНЕЦ), // КорАналитикаФинансовогоУчета
	|	(ВЫБОР
	|		КОГДА УчетСебестоимости.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратТоваровОтКлиентаПрошлыхПериодов)
	|			ТОГДА УчетСебестоимости.ВидДеятельностиНДС
	|		ИНАЧЕ УчетСебестоимости.КорВидДеятельностиНДС КОНЕЦ), // КорВидДеятельностиНДС
...
Процедура ВключитьИсключитьНДСВСтоимостьПродаж24(ПараметрыРасчета) Экспорт
	...
	|			 И Партии.ВидДеятельностиНДСОтгрузки <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)
	|				ТОГДА Партии.ВидДеятельностиНДСОтгрузки
	|			ИНАЧЕ Партии.ВидДеятельностиНДС КОНЕЦ) КАК НалогообложениеПартии,
	|		Партии.КорВидДеятельностиНДС КАК НалогообложениеНДС,
	|		Партии.ДокументПоступления КАК ДокументПоступления,
	|		Партии.СтоимостьБезНДС КАК СтоимостьБезНДС,
	|		Партии.СтоимостьБезНДСУпр КАК СтоимостьБезНДСУпр,

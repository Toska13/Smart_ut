	...
	Если ПараметрыФормирования.Количество() Тогда
		...
		Для Каждого СтрокаПечатнойФормы Из ТаблицаПечатныхФорм Цикл
			Если СтрокаПечатнойФормы.ФормироватьЗаново Тогда
				...
				ПечатныеФормы = смартУправлениеПечатью.СформироватьПечатныеФормы("смартУправлениеПечатью", СтрокаПечатнойФормы.ИмяМакета,
...
Функция ОбъединитьОфисныеДокументы(ОфисныеДокументы, АдресЕдиногоДокумента)
	...
	Для каждого ОфисныйДокумент Из ОфисныеДокументы Цикл
		ДеревоМакета = смартУправлениеПечатью.ИнициализироватьМакетОфисногоДокументаСКД(ОфисныйДокумент.Значение);
...
Процедура ИнициализироватьСтруктуруМакетаСКД(Макет)
	...
	Если Файл.Существует() Тогда
		...
		Для Каждого УзелГиперссылки Из МассивУзловГиперссылок Цикл
			...
			ПараметрыСсылки = смартУправлениеПечатью.НайтиПараметрыВТексте(ТекстСсылки);
	...
	Если Файл.Существует() Тогда
		...
		СтруктураДокумента.ПараметрыТекста = смартУправлениеПечатью.НайтиПараметрыВТексте(ТекстXML);
	...
	Для Каждого Файл Из СтруктурныеФайлы Цикл
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СтруктураДокумента.ПараметрыТекста, смартУправлениеПечатью.НайтиПараметрыВТексте(ТекстXML), Истина);
...
Функция СтруктураОбластиИзМакета(ДеревоМакета, Области, ИндексОбласти, Гиперссылки)
	...
	Для Каждого СтрокаДерева Из ДеревоОбласти.Строки Цикл
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивПараметров, смартУправлениеПечатью.НайтиПараметрыВТексте(СтрокаДерева.ПолныйТекст));
	КонецЦикла;
	...
	Для Каждого УзелГиперссылки Из МассивУзловГиперссылок Цикл 
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивПараметров, смартУправлениеПечатью.НайтиПараметрыВТексте(ТекстГиперссылки));
	КонецЦикла;
			...
			Для Каждого ПараметрТекст Из ПараметрыИзТекста Цикл
				...
				Если ДанныеОбъекта.Свойство(ПараметрТекст, ЗначениеПараметра) Тогда
					Если ТипЗнч(ЗначениеПараметра) = Тип("Структура") Тогда
						...
						//++ #Б-3602 смарт: Лопато 01.07.2025 //Ошибка. Долгое формирование печатной формы в конструкторе IECHO.  
						Если ЗначениеПараметра.Свойство("ШиринаКлеткиТаблицы") Тогда
							ШиринаКлеткиТаблицы = ЗначениеПараметра.ШиринаКлеткиТаблицы;
						КонецЕсли;	
						//-- #Б-3602 смарт: Лопато 01.07.2025
...
Функция ОпределитьРасширениеФайлаДанныхПоСигнатуре(ДанныеИлиСтруктура) Экспорт
	...
	Если ТипЗнч(ДанныеИлиСтруктура) = Тип("Структура") Тогда
		Попытка
			МакетИДанныеОбъекта = смартУправлениеПечатью.МакетыИДанныеОбъектовДляПечати(ДанныеИлиСтруктура.МенеджерПечати,

...
Функция ПараметрыОперацииФискализацииЧекаДокументоваПоставкиСВидамиЗапасов(ДокументСсылка, СуммаПредоплатыКорректировка, ВерсияФФД = Неопределено)
	...
	Для Каждого СтрокаРасшифровкиПлатежа Из РасшифровкаПлатежа Цикл
		...
		Если ТипЗнч(ДокументПоставки) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
			СтатусДокументаРеализации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументПоставки, "Статус");
			Если СтатусДокументаРеализации = Перечисления.СтатусыРеализацийТоваровУслуг.КПредоплате Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
...
Функция ПараметрыОперацииФискализацииЧека(ДокументСсылка, СуммаПредоплатыКорректировка = Неопределено, ВерсияФФД = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Выборка.Контрагент) Тогда
		...
		Если ВерсияФФД = "1.2" Тогда
			СведенияОПокупателе = ПараметрыФискализацииЧека.СведенияОПокупателе;
			...
			ПараметрыФискализацииЧека.СведенияОПокупателе = СведенияОПокупателе;
		Иначе
	...
	ТорговыйОбъектНаименование = РозничныеПродажи.ТорговыйОбъектККТ(Выборка.Организация, Выборка.ТорговыйОбъект);
...
Процедура ЗаполнитьПозицииИТаблицуОплатЧека(ПараметрыФискализацииЧека, ДанныеДокумента, СуммаПредоплатыКорректировка, ВерсияФФД)
	...
	|	ВЫБОР
	|		КОГДА ДанныеДокумента.ОснованиеПлатежа ССЫЛКА Документ.СчетНаОплатуКлиенту
	|			ТОГДА ДанныеДокумента.ОснованиеПлатежа.ДокументОснование.Валюта
	|		ИНАЧЕ ДанныеДокумента.ОснованиеПлатежа.Валюта
	|	КОНЕЦ КАК ВалютаОснования,
	|	ВЫБОР
	|		КОГДА ДанныеДокумента.ОснованиеПлатежа ССЫЛКА Документ.СчетНаОплатуКлиенту
	|			ТОГДА ДанныеДокумента.ОснованиеПлатежа.ДокументОснование.СуммаДокумента
	|		ИНАЧЕ ДанныеДокумента.ОснованиеПлатежа.СуммаДокумента
	...
	|			ТОГДА ДанныеДокумента.ОснованиеПлатежа.ДокументОснование.СуммаДокумента
	|		ИНАЧЕ ДанныеДокумента.ОснованиеПлатежа.СуммаДокумента
	|	КОНЕЦ КАК СуммаВзаиморасчетовОплаты
	|ИЗ
	|	Документ.ОперацияПоЯндексКассе КАК ДанныеДокумента
	|ГДЕ
	|	ДанныеДокумента.Ссылка = &Ссылка
	...
	|			ВЫБОР
	|				КОГДА НЕ Расшифровка.Основание.Дата ЕСТЬ NULL
	|					И НАЧАЛОПЕРИОДА(Расшифровка.Основание.Дата, ДЕНЬ) <> НАЧАЛОПЕРИОДА(Расшифровка.Ссылка.Дата, ДЕНЬ)
	|						ТОГДА 0
	|				ИНАЧЕ
	|					ВЫБОР
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	Расшифровка.Ссылка.Дата,
	|	Расшифровка.Основание,
	|	Расшифровка.Основание.Дата,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(Выборка, "Упаковка")
	...
	Если ИтогоСуммаОплаты <> ИтогоСуммаЧека
		И СуммаПостоплаты > 0 Тогда
...
Функция ТекстЗапросаОплаченныеПозицииНоменклатуры()
	...
	|	ПО ОснованияПлатежа.Основание = ТаблицаДокумента.Ссылка
	|ГДЕ
	|	(НЕ ТаблицаДокумента.Ссылка.ВернутьМногооборотнуюТару
	...
	|	И КорректировкаРеализации.Ссылка ЕСТЬ NULL
	|	И ТаблицаДокумента.Ссылка.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.КПредоплате)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ

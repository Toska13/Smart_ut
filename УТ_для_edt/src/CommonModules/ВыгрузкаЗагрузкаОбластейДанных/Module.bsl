...
Функция ЗагрузитьТекущуюОбластьИзАрхива(Знач Архив,
	...
	Иначе
		...
		Если ЗаписьПолногоАрхива <> Неопределено Тогда
			ВремКаталог = ПолучитьИмяВременногоФайла("zip") + ПолучитьРазделительПути();
			СоздатьКаталог(ВремКаталог);
			Если ЗаписьПолногоАрхива.МетодСжатия = 0 И ЗаписьПолногоАрхива.НесжатыйРазмер > 1024 * 1024 * 1024 Тогда
				// Большой файл, нужно читать из МС
				ПолныйАрхив = РаботаВМоделиСервиса.ПрочитатьВложенныйНесжатыйАрхив(АрхивКопии, ИмяФайлаПолногоАрхива);
			Иначе
		...
		Для Каждого ВыгружаемыйТип Из ВыгружаемыеТипы Цикл
			...
			Если Выборка.Следующий() И Выборка.Регистратор = Неопределено Тогда
				...
				ТекстОшибки = СтрШаблон(
					...
					|Рекомендуется восстановить отсутствующие регистраторы или удалить проблемные записи.'"),
					ПолноеИмя) + " ";
				ОшибкиВыгружаемыхДанных.Добавить(ТекстОшибки);

...
Процедура ЗаполнитьПроцентыПоСуммам(Параметры) Экспорт
	...
	Пока Сч < Параметры.ЭтапыГрафикаОплаты.Количество() Цикл
		...
		Если СуммаПлатежей > 0 Тогда
			ТекущийПроцент = Параметры.ЭтапыГрафикаОплаты[Сч].СуммаПлатежа / СуммаПлатежей * 100;
			
			Параметры.ЭтапыГрафикаОплаты[Сч].ПроцентПлатежа = ?(Сч = Параметры.ЭтапыГрафикаОплаты.Количество()-1 ИЛИ СуммаПроцентов - ТекущийПроцент< 0,
																СуммаПроцентов,
																ТекущийПроцент);

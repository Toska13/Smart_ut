...
Процедура ПодготовитьДанныеКодовМаркировкиДляОбработки(ДанныеКодовМаркировки, ДанныеШтрихкодов, ПараметрыСканирования, КешДанныхРазбора = Неопределено) Экспорт
	...
	ЗаполнитьДанныеКодовМаркировкиПоДаннымУПД(ДанныеКодовМаркировки, ПараметрыСканирования);
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		Если ЗначениеЗаполнено(СтрокаДанных.Номенклатура)
			И (Не ЗначениеЗаполнено(СтрокаДанных.ВидПродукции)
				Или Не СтрокаДанных.МаркируемаяПродукция
				// Поддержка переходного периода: АТП, эмиттированые как табак
				Или СтрокаДанных.ВидПродукции = Перечисления.ВидыПродукцииИС.Табак) Тогда
...
Функция СоздатьМаркированныйТоварПоДаннымШтрихкода(ДанныеШтрихкода, ПараметрыСканирования) Экспорт
	...
	Если ДанныеНовойУпаковки.Количество = 1
		...
		ИначеЕсли ИнтеграцияИСПовтИсп.ЭтоПродукцияИСМП(ДанныеШтрихкода.ВидПродукции) И ДанныеШтрихкода.КоличествоВложенныхЕдиниц > 1 Тогда
...
//  ПараметрыСканирования - См. ПараметрыСканирования
Процедура ДобавитьВДанныеШтрихкодовВложенныеШтрихкоды(ДанныеПоШтрихкодам, ВложенныеШтрихкодыПоДаннымСервиса, ПараметрыСканирования, ОтборПоВидуПродукции = Неопределено) Экспорт
	...
	Для Каждого СтрокаИсточника Из ДеревоУпаковокПоДаннымСервиса.Строки Цикл
		...
		Иначе
			...
			ЗаполнитьЗначенияСвойств(НайденнаяСтрока, СтрокаИсточника,, "Входящий");
			Если НайденнаяСтрока.Строки.Количество() Тогда
				...
				Для Каждого КлючИЗначение Из СоответствиеВложенныхСтрокДереваУпаковокИсточника Цикл
					...
					Если СтрокаДереваПриемника.Строки.Количество() = 0
						И КлючИЗначение.Значение.Строки.Количество() > 0 Тогда
						...
						ДополнитьДеревоУпаковок(
							СтрокаДереваПриемника, КлючИЗначение.Значение,
		...
		ДополнитьДеревоУпаковок(
			СтрокаПриемника, СтрокаИсточника,
			СоответствиеСтарыхИНовыхСтрокДерева, ОтборПоВидуПродукции);
...
Процедура ЗаполнитьСведенияПоШтрихкодамEAN(ДанныеКодовМаркировки, ПараметрыСканирования) Экспорт
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		Если ДанныеПулаПоКоду <> Неопределено Тогда
			...
			Если Не ДанныеПулаПоКоду.СохраненоПриСканировании
			...
			Если Не (РезультатПоискаУникален = Неопределено И ВариантовВыбора = 1) Тогда
				...
				ИначеЕсли ДанныеДокументаОснования <> Неопределено
					И ДанныеДокументаОснования.НайтиСтроки(
						Новый Структура("Номенклатура,Характеристика", СтрокаИнформации.Номенклатура,СтрокаИнформации.Характеристика)).Количество() = 0 Тогда
					Продолжить;
		...
		Если Не ЗначениеЗаполнено(СтрокаДанных.ВидУпаковки)
			...
			И ШтрихкодированиеИСКлиентСервер.ВозможнаГрупповаяУпаковка(СтрокаДанных.ДанныеРазбора.ВидУпаковки, СтрокаДанных.ДанныеРазбора) Тогда
			СтрокаДанных.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая;
		КонецЕсли;
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		Если ДанныеРазбораИПримечание = Неопределено Тогда
			ДанныеРазбора = РазборКодаМаркировкиИССлужебный.РазобратьКодМаркировки(
				СтрокаДанных.Штрихкод,
				ВидыПродукции,
				СтрокаДанных.ПримечаниеКРазборуШтрихкода,
				НастройкиРазбораКодаМаркировки,,
				СохранятьКодыМаркировкиВПулИСМП);
			СтрокаДанных.ДанныеРазбора = ДанныеРазбора;
		КонецЕсли;
		...
		Иначе
			...
			Если Не ПроверятьСтруктуруКодовМаркировки
				И ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ИСМП")
				И ИнтеграцияИСПовтИсп.ЭтоПродукцияИСМП(СтрокаДанных.ВидПродукции, Истина) Тогда
				
				МодульШтрихкодированиеИСМП = ОбщегоНазначения.ОбщийМодуль("ШтрихкодированиеИСМПСлужебный");
				МодульШтрихкодированиеИСМП.ПроверитьПолныйКодМаркировкиПоДаннымРазбора(ДанныеРазбора, СтрокаДанных, ПараметрыСканирования);
...
Функция ПроверкаШтрихкодаНаУжеДобавленный(СтрокаДанных, ПараметрыСканирования)
	...
	Если ОбработанныеКодыМаркировки.Получить(Штрихкод) = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция РезультатТребуетсяУточнениеКоэффициентаУпаковок(ДанныеШтрихкода, УточнениеКоэффициентовУпаковок, ПараметрыСканирования)
	...
	Если ЭтоАдресВременногоХранилища(ПараметрыСканирования.КэшМаркируемойПродукции) Тогда
		...
		КэшМаркируемойПродукции.ОтложенныеКодыМаркировки.Добавить(ОтложенныйКод);
...
Процедура ЗаполнитьДанныеИзвестнойМаркируемойПродукции(ДанныеПоШтрихкодам, ДобавленныеСтроки, ПараметрыСканирования) Экспорт
	...
	Для Каждого СтрокаДанных Из ТаблицаШтрихкодовУпаковок Цикл
		...
		НайденнаяСтрока.Идентифицирован        = ЗначениеЗаполнено(СтрокаДанных.Номенклатура);
		...
		Если Не ЗначениеЗаполнено(НайденнаяСтрока.ВидУпаковки) Тогда
			...
			НайденнаяСтрока.ТипУпаковки = Перечисления.ТипыУпаковок.МаркированныйТовар;
			Если НайденнаяСтрока.СтрокаДерева <> Неопределено Тогда
				...
				НайденнаяСтрока.СтрокаДерева.ТипУпаковки = Перечисления.ТипыУпаковок.МаркированныйТовар;
			КонецЕсли;
		КонецЕсли;
		...
		ИначеЕсли Не НайденнаяСтрока.ЭтоКодВводаОстатков
		...
		Если НайденнаяСтрока.ЭтоКодВводаОстатков Тогда
...
Функция ЗаполнитьСтрокуДереваПоДаннымРазбораШтрихкода(СтрокаДерева, ОбщиеМодули, НастройкиРазбораКодаМаркировки = Неопределено, ПроверятьАлфавитЭлементов = Истина, ПараметрыСканирования = Неопределено)
	...
	ПримечаниеКРезультатуРазбора = Неопределено;
	ДанныеРазбора = РазборКодаМаркировкиИССлужебный.РазобратьКодМаркировки(
		СтрокаДерева.Штрихкод, ВидыПродукции, ПримечаниеКРезультатуРазбора,
		НастройкиРазбораКодаМаркировки, ПроверятьАлфавитЭлементов);
	
	Если ДанныеРазбора = Неопределено
		И НастройкиРазбораКодаМаркировки.ТолькоУчитываемыеВидыПродукции
		...
		КонецЕсли;
		ВыключенныеВидыПродукции = ОбщегоНазначенияКлиентСервер.РазностьМассивов(Виды, НастройкиРазбораКодаМаркировки.ДоступныеВидыПродукции);
	...
	Для Каждого СтрокаДерева Из ДеревоУпаковок.Строки Цикл
		...
		Если Не ЭтоЗагрузкаИзТСД И НайденныеСтроки.Количество() Тогда
			...
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				...
				Если Не ЗначениеЗаполнено(НайденнаяСтрока.ТипУпаковки)
...
Процедура ДополнитьДеревоУпаковок(СтрокаДереваПриемник, СтрокаДереваИсточник, СоответствиеСтарыхИНовыхСтрокДерева = Неопределено, ОтборПоВидуПродукции = Неопределено)
	...
	Для Каждого СтрокаИсточника Из СтрокаДереваИсточник.Строки Цикл
		...
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаИсточника,, "Входящий");
...
Процедура ЗаполнитьПоДокументуИОснованию(ДанныеПоШтрихкодам, ПараметрыСканирования, Форма)
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если ИнтеграцияИСПовтИсп.ЭтоПродукцияИСМП(СтрокаДанных.ВидПродукции) Тогда
			Если МодульСпецификиИСМП = Неопределено Тогда
...
Процедура ЗаполнитьДанныеКодовМаркировкиПоДаннымУПД(ДанныеКодовМаркировки, ПараметрыСканирования)
	
	Если ДопустимаТабачнаяПродукция(ПараметрыСканирования) Или ДопустимаПродукцияИСМП(ПараметрыСканирования) Тогда
		МодульШтрихкодированиеИСМПСлужебный = ОбщегоНазначения.ОбщийМодуль("ШтрихкодированиеИСМПСлужебный");
		МодульШтрихкодированиеИСМПСлужебный.ЗаполнитьДанныеКодовМаркировкиПоДаннымУПД(ДанныеКодовМаркировки, ПараметрыСканирования);
	КонецЕсли;

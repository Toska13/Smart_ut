...
Функция ПолучитьНастройкуОбмена(ПрограммаOutlook, ИмяНастройки) Экспорт 
	Возврат CRM_ЛицензированиеЭкспортныеМетоды.ПолучитьЗащищеннуюОбработку().ОбменСMSOutlookПолучитьНастройкуОбмена(ПрограммаOutlook, ИмяНастройки);
КонецФункции
...
Функция ЗаполнитьПисьмоПоСтруктуре(Ссылка, СтруктураОбъекта, ОчиститьУчастников = Ложь, СтрокаСобытия) Экспорт
	...
	Попытка
		мОбъект.Записать();
		Взаимодействия.УстановитьПапкуЭлектронногоПисьма(мОбъект.Ссылка, СтруктураОбъекта.Реквизиты.Папка);
		СтрокаСобытия.Объект_1С			= мОбъект.Ссылка;
		СтрокаСобытия.ДатаИзменения_1С	= мОбъект.Дата;

...
Процедура ЗаписатьСоответствиеНоменклатуры(Объект, ОписаниеТабличнойЧасти = "Товары") Экспорт
	...
	|	И НЕ Т.Серия В (&НезаполненныеЗначенияСерии)
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	Т.Номенклатура,
...
Процедура ЗаполнитьДокументМаркировкаТоваровИСМП(ЗаполнениеВыполнено, ДанныеЗаполнения, ДокументОбъект) Экспорт
	
	Если Не ЗначениеЗаполнено(ДанныеЗаполнения) Тогда
		Возврат;
	КонецЕсли;
	Если ТипЗнч(ДанныеЗаполнения) <> Тип("ДокументСсылка.ПроизводственнаяОперацияВЕТИС")
			И ТипЗнч(ДанныеЗаполнения) <> Тип("ДокументСсылка.ВходящаяТранспортнаяОперацияВЕТИС") Тогда
		Возврат;
	КонецЕсли;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|"
	+ ВТМаркируемаяПродукция("ТоварыСерии", "Номенклатура", "МаркируемаяПродукция")
	+"
	|ВЫБРАТЬ
	|	ТаблицаТовары.Номенклатура                    КАК Номенклатура,
	|	ТаблицаТовары.Характеристика                  КАК Характеристика,
	|	ТаблицаТовары.Серия                           КАК Серия,
	|	ТаблицаТовары.ИдентификаторПроисхожденияВЕТИС КАК ИдентификаторПроисхожденияВЕТИС,
	|	ТаблицаТовары.Количество                      КАК Количество
	|ПОМЕСТИТЬ МаркируемыеТовары
	|ИЗ ТоварыСерии КАК ТаблицаТовары
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ МаркируемаяПродукция КАК МаркируемаяПродукция
	|	ПО МаркируемаяПродукция.Номенклатура = ТаблицаТовары.Номенклатура
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	ИмяДокументаМаркировка = ДокументОбъект.Метаданные().Имя;
	Запрос = Новый Запрос;
	...
	ВыборкаПоТоварам = МассивРезультатов[МассивРезультатов.Количество() - 2].Выбрать();
	...
	Если ДокументОбъект.ДополнительныеСвойства.Свойство("ОтчетПроизводственнойЛинии") Тогда
		...
		Если ВыборкаШапка.Следующий() Тогда
			...
			ДокументОбъект.ИдентификаторПроисхожденияВЕТИС = ВыборкаШапка.ИдентификаторПроисхожденияВЕТИС;
...
Функция ВТМаркируемаяПродукция(ТаблицаИсточник, ИмяПоляНоменклатура, ТаблицаПриемник)
	...
	|//////////////////////////////////////////////////
	|";
	ИнтеграцияВЕТИСПереопределяемый.ПриОпределенииЗапросаМаркируемойПродукции(Шаблон, ТаблицаИсточник, ИмяПоляНоменклатура, ТаблицаПриемник);
	Возврат СтрШаблон(Шаблон, ТаблицаИсточник, ИмяПоляНоменклатура, ТаблицаПриемник);
...
Функция СостояниеОбмена(ОрганизацииВЕТИС, Документ = Неопределено) Экспорт
	...
	ТекстыЗапроса.Добавить(
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ИСТИНА КАК НеЗаполненРегистрВидыПродукцииПоГруппамВЕТИС
	|ИЗ
	|	РегистрСведений.ВидыПродукцииПоГруппамВЕТИС КАК ВидыПродукцииПоГруппамВЕТИС
	|ИМЕЮЩИЕ
	|	ЕСТЬNULL(КОЛИЧЕСТВО(ВидыПродукцииПоГруппамВЕТИС.РольПользователя), 0) = 0
	|",
	"НеЗаполненРегистрВидыПродукцииПоГруппамВЕТИС");
	
	ТекстыЗапроса.Добавить(
	"ВЫБРАТЬ
	|	ИСТИНА КАК НеЗаполненРегистрДопустимыеЦелиПоГруппамВЕТИС
	|ИЗ
	|	РегистрСведений.ДопустимыеЦелиПоГруппамВЕТИС КАК ДопустимыеЦелиПоГруппамВЕТИС
	|ИМЕЮЩИЕ
	|	ЕСТЬNULL(КОЛИЧЕСТВО(ДопустимыеЦелиПоГруппамВЕТИС.ГруппаПриказа), 0) = 0
	|",
	"НеЗаполненРегистрДопустимыеЦелиПоГруппамВЕТИС");

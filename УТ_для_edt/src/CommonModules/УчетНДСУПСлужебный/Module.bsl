...
Функция РассчитатьЗаписиНДСПредъявленныйВосстановлениеПодВидДеятельности(ПараметрыРасчета)
	...
	|	
	|СГРУППИРОВАТЬ ПО
	|	ЕСТЬNULL(ИсправительныеСчетаФактуры.Ссылка, СчетаФактурыДокументы.Ссылка),
	|	ДД.Организация,
	|	ДД.СчетФактура,
	|	ДД.Поставщик,
	|	ДД.ВидЦенности,
...
Функция РассчитатьЗаписиНДСПредъявленныйПринятиеКВычету(ПараметрыРасчета)
	...
	|		И НДСЗаписиКнигиПокупок.КодВидаОперации = ""25"") КАК НДСПредъявленный
	|
	|СГРУППИРОВАТЬ ПО
	|	НДСПредъявленный.Организация,
	|	НДСПредъявленный.СчетФактура
	...
	Пока ВыборкаОстатковНДСПредъявленныйПоСФ.Следующий() Цикл
		...
		Если НайденнаяБлокировкаНаСФ.Количество() > 0 Тогда
			...
			Иначе
				СуммаБлокировки = Мин(НайденнаяБлокировкаНаСФ[0].НДС, ВыборкаОстатковНДСПредъявленныйПоСФ.НДС);
			КонецЕсли;
		Иначе
			СуммаБлокировки = 0;
		КонецЕсли;
		
		НДСПоСФ = ВыборкаОстатковНДСПредъявленныйПоСФ.НДС - СуммаБлокировки;
		
		Если НДСПоСФ = 0 Тогда
			Продолжить;
		КонецЕсли;
		...
		Знак = ?(НДСПоСФ > 0, 1, -1);
		Пока ВыборкаВРазрезеИзмеренийСФ.Следующий() И НДСПоСФ * Знак > 0 Цикл
			
			Если ВыборкаВРазрезеИзмеренийСФ.НДС = 0 Тогда
				Продолжить;
			КонецЕсли;
			
			НДСКВычету = Мин(ВыборкаВРазрезеИзмеренийСФ.НДС, НДСПоСФ);
			НДСПоСФ = НДСПоСФ - НДСКВычету;
			
			Суммы = Новый Структура("НДСКВычету, НДС, СуммаБезНДС, НДСУпр");
			Суммы.НДСКВычету = НДСКВычету;
			ЗаполнитьЗначенияСвойств(Суммы, ВыборкаВРазрезеИзмеренийСФ);
			
			СтруктураОтбораПартии = Новый Структура("Организация, СчетФактура, Поставщик, ВидЦенности, СтавкаНДС, ВидДеятельностиНДС, НаправлениеДеятельности, РеализацияЭкспорт");
...
Процедура ПолучитьПериодичностьФормированияВычетовИВосстановленийНДС(ПараметрыРасчета)
	...
	|	УчетныеПолитики.УчетнаяПолитика.ПериодичностьФормированияВычетовИВосстановленийНДС КАК ПериодичностьФормированияВычетовИВосстановленийНДС

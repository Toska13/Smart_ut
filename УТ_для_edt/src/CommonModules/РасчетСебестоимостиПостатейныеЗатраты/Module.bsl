...
Процедура РаспределитьРасходыБудущихПериодов(ПараметрыРасчета) Экспорт
	...
	|	ВЫБОР
	|		КОГДА Распределение.РаспределитьУпр
	|			ТОГДА Расходы.Сумма
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК Сумма,
	|	ВЫБОР
	...
	|	КОНЕЦ КАК СуммаУпр,
	|	ВЫБОР
	|		КОГДА Распределение.РаспределитьРегл
	...
	|ПОМЕСТИТЬ ВтРасходыКРаспределениюРБП
	|ИЗ
	...
	|	МАКСИМУМ(Распределение.ДоляСтоимостиУпр) КАК ДоляСтоимостиУпр,
	|	МАКСИМУМ(Распределение.ДоляСтоимостиРегл) КАК ДоляСтоимостиРегл,
	...
	|ИЗ
	|	ВтРасходыКРаспределениюРБП КАК Расходы
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтРаспределенияРБП КАК Распределение
	|		ПО (Распределение.Организация = Расходы.Организация)
	|			И (Распределение.Подразделение = Расходы.Подразделение)
	|			И (Распределение.СтатьяРасходов = Расходы.СтатьяРасходов)
	...
	|	Строки.СтатьяРасходов.ПринятиеКНалоговомуУчету,
...
Процедура СформироватьДопРасходыВСебестоимостьТоваров(ПараметрыРасчета, РасчетныеПриходыПартийРасходов)
	...
	Для Каждого Движение Из РасчетныеПриходыПартийРасходов Цикл
		...
		Если ПараметрыРасчета.ТипыНалогообложенияНДСУчитываетсяВСтоимости[РасчетнаяПартия.ВидДеятельностиНДС] <> Неопределено Тогда 
...
Процедура СформироватьДопРасходыВПрочиеРасходы(ПараметрыРасчета, РасчетныеПриходыПартийРасходов)
	...
	Для Каждого Движение Из РасчетныеПриходыПартийРасходов Цикл
		...
		Если ПараметрыРасчета.ТипыНалогообложенияНДСУчитываетсяВСтоимости[Движение.КорВидДеятельностиНДС] <> Неопределено Тогда
...
Функция ТекстБазисПрошлыеФактуровкаПоставки() // использует НеотфактурованныеПоставки
	...
	ТекстЗапроса = 
		"ВЫБРАТЬ
		|	""ТекстБазисПрошлыеФактуровкаПоставки"" КАК ЗапросИсточник,
		|	90 КАК Приоритет,
		|	ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Потребление) КАК ТипЗаписи,
		|	ЛОЖЬ КАК РасчетЗавершен,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
...
Функция ТекстРаспределениеРасходовНаВыбытиеТоваровПрошлыхПериодов() // ВТ РаспределениеРасходов - общая для регистров себестоимости, выручки, прочих расходов
	...
	|			КОГДА ДД.Партия <> НЕОПРЕДЕЛЕНО ТОГДА ДД.Партия
...
Процедура СформироватьДвиженияРаспределениеРБП(ПараметрыРасчета, ДанныеДвижения, Суммы)
	...
	ЗаписьПриход.ПостояннаяРазница = ?(ДанныеДвижения.ПринятиеКНалоговомуУчету, Суммы.ПостояннаяРазница, 
										Суммы.СуммаРегл);
	ЗаписьПриход.ВременнаяРазница = ?(ДанныеДвижения.ПринятиеКНалоговомуУчету, Суммы.ВременнаяРазница, 0);
	
	ЗаполнитьЗначенияСвойств(ЗаписьПриход, Суммы, "Сумма, СуммаУпр, СуммаРегл");

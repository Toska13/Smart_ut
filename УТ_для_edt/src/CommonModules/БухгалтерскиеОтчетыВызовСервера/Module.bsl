...
Функция ПодготовитьОтчет(ПараметрыОтчета) Экспорт
	...
	РезультатФормирования = РезультатФормированияОтчета();
	РезультатФормирования.ДанныеРасшифровки = ПараметрыОтчета.ДанныеРасшифровки;
	...
	Если ВыводитьРаздел Тогда
		...
		Если ПараметрыИсполненияОтчета.ИспользоватьПриВыводеЗаголовка Тогда
			МенеджерОтчета.ПриВыводеЗаголовка(ПараметрыОтчета, КомпоновщикНастроек, РезультатФормирования.Результат);
		Иначе
			ВывестиЗаголовокОтчета(ПараметрыОтчета, КомпоновщикНастроек, РезультатФормирования.Результат);
		КонецЕсли;
	...
	Попытка
		...
		ПроцессорВывода.УстановитьДокумент(РезультатФормирования.Результат);
		...
		Если ПараметрыИсполненияОтчета.ПроверкаИтогов Тогда
			МенеджерОтчета.ПроверитьКорректностьИтогов(ПараметрыОтчета, РезультатФормирования.Результат);
		КонецЕсли;
		...
		РезультатФормирования.ДанныеРасшифровки = ПоместитьВоВременноеХранилище(
			ДанныеДляРасшифровки, РезультатФормирования.ДанныеРасшифровки);
	...
	Исключение
		...
		РезультатФормирования.КраткоеПредставлениеОшибки   = ТекстСообщения;
		РезультатФормирования.ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки;
	...
	Если ВыводитьРаздел Тогда
		...
		Если ПараметрыИсполненияОтчета.ИспользоватьПриВыводеПодвала Тогда 
			МенеджерОтчета.ПриВыводеПодвала(ПараметрыОтчета, РезультатФормирования.Результат);
		Иначе
			ВывестиПодвалОтчета(ПараметрыОтчета, КомпоновщикНастроек, РезультатФормирования.Результат);
		КонецЕсли;

	КонецЕсли;
	
	Если ПараметрыИсполненияОтчета.ИспользоватьПослеВыводаРезультата Тогда
		МенеджерОтчета.ПослеВыводаРезультата(ПараметрыОтчета, РезультатФормирования.Результат);
	Иначе
		ОбработкаРезультатаОтчета(ПараметрыОтчета.ИдентификаторОтчета, РезультатФормирования.Результат);
	КонецЕсли;
	...
	Если ПараметрыИсполненияОтчета.ПроверкаИтогов
	   И ПараметрыОтчета.НастройкиКомпоновкиДанных.ДополнительныеСвойства.Свойство("КонтрольноеСоотношениеИтоговВыполняется") Тогда
		...
		РезультатФормирования.КонтрольноеСоотношениеИтоговВыполняется =

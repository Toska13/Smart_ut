...
Функция ТаблицаПолейДанныхПечати() Экспорт
	...
	Возврат смартКонструкторФормул.ТаблицаПолей();
...
Функция ДеревоПолейДанныхПечати() Экспорт
	...
	Возврат смартКонструкторФормул.ДеревоПолей();
...
Функция КомандыПечатиФормы(Форма, СписокОбъектов = Неопределено) Экспорт
	...
	смартУправлениеПечатьюПереопределяемый.ПередДобавлениемКомандПечати(ИмяФормы, КомандыПечати, СтандартнаяОбработка);
	...
	Если СтандартнаяОбработка Тогда
		...
		Иначе
			...
			Если ЭтоЖурналДокументов Тогда
				...
				смартУправлениеПечатьюПереопределяемый.ПриПолученииНастроекСпискаКомандПечати(НастройкиСписка);
			КонецЕсли;
...
Функция НастройкиСохранения() Экспорт
	...
	Возврат смартУправлениеПечатьюКлиентСервер.НастройкиСохранения();
...
Функция НастройкиПечатиОбъекта(МенеджерОбъекта) Экспорт
	...
	СписокОбъектов = Новый Массив;
	смартУправлениеПечатьюПереопределяемый.ПриОпределенииОбъектовСКомандамиПечати(СписокОбъектов); // АПК:222 Вызов устаревшей процедуры для обратной совместимости.
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ОбъектыСКомандамиПечати, СписокОбъектов, Истина);
...
Функция ИнициализироватьПечатнуюФорму(Знач УдалитьТипДокумента, Знач УдалитьНастройкиСтраницыМакета = Неопределено, Макет = Неопределено) Экспорт
	...
	ПечатнаяФорма = смартУправлениеПечатьюСлужебный.ИнициализироватьПечатнуюФорму(Макет);
...
Функция ИнициализироватьМакетОфисногоДокумента(ДвоичныеДанныеМакета, Знач УдалитьТипМакета, Знач УдалитьИмяМакета = "") Экспорт
	...
	Макет = смартУправлениеПечатьюСлужебный.МакетИзДвоичныхДанных(ДвоичныеДанныеМакета);
	Если Макет <> Неопределено Тогда
...
Процедура ОчиститьСсылки(ПечатнаяФорма, Знач УдалитьЗакрытьПриложение = Истина) Экспорт
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		смартУправлениеПечатьюСлужебный.ЗакрытьСоединение(ПечатнаяФорма);
...
Функция СформироватьДокумент(Знач ПечатнаяФорма) Экспорт
	...
	АдресХранилищаПечатнойФормы = смартУправлениеПечатьюСлужебный.СформироватьДокумент(ПечатнаяФорма);
...
Функция ОбластьМакета(СсылкаНаМакет, ОписаниеОбласти) Экспорт
	...
	Если ОписаниеОбласти.ТипОбласти = "ВерхнийКолонтитул" Или ОписаниеОбласти.ТипОбласти = "ВерхнийЧетныйКолонтитул" 
		...
		Область = смартУправлениеПечатьюСлужебный.ПолучитьОбластьВерхнегоКолонтитула(СсылкаНаМакет, ОписаниеОбласти.ИмяОбласти);
	ИначеЕсли ОписаниеОбласти.ТипОбласти = "НижнийКолонтитул"  Или ОписаниеОбласти.ТипОбласти = "НижнийЧетныйКолонтитул"  
		...
		Область = смартУправлениеПечатьюСлужебный.ПолучитьОбластьНижнегоКолонтитула(СсылкаНаМакет, ОписаниеОбласти.ИмяОбласти);
	ИначеЕсли ОписаниеОбласти.ТипОбласти = "Общая" 
		...
		Область = смартУправлениеПечатьюСлужебный.ПолучитьОбластьМакета(СсылкаНаМакет, ОписаниеОбласти.ИмяОбласти);
	Иначе
...
Процедура ПрисоединитьОбласть(ПечатнаяФорма, ОбластьМакета, Знач ПереходНаСледующуюСтроку = Ложь) Экспорт
	...
	Попытка
		...
		Если ОписаниеОбласти.ТипОбласти = "ВерхнийКолонтитул" Или ОписаниеОбласти.ТипОбласти = "ВерхнийЧетныйКолонтитул" 
			Или ОписаниеОбласти.ТипОбласти = "ВерхнийТитульныйКолонтитул" Тогда
				ВыведеннаяОбласть = смартУправлениеПечатьюСлужебный.ДобавитьВерхнийКолонтитул(ПечатнаяФорма, ОбластьМакета);
		ИначеЕсли ОписаниеОбласти.ТипОбласти = "НижнийКолонтитул"  Или ОписаниеОбласти.ТипОбласти = "НижнийЧетныйКолонтитул"
			Или ОписаниеОбласти.ТипОбласти = "НижнийТитульныйКолонтитул" Тогда
			ВыведеннаяОбласть = смартУправлениеПечатьюСлужебный.ДобавитьНижнийКолонтитул(ПечатнаяФорма, ОбластьМакета);
		ИначеЕсли ОписаниеОбласти.ТипОбласти = "Общая" Или ОписаниеОбласти.ТипОбласти = "Список" 
			...
			ВыведеннаяОбласть = смартУправлениеПечатьюСлужебный.ПрисоединитьОбласть(ПечатнаяФорма, ОбластьМакета, ПереходНаСледующуюСтроку);
		Иначе
...
Процедура ЗаполнитьПараметры(ПечатнаяФорма, Данные) Экспорт
	...
	Если ОписаниеОбласти.ТипОбласти = "ВерхнийКолонтитул" ИЛИ ОписаниеОбласти.ТипОбласти = "ВерхнийЧетныйКолонтитул" ИЛИ ОписаниеОбласти.ТипОбласти = "ВерхнийТитульныйКолонтитул" Тогда
		смартУправлениеПечатьюСлужебный.ЗаполнитьПараметрыВерхнегоКолонтитула(ПечатнаяФорма, ОписаниеОбласти.Область, Данные);
	ИначеЕсли ОписаниеОбласти.ТипОбласти = "НижнийКолонтитул"  ИЛИ ОписаниеОбласти.ТипОбласти = "НижнийЧетныйКолонтитул"  ИЛИ ОписаниеОбласти.ТипОбласти = "НижнийТитульныйКолонтитул" Тогда
		смартУправлениеПечатьюСлужебный.ЗаполнитьПараметрыНижнегоКолонтитула(ПечатнаяФорма, ОписаниеОбласти.Область, Данные);
	ИначеЕсли ОписаниеОбласти.ТипОбласти = "Общая"
			ИЛИ ОписаниеОбласти.ТипОбласти = "СтрокаТаблицы"
			ИЛИ ОписаниеОбласти.ТипОбласти = "Список" Тогда
		смартУправлениеПечатьюСлужебный.ЗаполнитьПараметры(ПечатнаяФорма, ОписаниеОбласти.Область, Данные);
	Иначе
		ВызватьИсключение ТекстТипОбластиУказанНекорректно();
	КонецЕсли;
...
Процедура ПрисоединитьИЗаполнитьКоллекцию(ПечатнаяФорма, ОбластьМакета, Данные, Знач ПереходНаСледСтроку = Ложь) Экспорт
	...
	Если ОписаниеОбласти.ТипОбласти = "СтрокаТаблицы" ИЛИ ОписаниеОбласти.ТипОбласти = "Список" Тогда
		смартУправлениеПечатьюСлужебный.ПрисоединитьИЗаполнитьНабор(ПечатнаяФорма, ОбластьМакета, Данные, ПереходНаСледСтроку);
	Иначе
		ВызватьИсключение ТекстТипОбластиУказанНекорректно();
	КонецЕсли;
...
Процедура ВставитьРазрывНаНовуюСтроку(ПечатнаяФорма) Экспорт
	...
	смартУправлениеПечатьюСлужебный.ВставитьРазрывНаНовуюСтроку(ПечатнаяФорма);
...
Функция ИнициализироватьМакетОфисногоДокументаСКД(ДвоичныеДанныеМакета) Экспорт
	...
	ДеревоМакета = смартУправлениеПечатьюСлужебный.МакетИзДвоичныхДанныхСКД(ДвоичныеДанныеМакета);
	СтруктураДокумента = ДеревоМакета.СтруктураДокумента;
	
	УзелНачалаТекста = смартУправлениеПечатьюСлужебный.ПолучитьУзел(СтруктураДокумента.ДеревоДокумента, "w:document/w:body");
	СобратьСтроки(УзелНачалаТекста);
	смартУправлениеПечатьюСлужебный.ВосстановитьПолныйТекст(СтруктураДокумента.ДеревоДокумента, СтруктураДокумента.Гиперссылки);
...
Процедура ДобавитьШтампыВОфисныйДокумент(АдресДокумента, ЭлектронныеПодписи) Экспорт
	...
	ДеревоМакетаШтампа = смартУправлениеПечатьюСлужебный.ПрочитатьXMLВДерево(ЧтениеXML);
	...
	ПараметрыПоискаУзлов = смартУправлениеПечатьюСлужебный.ПараметрыПоискаУзлов();
	ПараметрыПоискаУзлов.ИмяАтрибута = "w:name";
	ПараметрыПоискаУзлов.ЗначенияАтрибута = "V8DSStamp";
	смартУправлениеПечатьюСлужебный.НайтиУзлыПоСодержимому(СтруктураДокумента.ДеревоДокумента, "w:bookmarkStart", МассивЗакладок, ПараметрыПоискаУзлов);
	...
	смартУправлениеПечатьюСлужебный.ПеренестиПараметрыФорматирования(УзелШтампа, УзелЗакладкиШтампа);
	 
	ИндексРазмещения = Неопределено;
	смартУправлениеПечатьюСлужебный.НайтиУзелРазмещенияШтампа(РодительУзлаШтампа, ИндексРазмещения);
	...
	Для Каждого Подпись Из ЭлектронныеПодписи Цикл
		...
		УзелРазмещенияШтампа = смартУправлениеПечатьюСлужебный.СкопироватьУзел(РодительУзлаШтампа, ИндексРазмещения, УзелШтампа);
		смартУправлениеПечатьюСлужебный.СоздатьПоследующие(УзелРазмещенияШтампа, УзелШтампа);
		УстановитьПараметрыВДереве(ЗначенияДляЗаполнения, УзелРазмещенияШтампа, ДеревоМакета);
	КонецЦикла;
	ПутьКДокументу = смартУправлениеПечатьюСлужебный.СобратьФайлОфисногоДокумента(ДеревоМакета);
...
Процедура КонвертироватьПараметры(Дерево) Экспорт
	...
	смартУправлениеПечатьюСлужебный.КонвертироватьПараметры(Дерево);
...
Процедура УдалитьНезначащиеАтрибуты(Дерево) Экспорт
	...
	смартУправлениеПечатьюСлужебный.УдалитьНезначащиеАтрибуты(Дерево, НезначащиеАтрибуты);
...
Функция НастройкиПечати() Экспорт
	...
	смартУправлениеПечатьюПереопределяемый.ПриОпределенииНастроекПечати(Настройки);
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	Для Каждого КомандаПечати Из КомандыПечати Цикл
		...
		Команда.Обработчик = "смартУправлениеПечатьюСлужебныйКлиент.ОбработчикКоманды";
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "смартУправлениеПечатью.ДобавитьРольРедактированиеПечатныхФормВПрофилиСБазовымиПравами";
...
Функция ОбъектМетаданныхМакета(ПутьКМакету) Экспорт
	...
	Если Не ЭтоОбщийМакет Тогда
		ОбъектМетаданных = ОбъектМетаданныхПоПолномуИмени(ИмяОбъекта);
		Если ОбъектМетаданных = Неопределено Тогда
			Возврат Неопределено;
		КонецЕсли;
...
Функция КоллекцииПолейИсточниковДанных(ИсточникиДанных) Экспорт
	...
	Для Каждого ОписаниеОбъекта Из ОписанияОбъектов Цикл
		...
		Для Каждого СхемаКомпоновкиДанных Из СхемыКомпоновкиДанныхИсточникаПолей(ИмяОбъектаМетаданных, Ложь) Цикл
			Результат.Добавить(смартКонструкторФормул.КоллекцияПолей(СхемаКомпоновкиДанных.Значение), ИмяИсточникаДанных);
		КонецЦикла;
	КонецЦикла;
...
Функция ОбщиеПоляИсточниковДанных(ИсточникиДанных) Экспорт
	...
	Для Каждого ОписаниеОбъекта Из ОписанияОбъектов Цикл
		...
		Для Каждого СхемаКомпоновкиДанных Из СхемыКомпоновкиДанныхИсточникаПолей(ИмяОбъектаМетаданных, Ложь) Цикл
			КоллекцияПолей = смартКонструкторФормул.КоллекцияПолей(СхемаКомпоновкиДанных.Значение);
...
Процедура ОбновитьСписокДоступныхПолей(Форма, КоллекцииПолей, ИмяСпискаПолей) Экспорт
	...
	смартКонструкторФормул.ОбновитьКоллекцииПолей(Форма, КоллекцииПолей, ИмяСпискаПолей);
	
КонецПроцедуры

Функция СписокОператоров(ДополнительныеПоля = Неопределено) Экспорт
	
	СписокОператоров = смартКонструкторФормул.СписокОператоров();
	...
	Возврат смартКонструкторФормул.КоллекцияПолей(СписокОператоров);
...
Функция ФормулаИзПредставления(Форма, Представление) Экспорт
	...
	Возврат смартКонструкторФормул.ФормулаИзПредставления(Форма, Представление);
...
Функция ПредставлениеФормулы(Знач Форма, Знач Формула) Экспорт
	...
	Возврат смартКонструкторФормул.ПредставлениеФормулыПоДаннымФормы(Форма, Формула);
	
КонецФункции

Функция ИсточникиДанныхМакета(ПутьКМакету) Экспорт
	
	ИсточникиДанныхМакета = Справочники.смартМодулиПечатныхФорм.ИсточникиДанныхМакета(ПутьКМакету);
	...
	Для Каждого ПутьКДанным Из Поля Цикл
		Пока ЗначениеЗаполнено(ПутьКДанным) Цикл
			Если ТребуемыеПоля[ПутьКДанным] = Неопределено Тогда
				ОписаниеПоля = смартКонструкторФормулСлужебный.ОписаниеПоля(ПутьКДанным, СпискиПолей);
				...
				Если ЗначениеЗаполнено(КодЯзыкаБезРегиона) И Не СтрЗаканчиваетсяНа(ПутьКДанным, "." + КодЯзыкаБезРегиона)
					...
					ОписаниеПоля = смартКонструкторФормулСлужебный.ОписаниеПоля(ПутьКДанным + "." + КодЯзыкаБезРегиона, СпискиПолей);
...
Функция ИдентификаторОбласти(Область) Экспорт
	...
	Возврат смартУправлениеПечатьюКлиентСервер.ИдентификаторОбласти(Область);
...
Функция СформироватьОфисныйДокумент(Макет, МассивОбъектов, ОбъектыПечати, КодЯзыка, ПараметрыПечати) Экспорт
	...
	Для Каждого Элемент Из СтруктураДокумента.ПараметрыТекста Цикл
		...
		Для Каждого Выражение Из ПараметрыТекста Цикл
			...
			ЭлементыФормулы = смартКонструкторФормулСлужебный.ЭлементыФормулы(Выражение);
	...
	смартУправлениеПечатьюСлужебный.ПроиндексироватьУзлы(СтруктураДокумента.ДеревоДокумента);
	смартУправлениеПечатьюСлужебный.НайтиОбласти(СтруктураДокумента, ИменаТабличныхЧастейОбъекта);
	...
	Для Каждого Область Из СтруктураДокумента.Области Цикл
		...
		Иначе
			...
			Для Каждого Параметр Из Область.Параметры Цикл
				ЭлементыФормулы = смартКонструкторФормулСлужебный.ЭлементыФормулы(Параметр);
	...
	Для Каждого ОбъектПечати Из МассивОбъектов Цикл
		...
		Для ИндексОбласти = 0 По ДеревоМакетаДляЗаполнения.ОбластиДляВывода.ВГраница() Цикл
			...
			Иначе
				...
				Для Каждого УзелДобавления Из ОбластьДляВывода.Строки Цикл
					НовыйУзел = смартУправлениеПечатьюСлужебный.СкопироватьУзел(РодительДобавления, ИндексДобавления, УзелДобавления);
					УказателиНаОбласти.Вставить(НовыйУзел.Индекс, НовыйУзел);
					смартУправлениеПечатьюСлужебный.СоздатьПоследующие(НовыйУзел, УзелДобавления);
		...
		АдресХранилищаПечатнойФормы = смартУправлениеПечатьюСлужебный.ПолучитьПечатнуюФорму(ДеревоМакетаДляЗаполнения);
...
Функция ПолучитьПечатнуюФорму(ДеревоМакета, АдресХранилища = Неопределено) Экспорт
	...
	Возврат смартУправлениеПечатьюСлужебный.ПолучитьПечатнуюФорму(ДеревоМакета, АдресХранилища);
	
КонецФункции

Функция ИмяТегаУсловие() Экспорт

	Возврат смартУправлениеПечатьюКлиентСервер.ИмяТегаУсловие();

КонецФункции

Функция ПолучитьУзлыТекстов(УзелДокумента, СоответствиеУзлов = Неопределено) Экспорт
	
	Возврат смартУправлениеПечатьюСлужебный.ПолучитьУзлыТекстов(УзелДокумента, СоответствиеУзлов);
...
Функция СформироватьПечатныеФормы(Знач ИмяМенеджераПечати, Знач ИменаМакетов, Знач МассивОбъектов, Знач ПараметрыПечати, 
	...
	Для Каждого ИмяМакета Из ИменаМакетов Цикл
		...
		Если ДопустимыеТипыОбъектовПечати <> Неопределено И ДопустимыеТипыОбъектовПечати.Количество() > 0 Тогда
			...
			КонецЕсли;
		КонецЕсли;
		смартУправлениеПечатьюПереопределяемый.ПередПечатью(Идентификатор, ОбъектыСоответствующиеПечатнойФорме, ПараметрыПечати);
	...
	смартУправлениеПечатьюПереопределяемый.ПриПечати(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода);
...
Функция КомандыПечатиОбъекта(ОбъектМетаданных, СПечатнымиФормами = Истина) Экспорт
	...
	смартУправлениеПечатьюПереопределяемый.ПриПолученииКомандПечати(Источник.ПолноеИмя, КомандыПечати);
	...
	Для Каждого КомандаПечати Из КомандыПечати Цикл
		...
		Если КомандаПечати.ВыполнятьВФоновомЗадании = Неопределено Тогда
			КомандаПечати.ВыполнятьВФоновомЗадании = НРег(КомандаПечати.МенеджерПечати) = НРег("смартУправлениеПечатью")
...
Процедура ПроверитьРазметкуТабличногоДокументаПоОбъектамПечати(ТабличныйДокумент, ОбъектыПечати, Знач МенеджерПечати, Знач Идентификатор)
	...
	ТекстОшибкиРазметки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
		...
		Идентификатор, "смартУправлениеПечатью.ЗадатьОбластьПечатиДокумента");
...
Функция ПодписиИПечатиОбъектов(Знач ОбъектыПечати) Экспорт
	...
	смартУправлениеПечатьюПереопределяемый.ПриПолученииПодписейИПечатей(СписокОбъектов, ПодписиИПечати);
...
Функция ПодготовитьКоллекциюПечатныхФорм(Знач Идентификаторы) Экспорт
	...
	Результат = Новый ТаблицаЗначений;
	Для Каждого ИмяКолонки Из смартУправлениеПечатьюКлиентСервер.ИменаПолейКоллекцииПечатныхФорм() Цикл
...
Функция СкомпоноватьДанные(Параметры)
	...
	Для Каждого НаборДанных Из СхемаКомпоновкиДанных.НаборыДанных Цикл
		Если ТипЗнч(НаборДанных) = Тип("НаборДанныхОбъектСхемыКомпоновкиДанных") Тогда
			...
			Прервать;
		КонецЕсли;
		//++ #Б-3312 смарт: Лопато 20.05.2025 //Задание на подготовку проекта договора поставки. Доработка Условий оплаты. Этап 1.  
		Если ТипЗнч(НаборДанных) = Тип("НаборДанныхЗапросСхемыКомпоновкиДанных") Тогда
			ТекстЗапроса = НаборДанных.Запрос;
			Если СтрНайти(ТекстЗапроса, "&Предмет") <> 0 Тогда
				ПараметрПредмет = СхемаКомпоновкиДанных.Параметры.Добавить(); 
				...
				Если Объекты.Количество() > 0 Тогда
					ПараметрПредмет.Значение = Объекты[0];
				КонецЕсли;	
			КонецЕсли;	
		КонецЕсли;
		//-- #Б-3312 смарт: Лопато 20.05.2025
	КонецЦикла;
	...
	Если КлючевоеПоле = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			...
			"смартУправлениеПечатьюПереопределяемый.ПриОпределенииИсточниковДанныхПечати");
	КонецЕсли;
...
Функция ИмяМодуляПечати()
	Возврат Метаданные.ОбщиеМодули.смартУправлениеПечатью.Имя;
КонецФункции
...
Функция ОписаниеСписковПолей(ИсточникиДанныхПечати, Операторы = Неопределено, ИсточникДанных = Неопределено) Экспорт
	...
	ОписаниеСписковПолей = смартКонструкторФормулСлужебный.ОписаниеСписковПолей();
	
	ИсточникиДоступныхПолей = смартКонструкторФормулСлужебный.КоллекцияИсточниковДоступныхПолей();
	Для Каждого Элемент Из ИсточникиДанныхПечати Цикл
		...
		ИсточникДоступныхПолей.КоллекцияПолей = смартКонструкторФормулСлужебный.КоллекцияПолей(СхемаКомпоновкиДанных);
	...
	ОписаниеСпискаПолей.ПриОпределенииИсточниковДоступныхПолей = "смартУправлениеПечатью";
	
	Если Операторы <> Неопределено Тогда
		ИсточникиДоступныхПолей = смартКонструкторФормулСлужебный.КоллекцияИсточниковДоступныхПолей();
		ИсточникДоступныхПолей = ИсточникиДоступныхПолей.Добавить(); 
		ИсточникДоступныхПолей.КоллекцияПолей = смартКонструкторФормулСлужебный.КоллекцияПолей(Операторы);
...
Процедура ПриОпределенииИсточниковДоступныхПолей(ИмяИсточникаПолей, ТипИсточникаПолей, ИсточникиДоступныхПолей, УникальныйИдентификаторФормы) Экспорт
	...
	Для Каждого Элемент Из СхемыКомпоновкиДанных Цикл
		...
		ИсточникДоступныхПолей.ИдентификаторСхемыКомпоновкиДанных = ИдентификаторСхемыКомпоновкиДанных;
		ИсточникДоступныхПолей.КоллекцияПолей = смартКонструкторФормул.КоллекцияПолей(СхемаКомпоновкиДанных);
		ИсточникДоступныхПолей.Замещать = Истина;
	КонецЦикла;
	
КонецПроцедуры

Функция СхемыКомпоновкиДанныхИсточникаПолей(ИмяИсточникаПолей, ДобавитьОбщиеРеквизиты = Ложь, ТипИсточникаПолей = Неопределено) Экспорт
	
	ИсточникиПолей = Новый СписокЗначений;
	
	ОбъектМетаданных = ОбъектМетаданныхПоПолномуИмени(ИмяИсточникаПолей);
	Если ОбъектМетаданных <> Неопределено Тогда
		Если ОбъектМетаданных.Макеты.Найти("ДанныеПечати") <> Неопределено Тогда
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПрефиксацияОбъектов") Тогда
				...
				ДобавитьРасширениеПоляНомер(СхемаКомпоновкиДанных);
			КонецЕсли;
	...
	Если ДобавитьОбщиеРеквизиты Тогда
		ИсточникиПолей.Добавить(ПолучитьОбщийМакет("смартДанныеПечатиОбщиеРеквизиты"), "ОбщиеРеквизиты");	
	КонецЕсли;
	...
	Если СтрЗаканчиваетсяНа(ИмяИсточникаПолей, ".Наименование") Или СтрЗаканчиваетсяНа(ИмяИсточникаПолей, ".НаименованиеДляПечати") Тогда
		ИсточникиПолей.Добавить(ПолучитьОбщийМакет("смартДанныеПечатиРегистрСимволов"), "ДанныеПечатиРегистрСимволов");	
	КонецЕсли;
...
Процедура ПриПодготовкеДанныхПечати(Объекты, ВнешниеНаборыДанных, ИдентификаторСхемыКомпоновкиДанных, КодЯзыка, ДополнительныеПараметры)
	...
	смартУправлениеПечатьюПереопределяемый.ПриПодготовкеДанныхПечати(Объекты, ВнешниеНаборыДанных, ИдентификаторСхемыКомпоновкиДанных, КодЯзыка, ДополнительныеПараметры);
...
Функция ТекстЗапроса(ТипыИзменяемыхОбъектов, ОграничитьВыборку = Ложь)
	...
	Для Каждого ИмяОбъекта Из СтрРазделить(ТипыИзменяемыхОбъектов, ",", Ложь) Цикл
		ОбъектыМетаданных.Добавить(ОбъектМетаданныхПоПолномуИмени(ИмяОбъекта));
	КонецЦикла;
...
Функция СхемаКомпоновкиДанныхИзТаблицыЗначений(ТаблицаЗначений)
	...
	Возврат смартКонструкторФормулСлужебный.СхемаКомпоновкиДанныхИзТаблицыЗначений(ТаблицаЗначений);
	
КонецФункции

Функция СхемаКомпоновкиДанныхИзДереваЗначений(ДеревоЗначений)
	
	Возврат смартКонструкторФормулСлужебный.СхемаКомпоновкиДанныхИзДереваЗначений(ДеревоЗначений);
...
Функция СхемаКомпоновкиДанныхДополнительныхРеквизитовИСведений(ИмяОбъектаМетаданных)
	...
	Если МодульУправлениеСвойствамиСлужебный <> Неопределено Тогда
		Для Каждого ВидСвойства Из ВидыСвойств Цикл
			СписокСвойств = СписокСвойствДляВидаОбъектов(ИмяОбъектаМетаданных, ВидСвойства);
...
Функция ДополнительныеРеквизитыИСведения(Объекты, ИдентификаторСхемыКомпоновкиДанных, КодЯзыка)
	...
	МодульУправлениеСвойствамиСлужебный = ОбщегоНазначения.ОбщийМодуль("УправлениеСвойствамиСлужебный");
	Для Каждого ВидСвойства Из ВидыСвойств Цикл
		Свойства = СписокСвойствДляВидаОбъектов(ИмяОбъектаМетаданных, ВидСвойства).ВыгрузитьКолонку("Свойство");
...
Процедура ПодготовитьУзлыУсловныхОбластей(ДеревоXML, Гиперссылки)
	...
	Для Каждого СоответствиеЗамены Из ТаблицаСортировки Цикл
		...
		смартУправлениеПечатьюСлужебный.НайтиУзлы(ДеревоXML, СоответствиеЗамены.Ключ, УзлыУсловия);

		Для Каждого Узел Из УзлыУсловия Цикл
			Если Узел.Строки.Количество() Тогда
				СобратьСтроки(Узел);
				смартУправлениеПечатьюСлужебный.ВосстановитьПолныйТекст(Узел, Гиперссылки);
			КонецЕсли;
		КонецЦикла;
...
Процедура РазделитьПрогон(УзелУсловия, ФрагментыУзла, Гиперссылки)
	...
	Для ИндексФрагментаУзла = 0 По ФрагментыУзла.ВГраница() Цикл
		...
		Иначе
			УзелДляОбработки = смартУправлениеПечатьюСлужебный.СкопироватьУзел(РодительДобавления, ИндексДобавления, УзелУсловия);
		КонецЕсли;
		ИндексДобавления = ИндексДобавления + 1;
		
		УзелДляОбработки.Текст = ФрагментыУзла[ИндексФрагментаУзла];
	КонецЦикла;
	смартУправлениеПечатьюСлужебный.ВосстановитьПолныйТекст(РодительДобавления, Гиперссылки);
КонецПроцедуры
...
Функция КопияДереваМакета(ДеревоМакета)
	...
	смартУправлениеПечатьюСлужебный.КопироватьСодержимоеКаталога(КаталогИсточник, КаталогПриемник);
...
Процедура УстановитьПараметрыВДереве(Знач СоответствиеЗамен, ДеревоXML, ДеревоМакета = Неопределено, ДобавлятьСвязи = Истина)
	...
	Для Каждого СоответствиеЗамены Из ТаблицаСортировки Цикл
		...
		смартУправлениеПечатьюСлужебный.НайтиУзлы(ДеревоXML, СоответствиеЗамены.Ключ, МассивУзлов);
		
		Для Каждого Узел Из МассивУзлов Цикл
			Если Узел.Строки.Количество() = 0 ИЛИ Узел.ИмяТега = "w:hyperlink" Тогда
				смартУправлениеПечатьюСлужебный.УстановитьЗначениеВДокумент(Узел, СоответствиеЗамены, ДеревоМакета, ДобавлятьСвязи);
			Иначе
				...
				Для Каждого СобранныйУзел Из МассивСобранныхУзлов Цикл
					смартУправлениеПечатьюСлужебный.УстановитьЗначениеВДокумент(СобранныйУзел, СоответствиеЗамены, ДеревоМакета, ДобавлятьСвязи);
				КонецЦикла;
			КонецЕсли;
		КонецЦикла;
...
Функция КоллекцияПолейОбщиеРеквизиты()
	...
	Возврат смартКонструкторФормул.КоллекцияПолей(ПолучитьОбщийМакет("смартДанныеПечатиОбщиеРеквизиты"));
...
Функция ПолучитьПропущенногоРодителя(СтрокаДобавления, ДеревоДляВывода)
	...
	Для Каждого Родитель Из Родители Цикл
		РодительДобавления = смартУправлениеПечатьюСлужебный.СкопироватьУзел(РодительДобавления, ИндексДобавления, Родитель);
		ИндексДобавления = 0;
	КонецЦикла;
...
Функция СписокСвойствДляВидаОбъектов(ВидОбъектов, Знач ВидСвойств) Экспорт
	...
	Если ВидОбъектов = "Справочник.ШаблоныДокументов" Тогда 
		ИмяПредопределенныхДанных = "Справочник.ДокументыПредприятия";

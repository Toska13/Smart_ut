...
Процедура Выполнить_Задачу(ЗадачаСсылка, ДействиеПоУмолчанию = Ложь, РазрешитьВыполнитьСПодчиненнымиЗадачами = Ложь) Экспорт
	...
	Если Не РазрешитьВыполнитьСПодчиненнымиЗадачами И ЕстьПодчиненныеЗадачи(ЗадачаСсылка) Тогда
...
Процедура ВыполнитьЗадачу(ЗадачаСсылка, ВариантВыполнения = Неопределено, ОтложитьНаДату = Неопределено, РезультатВыполнения = "", ДополнительныеПараметры = Неопределено) Экспорт
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ЗадачаОбъект.БизнесПроцесс) И ЗадачаОбъект.БизнесПроцесс.ЗавершенДосрочно Тогда
		...
		Если НЕ ДополнительныеПараметры = Неопределено Тогда
			...
			КонецЕсли;
			Если ДополнительныеПараметры.Свойство("ПричинаДосрочногоЗавершения") Тогда
				ЗадачаОбъект.ДополнительныеСвойства.Вставить("ПричинаДосрочногоЗавершения", ДополнительныеПараметры.ПричинаДосрочногоЗавершения);
			КонецЕсли;
		КонецЕсли;
...
Функция ПеренаправитьЗадачи(Знач МассивЗадач, Знач ИнфоОПеренаправлении, Знач ТолькоПроверка = Ложь,
	...
	Для Каждого Задача Из МассивЗадач Цикл
		...
		Если НЕ CRM_БизнесПроцессыСервер.ЗадачаДоступнаДляПринятияКИсполнению(Задача.Ссылка) Тогда
			Результат = Ложь;
			Если ТолькоПроверка Тогда
				ПричинаОтказа = НСтр("ru='Невозможно перенаправить чужую задачу.';en='It is impossible to redirect someone else task.'");
				Прервать;
			КонецЕсли;
		КонецЕсли;
...
Функция ФормаВыполненияЗадачи(Знач ЗадачаСсылка, Знач ИзКалендаря = 0) Экспорт
	...
	Если ТипЗнч(ЗадачаСсылка) <> Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неправильный тип параметра ЗадачаСсылка (передан: %1; ожидается: %2)';en='Invalid type of parameter ЗадачаСсылка (passed: %1; expected: %2)'"),
	...
	Если ТипБизнесПроцесса.Имя = "CRM_БизнесПроцесс" Тогда
		...
		Если НЕ ЗадачаСсылка.CRM_ТочкаМаршрута.Вид = Перечисления.CRM_ВидыТочекМаршрута.ВложенныйБизнесПроцесс Тогда
			...
			ИначеЕсли ЗадачаСсылка.БизнесПроцесс.КартаМаршрута.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта
				...
				ПараметрыФормы.ПараметрыФормы.Вставить("ОткрываемаяЗадача", ЗадачаСсылка);
...
// Проверить возможность в следующее состояние интереса.
//
// Параметры:
//  Объект   - Данные формы - Интерес.
...
//
Функция ПроверитьВозможностьПереходаБПИнтереса(Знач Объект, ЗадачаИсполнителя, ВариантВыполнения) Экспорт
...
//
// Параметры:
//	ЗадачаИсполнителя - ЗадачаСсылка - Задача исполнителя.
//
// Возвращаемое значение:
//	Структура - настройки этапа.
//
Функция ПолучитьНастройкиЭтапа(ЗадачаИсполнителя) Экспорт
	
	Возврат CRM_БизнесПроцессыСервер.ПолучитьНастройкиЭтапа(ЗадачаИсполнителя);
...
//
//
Процедура ПолучитьТекущиеЗадачиПеренаправитьНаОтветственного(ОбъектБП, СтарыйОтветственный) Экспорт
	
	Исполнитель		= ОбъектБП.Ответственный;
	Ответственный	= ОбъектБП.Ответственный;
	
	Запрос			= Новый Запрос;
	Запрос.УстановитьПараметр("ОбъектБП", ОбъектБП);
	Запрос.УстановитьПараметр("Исполнитель", СтарыйОтветственный);
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	ЗадачаИсполнителя.Предмет = &ОбъектБП
	...
	МассивЗадач = Результат.ВыгрузитьКолонку("Задача");
	МассивПеренаправленныхЗадач = Неопределено;
	ЗадачиПеренаправлены = ПеренаправитьЗадачи(МассивЗадач, Исполнитель, Ложь, МассивПеренаправленныхЗадач);
	
	Результат.Свернуть("БизнесПроцесс");
	Для Каждого Процесс Из Результат Цикл
		Если НЕ Процесс.БизнесПроцесс.Ответственный = Ответственный Тогда
			БизнесПроцесОбъект = Процесс.БизнесПроцесс.ПолучитьОбъект();
...
Функция ПеренестиСобытияЗадачи(СобытияЗадачи, ДатаЗадачи, Знач ВариантЗавершенияСобытий = Неопределено, Знач ДатаПереноса = Неопределено,
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось ';en='Failed '")
			+ ?(ВариантЗавершенияСобытий = "ЗавершатьСобытия", НСтр("ru='завершить ';en='complete '"), НСтр("ru='перенести '"))
			+ НСтр("ru='события: ';en='events: '")
...
Функция ВыполнитьПереносЗадачи(ЭтаФормаИлиЗадача, ВариантПереноса, Знач ДатаПереноса = Неопределено, ПроверятьНаРабочийДень = Истина,
	...
	Исключение
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось перенести задачу: ';en='Failed to relocate task:'")
...
Процедура НазначитьРодителя(Задача, Родитель, Партнер = Неопределено, КонтактноеЛицо = Неопределено, Проект = Неопределено, ДополнительныеРеквизиты = Неопределено, НазначитьВсемВИерархии = Истина) Экспорт
	...
	Исключение
		...
		СтрОшибка = КраткоеПредставлениеОшибки(ИнфОбОшибке);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось назначить нового родителя задаче: ';en='Failed to appoint new parent for task: '")
			+ """" + Строка(Задача) + """"
			+ ?(ЗначениеЗаполнено(СтрОшибка), ": " + СтрОшибка, "."));
		//
	КонецПопытки;
...
Функция ПометитьНаУдаление(Задача, ПометкаУдаления) Экспорт
	...
	Исключение
		ИнфОбОшибке = ИнформацияОбОшибке();
		СтрОшибка = КраткоеПредставлениеОшибки(ИнфОбОшибке);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось удалить задачу: ';en='Failed to delete task: '")
			+ """" + Строка(Задача) + """"
			+ ?(ЗначениеЗаполнено(СтрОшибка), ": " + СтрОшибка, "."));
		//
		Возврат Ложь;
...
Функция ВыполнитьЛичнуюЗадачуСПодчиненными(Задача) Экспорт
	...
	Попытка
		...
		Если ЗначениеЗаполнено(Задача.Предмет) И ТипЗнч(Задача.Предмет) = Тип("ДокументСсылка.УдалитьCRM_Событие") Тогда
			Попытка
				...
				СобытиеОбъект.Записать();
			Исключение
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось изменить состояние, вероятно событие заблокировано:';en='Failed to change state, event is block:'")
	...
	Исключение
		ИнфОбОшибке = ИнформацияОбОшибке();
		СтрОшибка = КраткоеПредставлениеОшибки(ИнфОбОшибке);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось выполнить задачу: ';en='Failed to perform task: '")
			+ """" + Строка(Задача) + """"
			+ ?(ЗначениеЗаполнено(СтрОшибка), ": " + СтрОшибка, "."));
		//
		Возврат Ложь;
...
Процедура ИзменитьМаркетинговоеМероприятие(СправСсылка) Экспорт
	...
	Исключение
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось изменить состояние, вероятно элемент справочника заблокирован';en='Failed to change state, probably catalog item blocked'"));	
	КонецПопытки;
...
Процедура ЗадачаИсполнителяМодульОбъектаОбработкаЗаполнения(Объект, ДанныеЗаполнения) Экспорт
	...
	// +Рарус alexpr
	ИЛИ ТипЗнч(ДанныеЗаполнения) = Тип("СправочникСсылка.CRM_Диалоги")
	// -Рарус alexpr
...
Процедура ЗадачаИсполнителяМодульОбъектаПриЗаписи(Объект, Отказ) Экспорт
	...
	Если (НЕ Отказ И ЗначениеЗаполнено(Объект.CRM_Родитель))
		...
		Если НЕ Объект.ДополнительныеСвойства.Свойство("НеОбновлятьПоказателиЗадачВИерархии") Тогда
			Если Объект.ДополнительныеСвойства.ПредыдущееЗначениеВыполнена И Не Объект.Выполнена Тогда
				ПодчиненныеЗадачи = ПолучитьПодчиненныеЗадачи(Объект.Ссылка);
				Если ПодчиненныеЗадачи.Количество() > 0 Тогда
					ОбновитьПоказателиЗадачВИерархии(ПодчиненныеЗадачи[0].Ссылка);
				Иначе
					Если ЗначениеЗаполнено(Объект.CRM_Родитель) Тогда
						ОбновитьПоказателиЗадачВИерархии(Объект.Ссылка);
					КонецЕсли;
				КонецЕсли;
			Иначе
				ОбновитьПоказателиЗадачВИерархии(Объект.Ссылка);
	...
	Если НЕ Объект.CRM_Личная И Не Отказ И Объект.ДополнительныеСвойства.Свойство("ЭтоНовый") И Объект.ДополнительныеСвойства.ЭтоНовый Тогда
		Если Объект.БизнесПроцесс.КартаМаршрута.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта Тогда
			НастройкаЭтапа = CRM_БизнесПроцессыСервер.ПолучитьНастройкиЭтапа(Объект.CRM_ТочкаМаршрута);
			Если ЗначениеЗаполнено(НастройкаЭтапа.Состояние) Тогда
				Попытка
					Если Объект.ДополнительныеСвойства.Свойство("ОбъектПроцесса") Тогда
						ОбъектБП = Объект.ДополнительныеСвойства.ОбъектПроцесса;
						БлокироватьОбъект = Ложь;
					Иначе
						ОбъектБП = Объект.БизнесПроцесс.Интерес.ПолучитьОбъект();
						БлокироватьОбъект = Истина;
					КонецЕсли;
					Если СтрНайти(Объект.БизнесПроцесс.РеквизитОбъектаСтрокой,"%Свойство%") Тогда
						
						СсылкаНаСвойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.ПолучитьСсылку(Новый УникальныйИдентификатор(СтрЗаменить(Объект.БизнесПроцесс.РеквизитОбъектаСтрокой,"%Свойство%","")));
						
						МассивСтрок = ОбъектБП.ДополнительныеРеквизиты.НайтиСтроки(Новый Структура("Свойство",СсылкаНаСвойство));
						
						Если МассивСтрок.Количество() > 0 Тогда
							Если МассивСтрок[0].Значение <> НастройкаЭтапа.Состояние Тогда
								Попытка
									Если БлокироватьОбъект Тогда
										ОбъектБП.Заблокировать();
									КонецЕсли;
									МассивСтрок[0].Значение = НастройкаЭтапа.Состояние;
									ОбъектБП.Записать();
								Исключение
								КонецПопытки;
							КонецЕсли;
						Иначе
							Попытка
								Если БлокироватьОбъект Тогда
									ОбъектБП.Заблокировать();
								КонецЕсли;
								НовСтрока = ОбъектБП.ДополнительныеРеквизиты.Добавить();
								НовСтрока.Свойство = СсылкаНаСвойство;
								НовСтрока.Значение = НастройкаЭтапа.Состояние;
								ОбъектБП.Записать();
							Исключение
							КонецПопытки;
						КонецЕсли;
					Иначе
						Если Объект.БизнесПроцесс.Интерес[Объект.БизнесПроцесс.РеквизитОбъектаСтрокой]<>НастройкаЭтапа.Состояние Тогда
							Попытка
								Если БлокироватьОбъект Тогда
									ОбъектБП.Заблокировать();
								КонецЕсли;
								ОбъектБП[Объект.БизнесПроцесс.РеквизитОбъектаСтрокой] = НастройкаЭтапа.Состояние;
								ОбъектБП.Записать();
							Исключение
							КонецПопытки;
						КонецЕсли;
	...
	// +Рарус alexpr
	Если ТипЗнч(Объект.Предмет) = Тип("СправочникСсылка.CRM_Диалоги") И Объект.CRM_Личная И Не Отказ
		 И Объект.ДополнительныеСвойства.Свойство("ЭтоНовый")
		 И Объект.ДополнительныеСвойства.ЭтоНовый Тогда
		Справочники.CRM_Диалоги.ДобавитьДействие(Объект.Предмет,
			 Перечисления.CRM_ДействияСДиалогами.СозданаЗадача, Объект.Дата, Объект.Исполнитель,
			 Объект.Ссылка);
	КонецЕсли;	
	// -Рарус alexpr
	
	Если Объект.РольИсполнителя = Справочники.РолиИсполнителей.CRM_ДинамическаяАдресация
		И Объект.ДополнительныеСвойства.Свойство("ИсполнителиДинамическойАдресации") Тогда
		РегистрыСведений.CRM_ИсполнителиДинамическойАдресации.ЗаписатьИсполнителейДинамическойАдресации(Объект.Ссылка,
			Объект.ДополнительныеСвойства.ИсполнителиДинамическойАдресации); 
	КонецЕсли;	
...
Функция ЗавершитьЛичныеЗадачиПоПредметуВПривилигированномРежмиме(Ссылка, ИсполнительЗадачи, ПредметЗадачи)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ЕстьПодчиненныеЗадачи(Выборка.Задача) Тогда

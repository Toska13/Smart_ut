		...
		Если ДополнительныеПараметры.Свойство("МассивОбъектов") Тогда
			Если Форма.Свойства_ИспользоватьСвойства Тогда
				...
				Для каждого ОбъектДопРеквизитов из ДополнительныеПараметры.МассивОбъектов Цикл
					...
					КонецЕсли;
					ЗаполнитьДополнительныеРеквизитыВФорме(Форма, ОбъектДопРеквизитов.Объект, , Контекст.СкрытьУдаленные, УдалятьСтарые, НеУчитыватьЗависимости);
					УдалятьСтарые = Ложь;
				КонецЦикла;
			КонецЕсли;
		Иначе
...
//                    При вызове процедуры ПередЗаписьюНаСервере в режиме скрытия удаленных, удаленные значения
//                    очищаются (не переносятся обратно в объект), а режим СкрытьУдаленные устанавливается Ложь.
//  УдалятьСтарые   - Булево - удалять старые.
//  НеУчитыватьЗависимости - Булево - не учитывать зависимости.
//
Процедура ЗаполнитьДополнительныеРеквизитыВФорме(Форма, Объект = Неопределено, ПоляНадписей = Ложь, СкрытьУдаленные = Неопределено, УдалятьСтарые = Истина, НеУчитыватьЗависимости = Ложь) Экспорт
...
// -CRM
		...
		Если ОписаниеСвойства.СтрокаСсылочногоТипа Тогда
			...
			Иначе
				Значение = НСтр("ru='не задано';en='not specified'");
...
// -CRM
	...
	Для Каждого ОписаниеЗависимогоРеквизита Из Форма.Свойства_ОписаниеЗависимыхДополнительныхРеквизитов Цикл
		...
		// +CRM
		Если НеУчитыватьЗависимости ИЛИ ОписаниеСвойств.Найти(ОписаниеЗависимогоРеквизита.ИмяРеквизитаЗначение, "ИмяРеквизитаЗначение") = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		// -CRM
...
// -CRM
			...
			Команда.Заголовок   = НСтр("ru='Начать/закончить редактирование';en='Start/end editing'");
			Команда.Действие    = "Подключаемый_СвойстваВыполнитьКоманду";
			Команда.Подсказка   = НСтр("ru='Начать/закончить редактирование';en='Start/end editing'");

...
Процедура ДобавитьНапоминание(Объект, ТекстНапоминания = Неопределено, ДопПараметры = Неопределено, ДатаНапоминания = Неопределено, ДляТекущего = Ложь) Экспорт
	...
	Иначе
		...
		ИначеЕсли ДопПараметры <> Неопределено И ДопПараметры.Свойство("Пользователь") И ЗначениеЗаполнено(ДопПараметры.Пользователь) Тогда
...
Процедура СоздатьНапоминаниеОНовомИнтересе(Ссылка) Экспорт 
	...
	Если Ссылка.ТипОбращения = Справочники.CRM_ТипыОбращений.Интерес Тогда
		ТекстНапоминания = НСтр("ru='Создан новый интерес клиента '") + ?(ЗначениеЗаполнено(Ссылка.Партнер), Ссылка.Партнер, Ссылка.ПотенциальныйКлиент);
	Иначе
		ТекстНапоминания = НСтр("ru='Создано новое обращение ('") + СокрЛП(Ссылка.ТипОбращения) + ") " + ?(ЗначениеЗаполнено(Ссылка.Партнер), Ссылка.Партнер, Ссылка.ПотенциальныйКлиент);
	КонецЕсли;

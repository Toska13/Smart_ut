...
Процедура ЗагрузитьДанныеКалендаря(Календарь, СеансовыеДанные) Экспорт
	...
	ЦветФонаКалендаря		= КалендарьОбъект.backgroundColor;
	...
	Для Каждого ТекОписаниеКалендаря Из ОписанияВсехКалендарей["items"] Цикл
		...
		Если ИдентификаторКалендаря = ТекОписаниеКалендаря["id"] Тогда
			...
			ЦветФона = ТекОписаниеКалендаря["backgroundColor"];
			Если (ИндексЦвета <> ИндексЦветаКалендаря) ИЛИ (ЦветФонаКалендаря <> ЦветФона) Тогда
				КалендарьОбъект.backgroundColor	= ЦветФона;
	...
	Пока ЗначениеЗаполнено(СтраничныйТокен) Или СтраничныйТокен= "" Цикл
		...
		Если СобытияКалендаря["error"] <> Неопределено Тогда
			ВызватьИсключение 
				СтрШаблон(НСтр("ru='Произошла ошибка синхронизации с Google Calendar: %1';en='Synchronization error occurred with Google Calendar: %1'"),
...
Функция ИдентификацияПриложенияGoogleКорректна(ИдентификацияПриложенияGoogle) Экспорт
	...
	Исключение
		Результат = НСтр("ru='Требуется указать данные в формате JSON';en='It is necessary to specify data in JSON format'");
		Возврат Результат;
	КонецПопытки;
	...
	Результат = СтрШаблон(
		НСтр("ru='Некорректный формат идентификации клиента Google: ожидается один из ключей: %1';en='Incorrect format of client Google identification: one of this keys expect: %1'"),
...
Функция ВосстановитьДатуJSON(Свойство, Значение, Параметры) Экспорт
	...
	Исключение
		...
		НСтр("ru='Синхронизация календаря Google.';en='Google calendar synchronization.'"),
		УровеньЖурналаРегистрации.Предупреждение,,,
		СтрШаблон(
		НСтр("ru='Не удалось прочитать дату JSON для свойства ""%1"": ""%2"".';en='Failed to read date JSON for property ""%1"": ""%2"".'"), 
		Свойство,
		Значение));
...
Процедура ОтключитьКаналСинхронизацииGoogle(Пользователь) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Не удалось отключить канал синхронизации Google.';en='Failed to disable synchronization channel Google.'") + Символы.ПС +
			НСтр("ru='Пользователь: ';en='User: '") + Строка(Пользователь) + Символы.ПС +
			НСтр("ru='Текст ошибки: ';en='Error text: '") + ОписаниеОшибки();
...
Функция ПолучитьКодАвторизации(Пользователь) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Не удалось получить код доступа к Google API.';en='Failed to get access code to Google API.'") + Символы.ПС +
			НСтр("ru='Пользователь: ';en='User: '") + Строка(Пользователь) + Символы.ПС +
			НСтр("ru='Текст ошибки: ';en='Error text: '") + ОписаниеОшибки();
		ЗаписьЖурналаРегистрации("ru='Синхронизация Google'", УровеньЖурналаРегистрации.Предупреждение,,, ТекстСообщения);
...
Функция УдалитьЗаписьОКаналеНаСайте(Пользователь, ИдКалендаря, ИдКанала)
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Не удалось удалить запись о канале синхронизации на сайте pushtest.rarus.ru.';en='Failed to delete record of sync channel on website pushtest.rarus.ru.'") + Символы.ПС +
		НСтр("ru='Пользователь: ';en='User: '") + Строка(Пользователь) + Символы.ПС +
		НСтр("ru='Текст ошибки: ';en='Error text: '") + ОписаниеОшибки();
		ЗаписьЖурналаРегистрации("ru='Синхронизация Google'", УровеньЖурналаРегистрации.Предупреждение,,, ТекстСообщения);

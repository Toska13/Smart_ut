...
//
Функция ПараметрыФискализацииЧека(ДанныеДокумента, СуммаПредоплатыКорректировка = Неопределено, ЭтоВозврат = Ложь, ВерсияФФД = Неопределено) Экспорт
	...
	// Параметры фискализации чека - Структуру для заполнения
	...
	ПараметрыФискализацииЧекаСистемаНалогообложения(ПараметрыФискализацииЧека, Шапка);
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		ПараметрыФискализацииЧекаПозицииЧекаЗаполнить(СтрокаПозицииЧека, СтрокаТЧ, СуммыПоБезЗаказов, СуммаПредоплаты, ВерсияФФД);
...
Процедура ПараметрыФискализацииЧекаСистемаНалогообложения(Знач ПараметрыФискализацииЧека, Знач Шапка)
...
Процедура ПараметрыФискализацииЧекаТорговыйОбъект(Знач ПараметрыФискализацииЧека, Знач Шапка)
	...
	СтруктураДанных.Вставить("ТорговыйОбъект");
	
	ЗаполнитьЗначенияСвойств(СтруктураДанных, Шапка);

	ТорговыйОбъектНаименование = РозничныеПродажи.ТорговыйОбъектККТ(ПараметрыФискализацииЧека.Организация, СтруктураДанных.ТорговыйОбъект);
	ПараметрыФискализацииЧека.ТорговыйОбъект = СтруктураДанных.ТорговыйОбъект;
...
Процедура ПараметрыФискализацииЧекаПозицииЧекаЗаполнить(Знач СтрокаПозицииЧека, Знач СтрокаТЧ, Знач СуммыПоБезЗаказов, Знач СуммаПредоплаты, Знач ВерсияФФД)
	...
	ПараметрыФискализацииЧекаПозицииЧекаШтрихкод(СтрокаПозицииЧека, СтрокаТЧ, ВерсияФФД);
	...
	ПараметрыФискализацииЧекаПозицииЧекаДанныеПоАкцизам(СтрокаПозицииЧека, СтрокаТЧ, ВерсияФФД);
...
Процедура ПараметрыФискализацииЧекаПозицииЧекаДанныеМаркированногоТовара(Знач СтрокаПозицииЧека, Знач СтрокаТЧ)
	...
	Если НЕ (ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаТЧ, "ОсобенностьУчета")
				И ЗначениеЗаполнено(СтрокаТЧ.ОсобенностьУчета)) // ОсобенностьУчета
	...
	ВидПродукции = ИнтеграцияИСУТКлиентСервер.ОсобенностьУчетаПоВидуПродукции(СтрокаТЧ.ОсобенностьУчета);
...
Процедура ПараметрыФискализацииЧекаПозицииЧекаШтрихкод(Знач СтрокаПозицииЧека, Знач СтрокаТЧ, Знач ВерсияФФД)
...
Процедура ПараметрыФискализацииЧекаПозицииЧекаДанныеКомиссионнойПоставки(Знач СтрокаПозицииЧека, Знач СтрокаТЧ)
	...
	Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаТЧ, "ТипЗапасов")
		И ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаТЧ, "Комитент")
		И СтрокаТЧ.ТипЗапасов = Перечисления.ТипыЗапасов.КомиссионныйТовар Тогда
		
		РеквизитыКомитента = ФормированиеПечатныхФорм.СведенияОЮрФизЛице(СтрокаТЧ.Комитент, ТекущаяДатаСеанса());
...
Процедура ПараметрыФискализацииЧекаПозицииЧекаДанныеПоАкцизам(СтрокаПозицииЧека, СтрокаТЧ, ВерсияФФД)
	
	Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаТЧ, "ПодакцизныйТовар")
		И ВерсияФФД = "1.2"
		И СтрокаТЧ.ПодакцизныйТовар = Истина Тогда

...
//		** Значение - (См. ИнтерфейсИСМПСлужебный.ПараметрыКодаМаркировки).
//  * ТекстОшибки - Строка - Текст сообщения об ошибке.
Функция СтатусыКодовМаркировкиПакетно(МассивИсходныхСтрок, СтатусыКодовМаркировкиКеш = Неопределено, Организация = Неопределено, ВключатьСтатусыВложенныхКодов = Ложь, НастройкиРазбора = Неопределено) Экспорт
					...
					Для Каждого КлючИЗначение Из ДанныеОбработки Цикл
						...
						СтатусыПоЗначениямДляПоиска.Вставить(
							...
							ИнтерфейсИСМПСлужебный.ИнициализироватьПараметрыКодаМаркировки(
								КлючИЗначение.Значение, ВидПродукции, Ложь, НастройкиРазбораКодаМаркировки));
			...
			Иначе
				...
				Если ТипЗнч(ДанныеОбработки) = Тип("Массив") Тогда
					...
					Для Каждого ЭлементДанных Из ДанныеОбработки Цикл
						...
						Иначе
							ПараметрыКодаМаркировки = ИнтерфейсМОТПСлужебный.ПараметрыКодаМаркировки(ДанныеКодаМаркировки,, НастройкиРазбора);
	...
	Если ИнтеграцияИСКлиентСервер.ЭтоПродукцияМОТП(ВидПродукции) Тогда
		...
		Иначе
			ПараметрыЗапроса.Добавить(
				СтрШаблон(
					"prefix=%1",
					КодироватьСтроку("2402,2403", СпособКодированияСтроки.КодировкаURL)));
		КонецЕсли;
		...
		URLЗапроса = СтрШаблон(
			"api/v3/true-api/facade/tnved/search%1",
			ПараметрыЗапроса(ПараметрыЗапроса));
		РезультатЗапроса = ИнтеграцияИСМП.ПолучитьДанныеИзСервиса(
			URLЗапроса,
			КлючСессии,
	...
	Если РезультатОтправкиЗапроса.ОтветПолучен Тогда
		...
		Если РезультатОтправкиЗапроса.КодСостояния = 200 Тогда
			...
			Иначе
				...
				Для Каждого СтрокаКода Из  ДанныеОбработки.records Цикл
...
Функция СтатусОбработкиДокументаПоИдентификатору(Идентификатор, Организация = Неопределено, ВидПродукции = Неопределено, Операция = Неопределено) Экспорт
	...
	Если РезультатОтправкиЗапроса.ОтветПолучен Тогда
		...
		Если РезультатОтправкиЗапроса.КодСостояния = 200 Тогда
			...
			Если ДанныеОбработки = Неопределено Или ДанныеОбработки["status"] = Неопределено Тогда
			...
			Иначе
				...
				Если Статус = Перечисления.СтатусыДокументовИСМП.ЕстьОшибки 
					...
					Если ДанныеОбработки["errors"] <> Неопределено Тогда

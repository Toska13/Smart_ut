...
Функция ТекстОшибкиПотериДоступа(ТекстПричиныОшибки = "", Расширенный = Ложь)
	...
	Если НЕ ПустаяСтрока(ТекстПричиныОшибки) Тогда
		ТекстПричиныОшибки = Символы.ПС + ТекстПричиныОшибки;
	КонецЕсли;
	
	Результат = СтрШаблон(Шаблон, ТекстПричиныОшибки);
...
Процедура СообщитьОбОшибкеЗаписатьВЖурнал(ТипСообщения, Отказ = Ложь, ИнформацияОбОшибке = "")
	...
	ИначеЕсли ТипСообщения = "НеУдалосьПрочитатьОбновитьКлючДоступа" Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Ошибка выполнения запроса к сервису:'") 
				+ Символы.ПС 
				+ ПричинаОшибки 
				+ Символы.ПС 

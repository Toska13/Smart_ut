...
Процедура ВыполнениеLongPullingФоновыхЗаданий() Экспорт
	...
	Если ЗначениеЗаполнено(НужныйПлан) тогда
		...
		Пока Продолжаем Цикл
			...
			ТекВремя 	= ТекущаяДатаСеанса() - ТекДата;
			...
			Если  ((ТекВремя < лВремяНачала И ЗначениеЗаполнено(НашеРасписание.ВремяНачала)) 
				ИЛИ (ТекущаяДатаСеанса() < НашеРасписание.ДатаНачала И ЗначениеЗаполнено(НашеРасписание.ВремяНачала))
				ИЛИ (ТекущаяДатаСеанса() > НашеРасписание.ДатаКонца И ЗначениеЗаполнено(НашеРасписание.ДатаКонца))
...
Функция ВыгрузитьЛогФайлНаСайт(ПараметрыОбмена)
	...
	ПолноеИмяФайлаАрхива = ПараметрыОбмена.КаталогЛога + "\reports\" + "Exchange_" + "(" + ПараметрыОбмена.ссылка.Наименование +  ")" +  СокрЛП(Формат(ПараметрыОбмена.ДатаФормирования, "ДФ=yyyy-MM-dd")) + ".zip";
	ЗаписьАрхива = Новый ЗаписьZipФайла(ПолноеИмяФайлаАрхива);
	НовыйСписокФайлов = Новый СписокЗначений;
	ЗаписьАрхива.Добавить(Б_ОбщиеПроцедурыИФункцииСервер.ПолучитьПутьДляПлатформы(ПараметрыОбмена.ПлатформаWindows, ПараметрыОбмена.КаталогЛога + "\reports\" + "Exchange_" + "(" + ПараметрыОбмена.ссылка.Наименование +  ")" +  СокрЛП(Формат(ПараметрыОбмена.ДатаФормирования, "ДФ=yyyy-MM-dd")) + ".txt"));
	...
	ПредставлениеФайла 	=  "\Reports\" + "Exchange_" + "(" + ПараметрыОбмена.ссылка.Наименование +  ")" +  СокрЛП(Формат(ПараметрыОбмена.ДатаФормирования, "ДФ=yyyy-MM-dd")) + ".zip";
...
Функция ПолучитьСписокФайловДляОтправки(ПараметрыОбмена,КаталогНаДиске, МассивПодкаталогов);
	...
	Если МассивПодкаталогов <> Неопределено Тогда
		...
		Для Каждого Подкаталог Из МассивПодкаталогов Цикл
			...
			ФайлыВПодкаталоге = НайтиФайлы(Б_ОбщиеПроцедурыИФункцииСервер.ПолучитьПутьДляПлатформы(ПараметрыОбмена.ПлатформаWindows, КаталогНаДиске + "\" + Подкаталог), МаскаФайлыИКаталоги);
...
Функция ПроверкаОбновленияДанныхМодуляОбменаССайтом(ОсновныеПараметры) Экспорт
	...
	Пока Константы.Б_НомерВерсииМодуляОбменаССайтом.Получить() <> Версия Цикл
		...
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.4" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.4 до 7.0.1.5.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.5");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.5" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.5 до 7.0.1.6.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.6");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.6" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.6 до 7.0.1.7.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.7");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.7" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.7 до 7.0.1.8.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.8");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.8" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.8 до 7.0.1.9.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.9");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.9" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.9 до 7.0.1.10.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.10");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.10" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.10 до 7.0.1.11.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.11");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.11" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.11 до 7.0.1.12.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.12");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.12" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.12 до 7.0.1.13.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.13");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.13" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.13 до 7.0.1.14.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.14");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.14" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.14 до 7.0.1.15.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.15");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.15" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.15 до 7.0.1.16.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.16");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.16" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.16 до 7.0.1.17.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.17");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.17" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.17 до 7.0.1.18.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.18");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.18" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.18 до 7.0.1.19.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.19");
			
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.19" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.19 до 7.0.1.20.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.20");
			
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.20" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.20 до 7.0.1.21.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.21");
			
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.21" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.21 до 7.0.1.22.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.22");
			
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.22" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.21 до 7.0.1.23.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.23");	
			
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.23" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.23 до 7.0.1.24.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.24");	
			
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.24" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.24 до 7.0.1.25.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.25");	
			
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.25" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.25 до 7.0.1.26.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.26");	
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.26" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.26 до 7.0.1.27.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.27");	
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.27" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.27 до 7.0.1.28.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.28");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.28" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.28 до 7.0.1.29.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.29");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.29" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.29 до 7.0.1.30.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.30");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.30" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.30 до 7.0.1.31.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.31");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.31" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.31 до 7.0.1.32.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.32");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.32" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.32 до 7.0.1.33.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.33");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.33" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.33 до 7.0.1.34.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.34");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.34" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.34 до 7.0.1.35.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.35");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.35" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.35 до 7.0.1.36.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.36");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.36" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.36 до 7.0.1.37.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.37");
			
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.37" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.37 до 7.0.1.38.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.38");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.38" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.38 до 7.0.1.39.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.39");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.39" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.39 до 7.0.1.40.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.40");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.40" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.40 до 7.0.1.41.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.41");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.41" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.41 до 7.0.1.42.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.1.42");
		ИначеЕсли ПредыдущаяВерсия = "7.0.1.42" Тогда
			СообщитьПодробно("Обновление с версии 7.0.1.42 до 7.0.2.0.", ОсновныеПараметры);	
			
			Результат = ОбновлениеДоВерсии7_0_2_0(ОсновныеПараметры);
			Если Результат = Истина тогда
				Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.2.0");
			Иначе
				Возврат Результат;
			КонецЕсли;   
			
		ИначеЕсли ПредыдущаяВерсия = "7.0.2.0" Тогда
			СообщитьПодробно("Обновление с версии 7.0.2.0 до 7.0.2.1.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.2.1");
		ИначеЕсли ПредыдущаяВерсия = "7.0.2.1" Тогда
			СообщитьПодробно("Обновление с версии 7.0.2.1 до 7.0.2.2.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.2.2");
		ИначеЕсли ПредыдущаяВерсия = "7.0.2.2" Тогда
			СообщитьПодробно("Обновление с версии 7.0.2.2 до 7.0.2.3.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.2.3");
		ИначеЕсли ПредыдущаяВерсия = "7.0.2.3" Тогда
			СообщитьПодробно("Обновление с версии 7.0.2.3 до 7.0.2.4.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.2.4");
		ИначеЕсли ПредыдущаяВерсия = "7.0.2.4" Тогда
			СообщитьПодробно("Обновление с версии 7.0.2.4 до 7.0.2.5.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.2.5");
		ИначеЕсли ПредыдущаяВерсия = "7.0.2.5" Тогда
			СообщитьПодробно("Обновление с версии 7.0.2.5 до 7.0.2.6.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.2.6");   
		ИначеЕсли ПредыдущаяВерсия = "7.0.2.6" Тогда
			СообщитьПодробно("Обновление с версии 7.0.2.6 до 7.0.2.7.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.2.7"); 
		ИначеЕсли ПредыдущаяВерсия = "7.0.2.7" Тогда
			СообщитьПодробно("Обновление с версии 7.0.2.7 до 7.0.2.8.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.2.8");    
		ИначеЕсли ПредыдущаяВерсия = "7.0.2.8" Тогда
			СообщитьПодробно("Обновление с версии 7.0.2.8 до 7.0.2.9.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.2.9");
		ИначеЕсли ПредыдущаяВерсия = "7.0.2.9" Тогда
			СообщитьПодробно("Обновление с версии 7.0.2.9 до 7.0.2.10.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.2.10"); 
		ИначеЕсли ПредыдущаяВерсия = "7.0.2.10" Тогда
			СообщитьПодробно("Обновление с версии 7.0.2.1- до 7.0.2.11.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.2.11");   
		ИначеЕсли ПредыдущаяВерсия = "7.0.2.11" Тогда
			СообщитьПодробно("Обновление с версии 7.0.2.11- до 7.0.2.12.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.2.12");
		ИначеЕсли ПредыдущаяВерсия = "7.0.2.12" Тогда
			СообщитьПодробно("Обновление с версии 7.0.2.12- до 7.0.2.14.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.2.14");    
		ИначеЕсли ПредыдущаяВерсия = "7.0.2.14" Тогда
			СообщитьПодробно("Обновление с версии 7.0.2.14 - до 7.0.2.15.", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.2.15");    
		ИначеЕсли ПредыдущаяВерсия = "7.0.2.15" Тогда
			СообщитьПодробно("Обновление с версии 7.0.2.15 - до 7.0.2.16.", ОсновныеПараметры);	
			
			Результат = ОбновлениеДоВерсии7_0_2_16(ОсновныеПараметры);
			Если Результат = Истина тогда
				Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.2.16");
			Иначе
				Возврат Результат;
			КонецЕсли;
		ИначеЕсли ПредыдущаяВерсия = "7.0.2.16" Тогда
			СообщитьПодробно("Обновление с версии 7.0.2.16 - до 7.0.2.17", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.2.17"); 
		ИначеЕсли ПредыдущаяВерсия = "7.0.2.17" Тогда
			СообщитьПодробно("Обновление с версии 7.0.2.17 - до 7.0.2.18", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.2.18"); 
		ИначеЕсли ПредыдущаяВерсия = "7.0.2.18" Тогда
			СообщитьПодробно("Обновление с версии 7.0.2.18 - до 7.0.2.19", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.2.19"); 
		ИначеЕсли ПредыдущаяВерсия = "7.0.2.19" Тогда
			СообщитьПодробно("Обновление с версии 7.0.2.19 - до 7.0.2.20", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.2.20"); 
		ИначеЕсли ПредыдущаяВерсия = "7.0.2.20" Тогда
			СообщитьПодробно("Обновление с версии 7.0.2.20 - до 7.0.2.21", ОсновныеПараметры);	
			Константы.Б_НомерВерсииМодуляОбменаССайтом.Установить("7.0.2.21"); 
		Иначе 
...
Функция ОбновлениеДоВерсии7_0_2_0(ОсновныеПараметры)
	
	СообщитьПодробно("Начало обновления до версии 7.0.2.0", ОсновныеПараметры);	
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл 
			...
			Если НЕ УзелОбменаЭтаИБ тогда
				...
				Попытка
					...
					СохраненныеНастройкиЗагружаемыхДокументов = лСохраненныеНастройки.Заказы.СохраненныеНастройкиЗагружаемыхДокументов;
					
					Для каждого ТекСтрока Из СохраненныеНастройкиЗагружаемыхДокументов.ВсеСоответствияШапокДокументов Цикл
						
						Если (ТекСтрока.Документ = "ЗаказКлиента" ИЛИ ТекСтрока.Документ = "РеализацияТоваровУслуг")
							И ТекСтрока.НаименованиеРеквизита1С = "ПорядокОплаты" Тогда
							ТекСтрока.НаименованиеРеквизита1С 	= "Сборщик";	
							ТекСтрока.СинонимРеквизита1С 		= "Сборщик";	
							ТекСтрока.ТипДанных 				= "";	
							ТекСтрока.Значение 					= "";	
						КонецЕсли;
					...
					Для каждого ТекСтрока Из СохраненныеНастройкиЗагружаемыхДокументов.ВсеСоответствияТабличныхЧастейДокументов Цикл
						
						Если ТекСтрока.Документ = "ПоступлениеБезналичныхДенежныхСредств" 
							ИЛИ ТекСтрока.Документ = "ОперацияПоПлатежнойКарте"
							ИЛИ ТекСтрока.Документ = "ПриходныйКассовыйОрдер" Тогда
							
							Если ТекСтрока.НаименованиеРеквизита1С = "Заказ" Тогда
								ТекСтрока.НаименованиеРеквизита1С 	= "ОбъектРасчетов";	
								ТекСтрока.СинонимРеквизита1С 		= "Объект расчетов";	
								ТекСтрока.Значение 					= "Объект расчетов";	
							КонецЕсли;
					...
					НайденныеСтроки = СохраненныеНастройкиЗагружаемыхДокументов.ВсеСоответствияТабличныхЧастейДокументов.НайтиСтроки(Новый Структура("Документ, ТабличнаяЧасть, НаименованиеРеквизита1С", "РеализацияТоваровУслуг", "Товары", "ОбъектРасчетов"));
					Если НайденныеСтроки.Количество() = 0 Тогда
						НоваяСтрока = СохраненныеНастройкиЗагружаемыхДокументов.ВсеСоответствияТабличныхЧастейДокументов.Добавить();
						НоваяСтрока.Документ					= "РеализацияТоваровУслуг";
						НоваяСтрока.ТабличнаяЧасть				= "Товары";
						НоваяСтрока.ПолеXML						= "Товары";
						НоваяСтрока.НаименованиеРеквизита1С 	= "ОбъектРасчетов";	
						НоваяСтрока.СинонимРеквизита1С 			= "Объект расчетов";	
						НоваяСтрока.ТипДанных 					= "Из поля XML документа";	
						НоваяСтрока.Значение 					= "Объект расчетов";	
					КонецЕсли;
					
					НайденныеСтроки = СохраненныеНастройкиЗагружаемыхДокументов.ВсеСоответствияТабличныхЧастейДокументов.НайтиСтроки(Новый Структура("Документ, ТабличнаяЧасть, НаименованиеРеквизита1С", "ПриходныйКассовыйОрдер", "РасшифровкаПлатежа", "ОснованиеПлатежа"));
					Если НайденныеСтроки.Количество() = 0 Тогда
						НоваяСтрока = СохраненныеНастройкиЗагружаемыхДокументов.ВсеСоответствияТабличныхЧастейДокументов.Добавить();
						НоваяСтрока.Документ					= "ПриходныйКассовыйОрдер";
						НоваяСтрока.ТабличнаяЧасть				= "РасшифровкаПлатежа";
						НоваяСтрока.ПолеXML						= "";
						НоваяСтрока.НаименованиеРеквизита1С 	= "ОснованиеПлатежа";	
						НоваяСтрока.СинонимРеквизита1С 			= "Основание платежа";	
						НоваяСтрока.ТипДанных 					= "Из поля XML документа";	
						НоваяСтрока.Значение 					= "Основание";	
					КонецЕсли;
					
					НайденныеСтроки = СохраненныеНастройкиЗагружаемыхДокументов.ВсеСоответствияТабличныхЧастейДокументов.НайтиСтроки(Новый Структура("Документ, ТабличнаяЧасть, НаименованиеРеквизита1С", "ОперацияПоПлатежнойКарте", "РасшифровкаПлатежа", "ОснованиеПлатежа"));
					Если НайденныеСтроки.Количество() = 0 Тогда
						НоваяСтрока = СохраненныеНастройкиЗагружаемыхДокументов.ВсеСоответствияТабличныхЧастейДокументов.Добавить();
						НоваяСтрока.Документ					= "ОперацияПоПлатежнойКарте";
						НоваяСтрока.ТабличнаяЧасть				= "РасшифровкаПлатежа";
						НоваяСтрока.ПолеXML						= "";
						НоваяСтрока.НаименованиеРеквизита1С 	= "ОснованиеПлатежа";	
						НоваяСтрока.СинонимРеквизита1С 			= "Основание платежа";	
					...
					НайденныеСтроки = СохраненныеНастройкиЗагружаемыхДокументов.ВсеСоответствияТабличныхЧастейДокументов.НайтиСтроки(Новый Структура("Документ, ТабличнаяЧасть, НаименованиеРеквизита1С", "ПоступлениеБезналичныхДенежныхСредств", "РасшифровкаПлатежа", "ОснованиеПлатежа"));
					Если НайденныеСтроки.Количество() = 0 Тогда
						НоваяСтрока = СохраненныеНастройкиЗагружаемыхДокументов.ВсеСоответствияТабличныхЧастейДокументов.Добавить();
						НоваяСтрока.Документ					= "ПоступлениеБезналичныхДенежныхСредств";
						НоваяСтрока.ТабличнаяЧасть				= "РасшифровкаПлатежа";
				...
				Исключение
					СообщитьПодробно("Ошибка при указаниле полей документов:" + Выборка.Наименование, ОсновныеПараметры);
				КонецПопытки;
		...
		СообщитьПодробно("Обновление до версии 7.0.2.0 завершено.", ОсновныеПараметры);
		Возврат Истина;
		
	Исключение
		
		СообщитьПодробно("Обновление до версии 7.0.2.0 завершено с ошибками. Продолжение обмена невозможно.", ОсновныеПараметры, Истина, Истина);
		СообщитьПодробно(ОписаниеОшибки(), ОсновныеПараметры, Истина, Истина);
		Возврат Ложь;
...
Функция ОбновлениеДоВерсии7_0_2_16(ОсновныеПараметры)
	
	СообщитьПодробно("Начало обновления до версии 7.0.2.16", ОсновныеПараметры);	
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл 
			...
			Если НЕ УзелОбменаЭтаИБ И Не Выборка.ПометкаУдаления тогда
				
				лУзелОбмена = Выборка.ПолучитьОбъект();	
		      	лУзелОбмена.УдалениеНеВыгружаемыхДанных = Истина;
				лУзелОбмена.Записать();
	...
	Исключение
		...
		СообщитьПодробно("Обновление до версии 7.0.2.16 завершено с ошибками. Продолжение обмена невозможно.", ОсновныеПараметры, Истина, Истина);
		СообщитьПодробно(ОписаниеОшибки(), ОсновныеПараметры, Истина, Истина);
		Возврат Ложь;
	...
	СообщитьПодробно("Обновление до версии 7.0.2.16 завершено.", ОсновныеПараметры);
	Возврат Истина;
...
Процедура Таймаут(ДлительностьСек) Экспорт
	...
	ОбщегоНазначенияБТС.Пауза(ДлительностьСек);
	...
	Пока Истина Цикл
		...
		Если Сч % 10000 = 0 Тогда
			Если ТекущаяДатаСеанса() - Начало > ДлительностьСек Тогда
				Возврат;
			КонецЕсли;
		КонецЕсли;
...
Процедура ДобавитьКлассификаторыВТаблицуКаталогов(ТаблицаКаталогов) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаКаталогов Цикл
		...
		Если ЗначениеЗаполнено(НайденноеДеревоГрупп) тогда
			...
			Если НЕ Выборка.Следующий() И НайденноеДеревоГрупп.Товары.Количество() = 0 Тогда

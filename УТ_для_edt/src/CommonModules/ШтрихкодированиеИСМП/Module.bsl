...
// 	 * Упаковка           - ОпределяемыйТип.Упаковка                   - Упаковка таблицы товаров (опционально).
...
// 	* Ключ     - СтрокаТабличнойЧасти                                         - Исходная строка таблицы товаров.
// 	* Значение - Массив из См. НовыйРезультатРаспределенияШтрихкодовПоТоварам - Остаток для распределенных ранее строк.
Функция РаспределениеШтрихкодовУпаковокПоТоварам(ПараметрыРаспределения, Товары, МассивУпаковок) Экспорт
	...
	Для Каждого СтрокаШтрихкод Из ТаблицаШтрихкоды Цикл
		...
		Для Каждого СтрокаТовары Из НайденныйСтрокиТовары Цикл
			...
			РезультатСписания.ВидПродукции         = СтрокаШтрихкод.ВидПродукции;
			РезультатСписания.ВидУпаковки          = СтрокаШтрихкод.ВидУпаковки;
			РезультатСписания.ПолныйКодМаркировки  = СтрокаШтрихкод.ПолныйКодМаркировки;
			РезультатСписания.КодМаркировки        = СтрокаШтрихкод.Штрихкод;
			РезультатСписания.СоставКодаМаркировки = СтрокаШтрихкод.СоставКодаМаркировки;
			РезультатСписания.ШтрихкодУпаковки     = СтрокаШтрихкод.ШтрихкодУпаковки;
			РезультатСписания.ТипШтрихкода         = СтрокаШтрихкод.ТипШтрихкода;
			...
			Если РезультатСписания.ВидПродукции = Перечисления.ВидыПродукцииИС.АльтернативныйТабак
				...
				И Прав(РезультатСписания.Штрихкод, 4) <> "AAAA" Тогда
	...
	Для Каждого СтрокаТовары Из ТаблицаТовары Цикл
		...
		Если ВидыПродукцииТестовогоПериода.Найти(ВидПродукции) <> Неопределено
			Или УчитываемыеВидыПродукции.Найти(ВидПродукции) = Неопределено

...
Функция ЗаявленияОВвозеТоваровПоДокументуЗакупки(ПараметрыРегистрации)
	...
	ИначеЕсли ЕстьПравоИнтерактивногоДобавленияДокументаПоОснованию Тогда
		ТекстЗаявленияОВвозе = НСтр("ru='Оформить заявление о ввозе товаров'");
		НавигационнаяСсылка = "ВвестиНовыйЗаявлениеОВвозеТоваров";
	Иначе
		ТекстЗаявленияОВвозе = НСтр("ru='Заявление о ввозе товаров не оформлено'");
	КонецЕсли;
...
Процедура ПолучитьРазрезыВычетаНДС(ПараметрыРасчета) Экспорт
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	|		РегистрСведений.УчетнаяПолитикаОрганизаций.СрезПоследних(&НачалоПериода) КАК УчетнаяПолитикаОрганизаций
	|	ПО
	|		РазрезыВычетаНДС.Организация = УчетнаяПолитикаОрганизаций.Организация
	|ГДЕ
	|	(УчетнаяПолитикаОрганизаций.УчетнаяПолитика.ПериодичностьФормированияВычетовИВосстановленийНДС = ЗНАЧЕНИЕ(Перечисление.Периодичность.Месяц)
...
Процедура ПолучитьБлокировкиНДС(ПараметрыРасчета) Экспорт
	...
	|	МАКСИМУМ(Оплаты.НДСПриобретения) КАК НДСПриобретения
	...
	|			ТОГДА 0
	|			ИНАЧЕ ВЫРАЗИТЬ(ОплаченныеЭлектронныеУслуги.НДСПриобретения * ((ОплаченныеЭлектронныеУслуги.СуммаПриобретения - ОплаченныеЭлектронныеУслуги.СуммаОплаты) / ОплаченныеЭлектронныеУслуги.СуммаПриобретения) КАК ЧИСЛО(15, 2))
	|		КОНЕЦ
	|	ИЗ
...
Функция ТекстЗапросаИнформацияПоТаможеннымДекларациямПродажи()
	...
	|	И НЕ ТаблицаТоваров.НомерГТД.РНПТ
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ

...
Функция ОбработатьДанныеСервисаХарактеристикиКатегорий(ДанныеСервиса, Отказ)
	...
	Если Не ЕстьСвойство(ДанныеСервиса, "_embedded") Или 
		Не ЕстьСвойство(ДанныеСервиса._embedded.summary, "features", ДополнительныеРеквизиты) Или 
		Не ТипЗнч(ДополнительныеРеквизиты) = Тип("Массив") Или
		ДополнительныеРеквизиты.Количество() = 0 Тогда
	...
	Для Каждого ДополнительныйРеквизит Из ДополнительныеРеквизиты Цикл
		...
		Если ЕстьСвойство(ДополнительныйРеквизит, "numberValues",       СписокЗначений) Или 
			ЕстьСвойство(ДополнительныйРеквизит, "dateValues",    СписокЗначений) Тогда
		...
		ИначеЕсли ЕстьСвойство(ДополнительныйРеквизит, "booleanValues", СписокЗначений) Тогда
			...
			СтрокаДополнительныхРеквизитов = ДобавитьБулевоеЗначениеВТаблицуДополнительныхРеквизитов(
				СписокЗначений, ТаблицаДополнительныхРеквизитов);
...
Функция ОписаниеТаблицыДополнительныхРеквизитов()
	...
	// Наименование значения характеристики (в тех случаях, когда значение определяется ограниченным набором/словарём/справочником)/
...
Функция ДобавитьБулевоеЗначениеВТаблицуДополнительныхРеквизитов(СписокЗначений, Таблица)
		...
		Для Каждого ЭлементКоллекции Из Свойства Цикл
			...
			Если ЭлементКоллекции.Тип = "date" Тогда
				...
				Если Дата <> Дата(1,1,1) Тогда
					Значение = Формат(Дата, "ДФ=dd.MM.yyyy;");
				КонецЕсли;
			КонецЕсли;
...
Процедура ВыполнитьОбновлениеТорговыхПредложенийТорговойПлощадки(Знач ПараметрыПроцедуры, АдресРезультата, КоличествоЗаписей = 0)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(СопоставлениеНоменклатурыКонтрагентовСлужебный.ЗаписатьВЖурналРегистрации(

...
Процедура ИнициализироватьОбщиеПараметрыРасчета(ПараметрыИнициализации, ПараметрыРасчета, ПараметрыОтладки = Неопределено) Экспорт
	...
	ПараметрыРасчета.Вставить("УчетныеПолитикиКэш", УчетныеПолитики);
	ПараметрыРасчета.Вставить("УчетныеПолитикиПрошлогоПериодаКэш", УчетныеПолитикиПрошлогоПериода);
	ПараметрыРасчета.Вставить("НастройкиУчетаНДС", УчетНДСУП.НастройкиУчета());
...
//
Функция СхемаРасчетаПартий(Знач Дата = Неопределено, ОтборОрганизации = Неопределено, НомерЗаданияДляБлокировки = Неопределено) Экспорт
...
Процедура ПроверкаКорректностиИсходныхДанныхДляРасчетаСебестоимости(ПараметрыПроверки) Экспорт
	...
	|	ПО ДД.СтатьяРасходов.ВариантРаспределенияРасходовУпр = Соответствие.ВариантРаспределения
	|ГДЕ
	|	ДД.Проведен
	|	И ДД.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	|	И ДД.Организация В (&МассивОрганизаций)
	|	И Соответствие.НазначениеПравила <> ДД.НазначениеНастройкиРаспределения
	...
	|ИЗ Документ.РаспределениеПрочихЗатрат КАК ДД
	|ЛЕВОЕ СОЕДИНЕНИЕ ВтСоответствиеВариантовРаспределенияНазначениямПравил КАК Соответствие
	|	ПО ДД.СтатьяРасходов.ВариантРаспределенияРасходовРегл = Соответствие.ВариантРаспределения
	|ГДЕ
	|	ДД.Проведен
	|	И ДД.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	|	И ДД.Организация В (&МассивОрганизаций)
	...
	ТекстВтНесоответствиеНастроекДокументовРаспределения = 
		СтрЗаменить(ТекстВтНесоответствиеНастроекДокументовРаспределения, "%1", 
			НСтр("ru='Упр. учет'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ТекстВтНесоответствиеНастроекДокументовРаспределения = 
		СтрЗаменить(ТекстВтНесоответствиеНастроекДокументовРаспределения, "%2", 
			НСтр("ru='Регл. учет'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	СписокПолей.Добавить("Документ",				НСтр("ru='Документ'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("ВидУчета",				НСтр("ru='Вид учета'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	СписокПолей.Добавить("ВариантРаспределения",	НСтр("ru='Вариант распределения'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("НазначениеПоСтатье",		НСтр("ru='Настройка по статье расходов'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("НазначениеПоДокументу",	НСтр("ru='Настройка документа'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
	ЗакрытиеМесяцаСервер.ДополнитьПараметрыРегистрацииПроблемПроверки(ПараметрыРегистрации,
		"ВтНесоответствиеНастроекДокументовРаспределения",
		НСтр("ru = 'Обнаружены документы распределения расходов, настройки которых не соответсвуют варианту распределения в статье расходов.
				...
				|Организация: ""%1"", период: %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		"Документ");

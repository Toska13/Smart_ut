...
Функция oauth2callbackGET(Запрос)
	...
	Если Не ЗначениеЗаполнено(code) Тогда
		Возврат СообщениеОбОшибке(
			400,
			"Bad Request",
			НСтр("ru='Ошибка авторизации: отсутствует параметр ""code""';en='Authorization error: missing parameter ""code""'"));
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(state) Тогда
		Возврат СообщениеОбОшибке(
			400,
			"Bad Request",
			НСтр("ru='Ошибка авторизации: отсутствует параметр ""state""';en='Authorization error: missing parameter ""state""'"));
	КонецЕсли;
	...
	Если Не ПараметрыАвторизации.Свойство("id") Тогда
		Возврат СообщениеОбОшибке(
			400,
			"Bad Request",
			НСтр("ru='Ошибка авторизации: отсутствует ключ ""id"" в параметре ""state""';en='Authorization error: missing key ""id"" in parameter ""state""'"));
	КонецЕсли;
...
Процедура УстановитьРазделениеСеанса(ПараметрыАвторизации)
	...
	Если Не ПараметрыАвторизации.Свойство("zone") Тогда
		ВызватьИсключение НСтр("ru='Отсутствует ключ ""zone"" в параметре ""state""';en='Missing key ""zone"" in parameter ""state""'");
	КонецЕсли;
	...
	Если Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		РаботаВМоделиСервиса.УстановитьРазделениеСеанса(Истина, РазделительСеанса);
	КонецЕсли;

...
Функция itoolabsEventPOST(Запрос)
	...
	Попытка
		Если ТипОперации = "contact" Тогда
			...
			//Попытка    ВнутреннийНомер = Число(ПараметрыПереадресации.ВнутреннийНомер);
	...
	Попытка
		Если НРег(ПараметрыЗапросаJSON.location) = "ivr" Тогда
			...
			Если ЗначениеЗаполнено(ВнутреннийНомер) Тогда
				...
				РезультатЗапроса = сфпЛицензированиеЭкспортныеМетоды.ВыполнитьЗапросОблачнойАТС("ПереводНеотвеченногоЗвонка", ИдентификаторЗвонка, ВнутреннийНомер, ВнутреннийНомер, Ложь);
			КонецЕсли;
...
Функция calltrackingWebhook(Запрос)
	...
	Если НЕ ЗначениеЗаполнено(НомерТелефона) Тогда
		Попытка
			...
			Оператор = Перечисления.CRM_CallTrakingСценарии.Mango;
		Исключение КонецПопытки;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(НомерТелефона) И ЗначениеЗаполнено(ТекстОписание) Тогда
		...
		Если ЗначениеЗаполнено(utmCampaign) Тогда
			КаналПервичногоИнтереса = CRM_СистемаСквознойАналитики.ОпределитьКаналРекламы(utmMedium, utmSource);

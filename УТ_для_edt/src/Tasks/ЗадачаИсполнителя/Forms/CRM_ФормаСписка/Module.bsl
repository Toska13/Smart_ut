...
Процедура УстановитьПометкуУдаления(Команда)
	...
	Если НЕ Элементы.Список.ТекущиеДанные = Неопределено Тогда
		...
		Если Элементы.Список.ТекущиеДанные.ПометкаУдаления Тогда
			ТекстВопроса = НСтр("ru='Снять с ""';en='Remove from ""'") + Строка(Элементы.Список.ТекущаяСтрока) + НСтр("ru='"" пометку на удаление?'");
			
		Иначе
			ТекстВопроса = НСтр("ru='Пометить ""';en='Mark ""'") + Строка(Элементы.Список.ТекущаяСтрока) + НСтр("ru='"" на удаление?'");

...
Функция ВременнаяТаблицаЗаказыКПоступлениюИОтгрузке(ВТаблицеОтбораЕстьНазначения)
	...
	Текст =
		...
		|
		|	СУММА(ДвижениеТоваров.ПланируемоеПоступлениеСНеподтвержденными
		|		+ ДвижениеТоваров.ПланируемоеПоступлениеПодЗаказСНеподтвержденными) КАК КоличествоВсего
		|
		...
		|	СУММА(НаборПоТовару.КоличествоВсего)
		|		- СУММА(ЕСТЬNULL(НаборПоТовару.КоличествоОтгрузить, 0))
		|		- СУММА(ЕСТЬNULL(НаборПоТовару.КоличествоВРезерве, 0))  КАК КРезервированию

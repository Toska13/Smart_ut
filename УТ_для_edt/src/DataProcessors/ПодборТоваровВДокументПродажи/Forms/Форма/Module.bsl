...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ смарт:ММ #087 424 23.12.2020 
	ТекстБыло = "РегистрСведений.КурсыВалют.СрезПоследних(, Валюта = &Валюта)";
	ТекстСтало = "РегистрСведений.КурсыВалют.СрезПоследних(&Период, Валюта = &Валюта)";
	...
	СписокХарактеристики.ТекстЗапроса = СтрЗаменить(СписокХарактеристики.ТекстЗапроса, 
												ТекстБыло, 
												ТекстСтало);
	ПодборТоваровКлиентСервер.УстановитьПараметрДинамическогоСписка(СписокНоменклатура, "Период", КонецДня(Дата)); //смарт:rnp 456 11.02.2021  добавлено КонецДня
	ПодборТоваровКлиентСервер.УстановитьПараметрДинамическогоСписка(СписокХарактеристики, "Период", КонецДня(Дата)); //смарт:rnp 456 11.02.2021  добавлено КонецДня
	//-- смарт:ММ 23.12.2020
    ...
    //++ смарт:Подкин #1047 07.06.2022 10:15:39
    Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Параметры,"смартРазрешитьПовышеннуюСкидку") Тогда
        Объект.смартПроцентРазрешеннойМаксимальнойСкидки = Параметры.смартПроцентРазрешеннойМаксимальнойСкидки; 
        Объект.смартРазрешитьПовышеннуюСкидку = Параметры.смартРазрешитьПовышеннуюСкидку;
    КонецЕсли;
    //-- смарт:Подкин #1047 07.06.2022 10:15:39	 
...
Процедура КорзинаПроцентРучнойСкидкиПриИзменении(Элемент)
	...
	Если НЕ ДоступностьРоли("смартИсключениеОграниченияУстанавливатьРучныеСкидкиНаТоварыИзСегмента,ПолныеПрава") Тогда 
		
		СегментОтменаСкидкиНаТовар = ПолучитьСегментПоИдентификатору("ОтменаСкидкиНаТовар");
		
		Если ПроверитьВхождениеВСегмент(ТекущаяСтрока.Номенклатура, СегментОтменаСкидкиНаТовар) 
			И (ТекущаяСтрока.ПроцентРучнойСкидки > 0 ИЛИ ТекущаяСтрока.СуммаРучнойСкидки > 0) Тогда 
	...
	Если НЕ ДоступностьРоли("смартРазрешитьСкидкуБольшеМаксимальной,ПолныеПрава") Тогда
		...
		СтруктураПараметровДопустимаяСкидка.Вставить("Дата",					  Дата);
        ...
        Если НЕ СтруктураКонтроля=Неопределено И СтруктураКонтроля.ПроцентРучнойСкидки<ТекущаяСтрока.ПроцентРучнойСкидки Тогда
            ...
            ПоказатьВопрос(Новый ОписаниеОповещения("ОКЗавершениеПревышение", ЭтотОбъект, СтруктураКонтроля), ТекстСообщения + ". ", РежимДиалогаВопрос.ОК); 
...
Процедура КорзинаСуммаРучнойСкидкиПриИзменении(Элемент) 
	...
	Если НЕ ДоступностьРоли("смартРазрешитьСкидкуБольшеМаксимальной,ПолныеПрава") Тогда
		...
		Если НЕ СтруктураКонтроля=Неопределено И СтруктураКонтроля.МаксимальнаяСуммаСкидкиДляКонтроля<ТекущаяСтрока.СуммаРучнойСкидки Тогда
            ТекстСообщения = НСтр("ru = 'Сумма скидки превышает максимальное возможное значение:'") + " " + СтруктураКонтроля.МаксимальнаяСуммаСкидкиДляКонтроля;
...
Функция ДоступностьРоли(ИменаРолей)
...
//++ смарт:Подкин #914 01.03.2022 15:16:46  ограничения применения скидки больше максимальной, указанной в соглашении
&НаКлиенте
Процедура ОКЗавершениеПревышение(РезультатВопроса, ДополнительныеПараметры) Экспорт   
...
Функция ДобавитьВКорзинуНаКлиенте(ПараметрыТовара, НовыеСтроки)
	...
	Для Каждого НоваяСтрока Из НовыеСтроки Цикл
		...
		Если Не ЗначениеЗаполнено(Отбор.ВариантОбеспечения) Тогда
			Если Отбор.Склад = Неопределено Тогда // работа в комплектующих набора
				Отбор.Склад = ПредопределенноеЗначение("Справочник.Склады.ПустаяСсылка");
			КонецЕсли;
		...
		Если ПараметрыТовара.Свойство("КоличествоПоХарактеристикеРассчитано") Тогда 
			ТекущаяСтрока.КоличествоУпаковок = ТекущаяСтрока.КоличествоУпаковок + НоваяСтрока.КоличествоУпаковок;
		Иначе	
			КратностьПоХарактеристике=?(НоменклатураВРулонахПоХарактеристикам(ТекущаяСтрока.Номенклатура),СтрокуВЧисло(ТекущаяСтрока.Характеристика),1);
			ТекущаяСтрока.КоличествоУпаковок=ТекущаяСтрока.КоличествоУпаковок+(НоваяСтрока.КоличествоУпаковок*КратностьПоХарактеристике);	
		КонецЕсли;	
		// Фролов-
		//-- смарт:ММ 27.03.2020
...
Процедура ПеретащитьВКорзинуНаСервере(Данные)
	...
	Для Каждого НоваяСтрока Из НовыеСтроки Цикл
		...
		//++ смарт:ММ #042 222 27.03.2020 Рефакторинг. Перенесено из расширения (отмена &Вместо) 
		// Фролов+ Отменил для правильной работы поиска строки
		//Если ИспользоватьДатыОтгрузки Тогда
		//	Отбор.Вставить("ДатаОтгрузки", НоваяСтрока.ДатаОтгрузки);
		//КонецЕсли;
		//
...
Функция СтрокуВЧисло(ИсходнаяСтрока, ЗначениеПоУмолчанию = 0)
	...
	Исключение
		ТекстСообщения = "не удалось преобразовать характеристику в число";
...
// Каратеев+
&НаКлиенте
Функция КоличествоВыбраноВерно(КоличествоУпаковок, Характеристика)
	
	Намотка = СтрокуВЧисло(Характеристика);
	КоличествоКОтгрузке = КоличествоУпаковок;
...
Процедура смартУстановитьВСтрокеПроцентИндивидуальнойСкидкиПоНоменклатуре(СтрокаТовары)
	...
	ПроцентИндивидуальнойСкидки = смартПолучитьИндивидуальнуюСкидкуКлиентаПоНоменклатуреСервер(Дата, Объект.Партнер, СтрокаТовары.Номенклатура);
	
	СтрокаТовары.ПроцентРучнойСкидки = ПроцентИндивидуальнойСкидки;
	
	смартКорзинаПроцентРучнойСкидкиПриИзменении(СтрокаТовары);
	
КонецПроцедуры //--#1868 смарт: Лопато 19.06.2024

//++#1868 смарт: Лопато 19.06.2024 //Доработать механизм индивидуальных скидок (прод 1323)  
&НаКлиенте
Процедура смартКорзинаПроцентРучнойСкидкиПриИзменении(ТекущаяСтрока)

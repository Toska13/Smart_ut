...
Процедура ОК(Команда)
	...
	КонецЕсли;
	//++ Смарт:Белоусова 571 30.06.2021	
	//++ смарт:ММ #042 222 27.03.2020 Рефакторинг. Перенесено из расширения (отмена &Вместо) 
	// Фролов+
	Если ЗначениеЗаполнено(Характеристика) И Не КоличествоВыбраноВерно(КоличествоУпаковок,Характеристика) Тогда 
		Текст="Количество погонных метров должно быть кратно намотке рулона!
			|Укажите другое количество погонных метров.";
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст,,"КоличествоУпаковок",,Отказ);
	КонецЕсли;	
	// Фролов-
	//-- смарт:ММ 27.03.2020
	//-- Смарт:Белоусова 571 30.06.2021	

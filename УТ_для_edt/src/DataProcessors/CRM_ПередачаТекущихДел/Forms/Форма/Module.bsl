...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.СотрудникИсточник = Пользователи.АвторизованныйПользователь();
	...
	Иначе
		СписокТипов.Добавить(Новый Структура(КлючиСтруктуры, "Документ.КоммерческоеПредложениеКлиенту"	,"Клиент"				,""			,""		,"Менеджер"));		
	КонецЕсли; 
	...
	Для каждого Элемент Из СписокТипов Цикл
		...
		Если Элемент.ПолноеИмя = "Задача.ЗадачаИсполнителя" Тогда
			НоваяСтрока.Представление = НСтр("ru = 'Задачи бизнес-процессов и личные задачи (за исключением задач по Интересам)'");
		Иначе
...
Процедура КомандаДалее(Команда)
	...
	Если Элементы.ПанельСтраницы.ТекущаяСтраница = Элементы.СтраницаОтбор Тогда
		...
		Элементы.КомандаДалее.Заголовок = НСтр("ru='Выполнить';en='Perform'");
		
		ОбработкаОтбор();
	ИначеЕсли Элементы.ПанельСтраницы.ТекущаяСтраница = Элементы.СтраницаОбработка Тогда
		ОбработкаВыполнить();
		ПоказатьПредупреждение(, НСтр("ru='Выполнение обработки завершено.';en='Execution of handling is completed.'"), 60, НСтр("ru='Передача текущих дел'"));
	КонецЕсли;
...
Процедура ОбработкаОтбор()
	...
	Для Каждого Строка Из СписокТиповОбъектов Цикл
		...
		// Соединения
		Если ПолноеИмяОбъекта = "Документ.ЭлектронноеПисьмоВходящее"
			ИЛИ ПолноеИмяОбъекта = "Документ.ЭлектронноеПисьмоИсходящее" Тогда
			Текст = СтрЗаменить(Текст, "//Соединения", "ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПредметыПапкиВзаимодействий КАК ПредметыПапкиВзаимодействий
														|	ПО Объект.Ссылка = ПредметыПапкиВзаимодействий.Взаимодействие");
		Иначе
		...
		ИначеЕсли ПолноеИмяОбъекта = "Документ.ЭлектронноеПисьмоВходящее" Тогда
			Текст = СтрЗаменить(Текст, "//Отбор", "И НЕ ПредметыПапкиВзаимодействий.Рассмотрено");
		ИначеЕсли ПолноеИмяОбъекта = "Документ.ЭлектронноеПисьмоИсходящее" Тогда
			Текст = СтрЗаменить(Текст, "//Отбор", "И НЕ ПредметыПапкиВзаимодействий.Рассмотрено");
		...
		ИначеЕсли ВидОбъекта = "Задача" Тогда
			...
			Текст = СтрЗаменить(Текст, "//Отбор", "И НЕ Объект.Выполнена И НЕ Объект.Предмет ССЫЛКА Документ.CRM_Интерес");
		Иначе
		...
		Если Документы.ТипВсеСсылки().СодержитТип(ТипОбъекта) Тогда
			Если ТипОбъекта = Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") ИЛИ ТипОбъекта = Тип("ДокументСсылка.ЭлектронноеПисьмоИсходящее") Тогда
				...
				Если ЗначениеЗаполнено(ОсновнаяУчетнаяЗапись) И ТекОбъект.УчетнаяЗапись<>ОсновнаяУчетнаяЗапись Тогда
					...
					Взаимодействия.УстановитьПапкуЭлектронногоПисьма(ОбъектСсылка, НоваяПапка, Ложь);
					ТекОбъект.Прочитать();
				КонецЕсли;
			КонецЕсли;
...
Процедура ОбработкаВыполнить()
	...
	Для каждого Строка Из Список Цикл
		...
		ИначеЕсли Результат = "НеПолучен" Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось заблокировать';en='It were not possible to disable'") + " """ + СокрЛП(Строка.Объект) + """. " + НСтр("ru='Обратитесь к администратору.'"));
		ИначеЕсли Результат = "НеЗаписан" ИЛИ Результат = "НеОбработанПартнер" Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось записать';en='It were not possible to write down'") + " """ + СокрЛП(Строка.Объект) + """. " + НСтр("ru='Обратитесь к администратору.'"));
	...
	Если КолвоБезДоступа > 0 Тогда
		Текст = Строка(КолвоБезДоступа) + НСтр("ru=' документов не было перенесено, поскольку новый ответственный не имеет уровня доступа к клиентам этих документов. ';en=' documents were not transferred, because new responsible does not have access level to customers of these documents. '");

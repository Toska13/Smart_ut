...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	CRM_ЛицензированиеЭкспортныеМетоды.ПолучитьЗащищеннуюОбработку().Обработка_CRM_ПланированиеПроектов_ПриСозданииНаСервере(ЭтотОбъект, Отказ, СтандартнаяОбработка);
...
Процедура СформироватьДиаграммуГанта(ТекСтрокаДерева)
	...
	Если РежимПланирования = 1 Тогда // по проектам
		...
		СписокИзмерений.Добавить("Исполнитель",	НСтр("ru='Исполнитель';en='Assignee'"));
...
Процедура ВывестиДеревоПланов()
	...
	Если НеобходимоОбновитьКолонкиДереваПланов Тогда
		...
		CRM_ЛицензированиеЭкспортныеМетоды.ПолучитьЗащищеннуюОбработку().Обработка_CRM_ПланированиеПроектов_ОбновитьКолонкиДереваПланов(ЭтотОбъект);
	...
	Если флВыводитьНераспланированныеРесурсы Тогда
		...
		Если НезапланированныеПроектыЭтапы.Количество() > 0 Тогда
			...
			ИмяВетки = ?(НЕ ЗначениеЗаполнено(Элементы.СписокПроектов.ТекущаяСтрока) ИЛИ Элементы.СписокПроектов.ТекущаяСтрока.ЭтоГруппа, НСтр("ru='[НЕЗАПЛАНИРОВАННЫЕ ПРОЕКТЫ]';en='[UNPLANNED PROJECTS]'"), НСтр("ru='[НЕЗАПЛАНИРОВАННЫЕ ЭТАПЫ]'"));
...
Процедура Подключаемый_ПриСменеСтраницыПанелиИнформации()
	Если Элементы.ПавнельИнформации.ТекущаяСтраница = Элементы.СтраницаДиаграммаГанта Тогда
		СформироватьДиаграммуГанта(Элементы.ПланыДерево.ТекущиеДанные);
	//ИначеЕсли Элементы.ПавнельИнформации.ТекущаяСтраница = Элементы.СтраницаПланы Или
	//Элементы.ПавнельИнформации.ТекущаяСтраница = Элементы.СтраницаКонтрольныеТочки Тогда Подключаемый_ОбработчикОжиданияПланыДеревоПриАктивизацииСтроки();
	КонецЕсли;
КонецПроцедуры

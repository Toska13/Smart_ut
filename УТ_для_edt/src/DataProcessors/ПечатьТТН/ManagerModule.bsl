...
Процедура ЗаполнитьРеквизитыСтрокиТовараТТН(ДанныеПечати, СтрокаТовары, ДанныеСтроки, ОбластьМакета, ЕдиницаИзмеренияВеса = Неопределено, КоэффициентПересчетаВТонны = 0)
	...
	// Фролов+
	ПересчитатьВВалютуРегламентированногоУчета(ДанныеСтроки,СтрокаТовары);
	// Фролов-
	//-- смарт:ММ 27.03.2020
...
//++ смарт:ММ #042 222 27.03.2020 Рефакторинг. Перенесено из расширения (отмена &Вместо) 
// Фролов+
Процедура ПересчитатьВВалютуРегламентированногоУчета(ДанныеСтроки,СтрокаТовары)
	ВалютаРУ=Константы.ВалютаРегламентированногоУчета.Получить();
	Если ТипЗнч(СтрокаТовары.Ссылка)=Тип("ДокументСсылка.ПеремещениеТоваров") И СтрокаТовары.Ссылка.ВидЦены.ВалютаЦены<>ВалютаРУ Тогда 
		ДанныеВалютыДок=РаботаСКурсамиВалют.ПолучитьКурсВалюты(СтрокаТовары.Ссылка.ВидЦены.ВалютаЦены,СтрокаТовары.Ссылка.Дата);
		ДанныеВалютыРУ=РаботаСКурсамиВалют.ПолучитьКурсВалюты(ВалютаРУ,СтрокаТовары.Ссылка.Дата);

		ДанныеСтроки.Сумма=РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(ДанныеСтроки.Сумма,ДанныеВалютыДок,ДанныеВалютыРУ);
	КонецЕсли;	
КонецПроцедуры	//-- смарт:ММ 27.03.2020

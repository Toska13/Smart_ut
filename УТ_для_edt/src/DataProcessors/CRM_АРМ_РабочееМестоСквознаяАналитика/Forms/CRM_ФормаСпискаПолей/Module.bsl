...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Режим="ПоляГруппировки" Тогда
		Заголовок = НСтр("ru='Поля группировки';en='Grouping fields'");
		ДобавитьПоляГруппировок();
	ИначеЕсли Режим="ПоляВыбор" Тогда
		Заголовок = НСтр("ru='Доступные поля';en='Available fields'");
		ДобавитьПоляВыбор();
	ИначеЕсли Режим="ПоляОтбора" Тогда
		Заголовок = НСтр("ru='Поля фильтрации';en='Filter fields'");
		ДобавитьФильтры();
	КонецЕсли; 
...
Процедура ДобавитьФильтры()
	...
	Для каждого Поле Из Компоновщик.Настройки.Выбор.Элементы Цикл
		...
		//Если Поле.ТипЗначения.Типы().Количество()=1 И
		//Перечисления.ТипВсеСсылки().СодержитТип(Поле.ТипЗначения.Типы().Получить(0)) Тогда ПропуститьВложенные = Истина;
		//КонецЕсли; 

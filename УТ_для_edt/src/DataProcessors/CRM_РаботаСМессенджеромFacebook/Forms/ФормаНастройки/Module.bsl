...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	АдресКартинки =
		ПоместитьВоВременноеХранилище(Обработки.CRM_РаботаСМессенджеромFacebook.ПолучитьМакет("Картинка_login"));
...
Процедура ГруппаПриИзмененииСервер(СтраницаID, Токен)
	Объект.ТокенГруппы = Обработки.CRM_РаботаСМессенджеромFacebook.ПолучитьТокенСтраницыПользователя(СтраницаID,
		 Объект.Токен, Токен,
		 ПараметрыПрокси);
КонецПроцедуры
...
Процедура ВходНаСервере()
	...
	СтруктураОтвета =
		CRM_ЛицензированиеЭкспортныеМетоды.ПолучитьЗащищеннуюОбработку().ДанныеВерификацииFacebook(ПараметрыПрокси);
...
Процедура КомандаСправка(Команда)
	...
	ПерейтиПоНавигационнойСсылке(CRM_ОбщегоНазначенияСервер.ПолучитьСсылкуНаРазделСправки("Facebook"));
...
Процедура ЗаполнитьСписокСтраниц()
	Попытка
		ДанныеАккаунта = НСтр("en='Facebook connected';ru='Facebook подключен'") + ": " 
			+ Обработки.CRM_РаботаСМессенджеромFacebook.ДанныеГруппы(Объект.Токен, ,
			 ПараметрыПрокси);
		...
		СписокГрупп = Обработки.CRM_РаботаСМессенджеромFacebook.ПолучитьСписокСтраницПользователя(Объект.Токен,
			 ПараметрыПрокси);
...
Процедура ПроверитьПодключениеСервер()
	ДанныеГруппы = Обработки.CRM_РаботаСМессенджеромFacebook.ДанныеГруппы(Объект.ТокенГруппы,
		 Объект.IDГруппы,
		 ПараметрыПрокси);
КонецПроцедуры

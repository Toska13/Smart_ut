...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Спецификация") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			СформироватьПечатнуюФормуСпецификации(СтруктураТипов, ОбъектыПечати, ПараметрыПечати));
	КонецЕсли;
	//-- #Б-7584 смарт: Фролов 14.08.2026	
...
//++ #Б-7584 смарт: Фролов 13.08.2026 //Создать печатную форму счету на предоплату по договору
Функция СформироватьПечатнуюФормуСпецификации(СтруктураТипов, ОбъектыПечати, ПараметрыПечати) Экспорт
	...
	Для Каждого СтруктураОбъектов Из СтруктураТипов Цикл
		...
		ЗаполнитьТабличныйДокументСпецификация(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ПараметрыПечати);
...
КонецФункции //-- #Б-7584 смарт: Фролов 14.08.2026

//++ #Б-7584 смарт: Фролов 13.08.2026 //Создать печатную форму счету на предоплату по договору
Процедура ЗаполнитьТабличныйДокументСпецификация(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ПараметрыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Макет = ПолучитьМакет("смартПФ_MXL_Спецификация");
		...
		ОбластьШапкаТаблицы.Параметры.ДатаЗаказа = Формат(ДанныеПечати.Дата, "ДЛФ=DD");
		ОбластьШапкаТаблицы.Параметры.НомерДоговора = Договор.Номер;
		ОбластьШапкаТаблицы.Параметры.ДатаДоговора = Формат(Договор.Дата, "ДЛФ=DD");
		//ОбластьСуммаПрописью.Параметры.ДатаЗаказа = Формат(ДанныеПечати.Дата, "ДЛФ=DD");
			
		Если ЗначениеЗаполнено(Договор) Тогда
			
			ОбластьСуммаПрописью.Параметры.СрокДействия = Договор.смартСрокПоставки;
			ОбластьСуммаПрописью.Параметры.СрокДействияПрописью = СтрЗаменить(ЧислоПрописью(Договор.смартСрокПоставки), "  00 ", "");
			Если ТипЗнч(Договор.смартОтветственноеЛицоОрганизации) = Тип("СправочникСсылка.ОтветственныеЛицаОрганизаций") Тогда
				ОбластьПодвалТаблицы.Параметры.ПодписантПродавца = ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(Договор.смартОтветственноеЛицоОрганизации.ФизическоеЛицо);
			Иначе
				ОбластьПодвалТаблицы.Параметры.ПодписантПродавца = Договор.смартОтветственноеЛицоОрганизации.Наименование;		
			КонецЕсли;	
			ОбластьПодвалТаблицы.Параметры.ПодписантПокупателя = ФизическиеЛицаКлиентСервер.ФамилияИнициалы(Договор.смартОтветственноеЛицоКонтрагента.Наименование);
		...
		Для Каждого СтрокаТовары Из ТаблицаТовары Цикл
			...
			ОбластьСтрокаТаблицы.Параметры.НомПП = НомерСтроки;
			ОбластьСтрокаТаблицы.Параметры.Номенклатура = СтрокаТовары.Номенклатура;
			ОбластьСтрокаТаблицы.Параметры.Цена = СтрокаТовары.Цена;
			ОбластьСтрокаТаблицы.Параметры.Количество = СтрокаТовары.Количество;
			ОбластьСтрокаТаблицы.Параметры.Сумма = СтрокаТовары.Сумма;
		...
		ОбластьСуммаПрописью.Параметры.ИтогоСумма = ИтогоСумма;
		ОбластьСуммаПрописью.Параметры.ИтогоСуммаНДС = ИтогоСуммаНДС;
		ОбластьСуммаПрописью.Параметры.НомПП = НомерСтроки;
		ОбластьСуммаПрописью.Параметры.ИтогоСуммаПрописью = РаботаСКурсамиВалют.СформироватьСуммуПрописью(ИтогоСумма, ДанныеПечати.Валюта);
		
		Если ДанныеПечати.Ссылка.СпособДоставки = Перечисления.СпособыДоставки.ДоКлиента Тогда
			ОбластьСуммаПрописью.Параметры.СпособДоставки = " п. 4.4.3 договора, посредством доставки Товара силами Продавца по адресу "
				+ ДанныеПечати.Ссылка.АдресДоставки;	
		ИначеЕсли ДанныеПечати.Ссылка.СпособДоставки = Перечисления.СпособыДоставки.Самовывоз Тогда
			ОбластьСуммаПрописью.Параметры.СпособДоставки = " п. 4.4.1 договора, в порядке его самовывоза со склада Продавца по адресу "
				+ ДанныеПечати.Ссылка.АдресДоставки;
		ИначеЕсли ДанныеПечати.Ссылка.СпособДоставки = Перечисления.СпособыДоставки.СиламиПеревозчикаПоАдресу Тогда	
			ОбластьСуммаПрописью.Параметры.СпособДоставки = " п. 4.4.2 договора, посредством доставки Товара силами Продавца до терминала транспортной компании "
				+ ДанныеПечати.Ссылка.ПеревозчикПартнер + " " +	ДанныеПечати.Ссылка.АдресДоставкиПеревозчика;
		КонецЕсли;	
		
		ТабличныйДокумент.Вывести(ОбластьСуммаПрописью);
		
		ОбластьПодвалТаблицы.Параметры.Организация = ДанныеПечати.Организация.Наименование;
		ОбластьПодвалТаблицы.Параметры.Контрагент = ДанныеПечати.Контрагент.НаименованиеПолное;
...
КонецПроцедуры //-- #Б-7584 смарт: Фролов 14.08.2026
...
Процедура ЗаполнитьРеквизитыШапкиСчетаНаОплату(ДанныеПечати, Макет, ТабличныйДокумент, ТаблицаЭтапыОплаты, ТаблицаТовары, СчетНаПредоплету = Ложь)
//-- #Б-7584 смарт: Фролов 13.08.2026	
	...
	//Макет = УправлениеПечатью.МакетПечатнойФормы("Обработка.ПечатьСчетовНаОплату.смартПФ_MXL_СчетНаОплату");
	...
	Иначе
		...
		//++ смарт:Каратеев #1570 06.12.2023 //Корректировка Счета на оплату
		//СтруктураДанныхШапки.Вставить("ИдентификаторПлатежа", ДанныеПечати.ИдентификаторПлатежа);
		//-- смарт:Каратеев #1570 06.12.2023
		...
		Если Не ЗначениеЗаполнено(ДанныеПечати.НазначениеПлатежа)
			...
			Если ТипЗнч(ДанныеПечати.Ссылка) = Тип("ДокументСсылка.ЗаказКлиента") 
				...
				//++ #Б-7584 смарт: Фролов 13.08.2026 //Создать печатную форму счету на предоплату по договору
				Если НЕ СчетНаПредоплету Тогда
				...
				Иначе
					...
					СтруктураДанныхШапки.Вставить("НазначениеПлатежа", 
						"Авансовый платеж по Договору № "  + ДанныеПечати.Ссылка.Договор.Номер 
						+ " от " + Формат(ДанныеПечати.Ссылка.Договор.ДатаНачалаДействия, "ДЛФ=D"));
			...
			Иначе
				Если ТипЗнч(ДанныеПечати.Ссылка) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
					...
					Иначе
						СтруктураДанныхШапки.Вставить("НазначениеПлатежа", 
							"Авансовый платеж по Договору № " + ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(ДанныеПечати,));	
					КонецЕсли;
					//-- #Б-7584 смарт: Фролов 13.08.2026	
				Иначе
					СтруктураДанныхШапки.Вставить("НазначениеПлатежа", Документы.СчетНаОплатуКлиенту.СформироватьНазначениеПлатежа(
	...
	//	// Фролов -
	//КонецЕсли;
	ДатаСрокаДействия = смартОбщегоНазначения.смартПолучитьДатуСрокаДействия(ДанныеПечати.Валюта, ДанныеПечати.Дата, ТаблицаЭтапыОплаты);
	//--#1311 смарт: Лопато 13.02.2023
...
Процедура ЗаполнитьРеквизитыПодвалаСчетаНаОплату(ДанныеПечати, Макет, ТабличныйДокумент, ТаблицаЭтапыОплаты, СоответствиеСтавокНДС, ПараметрыПечати)
	...
	//++ #Б-5122 смарт: Фролов 02.12.2025 //Заказ клиента. Печатная форма. Дополнить данными масса габаритами.
	смартОриентировочныеГабариты = смартПолучитьТекстОрриентировочныеГабариты(ДанныеПечати);
	
	Если НЕ смартОриентировочныеГабариты = "" Тогда
		
		ОбластьОриентировочныеГабариты = Макет.ПолучитьОбласть("ОбластьОриентировочныеГабариты");
	
		ОбластьОриентировочныеГабариты.Параметры.смартОриентировочныеГабариты = СокрЛП(смартОриентировочныеГабариты);

		МассивПроверкиВывода.Очистить();
		МассивПроверкиВывода.Добавить(ОбластьОриентировочныеГабариты);
		Если НЕ ТабличныйДокумент.ПроверитьВывод(МассивПроверкиВывода) Тогда
			ТабличныйДокумент.ВывестиГоризонтальныйРазделительСтраниц();
		КонецЕсли;
		
		ТабличныйДокумент.Вывести(ОбластьОриентировочныеГабариты);
	
	КонецЕсли;
	//-- #Б-5122 смарт: Фролов 02.12.2025
	...
	смартДобавитьТекстОферты(ТабличныйДокумент, ДанныеПечати);
	//--#1311 смарт: Лопато 10.02.2023 
	
	//++ #Б-5558 смарт: Фролов 02.02.2026 //Выставление счета на номенклатуру с ограниченной гарантией
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	ЗаказКлиентаТовары.Номенклатура КАК Номенклатура
	|ИЗ
	|	Справочник.смартПредопределенныеЗначения.НоменклатураСОграниченнойГарантией КАК смартПредопределенныеЗначенияНоменклатураСОграниченнойГарантией
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказКлиента.Товары КАК ЗаказКлиентаТовары
	|		ПО смартПредопределенныеЗначенияНоменклатураСОграниченнойГарантией.Номенклатура = ЗаказКлиентаТовары.Номенклатура
	|ГДЕ
	...
	Если НЕ Запрос.Выполнить().Пустой() Тогда
		...
		Область = Макет.ПолучитьОбласть("ОбластьОграниченнаяГарантия");
		Область.Параметры.Заполнить(ДанныеПечати);
		ТабличныйДокумент.Вывести(Область);
		
	КонецЕсли;
	//-- #Б-5558 смарт: Фролов 02.02.2026
	
	//++#1511 смарт: Лопато 18.10.2023 //Изменить ПФ Счет на оплату  
	Если ДанныеПечати.Валюта = Константы.ВалютаРегламентированногоУчета.Получить() Тогда
		//++ #Б-7584 смарт: Фролов 13.08.2026 //Создать печатную форму счету на предоплату по договору
		Если ПараметрыПечати.Свойство("СчетНаПредоплету") Тогда
			ОбластьПредупрежденияКлиенту = Макет.ПолучитьОбласть("ОбластьПредупрежденияКлиентуРУБПредоплета");
		Иначе
			ОбластьПредупрежденияКлиенту = Макет.ПолучитьОбласть("ОбластьПредупрежденияКлиентуРУБ");
		КонецЕсли;
		//ОбластьПредупрежденияКлиенту = Макет.ПолучитьОбласть("ОбластьПредупрежденияКлиентуРУБ");
		//-- #Б-7584 смарт: Фролов 13.08.2026
	Иначе
		ОбластьПредупрежденияКлиенту = Макет.ПолучитьОбласть("ОбластьПредупрежденияКлиенту");
		ОбластьПредупрежденияКлиенту.Параметры.Валюта = СокрЛП(ДанныеПечати.Валюта);
	КонецЕсли;	

	МассивПроверкиВывода.Очистить();
	МассивПроверкиВывода.Добавить(ОбластьПредупрежденияКлиенту);
	Если НЕ ТабличныйДокумент.ПроверитьВывод(МассивПроверкиВывода) Тогда
		ТабличныйДокумент.ВывестиГоризонтальныйРазделительСтраниц();
	КонецЕсли;
	
	ТабличныйДокумент.Вывести(ОбластьПредупрежденияКлиенту);
	//--#1511 смарт: Лопато 18.10.2023
...
Процедура ЗаполнитьТабличныйДокументСчетаНаОплату(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ПараметрыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		//++ #Б-7584 смарт: Фролов 13.08.2026 //Создать печатную форму счету на предоплату по договору
		Если ПараметрыПечати.Свойство("СчетНаПредоплету") Тогда
			СчетНаПредоплету = Истина;
		Иначе
			СчетНаПредоплету = Ложь;
		КонецЕсли;

		Макет = Новый ТабличныйДокумент;
		//ЗаполнитьРеквизитыШапкиСчетаНаОплату(ДанныеПечати, Макет, ТабличныйДокумент, ТаблицаЭтапыОплаты, ТаблицаТовары);
		ЗаполнитьРеквизитыШапкиСчетаНаОплату(ДанныеПечати, Макет, ТабличныйДокумент, ТаблицаЭтапыОплаты, ТаблицаТовары, СчетНаПредоплету);
		//-- #Б-7584 смарт: Фролов 13.08.2026
		...
		Иначе
			...
			Для Каждого СтрокаТовары Из ТаблицаТовары Цикл
				...
				Если ПараметрыПечати.Свойство("СчетНаПредоплету") Тогда
					...
					Если ЗначениеЗаполнено(ДанныеПечати.Ссылка.Договор) Тогда
						...
						СтруктураДанныхСтроки.Вставить("Товар", "Авансовый платеж по Договору № " + ДанныеПечати.Ссылка.Договор.Номер + " от " 
							+ Формат(ДанныеПечати.Ссылка.Договор.Дата, "ДЛФ=DD"));
				...
				//++ #Б-7584 смарт: Фролов 13.08.2026 //Создать печатную форму счету на предоплату по договору
				Если ПараметрыПечати.Свойство("СчетНаПредоплету") Тогда
					ОбластьСтрокаТаблицы.Параметры.ЕдиницаИзмерения = "";
					ОбластьСтрокаТаблицы.Параметры.Количество = "";
					ОбластьСтрокаТаблицы.Параметры.Цена = ОбластьСтрокаТаблицы.Параметры.Сумма;
				КонецЕсли;
				//-- #Б-7584 смарт: Фролов 13.08.2026
			...
			//++ #4880 смарт: Тимофеева 07.11.2025 / Заказ клиента. Печатная форма. Добавить показатели массогабаритов.
			СвойствоВесЗаказа	= ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя", "ВесЗаказа");
			ОриентировочныйВес 	= УправлениеСвойствами.ЗначениеСвойства(ДанныеПечати.Ссылка, СвойствоВесЗаказа);
			//++ #Б-7584 смарт: Фролов 13.08.2026 //Создать печатную форму счету на предоплату по договору
			Если НЕ ПараметрыПечати.Свойство("СчетНаПредоплету") Тогда
				ИтоговаяСтрока = СтрШаблон(НСтр("ru='Всего наименований %1, ориентировочный вес %2 кг., на сумму %3'"),
												НомерСтроки,
												?(ТипЗнч(ОриентировочныйВес) = Тип("Число") И ОриентировочныйВес > 0, ОриентировочныйВес, " "),
												ФормированиеПечатныхФорм.ФорматСумм(СуммаКПрописи, ДанныеПечати.Валюта));
			КонецЕсли;									
			//-- #Б-7584 смарт: Фролов 13.08.2026									
			//-- #4880 смарт: Тимофеева 07.11.2025
...
//  ВалютаДокумента - Справочник.Валюты - Валюта документа основания
//
// Возвращаемое значение:
//  ТекстНДСПоСтавке - Строка - строка.
//
Функция ТекстНДСПоСтавке(СтавкаНДС, ЦенаВключаетНДС, ВалютаДокумента)
	
	ТекстНДСПоСтавке = ?(ЦенаВключаетНДС, НСтр("ru='В т.ч. НДС (%СтавкаНДС%), %ВалютаДокумента%:'"), НСтр("ru='НДС (%СтавкаНДС%), %ВалютаДокумента%:'"));
	ТекстНДСПоСтавке = СтрЗаменить(ТекстНДСПоСтавке, "%СтавкаНДС%", СтавкаНДС);
	ТекстНДСПоСтавке = СтрЗаменить(ТекстНДСПоСтавке, "%ВалютаДокумента%", ВалютаДокумента);
	
	Возврат ТекстНДСПоСтавке;
	
КонецФункции //-- смарт:ММ 28.03.2020

#Область ДоработкиСмарт

//++#1311 смарт: Лопато 09.03.2023 //Изменить форму Счета на оплату (Счет-оферта)  
Процедура смартДобавитьТекстОферты(ТабличныйДокумент, ДанныеПечати)

	ЗаказКлиента = Неопределено;
	
	Если ТипЗнч(ДанныеПечати.Ссылка) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		ЗаказКлиента = ДанныеПечати.Ссылка;
	ИначеЕсли ТипЗнч(ДанныеПечати.Ссылка) = Тип("ДокументСсылка.СчетНаОплатуКлиенту") Тогда
		ЗаказКлиента = ДанныеПечати.ДокументОснование;
	КонецЕсли;
	
	ТекстОферта = смартОбщегоНазначения.смартСформироватьТекстОферты(ЗаказКлиента);
	
	Если ЗначениеЗаполнено(ТекстОферта) Тогда

		МакетОферта = УправлениеПечатью.МакетПечатнойФормы("Обработка.ПечатьСчетовНаОплату.смартПФ_MXL_СчетНаОплатуВнимание");
		
		ОбластьОферта = МакетОферта.ПолучитьОбласть("смартОферта");
		ОбластьОферта.Параметры.ТекстОферта = ТекстОферта;
		
		ТабличныйДокумент.Вывести(ОбластьОферта);
		
	КонецЕсли;	

КонецПроцедуры //--#1311 смарт: Лопато 09.03.2023

//++ #Б-5122 смарт: Фролов 02.12.2025 //Заказ клиента. Печатная форма. Дополнить данными масса габаритами.
Функция смартПолучитьТекстОрриентировочныеГабариты(ДанныеПечати)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ТоварыЗаказа.Номенклатура) КАК Номенклатура,
	|	УпаковкиЕдиницыИзмерения.Ширина * ТоварыЗаказа.Количество КАК Ширина,
	|	УпаковкиЕдиницыИзмерения.Глубина * ТоварыЗаказа.Количество КАК Глубина,
	|	УпаковкиЕдиницыИзмерения.Высота * ТоварыЗаказа.Количество КАК Высота,
	|	УпаковкиЕдиницыИзмерения.Вес * ТоварыЗаказа.Количество КАК Брутто,
	|	ЕСТЬNULL(ВЫБОР
	|			КОГДА ТоварыЗаказа.Номенклатура.ВесЗнаменатель > 0
	|				ТОГДА ТоварыЗаказа.Номенклатура.ВесЧислитель * ТоварыЗаказа.Количество / ТоварыЗаказа.Номенклатура.ВесЗнаменатель
	|			ИНАЧЕ 0
	|		КОНЕЦ, 0) КАК Нетто
	|ИЗ
	|	ТоварыЗаказа КАК ТоварыЗаказа
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.УпаковкиЕдиницыИзмерения КАК УпаковкиЕдиницыИзмерения
	|		ПО ТоварыЗаказа.Номенклатура = УпаковкиЕдиницыИзмерения.Владелец
	|			И ТоварыЗаказа.Номенклатура.ЕдиницаИзмерения = УпаковкиЕдиницыИзмерения.ЕдиницаИзмерения");
	Запрос.УстановитьПараметр("Ссылка", ДанныеПечати.Ссылка);
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстДляВозврата = ТекстДляВозврата + ?(ТекстДляВозврата = "", "", Символы.ПС) + Выборка.Номенклатура + " - " + Выборка.Ширина + "x" + Выборка.Глубина + "x" + Выборка.Высота 
			+ " м ШГВ, " + ?(Выборка.Брутто > 0, "" + Выборка.Брутто + ?(НомПП = Выборка.Количество() и Выборка.Нетто = 0, "кг брутто.", "кг брутто, "), "") 
			+ ?(Выборка.Нетто > 0, "" + Выборка.Нетто + ?(НомПП = Выборка.Количество(), " кг нетто.", " кг нетто,"), "");		
...
КонецФункции //-- #Б-5122 смарт: Фролов 02.12.2025	

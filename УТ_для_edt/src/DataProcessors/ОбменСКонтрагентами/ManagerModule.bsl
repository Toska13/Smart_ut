...
Процедура ПодготовитьДанныеДляЗаполненияДокументов(Параметры, АдресХранилища) Экспорт
	...
	ОрганизацияПодключенаБС = Неопределено;
	Параметры.Свойство("ОрганизацияПодключенаБС", ОрганизацияПодключенаБС);
	
	МодульБизнесСеть = Неопределено;
	ВыводитьQRКодВПечатныхФормах = Ложь;
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.БизнесСеть") Тогда
		...
		ИспользоватьОбменБизнесСеть  = МодульБизнесСеть.ИспользоватьОбменБизнесСеть();
		ВыводитьQRКодВПечатныхФормах = ИспользоватьОбменБизнесСеть И МодульБизнесСеть.ВыводитьQRКодНаПечатныхФормахДокументов();
	КонецЕсли;
	...
	Для Каждого СтруктураОбмена Из МассивСтруктурОбмена Цикл
		...
		НоваяСтрока.Организация = СтруктураОбмена.СтруктураЭД.Организация;
		...
		Если МодульБизнесСеть <> Неопределено
			И СтруктураОбмена.СтруктураЭД.ОбменБезПодписи = Истина
			И ОрганизацияПодключенаБС = Истина
			И ВыводитьQRКодВПечатныхФормах Тогда
			
			Отказ = Ложь;
			ДанныеQRКода = МодульБизнесСеть.ДанныеQRКодаПоДокументу(СсылкаНаОбъект, НастройкиОбмена.Организация, Отказ);
			Если Не Отказ Тогда
				...
				НоваяСтрока.ИдентификаторВременнойСсылки = ДанныеQRКода.Идентификатор;
				
				СтруктураОбмена.Вставить("ДанныеQRКода", ДанныеQRКода);
			КонецЕсли;
		КонецЕсли;
...
Функция СформироватьДвоичныеДанныеПакета(СтруктураОбмена) Экспорт
	...
	Если Не ЗначениеЗаполнено(Ошибки) Тогда
		...
		Если СтруктураОбмена.Свойство("ДвоичныеДанныеПредставления") Тогда
			...
			ИмяФайлаПДФ = ОбменСКонтрагентамиВнутренний.ДвоичныеДанныеФайлаПредставленияДляБизнесСети(СтруктураОбмена,
																									НаименованиеФайла,
																									СтруктураОбмена.СтруктураЭД.Владелец,
																									СтруктураОбмена.СтруктураЭД.Организация);

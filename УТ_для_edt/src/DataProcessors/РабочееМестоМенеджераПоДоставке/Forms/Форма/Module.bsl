...
Процедура СоздатьЗаданияНаПеревозку(Команда)
	...
	//++ смарт:Демичев №1115 06.09.2022 ((Чтобы не переносить в расширения с признаком &Вместо
	...
	//	СписокСкладов.ЗагрузитьЗначения(смартСписокСкладов.ВыгрузитьЗначения());
	//		
	//	ОповещениеПослеВыбораЭлемента = Новый ОписаниеОповещения(
	//	"смартОбработкаВыбораСкладаИзСписка", ЭтотОбъект);
	//
	//	СписокСкладов.ПоказатьВыборЭлемента(
	//	ОповещениеПослеВыбораЭлемента,
	//	"Выберите склад для задания на перевозку");	
		
		Если Не ЗначениеЗаполнено(смартСкладПоУмолчаниюДляЗаданий) Тогда
			...
			Иначе
				...
				ОткрытьПодборТранспорта(Ложь,смартСкладПоУмолчаниюДляЗаданий); 
...
Процедура СкладПриИзмененииСервер(ТекстОшибки = "")
   ...
   // ДоставкаТоваров.ЗаполнитьСписокВыбораПеревозчиков(Элементы,,Склад);
   Если смартФлажокСписокСкладов Тогда
	   ДоставкаТоваров.ЗаполнитьСписокВыбораПеревозчиков(Элементы,,смартСписокСкладов);
   Иначе
	   ДоставкаТоваров.ЗаполнитьСписокВыбораПеревозчиков(Элементы,,Склад);	   
   КонецЕсли;
	//-- смарт:Демичев №1115 06.09.2022
...
Процедура СохранитьНастройкиИЗаданияНаПеревозкуСервер()
	...
	//++ смарт:Демичев №1115 06.12.2022((
	Настройки.Вставить("смартФлажокСписокСкладов",                           смартФлажокСписокСкладов);
	Настройки.Вставить("смартСписокСкладов",                           смартСписокСкладов);				
    Настройки.Вставить("смартСкладПоУмолчаниюДляЗаданий",                           смартСкладПоУмолчаниюДляЗаданий);	
	//-- смарт:Демичев №1115 06.12.2022)) 
...
Процедура ВосстановитьНастройки()
	...
	Если ТипЗнч(ЗначениеНастроек) = Тип("Соответствие") Тогда
        //++ смарт:Демичев №1115 06.12.2022((
		смартФлажокСписокСкладов                           = ЗначениеНастроек.Получить("смартФлажокСписокСкладов");
		смартСписокСкладов                           = ЗначениеНастроек.Получить("смартСписокСкладов");	
		смартСкладПоУмолчаниюДляЗаданий                           = ЗначениеНастроек.Получить("смартСкладПоУмолчаниюДляЗаданий");	
		//-- смарт:Демичев №1115 06.12.2022))
...
Процедура ПослеВопросаОСозданииЗаданий(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Если смартФлажокСписокСкладов Тогда
			...
			//СписокСкладов.ЗагрузитьЗначения(смартСписокСкладов.ВыгрузитьЗначения());
			//
			//мСтруктураПараметров = Новый Структура;
			//мСтруктураПараметров.Вставить("ПеренестиРаспоряженияВНовоеЗадание",Истина);
			//
			//ОповещениеПослеВыбораЭлемента = Новый ОписаниеОповещения(
			//"смартОбработкаВыбораСкладаИзСписка", ЭтотОбъект,мСтруктураПараметров);
			//
			//СписокСкладов.ПоказатьВыборЭлемента(
			//ОповещениеПослеВыбораЭлемента,
			//"Выберите склад для задания на перевозку");	
			...
			Иначе
				 ...
				 ОткрытьПодборТранспорта(Истина,смартСкладПоУмолчаниюДляЗаданий);
...
&НаКлиенте
//Процедура ОткрытьПодборТранспорта(ПеренестиРаспоряженияВНовоеЗадание = Ложь) ////++ смарт:Демичев №1115 06.09.2022

Процедура ОткрытьПодборТранспорта(ПеренестиРаспоряженияВНовоеЗадание = Ложь,СкладИзСписка = Неопределено)
	...
	Если СкладИзСписка <> Неопределено Тогда
		ПараметрыФормы.Вставить("Склад", СкладИзСписка);	
	Иначе
		ПараметрыФормы.Вставить("Склад", Склад);	
	КонецЕсли;
	//-- смарт:Демичев №1115 06.09.2022
	...
	Если СкладИзСписка <> Неопределено Тогда
		...
		ЭтотОбъект, Новый Структура("ПеренестиРаспоряженияВНовоеЗадание,СкладИзСписка",ПеренестиРаспоряженияВНовоеЗадание,СкладИзСписка));
...
Процедура ПослеПодбораТранспорта(Результат, ДополнительныеПараметры) Экспорт
	...
	//++ смарт:Демичев №1115 06.09.2022
	Если ДополнительныеПараметры.Свойство("СкладИзСписка") и ЗначениеЗаполнено(ДополнительныеПараметры.СкладИзСписка) Тогда
		
		ДобавитьНовыеЗаданияСервер(Результат,ДополнительныеПараметры.СкладИзСписка);		
	Иначе
...
&НаСервере
//Процедура ДобавитьНовыеЗаданияСервер(АдресНовыхЗаданийВХранилище)//	//++ смарт:Демичев №1115 06.09.2022
Процедура ДобавитьНовыеЗаданияСервер(АдресНовыхЗаданийВХранилище,СкладИзСписка = Неопределено)
...
Процедура ЗаполнитьСкладыПунктовИЗаданий(УчестьМодифицированность = Ложь);
	...
	Для Каждого СтрокаЗадание Из ЗаданияНаПеревозкуПланируемые.ПолучитьЭлементы() Цикл
		...
		Если КоличествоСкладов = 1
			...
			Если Не смартФлажокСписокСкладов Тогда //Если не наша доработка, то пусть работает как было потиповому 
				СтрокаЗадание.Склад = СкладыЗадания.Склад;
				УстановитьМодифицированность(СтрокаЗадание, ЗаданияМодифицированы);
			КонецЕсли;
			  //-- смарт:Демичев №1115 06.09.2022  
...
Процедура ОбновитьСтраницуФормированияЗаданий()
	...
	ЭтаФорма.ИтогоВесБрутто  = 0;
	//-- #Б4398 смарт: Шляпников 02.10.2025
	
	 //++ смарт:Демичев #1115 09.09.2022 ((
	//Если НЕ ЗначениеЗаполнено(Склад) Тогда
	//	Возврат;
	//КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(смартСписокСкладов) И  смартФлажокСписокСкладов Тогда
		
		Возврат;
		
	ИначеЕсли  Не смартФлажокСписокСкладов и НЕ ЗначениеЗаполнено(Склад) Тогда
	...
	//++ #Б4398 смарт: Шляпников 02.10.2025 / Задания на перевозку. Заполнение поля "Вес". Разработка. 
	|	ЗаданиеНаПеревозкуРаспоряжения.ВесБрутто КАК РаспоряжениеВесБрутто,
	|	ЗаданиеНаПеревозкуМаршрут.ВесБрутто КАК ВесБрутто,
	//-- #Б4398 смарт: Шляпников 02.10.2025
    ...
    //++ смарт:Подкин #1121 31.08.2022 16:42:12
    Если ЗначениеЗаполнено(смартСписокСкладов) И смартФлажокСписокСкладов Тогда
        Запрос.УстановитьПараметр("Склад", смартСписокСкладов);
    Иначе
        Запрос.УстановитьПараметр("Склад", Склад);
    КонецЕсли;
    //-- смарт:Подкин #1121 31.08.2022 16:42:12
	...
	Пока Выборка.СледующийПоЗначениюПоля("Ссылка") Цикл
		...
		Пока Выборка.СледующийПоЗначениюПоля("ПорядокВМаршруте")
			...
			//++ #Б4398 смарт: Шляпников 02.10.2025 / Задания на перевозку. Заполнение поля "Вес". Разработка. 
			СтрокаЗадание.ВесБрутто = СтрокаЗадание.ВесБрутто + Выборка.РаспоряжениеВесБрутто;
			//-- #Б4398 смарт: Шляпников 02.10.2025
			...
			Пока Выборка.Следующий() Цикл
				...
				//++ #Б4398 смарт: Шляпников 02.10.2025 / Задания на перевозку. Заполнение поля "Вес". Разработка. 
				СтрокаРаспоряжение.ВесБрутто  = Выборка.РаспоряжениеВесБрутто;
				//-- #Б4398 смарт: Шляпников 02.10.2025
...
Процедура СохранитьЗаданиеВСтроке(СтрокаЗадание, ПометитьНаУдаление = Ложь)
	...
	Если смартФлажокСписокСкладов и не ЗначениеЗаполнено(ДокОбъект.Склад) Тогда
...
Процедура УстановитьОтборЗаданияНаПеревозкуВРаботе()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(ЗаданияНаПеревозкуВРаботе,
	  ...
	  //КонецЕсли;
	  Если смартФлажокСписокСкладов и смартСписокСкладов.Количество() Тогда  		
		  //ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(ЗаданияНаПеревозкуВРаботе,
		  //"Склад", смартСписокСкладов, ВидСравненияКомпоновкиДанных.ВСписке,,Истина);
		  
		   ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(ЗаданияНаПеревозкуВРаботе,
		  "Склад", смартСписокСкладов, ВидСравненияКомпоновкиДанных.ВИерархии,,Истина);
...
//++ смарт:Демичев №1115 06.09.2022 
&НаКлиенте
Процедура смартОбработкаВыбораСкладаИзСписка(Элемент, Параметры)  Экспорт
	
	Если Элемент = Неопределено Тогда
		Возврат
	КонецЕсли;
	
	смартСкладПоУмолчаниюДляЗаданий = Элемент.Значение;
	
	Если Параметры <> Неопределено и Параметры.Свойство("ПеренестиРаспоряженияВНовоеЗадание") и Параметры.ПеренестиРаспоряженияВНовоеЗадание Тогда
		
		ОткрытьПодборТранспорта(Параметры.ПеренестиРаспоряженияВНовоеЗадание,смартСкладПоУмолчаниюДляЗаданий);

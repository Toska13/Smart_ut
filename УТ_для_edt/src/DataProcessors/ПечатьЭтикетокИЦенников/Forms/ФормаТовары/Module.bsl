...
Процедура ЗаполнитьТаблицуТоваровНаСервере(СохранятьШаблоныИКоличество, ОбновлениеДанныхПоВыбраннымТоварам = Ложь)
	...
	//++ смарт:ММ #042 222 27.03.2020 Рефакторинг. Перенесено из расширения (отмена &Вместо) 
	// Фролов+
	Если СтруктураНастроек.ИмяМакетаСхемыКомпоновкиДанных="ПоляШаблонаТовары"
		Или СтруктураНастроек.ИмяМакетаСхемыКомпоновкиДанных="ПоляШаблонаБДТовары" Тогда
		СтруктураНастроек.ИмяМакетаСхемыКомпоновкиДанных="смарт"+СтруктураНастроек.ИмяМакетаСхемыКомпоновкиДанных;
	КонецЕсли;	
	// Фролов-
	//-- смарт:ММ 27.03.2020
	...
	//++ смарт:ММ #042 222 27.03.2020 Рефакторинг. Перенесено из расширения (отмена &Вместо) 
	// Фролов+
	ЕстьСерия=Не СтруктураРезультата.Таблица.Колонки.Найти("Серия")=Неопределено;
	// Фролов-
	//-- смарт:ММ 27.03.2020
	...
	Для Каждого СтрокаТЧ Из СтруктураРезультата.Таблица Цикл
		...
		//++ смарт:ММ #042 222 27.03.2020 Рефакторинг. Перенесено из расширения (отмена &Вместо) 
		// Фролов+
		Если ЕстьСерия И ПолучитьФункциональнуюОпцию("ИспользоватьСерииНоменклатуры") Тогда 
			НоваяСтрока.Серия				 = СтрокаТЧ.Серия;
			НоваяСтрока.СерииИспользуются	 = НоваяСтрока.Номенклатура.ВидНоменклатуры.ИспользоватьСерии;
		КонецЕсли;	
		// Фролов-
		//-- смарт:ММ 27.03.2020

...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Константы.ИспользоватьРазделениеПоОбластямДанных.Получить() Тогда
		НеобходимаяВерсия = "3.0.20";
		Если НЕ слкМенеджерЗащиты.ПроверитьВерсиюКомпонентыСЛК(НеобходимаяВерсия) Тогда
			СообщениеИсключания = НСтр("ru='Устаревшая версия компоненты СЛК %1, для работы конфигурации необходима компонента СЛК %2 или выше'");
			СообщениеИсключания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СообщениеИсключания, ВерсияКомпонентыСЛК, НеобходимаяВерсия);
			Сообщить(СообщениеИсключания);
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	КонецЕсли;

...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДоступа("Администрирование", Метаданные) Тогда
		ВызватьИсключение НСтр("ru='Использование обработки в интерактивном режиме доступно только администратору.';en='Data processor use in interactive mode is available to administrator only.'");
	КонецЕсли;
	...
	ЗаголовокФормы = НСтр("ru='Универсальный обмен данными в формате XML (%ВерсияОбработки%)';en='Universal Data Exchange XML (%ВерсияОбработки%)'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если СохраненныйРежимЗагрузки
		...
		Если Объект.НастройкаАвтоматическойЗагрузкиДанных = 1 Тогда
			...
			ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Выполнить загрузку данных из файла обмена?';en='Import data from the exchange file? '"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
...
Процедура КаталогИнформационнойБазыДляПодключенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбораФайла.Заголовок = НСтр("ru='Выберите каталог информационной базы';en='Select an infobase directory'");
...
Процедура ИмяФайлаПравилПослеПроверкиСуществования(Существует, ДополнительныеПараметры) Экспорт
	...
	Если Не Существует Тогда
		СообщитьПользователю(НСтр("ru='Не найден файл правил обмена';en='File of exchange rules is not found'"), "ИмяФайлаПравил");
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Загрузить правила обмена данными?';en='Import data exchange rules?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
...
Процедура ПолучитьИнформациюОФайлеОбмена(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайла,НСтр("ru='Файл обмена';en='File of an exchange'"),, УникальныйИдентификатор);
...
Процедура УдалениеУдалить(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Удалить выбранные данные в информационной базе?';en='Delete the select data in information base?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
...
Процедура НастройкаОтладкиЗагрузки(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайлаОбменаВХранилище,НСтр("ru='Файл обмена';en='File of an exchange'"),, УникальныйИдентификатор);
...
Процедура ПрочитатьПравилаОбмена(Команда)
	...
	Если Не ЭтоWindowsКлиент() И ПрямаяВыгрузка = 1 Тогда
		ПоказатьПредупреждение(,НСтр("ru='Прямое подключение к информационной базе поддерживается только в клиенте под управлением ОС Windows.';en='Direct connection to the information base is supported only in customer running Windows.'"));
		Возврат;
	КонецЕсли;
...
"Выберите другой файл для выгрузки данных.';en='Exchange rules file may not be in the same data file."
"Please select another file to upload data.'"));
		Возврат Истина;

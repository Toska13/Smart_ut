...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаголовокФормы = НСтр("ru='Настройка отладки обработчиков при %Событие% данных';en='Configure handler debugging during data %Событие%'");	
	Событие = ?(Параметры.ОбработчикиСобытийЧитаемИзФайлаПравилОбмена, НСтр("ru='выгрузке';en='export'"), НСтр("ru='загрузке'"));
	ЗаголовокФормы = СтрЗаменить(ЗаголовокФормы, "%Событие%", Событие);
	Заголовок = ЗаголовокФормы;
	
	ЗаголовокКнопки = НСтр("ru='Сформировать модуль отладки %Событие%';en='Generate debug engine %Событие%'");
	Событие = ?(Параметры.ОбработчикиСобытийЧитаемИзФайлаПравилОбмена, НСтр("ru='выгрузки';en='exports'"), НСтр("ru='загрузки'"));
...
Процедура ИмяФайлаВнешнейОбработкиОбработчиковСобытийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбораФайла.Фильтр     = НСтр("ru='Файл внешней обработки обработчиков событий (*.epf)|*.epf';en='External data processor file of event handlers (*.epf)|*.epf'");
...
Процедура Готово(Команда)
	...
	Если ПустаяСтрока(Объект.ИмяФайлаВнешнейОбработкиОбработчиковСобытий) Тогда
		...
		СообщитьПользователю(НСтр("ru='Укажите имя файла внешней обработки.';en='Specify the file name of external data processor.'"), "ИмяФайлаВнешнейОбработкиОбработчиковСобытий");
		Возврат;
...
Процедура ПроверкаСуществованияФайлаВнешнейОбработкиОбработчиковСобытийЗавершение(Существует, ДополнительныеПараметры) Экспорт
	...
	Если Не Существует Тогда
		СообщитьПользователю(НСтр("ru='Указанный файл внешней обработки не существует.';en='Specified file of external processing does not exist.'"),
...
Процедура ВыгрузитьКодОбработчиков(Команда)
	...
	Если Не ПустаяСтрока(Объект.ИмяВременногоФайлаОбработчиковСобытий) Тогда
		...
		СписокКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Выгрузить повторно';en='Export again'"));
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Модуль отладки с кодом обработчиков уже выгружен.';en='Debug engine with handler code is already exported.'"), СписокКнопок,,КодВозвратаДиалога.Нет);
...
Процедура ПоказатьОбработчикиСобытийВОкне()
	...
	Если ОбработчикиСобытий <> Неопределено Тогда
		ОбработчикиСобытий.Показать(НСтр("ru='Модуль отладки обработчиков';en='Handler debug engine'"));
	КонецЕсли;
	
	
	ПротоколОбмена = ПротоколОбмена();
	Если ПротоколОбмена <> Неопределено Тогда
		ПротоколОбмена.Показать(НСтр("ru='Ошибки выгрузки модуля обработчиков';en='An error occurred when exporting the handler module'"));
	КонецЕсли;

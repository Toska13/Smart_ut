...
Перем МенеджерыДляПлановОбмена Экспорт; // Соответствие, содержащее менеджеры для планов обмена
...
Перем ФлагКомментироватьОбработкуОбъектов Экспорт; // Признак комментирования процесса обработки объектов
...
Перем одТипУзлаXML_КонецЭлемента  Экспорт; // Тип узла XML
Перем одТипУзлаXML_НачалоЭлемента Экспорт; // Тип узла XML
Перем одТипУзлаXML_Текст          Экспорт; // Тип узла XML

Перем ЗначениеПустаяДата Экспорт; // Содержит значение пустой даты

Перем одСообщения;             // Соответствие. Ключ - код ошибки,  Значение - описание ошибки.

Перем мСписокМакетовПравилОбмена Экспорт; // СписокЗначений, содержащий синонимы макетов правил обмена
...
Перем ЗагруженныеГлобальныеОбъекты Экспорт; // Соответствие
Перем ЧислоХранимыхЗагруженныхОбъектов Экспорт;  // Число хранимых загруженных объектов, 
                                          ...
                                          // очищается.
Перем ЗапоминатьЗагруженныеОбъекты Экспорт; // Признак необходимости запоминать ссылку на объект
...
Перем мБылиПрочитаныПравилаОбменаПриЗагрузке Экспорт; // Булево
...
Перем СобытияПослеЗагрузкиПараметров Экспорт; // Структура, содержащая алгоритмы, выполняемые после загрузки параметров
...
//
// Возвращаемое значение:
//  XMLЗначение	- Значение xml-элемента, приведенное к соответствующему типу.
//
...
Функция ВыгрузитьПоПравилу(Источник					= Неопределено,
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаПользователю = ?(ТолькоПолучитьУзелСсылки, НСтр("ru='Конвертация ссылки на объект: %1';en='Conversion of reference to object: %1 '"), НСтр("ru='Конвертация объекта: %1'"));
	...
	Если ЕстьГлобальныйОбработчикПередКонвертациейОбъекта Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеВыгрузкиОбработчикаПКО(64, ОписаниеОшибки(), ПКО, Источник, НСтр("ru='ПередКонвертациейОбъекта (глобальный)';en='ПередКонвертациейОбъекта (global)'"));
		КонецПопытки;
...
//
// Возвращаемое значение:
//   РезультатЗапроса - результат запроса для выгрузки очистки данных или неопределено.
//
...
Процедура ПередатьИнформациюОЗаписиВПриемник(ИнформацияДляЗаписиВФайл, СтрокаОшибкиВБазеПриемнике = "") Экспорт
	...
	Если Не ПустаяСтрока(СтрокаОшибкиВБазеПриемнике) Тогда
		...
		СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ru='Загрузка в приемнике: %1';en='Import in the target: %1'"), СтрокаОшибкиВБазеПриемнике);
	...
	ИначеЕсли ФлагРежимОтладкиОбработчиков Тогда
		...
		Попытка
			...
			Если ЭтоВнешняяОбработка() Тогда
				...
				ВызватьИсключение
					НСтр("ru='Внешняя обработка отладки, загружаемая из файла на диске, не поддерживается.';en='External debug data processor imported from the file on disk is not supported.'");
...
Процедура УдалитьВременныеФайлы(ИмяВременногоФайла) Экспорт
	...
	Если Не ПустаяСтрока(ИмяВременногоФайла) Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru='Универсальный обмен данными в формате XML';en='Universal Data Exchange (XML)'", КодОсновногоЯзыка()),
...
Функция ЗаписьТекстаВоВременныйФайл(СписокВременныхФайлов)
	...
	Исключение
		ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(1000,
			ОписаниеОшибки(),
			НСтр("ru='Ошибка при создании временного файла для выгрузки данных';en='An error occurred when creating a temporary file for data export'"));
		ВызватьИсключение;
	КонецПопытки;
...
Функция ЧтениеТекстаИзВременногоФайла(ИмяВременногоФайла)
	...
	Исключение
		ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(1000,
			ОписаниеОшибки(),
			НСтр("ru='Ошибка при открытии временного файла для переноса данных в файл обмена';en='Error opening temporary file to transfer data to exchange file'"));
		ВызватьИсключение;
	КонецПопытки;
...
Процедура ВыгрузитьОбработчикиСобытий(Отказ) Экспорт
	...
	Если ЭтоИнтерактивныйРежим Тогда
		...
		Если ФлагОшибки Тогда
			...
			СообщитьПользователю(НСтр("ru='При выгрузке обработчиков были обнаружены ошибки.';en='When unloading handlers, errors were found.'"));
			
		Иначе
			
			СообщитьПользователю(НСтр("ru='Обработчики успешно выгружены.';en='Handlers successfully unloaded.'"));
...
Функция ЗаменитьВызовыАлгоритмовКодомЭтихАлгоритмовВОбработчике(Обработчик, АлгоритмВладелец, МассивЗапрошенныхЭлементов, Знач СтрокаПрефикса = "")
	...
	ЗаписатьЗаголовокБлокаАлгоритма(ОбработчикНовый, АлгоритмВладелец, СтрокаПрефикса, НСтр("ru='{НАЧАЛО АЛГОРИТМА}';en='{ALGORITHM START}'"));
		...
		Если АлгоритмыОбработчика.Количество() <> 0 Тогда // В этой строке есть вызовы алгоритмов.
			...
			Для Каждого Алгоритм Из АлгоритмыОбработчика Цикл
				...
				Если МассивЗапрошенныхЭлементов.Найти(ВРег(Алгоритм)) <> Неопределено Тогда // Рекурсивный вызов алгоритма.
					...
					ЗаписатьЗаголовокБлокаАлгоритма(ОбработчикНовый, Алгоритм, СтрокаПрефиксаДляВложенногоКода, НСтр("ru='{РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА}';en='{RECURSIVE ALGORITHM CALL}'"));
					
					СтрокаОператора = НСтр("ru='ВызватьИсключение ""РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА: %1"";';en='CallException ""ALGORITHM RECURSIVE CALL: %1"";'");
	...
	ЗаписатьЗаголовокБлокаАлгоритма(ОбработчикНовый, АлгоритмВладелец, СтрокаПрефикса, НСтр("ru='{КОНЕЦ АЛГОРИТМА}';en='{ALGORITHM END}'"));
...
Функция СоздатьНовыйОбъект(Тип, СвойстваПоиска, Объект = Неопределено, 
	...
	Если ЗаписыватьОбъектСразуПослеСоздания Тогда
		...
		Если ГНПП <> 0
			...
			Если  ИмяТипа = "РегистрСведений" ИЛИ ИмяТипа = "Документ" ИЛИ ИмяТипа = "Справочник" ИЛИ ИмяТипа = "Задача" ИЛИ ИмяТипа = "БизнесПроцесс" Тогда
				Если Параметры.Свойство("ИдентификаторУзла") Тогда
					Объект.ДополнительныеСвойства.Вставить("ИдентификаторУзла", Параметры.ИдентификаторУзла);
				КонецЕсли;	
			КонецЕсли;
	...
	Пока НомерВариантаПоиска <= 10
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеЗагрузкиОбработчикаПКО(73, ОписаниеОшибки(), "", "",
				ТипОбъекта, Неопределено, НСтр("ru='Последовательность полей поиска';en='Sequence of search fields'"));
...
Процедура КомментарииКЗагрузкеОбъекта(НПП, ИмяПравила, Источник, ТипОбъекта, ГНПП = 0)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		Если НПП <> 0 Тогда
			СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ru='Загрузка объекта № %1';en='Import object No. %1'"), НПП);
		Иначе
			СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ru='Загрузка объекта № %1';en='Import object No. %1'"), ГНПП);
		КонецЕсли;
	...
	Если ЕстьГлобальныйОбработчикПередЗагрузкойОбъекта Тогда
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеЗагрузкиОбработчикаПКО(53, ОписаниеОшибки(), ИмяПравила, Источник,
				ТипОбъекта, Неопределено, НСтр("ru='ПередЗагрузкойОбъекта (глобальный)';en='Beforeloadinganobject (global)'"));
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		ИначеЕсли (ИмяУзла = "Объект") И (ФайлОбмена.ТипУзла = одТипУзлаXML_КонецЭлемента) Тогда
			...
			Если ЕстьГлобальныйОбработчикПослеЗагрузкиОбъекта Тогда
				...
				Исключение
					...
					ЗаписатьИнформациюОбОшибкеЗагрузкиОбработчикаПКО(54, ОписаниеОшибки(), ИмяПравила, Источник,
							ТипОбъекта, Объект, НСтр("ru='ПослеЗагрузкиОбъекта (глобальный)';en='ПослеЗагрузкиОбъекта (global)'"));
			...
			ИначеЕсли ИмяТипаОбъекта <> "Перечисление" Тогда
				...
				Если НужноЗаписатьОбъект Тогда
					Если  ИмяТипаОбъекта = "РегистрСведений" ИЛИ ИмяТипаОбъекта = "Документ" ИЛИ ИмяТипаОбъекта = "Справочник" ИЛИ ИмяТипаОбъекта = "Задача" ИЛИ ИмяТипаОбъекта = "БизнесПроцесс" Тогда
						Если Параметры.Свойство("ИдентификаторУзла") Тогда
							Объект.ДополнительныеСвойства.Вставить("ИдентификаторУзла", Параметры.ИдентификаторУзла);
						КонецЕсли;	
					КонецЕсли;	
...
Процедура ВыгрузкаОбъектаВыборки(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, ВыборкаДляВыгрузкиДанных = Неопределено)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ru='Выгрузка объекта: %1';en='Export object: %1'"), ПрОбъекта);
	...
	Если ЕстьГлобальныйОбработчикПередВыгрузкойОбъекта Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиПВД(65, ОписаниеОшибки(), Правило.Имя, НСтр("ru='ПередВыгрузкойОбъектаВыборки (глобальный)';en='ПередВыгрузкойОбъектаВыборки (global)'"), Объект);
		КонецПопытки;
	...
	Если ЕстьГлобальныйОбработчикПослеВыгрузкиОбъекта Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиПВД(69, ОписаниеОшибки(), Правило.Имя, НСтр("ru='ПослеВыгрузкиОбъектаВыборки (глобальный)';en='ПослеВыгрузкиОбъектаВыборки (global)'"), Объект);
		КонецПопытки;
...
Процедура ЗагрузитьЗначенияПараметровОбменаДанными()
	...
	Если СобытияПослеЗагрузкиПараметров.Свойство(Имя, АлгоритмПослеЗагрузкиПараметра)
		...
		Если ФлагРежимОтладкиОбработчиков Тогда
			...
			ВызватьИсключение НСтр("ru='Отладка обработчика ""После загрузки параметра"" не поддерживается.';en='Debugging of handler ""After parameter import"" is not supported.'");
	...
	Если Конвертация.Свойство("ПослеЗагрузкиПравилОбмена", ТекстСобытияПослеЗагрузкиПравилОбмена)
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				ВызватьИсключение НСтр("ru='Отладка обработчика ""После загрузки правил обмена"" не поддерживается.';en='Debugging of handler ""ПослеЗагрузкиПравилОбмена"" is not supported.'");
...
Функция ПроизвестиЧтениеДанных(РезультирующаяСтрокаСИнформациейОбОшибке = "") Экспорт
				...
				Если Не ПустаяСтрока(ТекстАлгоритма) Тогда
					...
					Попытка
						...
						Если ФлагРежимОтладкиОбработчиков Тогда
							...
							ВызватьИсключение НСтр("ru='Отладка обработчика ""После загрузки параметров"" не поддерживается.';en='Debugging of handler ""ПослеЗагрузкиПараметров"" is not supported.'");
						...
						Если Отказ = Истина Тогда
							...
							Если Не ПустаяСтрока(ПричинаОтказа) Тогда
								СтрокаИсключения = ПодставитьПараметрыВСтроку(НСтр("ru='Загрузка данных отменена по причине: %1';en='Data import was canceled as: %1'"), ПричинаОтказа);
								ВызватьИсключение СтрокаИсключения;
							Иначе
								ВызватьИсключение НСтр("ru='Загрузка данных отменена';en='Data import is canceled'");
							КонецЕсли;
				...
				Если Не ПустаяСтрока(ТекстАлгоритма) Тогда
					...
					Попытка
						...
						Если ФлагРежимОтладкиОбработчиков Тогда
							...
							ВызватьИсключение НСтр("ru='Отладка глобального алгоритма не поддерживается.';en='Global algorithm debugging is not supported.'");
	...
	//Исключение
	//	
	//	СтрокаОшибки = ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при загрузке данных: %1';en='An error occurred when importing data: %1'"), ОписаниеОшибки());
	//	
...
Функция ВыполнитьДействияПередЧтениемДанных(СтрокаДанных = "") Экспорт
	...
	Если Не ПустаяСтрока(Конвертация.ПередЗагрузкойДанных) Тогда
		...
		Исключение
			СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(22, ОписаниеОшибки(), НСтр("ru='ПередЗагрузкойДанных (конвертация)';en='Beforeloadingdata (conversion)'"));
			Отказ = Истина;
		КонецПопытки;
...
Процедура ВыполнитьДействияПослеЗавершенияЧтенияДанных() Экспорт
	...
	Если Не ПустаяСтрока(Конвертация.ПослеЗагрузкиДанных) Тогда
		...
		Исключение
			СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(23, ОписаниеОшибки(), НСтр("ru='ПослеЗагрузкиДанных (конвертация)';en='Afterdataloading(conversion)'"));
		КонецПопытки;
	...
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("ru='Окончание загрузки: %1';en='Import end: %1'"), ТекущаяДатаСеанса()), , Ложь, , , Истина);
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
	...
	Если ЭтоИнтерактивныйРежим Тогда
		СообщитьПользователю(НСтр("ru='Загрузка данных завершена.';en='Data import is complete.'"));
	КонецЕсли;
...
Процедура ВыполнитьЗагрузку() Экспорт
	...
	Попытка
		Результат = ПроизвестиЧтениеДанных();
...
Процедура ВыполнитьВыгрузкуCRM(МассивОбъектовДляВыгрузки, РезультатВыгрузки) Экспорт
	...
	//Попытка
		...
		Если Не Отказ Тогда
			...
			Исключение
				ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(62, ОписаниеОшибки(), НСтр("ru='ПередВыгрузкойДанных (конвертация)';en='ПередВыгрузкойДанных (conversion)'"));
				Отказ = Истина;
			КонецПопытки;
			...
			Если Не Отказ Тогда
				...
				Исключение
					ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(63, ОписаниеОшибки(), НСтр("ru='ПослеВыгрузкиДанных (конвертация)';en='ПослеВыгрузкиДанных (conversion)'"));
				КонецПопытки;
	...
	//	
	//	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
	//		НСтр("ru='Ошибка при выгрузке данных: %1';en='An error occurred when exporting data: %1'"), СтрокаОшибки), Неопределено, Истина, , , Истина);
	//	
	//	ВыполнитьПередачуИнформацииПриЗавершенииПередачиДанных();
	//	
	//	ЗавершитьВедениеПротоколаОбмена();
	...
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("ru='Окончание выгрузки: %1';en='Export end: %1'"), ТекущаяДатаСеанса()), , Ложь, , ,Истина);
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("ru='Выгружено объектов: %1';en='Exported objects: %1'"), мСчетчикВыгруженныхОбъектов), , Ложь, , , Истина);
	...
	Если ЭтоИнтерактивныйРежим Тогда
		СообщитьПользователю(НСтр("ru='Выгрузка данных завершена.';en='Data is exported.'"));
	КонецЕсли;
...
Процедура ИнициализацияСообщений()
	...
	одСообщения.Вставить(2,  НСтр("ru='Ошибка распаковки файла обмена. Файл заблокирован';en='An error occurred when unpacking an exchange file. The file is locked'"));
	...
	одСообщения.Вставить(4,  НСтр("ru='Ошибка при создании COM-объекта Msxml2.DOMDocument';en='An error occurred when creating COM object Msxml2.DOMDocument'"));
	одСообщения.Вставить(5,  НСтр("ru='Ошибка открытия файла обмена';en='An error occurred when opening the exchange file'"));
	одСообщения.Вставить(6,  НСтр("ru='Ошибка при загрузке правил обмена';en='An error occurred when importing exchange rules'"));
	одСообщения.Вставить(7,  НСтр("ru='Ошибка формата правил обмена';en='Error in exchange rule format'"));
	одСообщения.Вставить(8,  НСтр("ru='Некорректно указано имя файла для выгрузки данных';en='Incorrectly  specify file name for uploading data'"));
	одСообщения.Вставить(9,  НСтр("ru='Ошибка формата файла обмена';en='Error in exchange file format'"));
	одСообщения.Вставить(10, НСтр("ru='Не указано имя файла для выгрузки данных (Имя файла данных)';en='File name for data export is not specified (Data file name)'"));
	одСообщения.Вставить(11, НСтр("ru='Ссылка на несуществующий объект метаданных в правилах обмена';en='Reference to a non-existing metadata object in the exchange rules'"));
	одСообщения.Вставить(12, НСтр("ru='Не указано имя файла с правилами обмена (Имя файла правил)';en='File name with exchange rules is not specified (Rule file name)'"));
	
	одСообщения.Вставить(13, НСтр("ru='Ошибка получения значения свойства объекта (по имени свойства источника)';en='An error occurred when receiving a value of the object property (by the name of the source property)'"));
	одСообщения.Вставить(14, НСтр("ru='Ошибка получения значения свойства объекта (по имени свойства приемника)';en='An error occurred when receiving the object property value (by the target property name)'"));
	
	одСообщения.Вставить(15, НСтр("ru='Не указано имя файла для загрузки данных (Имя файла для загрузки)';en='File name for data import is not specified (File name for import)'"));
	
	одСообщения.Вставить(16, НСтр("ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства источника)';en='An error occurred while receiving value of subobject property (by source property name)'"));
	одСообщения.Вставить(17, НСтр("ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства приемника)';en='An error occurred while receiving value of subobject property (by target property name)'"));
	
	одСообщения.Вставить(19, НСтр("ru='Ошибка в обработчике события ПередЗагрузкойОбъекта';en='An error occurred in the handler of event ПередЗагрузкойОбъекта'"));
	одСообщения.Вставить(20, НСтр("ru='Ошибка в обработчике события ПриЗагрузкеОбъекта';en='An error occurred in the handler of event ПриЗагрузкеОбъекта'"));
	одСообщения.Вставить(21, НСтр("ru='Ошибка в обработчике события ПослеЗагрузкиОбъекта';en='An error occurred in the handler of event ПослеЗагрузкиОбъекта'"));
	одСообщения.Вставить(22, НСтр("ru='Ошибка в обработчике события ПередЗагрузкойДанных (конвертация)';en='An error occurred in the handler of event ПередЗагрузкойДанных (conversion)'"));
	одСообщения.Вставить(23, НСтр("ru='Ошибка в обработчике события ПослеЗагрузкиДанных (конвертация)';en='An error occurred in the handler of event ПослеЗагрузкиДанных (conversion)'"));
	одСообщения.Вставить(24, НСтр("ru='Ошибка при удалении объекта';en='An error occurred when removing an object'"));
	одСообщения.Вставить(25, НСтр("ru='Ошибка при записи документа';en='Error at record of the document'"));
	одСообщения.Вставить(26, НСтр("ru='Ошибка записи объекта';en='An error occurred when writing the object'"));
	одСообщения.Вставить(27, НСтр("ru='Ошибка в обработчике события ПередОбработкойПравилаОчистки';en='An error occurred in the handler of event ПередОбработкойПравилаОчистки'"));
	одСообщения.Вставить(28, НСтр("ru='Ошибка в обработчике события ПослеОбработкиПравилаОчистки';en='An error occurred in the handler of event ПослеОбработкиПравилаОчистки'"));
	одСообщения.Вставить(29, НСтр("ru='Ошибка в обработчике события ПередУдалениемОбъекта';en='An error occurred in the handler of event ПередУдалениемОбъекта'"));
	
	одСообщения.Вставить(31, НСтр("ru='Ошибка в обработчике события ПередОбработкойПравилаВыгрузки';en='An error occurred in the handler of event ПередОбработкойПравилаВыгрузки'"));
	одСообщения.Вставить(32, НСтр("ru='Ошибка в обработчике события ПослеОбработкиПравилаВыгрузки';en='An error occurred in the handler of event ПослеОбработкиПравилаВыгрузки'"));
	одСообщения.Вставить(33, НСтр("ru='Ошибка в обработчике события ПередВыгрузкойОбъекта';en='An error occurred in the handler of event ПередВыгрузкойОбъекта'"));
	одСообщения.Вставить(34, НСтр("ru='Ошибка в обработчике события ПослеВыгрузкиОбъекта';en='An error occurred in the handler of event ПослеВыгрузкиОбъекта'"));
	
	одСообщения.Вставить(39, НСтр("ru='Ошибка при выполнении алгоритма, содержащегося в файле обмена';en='An error occurred when executing the algorithm from the exchange file'"));
	
	одСообщения.Вставить(41, НСтр("ru='Ошибка в обработчике события ПередВыгрузкойОбъекта';en='An error occurred in the handler of event ПередВыгрузкойОбъекта'"));
	одСообщения.Вставить(42, НСтр("ru='Ошибка в обработчике события ПриВыгрузкеОбъекта';en='An error occurred in the handler of event ПриВыгрузкеОбъекта'"));
	одСообщения.Вставить(43, НСтр("ru='Ошибка в обработчике события ПослеВыгрузкиОбъекта';en='An error occurred in the handler of event ПослеВыгрузкиОбъекта'"));
	
	одСообщения.Вставить(45, НСтр("ru='Не найдено правило конвертации объектов';en='Object conversion rule is not found'"));
	
	одСообщения.Вставить(48, НСтр("ru='Ошибка в обработчике события ПередОбработкойВыгрузки группы свойств';en='An error occurred in the handler of event ПередОбработкойВыгрузки of the property group'"));
	одСообщения.Вставить(49, НСтр("ru='Ошибка в обработчике события ПослеОбработкиВыгрузки группы свойств';en='An error occurred in the handler of event ПослеОбработкиВыгрузки of the property group'"));
	одСообщения.Вставить(50, НСтр("ru='Ошибка в обработчике события ПередВыгрузкой (объекта коллекции)';en='Error in the handler of event ПередВыгрузкой (of collection object)'"));
	одСообщения.Вставить(51, НСтр("ru='Ошибка в обработчике события ПриВыгрузке (объекта коллекции)';en='Error in the handler of event ПриВыгрузке (of collection object)'"));
	одСообщения.Вставить(52, НСтр("ru='Ошибка в обработчике события ПослеВыгрузки (объекта коллекции)';en='Error in the handler of event ПослеВыгрузки (of collection object)'"));
	одСообщения.Вставить(53, НСтр("ru='Ошибка в глобальном обработчике события ПередЗагрузкойОбъекта (конвертация)';en='An error occurred in global handler of event ПередЗагрузкойОбъекта (conversion)'"));
	одСообщения.Вставить(54, НСтр("ru='Ошибка в глобальном обработчике события ПослеЗагрузкиОбъекта (конвертация)';en='An error occurred in global handler of event ПослеЗагрузкиОбъекта (conversion)'"));
	одСообщения.Вставить(55, НСтр("ru='Ошибка в обработчике события ПередВыгрузкой (свойства)';en='An error occurred in the handler of event ПередВыгрузкой (properties)'"));
	одСообщения.Вставить(56, НСтр("ru='Ошибка в обработчике события ПриВыгрузке (свойства)';en='An error occurred in the handler of event ПриВыгрузке (properties)'"));
	одСообщения.Вставить(57, НСтр("ru='Ошибка в обработчике события ПослеВыгрузки (свойства)';en='An error occurred in the handler of event ПослеВыгрузки (properties)'"));
	
	одСообщения.Вставить(62, НСтр("ru='Ошибка в обработчике события ПередВыгрузкойДанных (конвертация)';en='An error occurred in the handler of event ПередВыгрузкойДанных (conversion)'"));
	одСообщения.Вставить(63, НСтр("ru='Ошибка в обработчике события ПослеВыгрузкиДанных (конвертация)';en='An error occurred in the handler of event ПослеВыгрузкиДанных (conversion)'"));
	одСообщения.Вставить(64, НСтр("ru='Ошибка в глобальном обработчике события ПередКонвертациейОбъекта (конвертация)';en='An error occurred in global handler of event ПередКонвертациейОбъекта (conversion)'"));
	одСообщения.Вставить(65, НСтр("ru='Ошибка в глобальном обработчике события ПередВыгрузкойОбъекта (конвертация)';en='An error occurred in global handler of event ПередВыгрузкойОбъекта (conversion)'"));
	одСообщения.Вставить(66, НСтр("ru='Ошибка получения коллекции подчиненных объектов из входящих данных';en='An error occurred when receiving a subordinate object collection from the incoming data'"));
	одСообщения.Вставить(67, НСтр("ru='Ошибка получения свойства подчиненного объекта из входящих данных';en='An error occurred when receiving the subordinate object properties from the incoming data'"));
	одСообщения.Вставить(68, НСтр("ru='Ошибка получения свойства объекта из входящих данных';en='An error occurred when receiving the object properties from the incoming data'"));
	
	одСообщения.Вставить(69, НСтр("ru='Ошибка в глобальном обработчике события ПослеВыгрузкиОбъекта (конвертация)';en='An error occurred in global handler of event ПослеВыгрузкиОбъекта (conversion)'"));
	
	одСообщения.Вставить(71, НСтр("ru='Не найдено соответствие для значения Источника';en='Match for the Source value is not found'"));
	
	одСообщения.Вставить(72, НСтр("ru='Ошибка при выгрузке данных для узла плана обмена';en='An error occurred when exporting data for the exchange plan node'"));
	
	одСообщения.Вставить(73, НСтр("ru='Ошибка в обработчике события ПоследовательностьПолейПоиска';en='An error occurred in the handler of event ПоследовательностьПолейПоиска'"));
	
	одСообщения.Вставить(74, НСтр("ru='Необходимо перезагрузить правила обмена для выгрузки данных';en='Import exchange rules for data export again'"));
	
	одСообщения.Вставить(75, НСтр("ru='Ошибка при выполнении алгоритма после загрузки значений параметров';en='An error occurred when executing the algorithm after import of the parameter values'"));
	
	одСообщения.Вставить(76, НСтр("ru='Ошибка в обработчике события ПослеВыгрузкиОбъектаВФайл';en='An error occurred in the handler of event ПослеВыгрузкиОбъектаВФайл'"));
	
	одСообщения.Вставить(77, НСтр("ru='Не указан файл внешней обработки с подключаемыми процедурами обработчиков событий';en='File of external data processor with connected procedures of event handlers is not specified'"));
	
	одСообщения.Вставить(78, НСтр("ru='Ошибка создания внешней обработки из файла с процедурами обработчиков событий';en='An error occurred when creating an external data processor from a file with event handler procedures'"));
...
"и повторите выгрузку.';en='Algorithm code cannot be integrated into the handler due to detected recursive algorithm call."

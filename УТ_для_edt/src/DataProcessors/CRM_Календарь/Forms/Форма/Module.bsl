...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.КнопкаТекущееОтображениеДиспетчеризация.Видимость = CRM_ЛицензированиеЭкспортныеМетоды.ВариантПоставкиКОРП();
	...
	Если Пользователи.РолиДоступны("CRM_УправлениеПроектами, ПолныеПрава") И CRM_ЛицензированиеЭкспортныеМетоды.ВариантПоставкиКОРП() Тогда
...
Процедура КалендарьСписокТекстПоискаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СписокДляВыбора.Добавить("[ПоискПоПолномуНазванию]", НСтр("ru='[Поиск по полному названию]';en='[Search by Full Name]'"));
	СписокДляВыбора.Добавить("[ПоискПоПодстроке]", НСтр("ru='[Поиск по подстроке]';en='[Search]'"));
...
Процедура ДобавитьУдалитьЭлементВДеревеРесурсов(ДобавитьГруппу = Ложь,
	...
	Если ДобавитьГруппу Тогда
		...
		ПоказатьВводСтроки(ОписаниеОповещения, СтрокаОписание, НСтр("ru='Название группы';en='Group name'"),, Ложь);
	Иначе
	...
	Если бВключитьОтборПоТекущемуПользователю Тогда
		Если Не ЗначениеЗаполнено(ОтборЗначениеПользователь) Или Не ОтборИспользованиеПользователь Тогда
			...
			ПоказатьОповещениеПользователя(
				"",
				"",
				СтрЗаменить(НСтр("ru='Один из отборов должен быть установлен: %Отбор%';en='One of filter should be chosen: %Отбор%'"), "%Отбор%", СтрокаОбязательныеОтборы + "."));
			//
...
Процедура КомандаКонтекстногоМенюКалендаряОбщая(Команда)
	...
	Иначе
		ВызватьИсключение НСтр("ru='Для кнопки';en='For the button'") + " """ + ИмяКоманды + """ " + НСтр("ru='не определено действие!'");
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьКонтекстныеМеню()
	...
	Если	ПолучитьФункциональнуюОпцию("CRM_ИспользоватьПодсистемуУправленияПроектами")
		И	CRM_ЛицензированиеЭкспортныеМетоды.ВариантПоставкиКОРП()
		...
		Если СобытиеОбъект.Ответственный <> НаПользователя Тогда
			Если ЗначениеЗаполнено(СобытиеОбъект.Ответственный) Тогда
				...
				СобытиеОбъект.Описание = НСтр("ru='Переадресовано от';en='Redirected from'") + " " + СобытиеОбъект.Ответственный + " " + CRM_ОбщегоНазначенияСервер.ПолучитьТекущуюДатуСеанса()
					+ НСтр("ru=' (предыдущее состояние ""';en=' (previous state ""'") + СобытиеОбъект.СостояниеСобытия + """)" 
...
Процедура СоздатьСобытиеИзКалендаря(ТипСобытия = Неопределено, Знач ВидСобытия = Неопределено, Знач УпрощеннаяФорма = Ложь)
	...
	Если ЗадаватьВопросПередСозданиемСобытия(СтруктураДанныхСобытия) Тогда
		ТекстВопроса = НСтр("ru='День';en='Day'") + " "
			...
			+ НСтр("ru=' отмечен в производственном календаре как не рабочий.';en=' marked in production calendar as non working.'")
			+ Символы.ПС
	...
	Для Каждого СтрокаМассива Из МассивДокументов Цикл
		...
		Попытка
			...
			Если ТипЗнч(ТекущийДокументОбъект) = Тип("ДокументОбъект.УдалитьCRM_Событие") Тогда
				ТекущийДокументОбъект.Описание = НСтр("ru='Переадресовано от ';en='Forwarded from '") + ТекущийДокументОбъект.Ответственный + " " + CRM_ОбщегоНазначенияСервер.ПолучитьТекущуюДатуСеанса()
				+ НСтр("ru=' (предыдущее состояние ""';en=' (previous state ""'") + ТекущийДокументОбъект.СостояниеСобытия + """)" 
...
Процедура ПеренаправитьСобытиеНаКлиенте(МассивПеренаправляемыхСобытий, СтруктураДанныхКлиент = Неопределено)
	...
	Если МассивИзбранныхПолучателей.Количество() > 0 Тогда
		...
		КонецЦикла;
		СписокВыбора.Добавить(ПредопределенноеЗначение("Справочник.Пользователи.ПустаяСсылка"), НСтр("ru='Выбрать из списка ...';en='Choose from the list ...'"), , БиблиотекаКартинок.Лупа);
		...
		Для Каждого ЗадачаСсылка Из МассивНеЛичныхЗадач Цикл
			Если Не ЗадачаСсылка.ПринятаКИсполнению Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Задача';en='Task'") + " """ + Строка(ЗадачаСсылка) + """ "
					+ НСтр("ru='не принята к исполнению, изменение реквизитов недоступно!';en='it is not accepted to execution, change of attributes are inaccessible!'"));
				//
				Продолжить;
			КонецЕсли;
...
Процедура ОбновитьСписокЗадачНаСервере()
	...
	Если бДобавлятьОтборПоИсполнителю Тогда
		...
		ГруппаИли = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(НастройкиСКД.Отбор.Элементы, НСтр("ru='Исполнитель';en='Assignee'"), ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
...
Функция ДеньНеделиСтрокой(ПараметрДата)
	Стр = НСтр("ru=' ПнВтСрЧтПтСбВс';en='MonTuesWedThurFriSatSun'");
...
Функция ПолучитьСписокВыбораПериода()
	...
	Список.Добавить("ПроизвольныйПериод",	НСтр("ru='Произвольный период';en='Custom Period'"));
...
Функция ПолучитьСписокГруппировокСпискаКалендаря()
	...
	Список.Добавить("БезГруппировок", НСтр("ru='Без группировки';en='No grouping'"));
...
Процедура ЗагрузитьНастройкиФормы()
	...
	КонецЕсли;
	Если СохраненныйРежимОтображения = "Диспетчеризация" И НЕ CRM_ЛицензированиеЭкспортныеМетоды.ВариантПоставкиКОРП() Тогда
			...
			Иначе
				...
				Для Каждого СтрокаЗанятостьЯчейки Из СтрокиЗанятостьЯчейки Цикл
					...
					Если ОписаниеСобытия = Неопределено Тогда
						...
						// Если БлижайшееСобытиеСлева <> Неопределено И БлижайшееСобытиеСправа <> Неопределено И БлижайшийГорИндексСлева >
						// БлижайшийНезаполненныйГорИндексСлева И БлижайшийГорИндексСправа < БлижайшийНезаполненныйГорИндексСправа Тогда
...
Процедура СформироватьСписокНаСервере(СтруктураДанныхКлиент)
	...
	ОбластьКолонки.Параметры.ПериодЗаголовок = ?(СписокКалендаряГруппироватьПоДням, НСтр("ru='Время';en='Time'"), НСтр("ru='Период'"));
...
Процедура СформироватьРасписаниеНаСервере(СтруктураДанныхКлиент);
	...
	Пока ТекДата <= НачалоДня(МаксимальнаяДата) Цикл
		...
		ОбластьЛинияСнизу.Текст = Формат(ТекДата, "ДФ='дд ММММ гггг'") + НСтр("ru='г';en='year'");
...
Процедура ЗаполнитьДанныеОбработки()
	...
	Для н = -ДобавитьМесяцев По ДобавитьМесяцев Цикл
		...
		ПредставлениеМесяца = Формат(ДобавляемыйМесяц, "ДФ='ММММ гггг'") + " " + НСтр("ru='г.';en='year'");
		...
		Если СписокКалендаряГруппироватьПоДням Тогда
			...
			Если бИспользуетсяПолнотекстовыйПоискСписка Тогда
				...
				Если Не ЗначениеЗаполнено(ТекстОшибкиГлобальный) И РезультатТаблица.Количество() = 0 Тогда
					ТекстОшибкиГлобальный = НСтр("ru='Ничего не найдено';en='No results found'");
				КонецЕсли;
			Иначе
				МассивВыбранныхДат = СтруктураДанныхКлиент.МассивВыбранныхДат;
			КонецЕсли;
	...
	Если РасширитьВниз Тогда
		...
		Для н = 1 По ДобавитьМесяцев Цикл
			...
			Если н = 1 Тогда
				...
				СписокВыборМесяца[СписокВыборМесяца.Количество() - 1].Представление = Формат(ДобавляемыйМесяц, "ДФ='ММММ гггг'") + " " + НСтр("ru='г.';en='year'");
			Иначе
				СписокВыборМесяца.Добавить(ДобавляемыйМесяц, Формат(ДобавляемыйМесяц, "ДФ='ММММ гггг'") + " " + НСтр("ru='г.';en='year'"));
			КонецЕсли;
		КонецЦикла;
		СписокВыборМесяца.Добавить(Дата("99990101"), СтрЕще);
	Иначе 
		...
		Для н = 1 По ДобавитьМесяцев Цикл
			...
			Если н = 1 Тогда
				...
				СписокВыборМесяца[0].Представление = Формат(ДобавляемыйМесяц, "ДФ='ММММ гггг'") + " " + НСтр("ru='г.';en='year'");
			Иначе
				СписокВыборМесяца.Вставить(0, ДобавляемыйМесяц, Формат(ДобавляемыйМесяц, "ДФ='ММММ гггг'") + " " + НСтр("ru='г.';en='year'"));
			КонецЕсли;
		КонецЦикла;
		СписокВыборМесяца.Вставить(0, Дата("00010101"), СтрЕще);
	КонецЕсли;
...
"или выбранному периоду.';en='Event will not be displayed in calendar, as is not correspond to filter"
"or selected period.'"), 10);
				//
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;

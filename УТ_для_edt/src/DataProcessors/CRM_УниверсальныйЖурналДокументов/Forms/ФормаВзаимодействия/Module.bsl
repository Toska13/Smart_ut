...
Процедура ОтборОтветственныйПриИзменении(Элемент)
	Если ЗначениеЗаполнено(ОтборОтветственный) Тогда		
		CRM_ОбщегоНазначенияКлиентСервер.ИзменитьЭлементОтбораСписка(Список, "Ответственный", ОтборОтветственный, Истина);		
	Иначе 
		//++ #1441 смарт: Фролов 31.08.2023 //Описание  
		Если НЕ смартПроверитьРоли("ПолныеПрава,смартРуководительCRM") Тогда
			ОтборОтветственный=ПользователиКлиент.ТекущийПользователь();
			CRM_ОбщегоНазначенияКлиентСервер.ИзменитьЭлементОтбораСписка(Список, "Ответственный", ОтборОтветственный, Истина);
		Иначе
...
Процедура Подключаемый_УдалитьТекущуюНастройку(Команда)
	...
	Иначе
		...
		ПоказатьВопрос(Оповещение, "Удалить настройку """
			+ CRM_ПользовательскиеНастройкиСервер.ПолучитьПредставлениеНастройки(ИдентификаторПользовательскойНастройки, ИмяФормы + ".Список") + """ ?", РежимДиалогаВопрос.ДаНет);
	КонецЕсли;	
...
Функция смартПроверитьРоли(Роль)
    Возврат Пользователи.РолиДоступны(Роль);
КонецФункции	//-- #1441 смарт: Фролов 31.08.2023

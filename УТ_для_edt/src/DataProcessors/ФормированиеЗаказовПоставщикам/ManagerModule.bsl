...
Функция ТаблицаЗаказыКПоступлению()
	...
	Возврат
		...
		|	СУММА(Набор.ЗаказыКПоступлениюСвободные) КАК ЗаказыКПоступлениюСвободные
		|ИЗ(
		|	ВЫБРАТЬ
		|		Товары.Номенклатура КАК Номенклатура,
		|		Товары.Характеристика КАК Характеристика,
		|		Товары.Склад КАК Склад,
		|		ГрафикПоступления.КоличествоИзЗаказовСНеподтвержденными КАК ЗаказыКПоступлению,
		|		0 КАК ЗаказыКПоступлениюСвободные
		|		
		|	ИЗ
		|		ВтТовары КАК Товары
		|			
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ГрафикПоступленияТоваров КАК ГрафикПоступления
		...
		|		ДоступныеОстатки.Количество КАК ЗаказыКПоступлениюСвободные
		|	ИЗ
		|		ВтТовары КАК Товары
		|			
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ДоступныеОстаткиПланируемыхПоступлений КАК ДоступныеОстатки
		|			ПО Товары.Номенклатура = ДоступныеОстатки.Номенклатура
		|			 И Товары.Характеристика = ДоступныеОстатки.Характеристика
		|			 И Товары.Склад = ДоступныеОстатки.Склад
		|			 И ДоступныеОстатки.Количество > 0) КАК Набор
		|СГРУППИРОВАТЬ ПО
		|	Набор.Номенклатура,

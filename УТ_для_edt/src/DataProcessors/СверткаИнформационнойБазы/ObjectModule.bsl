	...
	Если ВидРасчетов = "РасчетыСКлиентами" Тогда
		...
		Пока Выборка.СледующийПоЗначениюПоля("ТипОперации") Цикл
			Пока Выборка.СледующийПоЗначениюПоля("Организация") Цикл
				НуженСФАванс = Ложь;
				Если Выборка.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПоАвансамПоставщикам 
					Или Выборка.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПоАвансамКлиентов Тогда
					ПараметрыУчетнойПолитики = ЗначениеНастроекПовтИсп.ПараметрыУчетнойПолитики(Выборка.Организация, КонецДня(ДатаСверткиИБ));
					Если ПараметрыУчетнойПолитики.ПрименяетсяУСН = Истина Тогда
						НуженСФАванс = Ложь; 
					КонецЕсли;
				КонецЕсли;
	...
	Пока Выборка.СледующийПоЗначениюПоля("Организация") Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.Множитель < 0 Тогда
				...
				НоваяСтрока.СуммаУпр = (-1) * НоваяСтрока.СуммаУпр; 
			КонецЕсли;
...
Процедура СформироватьДокументыВводаОстатковТоварыКомиссионные(ОрганизацииУСНФИФО)
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.СледующийПоЗначениюПоля("Организация") Цикл
		НалогообложениеПоУмолчанию = Перечисления.ТипыНалогообложенияНДС.ПоФактическомуИспользованию;
		ПараметрыУчетнойПолитики = ЗначениеНастроекПовтИсп.ПараметрыУчетнойПолитики(Выборка.Организация, КонецДня(ДатаСверткиИБ));
		Если ПараметрыУчетнойПолитики.ПрименяетсяУСН = Истина Тогда
			НалогообложениеПоУмолчанию = Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС;
		КонецЕсли;
					...
					Пока Выборка.СледующийПоЗначениюПоля("НалогообложениеНДС") Цикл
						Пока Выборка.СледующийПоЗначениюПоля("ДокументПартии") Цикл
							...
							Если НЕ ЗначениеЗаполнено(ДокОст.НалогообложениеНДС) Тогда
								ДокОст.НалогообложениеНДС = НалогообложениеПоУмолчанию;
							КонецЕсли;
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.СледующийПоЗначениюПоля("Организация") Цикл 
		НалогообложениеПоУмолчанию = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС;
		ПараметрыУчетнойПолитики = ЗначениеНастроекПовтИсп.ПараметрыУчетнойПолитики(Выборка.Организация, КонецДня(ДатаСверткиИБ));
		Если ПараметрыУчетнойПолитики.ПрименяетсяУСН = Истина Тогда
			НалогообложениеПоУмолчанию = Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС;
		КонецЕсли;
	...
	Пока ВыборкаЗаказ.Следующий() Цикл
		...
		Для Каждого СтрокаТЧ Из ДокОст.Товары Цикл
			...
			Если ВыборкаСтрокаЗаказа.НайтиСледующий(СтруктураОтбора) Тогда
				...
				ИначеЕсли СтрокаТЧ.Количество < ВыборкаСтрокаЗаказа.ЗаказаноКонечныйОстаток Тогда
					СтрокаТЧ.КоличествоУпаковок = СтрокаТЧ.КоличествоУпаковок * ВыборкаСтрокаЗаказа.ЗаказаноКонечныйОстаток / СтрокаТЧ.Количество;
			...
			// Если есть другие строки с такой же номенклатурой и характеристикой - их отменить.
			
			ТекИндекс = ДокОст.Товары.Индекс(СтрокаТЧ);
			Для Каждого СтрокаТЧДубль Из ДокОст.Товары Цикл
				Если СтрокаТЧДубль.Отменено
					ИЛИ ДокОст.Товары.Индекс(СтрокаТЧДубль) = ТекИндекс Тогда
					Продолжить;
				КонецЕсли;
				Если СтрокаТЧДубль.Номенклатура = СтрокаТЧ.Номенклатура
					И СтрокаТЧДубль.Характеристика = СтрокаТЧ.Характеристика
					И СтрокаТЧДубль.Серия = СтрокаТЧ.Серия Тогда
					СтрокаТЧДубль.Отменено = Истина;
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
		...
		Для Каждого СтрокаТЧ Из ДокОст.Товары Цикл
			...
			Если ВыборкаСтрокаЗаказа.НайтиСледующий(СтруктураОтбора) Тогда
				...
				ИначеЕсли СтрокаТЧ.Количество < ВыборкаСтрокаЗаказа.ЗаказаноКонечныйОстаток Тогда 
					...
					ПересчитатьСтрокуЗаказаПоКоэффициенту(СтрокаТЧ, КоэффициентПересчета, ДокОст);
...
Процедура ПересчитатьСтрокуЗаказаПоКоэффициенту(СтрокаТЧ, КоэффициентПересчета, ДокОст)
		...
		Для Каждого СтрокаТЧ Из ДокОст.Товары Цикл
			...
			Если ВыборкаСтрокаЗаказа.НайтиСледующий(СтруктураОтбора) Тогда
				...
				ИначеЕсли ВыборкаСтрокаЗаказа.ЗаказаноКонечныйОстаток <= 0 Тогда
					...
					КонецЕсли; 
				ИначеЕсли ВыборкаСтрокаЗаказа.ЗаказаноКонечныйОстаток > СтрокаТЧ.Количество Тогда
					КоличествоСтарое = СтрокаТЧ.Количество;
					СтрокаТЧ.Количество = ВыборкаСтрокаЗаказа.ЗаказаноКонечныйОстаток;
					КоэффициентПересчета = СтрокаТЧ.Количество / КоличествоСтарое;
					ПересчитатьСтрокуЗаказаПоставщикуПоКОэффициенту(СтрокаТЧ, КоэффициентПересчета);
				ИначеЕсли ВыборкаСтрокаЗаказа.ЗаказаноКонечныйОстаток < СтрокаТЧ.Количество Тогда
...
Процедура ПересчитатьСтрокуЗаказаПоставщикуПоКОэффициенту(СтрокаТЧ, КоэффициентПересчета)
...
Процедура СформироватьДокументыВводаОстатковПрочиеРасходы()
	...
	Пока ВыборкаОрганизация.Следующий() Цикл  
		...
		Пока ВыборкаПодразделение.Следующий() Цикл
			...
			ДокОст.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПрочихРасходов;
	...
	Если ЗначениеЗаполнено(ОбъектРасчетов) И 
		...
		Если СтруктураРеквизитовШапки.Свойство("Дата") И СтруктураРеквизитовШапки.Свойство("Номер") Тогда
			...
			Если ЗначениеЗаполнено(ПДСсылка) Тогда
				...
				КонецЕсли;
				// Подменить ссылки в документах оплаты после даты свертки.
				ЗаполнитьПДВДокументахОплаты(ОбъектРасчетов, ПДСсылка);
				Возврат ПДСсылка;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьПДВДокументахОплаты(ОбъектРасчетов, ПервичныйДокумент)
	...
	|ГДЕ Шапка.Дата > &ДатаСвертки И Шапка.ПометкаУдаления = ЛОЖЬ
	|И Расшифровка.Заказ = &ОбъектРасчетов
	|ОБЪЕДИНИТЬ ВСЕ
	|ВЫБРАТЬ
	|	Шапка.Ссылка КАК Ссылка,
	|	ЛОЖЬ КАК ЭтоДЗ,
	...
	Пока ВыборкаДок.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			КонецЕсли;
			СтрокаРасчетов.Заказ = ПервичныйДокумент;
			Если ТипЗнч(Выборка.Ссылка) = Тип("ДокументСсылка.ПоступлениеБезналичныхДенежныхСредств")
				И СтрокаРасчетов.ОснованиеПлатежа = ОбъектРасчетов Тогда
				СтрокаРасчетов.ОснованиеПлатежа = ПервичныйДокумент;
			КонецЕсли;
		КонецЦикла;
		ДокОбъект.Записать(РежимЗаписиДокумента.Проведение);
	КонецЦикла;
...
Процедура ОбработатьРезультатЗапросаСобственныеТовары(Выборка, КоличествоСформированныхДокументов, ЕстьСФ = Ложь, ТаблицаОстатковПоГТД = Неопределено)
	Пока Выборка.СледующийПоЗначениюПоля("Организация") Цикл
		НалогообложениеПоУмолчанию = Перечисления.ТипыНалогообложенияНДС.ПоФактическомуИспользованию;
		ПараметрыУчетнойПолитики = ЗначениеНастроекПовтИсп.ПараметрыУчетнойПолитики(Выборка.Организация, КонецДня(ДатаСверткиИБ));
		ЭтоУСН = (ПараметрыУчетнойПолитики.ПрименяетсяУСН = Истина);
		Если ЭтоУСН Тогда
			НалогообложениеПоУмолчанию = Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС;
		КонецЕсли;
						...
						Пока Выборка.СледующийПоЗначениюПоля("СчетФактура") Цикл
							...
							Пока Выборка.Следующий() Цикл
								...
								Если НЕ Выборка.ИспользоватьСкладскиеПомещения ИЛИ ВыбранныеВариантыПомещений.Количество() = 0 Тогда
									...
									КонецЕсли;
									НоваяСтрока = ДокОст.Товары.Добавить();
									ЗаполнитьСуммыИКоличестваТабличнойЧасти(НоваяСтрока, Выборка, ЕстьСФ, ЭтоУСН);
								Иначе
...
Процедура ЗаполнитьСуммыИКоличестваТабличнойЧасти(НоваяСтрока, Выборка, ЕстьСФ, ЭтоУСН)
	...
	КонецЕсли;
	Если ЭтоУСН Тогда
		// Сумма регл должна быть за минусом НДС, иначе суммы в проводках Дт завышены на сумму НДС
		Если ВалютаУправленческогоУчета = ВалютаРегламентированногоУчета Тогда
			НоваяСтрока.СуммаРегл = НоваяСтрока.СуммаБезНДС; 
		Иначе
			НоваяСтрока.СуммаРегл = НоваяСтрока.СуммаРегл - НоваяСтрока.НДСРегл; 
		КонецЕсли;
	КонецЕсли;
...
Процедура ДополнитьКомментарииСвертки()
	...
	Комментарий_СформированСверткойБазы = НСтр("ru = '[%1% Сформирован обработкой свертки базы]';
												|en = '[%1% Generated by infobase rollup data processor]'");
	Комментарий_СкорректированСверткойБазы = НСтр("ru = '[%1% Скорректирован обработкой свертки базы]';
													|en = '[%1% Corrected by infobase rollup data processor]'");

...
Процедура ОтобразитьСостояниеОбмена()
	...
	Если НеЗаполненРегистрВидыПродукцииПоГруппамВЕТИС
		Или НеЗаполненРегистрДопустимыеЦелиПоГруппамВЕТИС Тогда
		Если ПерваяПроблема <> Неопределено Тогда
			ЕщеПроблемы = ЕщеПроблемы + 1;
		Иначе
			ПерваяПроблема = НСтр("ru='Не заполнены служебные регистры'");

...
&НаКлиенте
//Процедура ЗагрузитьДокументы(Команда)//смарт:Подкин 537 28.05.2021
Процедура ЗагрузитьДокументы(Команда) Экспорт	
	Для каждого Счет Из Объект.БанковскиеСчета Цикл
...
Процедура ЗавершитьЗагрузку()
	...
	//Смарт {[+](фрагмент ДОБАВЛЕН), Подкин Алексей Иванович 15.04.2021 13:41:52 Директ-банк загрузка, задача 537
	Если глСмартАвтозагрузкаВыписок Тогда//ЗначениеПС() Тогда
		
		ЗавершитьРаботуСистемы(Ложь);	
		
	КонецЕсли; 
	//Смарт }Подкин Алексей Иванович 15.04.2021 13:41:52	
...
//Смарт {[+](фрагмент ДОБАВЛЕН), Подкин Алексей Иванович 15.04.2021 14:38:25 Директ-банк загрузка, задача 537
&НаСервере
Функция ЗначениеПС()
	
	Попытка
	    ЗаписьЖурналаРегистрации("Окончание загрузки выписок...",УровеньЖурналаРегистрации.Информация,,,,);

		Возврат ПараметрыСеанса.СмартАвтозагрузкаВыписок;

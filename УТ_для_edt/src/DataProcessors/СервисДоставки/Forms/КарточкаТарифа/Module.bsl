...
Процедура ПередЗакрытиемЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		Отказ = Ложь;
		ВыполнитьКомандуЗаписатьТариф(Истина);
...
Процедура УстановитьТарифПоУмолчанию(Команда)
	...
	Иначе
		...
		ПараметрыОперации.Вставить("ТарифИдентификатор", Идентификатор);
	КонецЕсли;
	
	ПараметрыОперации.НаименованиеОперации = НаименованиеОперации;
	ПараметрыОперации.ВыводитьОкноОжидания = Истина;
	ПараметрыОперации.Вставить("ГрузоперевозчикИдентификатор", ГрузоперевозчикИдентификатор);
...
Процедура Записать(Команда)
	...
	ВыполнитьКомандуЗаписатьТариф();
	
КонецПроцедуры

&НаКлиенте
Процедура ВыполнитьКомандуЗаписатьТариф(ЗакрытьФорму = Ложь)
...
Процедура СформироватьЗаголовокФормы()
	...
	Заголовок = СтрШаблон(ТексЗаголовка, ТекстНаименования);
...
Процедура ПроверитьЗаполнениеКарточкиТарифа(Отказ)
	...
	Для Каждого ТекущийТерминал Из ТерминалыПоУмолчанию Цикл
		...
		Если ТекущийТерминал.Направление = 0 Тогда
			ТекстСообщения = СтрШаблон(ТекстОшибки, "Направление", НомерСтроки);
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,,,Отказ);
		КонецЕсли;
		
		Если ТекущийТерминал.Идентификатор = "" Тогда
			ТекстСообщения = СтрШаблон(ТекстОшибки, "Терминал", НомерСтроки);
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,,,Отказ);
		КонецЕсли;
		
		Если ТекущийТерминал.АдресЗначение = "" Тогда
			ТекстСообщения = СтрШаблон(ТекстОшибки, "Адрес", НомерСтроки);
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,,,Отказ);
		КонецЕсли;

...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	МассивРеквизитов.Добавить(Новый РеквизитФормы("CRM_Интерес", Новый ОписаниеТипов("ДокументСсылка.CRM_Интерес"),,"Интерес"));
	ЭтотОбъект.ИзменитьРеквизиты(МассивРеквизитов);
	Если Параметры.Свойство("ОткрытИзИнтереса") Тогда
		CRM_ИнициализироватьПомощникПродаж();
	Иначе	
...
Процедура ПриОткрытии(Отказ)
	...
	// +CRM_Модуль
	Если НЕ ЭтотОбъект["CRM_Интерес"].Пустая() Тогда
...
&НаКлиенте
Процедура ОбработкаВыбораПодборНаКлиентеПослеВопроса(Ответ, ДополнительныеПараметры) Экспорт // АПК:78 - исключить из проверки.
...
// ЗавершениеСозданияИзмененияДокументов
	...
	// +CRM_Модуль
	Для Каждого СозданныйДокумент Из МассивНайденныхДокументов Цикл
		Если ТипЗнч(СозданныйДокумент) = Тип("ДокументСсылка.ЗаказКлиента") ИЛИ ТипЗнч(СозданныйДокумент) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
			МенеджерЗаписи = РегистрыСведений.CRM_ЖурналДокументов.СоздатьМенеджерЗаписи();
			МенеджерЗаписи.ГлавнаяЗапись = Ложь;
			МенеджерЗаписи.CRM_Интерес = ЭтотОбъект["CRM_Интерес"];
			МенеджерЗаписи.Объект = СозданныйДокумент.Ссылка;
			МенеджерЗаписи.Клиент = Объект.Партнер;
			МенеджерЗаписи.Дата = ТекущаяДатаСеанса();
			МенеджерЗаписи.Описание = CRM_ОбщегоНазначенияСервер.ПолучитьТекстовоеОписанияОбъекта(СозданныйДокумент.Ссылка);
			МенеджерЗаписи.Автор = Пользователи.АвторизованныйПользователь();
			ТипИсточника = ТипЗнч(СозданныйДокумент.Ссылка);
			МенеджерЗаписи.ТипОбъекта = CRM_КлиентыСервер.ПолучитьСсылкуПВХОбъектыЖурналаПоТипу(ТипИсточника);
			МенеджерЗаписи.Номер = СозданныйДокумент.Номер;
			МенеджерЗаписи.Дата = СозданныйДокумент.Дата;
...
// +CRM_Модуль
&НаСервере
Процедура CRM_ИнициализироватьПомощникПродаж()
	...
	ЭтотОбъект["CRM_Интерес"] = Параметры.Интерес;
	
	ЗаполнитьЗначенияСвойств(Объект, Параметры.Интерес,, "Товары,СкидкиНаценки");
	Объект.Менеджер       = Параметры.Интерес.Ответственный;
	Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиенту;
	
	Для Каждого Строка Из Параметры.Интерес.Товары Цикл
		Стр = Объект.Товары.Добавить();
		ЗаполнитьЗначенияСвойств(Стр, Строка);
		Если Стр.Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Услуга или Стр.Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Работа Тогда
			Стр.Содержание = Стр.Номенклатура.НаименованиеПолное;
		КонецЕсли;	
		Стр.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Отгрузить;
	КонецЦикла;	
	
	Для Каждого Строка Из Параметры.Интерес.СкидкиНаценки Цикл
		Стр = Объект.СкидкиНаценки.Добавить();
		ЗаполнитьЗначенияСвойств(Стр, Строка);
	КонецЦикла;	
	...
	ОбновитьИнформациюДосьеПартнераРасчеты();
	ОбновитьИнформациюДосьеПартнераКонтакты();
	CRM_СоглашениеПриИзмененииСервер();
	
КонецПроцедуры

&НаСервере
Процедура CRM_СоглашениеПриИзмененииСервер()
...
&НаКлиенте
Процедура ЗаполнитьВариантОбеспеченияПослеВопроса(Ответ, ДополнительныеПараметры) Экспорт // АПК:78 - исключить из проверки.

...
Функция СформироватьТекстЗапросаТаблицаВозвратыТоваровОтКомиссионеров() Экспорт
		...
		|	И ТоварыПереданныеНаКомиссиюОстатки.НомерГТД.РНПТ = ЛОЖЬ
		|	И ТоварыПереданныеНаКомиссиюОстатки.АналитикаУчетаНоменклатуры.Номенклатура.КодТНВЭД.ПрослеживаемыйТовар = ИСТИНА
		|	И ТоварыПереданныеНаКомиссиюОстатки.Соглашение.КомиссионерВедетУчетПоРНПТ
...
// Формирует текст запроса таблицы к фомормлению передач товаров.
// 
// Возвращаемое значение:
//  Строка - текс запроса
//
Функция СформироватьТекстЗапросаТаблицаКОформлениюПередач() Экспорт
	...
	ТекстЗапроса = 
		...
		|	И ВозвратТоваровОтКлиентаТовары.Номенклатура.КодТНВЭД.ПрослеживаемыйТовар
...
Функция ТекстЗапросаДокументовКОформлениюПоТоварамНаКомиссии()
	...
	|ГДЕ
	|	ЕСТЬNULL(Аналитика.Номенклатура.КодТНВЭД.ПрослеживаемыйТовар, ЛОЖЬ)
	|	И НЕ ЕСТЬNULL(ТоварыОрганизаций.НомерГТД.РНПТ, ЛОЖЬ)

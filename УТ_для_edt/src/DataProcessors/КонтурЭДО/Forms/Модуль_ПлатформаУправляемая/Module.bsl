...
Процедура Авторизация_НачатьПолучениеСессииЧерезБраузер(ПриПолученииСессии) Экспорт
	...
	КлючСессии = КлючСессииПорталБраузер();
	
	КонтурПортал = МодульКонтурПорталСлужебныйНаКлиенте();
	КонтурПортал.НачатьПолучениеСессии(ПриПолученииСессииЧерезБраузер, КлючСессии);
...
//
// Параметры:
//  КлючСессии - Строка - Ядро.МодельСессияПользователя.Ключ
//
&НаКлиенте
Процедура Авторизация_СброситьПортальнуюСессию(Знач КлючСессии) Экспорт
	
	КонтурПортал = МодульКонтурПорталСлужебныйНаКлиенте();
	КонтурПортал.СброситьСессию(КлючСессии);
...
Процедура АвторизацияСлужебныйОбработчик_ПриПолученииСессииЧерезБраузер(ПортальнаяСессия, ПараметрыПолученияСессии) Экспорт
	...
	Сессия = АвторизацияСлужебный_СессияПорталБраузерВызовСервера(КлючСессииПорталБраузер(), ПортальнаяСессия);
...
&НаКлиенте
Функция МодульКонтурПорталСлужебныйНаКлиенте()
	
	ПараметрыПортальногоКлиента = Новый Структура;
	ПараметрыПортальногоКлиента.Вставить("ИмяПриложения", АутхПриложение.Имя);
	ПараметрыПортальногоКлиента.Вставить("КлючПриложения", АутхПриложение.Ключ);
	ПараметрыПортальногоКлиента.Вставить("РазрешенияПриложения", АутхПриложение.Разрешения);
	ПараметрыПортальногоКлиента.Вставить("ХостАвторизации", АутхПровайдер.Хост);
	
	ИмяФормыАвторизатора = "Обработка.КонтурДиадокПортальнаяАвторизацияАвторизатор.Форма.Клиент";
	
	Если ЭтоВнешняяОбработка Тогда
		ИмяФормыАвторизатора = "ВнешняяОбработка.КонтурДиадокПортальнаяАвторизацияАвторизатор.Форма.Клиент";
	КонецЕсли;
	
	// BSLLS:GetFormMethod-off
	Результат = ПолучитьФорму(ИмяФормыАвторизатора, ПараметрыПортальногоКлиента);
	// BSLLS:GetFormMethod-on
...
&НаКлиентеНаСервереБезКонтекста
Функция КлючСессииПорталБраузер()
...
Функция СеансСлужебный_Сессии(СохраненныеНастройки)
	...
	Для Каждого НастройкаСеанса Из СохраненныеНастройки Цикл
		...
		СхемаАутентификации = НастройкаСеанса.СхемаАутентификации;
		Если ДоступныеСхемы.Найти(СхемаАутентификации) = Неопределено Тогда
			Ошибка = СтрЗаменить("Схема аутентификации '%1' отключена по функциональным опциям", "%1", СхемаАутентификации);
			ВызватьИсключение Ошибка;
		КонецЕсли;
		...
		ИначеЕсли СхемаАутентификации = СхемаАутентификацииПорталБраузер() Тогда
			КонтурПортал = МодульКонтурПорталСлужебныйНаКлиенте();
			ПортальнаяСессия = КонтурПортал.ПолучитьСессию(КлючСессии);
			Сессия = АвторизацияСлужебный_СессияПорталБраузерВызовСервера(КлючСессии, ПортальнаяСессия);
		Иначе

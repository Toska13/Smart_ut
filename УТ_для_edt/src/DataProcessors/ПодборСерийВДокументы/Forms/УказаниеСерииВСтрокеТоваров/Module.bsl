...
Процедура ОстаткиСерийВыборКлиент()
	...
	Если ТипЗНЧ(Этаформа.смартРегистратор)=Тип("ДокументСсылка.ЗаказКлиента") И ЗначениеЗаполнено(Этаформа.смартРегистратор) Тогда 
		ВозвращенныйВариант = смартПроверитьСерию(Новый Структура("Значение",ТекущиеДанные.Серия), Этаформа.смартНоменклатура,Этаформа.смартХарактеристика);
    КонецЕсли;   
	
	Если ВозвращенныйВариант=2 ТОгда
		ТекстПредупреждения = НСтр("ru = 'Оборудование не исправно, обратитесь в тех. отдел.'");	
		ПоказатьПредупреждение(Неопределено, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура ПодобратьСериюЗавершитьВвод()
    ...
    //++ смарт:Подкин #1143 17.08.2022 13:58:25 
    ВозвращенныйВариант = Ложь; 
	Если ТипЗНЧ(Этаформа.смартРегистратор)=Тип("ДокументСсылка.ЗаказКлиента") И ЗначениеЗаполнено(Этаформа.смартРегистратор) Тогда 
	//++ #1250 смарт: Фролов 21.11.2022 //Проверенное оборудование. "Запрет на отгрузку без проверки оборудования  #1143 "  
        //ВыводитьВопрос = смартПроверитьСерию(ВозвращаемоеЗначение, Этаформа.смартНоменклатура);  
		ВозвращенныйВариант = смартПроверитьСерию(ВозвращаемоеЗначение, Этаформа.смартНоменклатура,Этаформа.смартХарактеристика);
    КонецЕсли;   
    
    Если ВозвращенныйВариант=3 Тогда
        Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияПроверитьСерию", ЭтотОбъект, ВозвращаемоеЗначение);
        ПоказатьВопрос(Оповещение, "Данное оборудование не проверено, 
        |отгрузку осуществляете под свою ответственность. Установить выбранную серию?", РежимДиалогаВопрос.ДаНет, 0,);    
	КонецЕсли; 
	
	Если ВозвращенныйВариант=2 ТОгда
		Сообщить("Оборудование не исправно, обратитесь в тех. отдел."); 
		ВозвращаемоеЗначение.Значение=ПредопределенноеЗначение("Справочник.СерииНоменклатуры.ПустаяСсылка");
        ОповеститьОВыборе(ВозвращаемоеЗначение);
        Закрыть();
	КонецЕсли;	
   
    Если ВозвращенныйВариант=1 Тогда
        ОповеститьОВыборе(ВозвращаемоеЗначение);
        Закрыть();
    КонецЕсли;    
    Если ВозвращенныйВариант=Ложь ТОгда
	    ОповеститьОВыборе(ВозвращаемоеЗначение);    
		
		//++ #1686 смарт: Мосолов 18.03.2024 // Создать механизм ручного резервирования товаров (временного вывода из оборота)
		Если ПараметрыУказанияСерий.Свойство("ЭтоВыводИзОборота") Тогда
			Оповестить("ВыборСерииВВыводИзОборота", ВозвращаемоеЗначение); 
		КонецЕсли;
...
//++ смарт:Подкин #1143 17.08.2022 12:04:34
//&НаСервере
//Функция смартПроверитьСерию(ВозвращаемоеЗначение,Номен) 
//    ВходитВСегмент = Ложь;
//    Если ЗначениеЗаполнено(Номен) И ЗначениеЗаполнено(ВозвращаемоеЗначение.Значение) И ЗначениеЗаполнено(Справочники.смартПредопределенныеЗначения.СегментПроверкаОборудования.Значение) Тогда
//        ВходитВСегмент = СегментыСервер.ВходитВСегмент(Номен,Справочники.смартПредопределенныеЗначения.СегментПроверкаОборудования.Значение);
//    КонецЕсли;
//    
//    Если ВходитВСегмент Тогда
//        Возврат НЕ Серия.смартПроверен;
//    КонецЕсли;
//    
//    Возврат Ложь;  
//    
...
//++ #1250 смарт: Фролов 21.11.2022 //Внес параметр Характеристика и передаем его дальше 
&НаСервере
Функция смартПроверитьСерию(ВозвращаемоеЗначение,Номен,Характеристика) 
    ВходитВСегмент = Ложь;
    Если ЗначениеЗаполнено(Номен) И ЗначениеЗаполнено(ВозвращаемоеЗначение.Значение) И ЗначениеЗаполнено(Справочники.смартПредопределенныеЗначения.СегментПроверкаОборудования.Значение) Тогда
        ВходитВСегмент = СегментыСервер.ВходитВСегмент(Номен,Справочники.смартПредопределенныеЗначения.СегментПроверкаОборудования.Значение,Характеристика,Ложь);
    КонецЕсли;
    
	Если ВходитВСегмент Тогда 
		Если ВозвращаемоеЗначение.Значение.смартСтатусПроверкиОборудования=Перечисления.смартСтатусыПроверкиОборудованияПоСерии.ПровереноИсправно Тогда
			ВозвращаемыйВариант=1;
		ИначеЕсли ВозвращаемоеЗначение.Значение.смартСтатусПроверкиОборудования=Перечисления.смартСтатусыПроверкиОборудованияПоСерии.ПровереноНеИсправно Тогда
			ВозвращаемыйВариант=2;                                                                                              
		Иначе
			ВозвращаемыйВариант=3;	
		КонецЕсли;	
        Возврат ВозвращаемыйВариант;
    КонецЕсли;
...
//++ смарт:Подкин #1143 17.08.2022 12:21:56
&НаКлиенте
Процедура ПослеЗакрытияПроверитьСерию(Результат, Параметры) Экспорт 
    
    Если Результат = КодВозвратаДиалога.Нет Тогда 
        Параметры.Значение=ПредопределенноеЗначение("Справочник.СерииНоменклатуры.ПустаяСсылка");
...
КонецПроцедуры //-- смарт:Подкин #1143 17.08.2022 12:21:56
...
Процедура ВыполнитьЗапросЗаполненияТаблицыОстатков()
	...
	//++ #1250 смарт: Фролов 17.11.2022 //Описание  
	//Смарт {[+](фрагмент ДОБАВЛЕН), Подкин Алексей Иванович 27.01.2021 15:32:00 задача 444
	//|   ,ДанныеРегистров.Серия.смартПроверен КАК смартПроверен  
	//Смарт }Подкин Алексей Иванович 27.01.2021 15:32:00 
	|   ,ДанныеРегистров.Серия.смартСтатусПроверкиОборудования КАК смартПроверено

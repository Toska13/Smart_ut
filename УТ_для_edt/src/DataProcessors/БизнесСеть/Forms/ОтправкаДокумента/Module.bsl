...
Процедура ЗагрузитьПодготовленныеДанныеЭД(СсылкиНаДокументы, Отказ)
	...
	ТаблицаЭД = БизнесСеть.ТаблицаЭлектронныхДокументовСформированныхЧерезБизнесСеть(СсылкиНаДокументы, ОрганизацияЗарегистрирована, УникальныйИдентификатор, Отказ);
...
Процедура ОтправитьДокументыВСервис()
	...
	ОперацияОтправкиДокументов = ОтправитьДанныхДокументовЧерезБизнесСетьНаСервере();
	
	ОповещениеПриЗавершении = Новый ОписаниеОповещения("ОтправитьДанныхДокументовЧерезБизнесСетьЗавершение", ЭтотОбъект);
...
&НаСервере
Функция ОтправитьДанныхДокументовЧерезБизнесСетьНаСервере()
	...
	Возврат БизнесСетьВызовСервера.ОтправлениеДанныхДокументовЧерезБизнесСетьВФоне(Новый УникальныйИдентификатор,
																					Неопределено,
...
&НаКлиенте
Процедура ОтправитьДанныхДокументовЧерезБизнесСетьЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ОбновитьФайлПредставления()
	...
	Для Каждого СтрокаДанных Из ТаблицаДанных Цикл
		...
		ПараметрыОбмена.Вставить("ПолноеИмяДопФайла", СтрокаДанных.ПолноеИмяДопФайла);
		ПараметрыОбмена.Вставить("ПолноеИмяФайла"   , СтрокаДанных.ПолноеИмяФайла);
		ПараметрыОбмена.Вставить("ВидЭД"            , СтрокаДанных.ВидЭД);
		ПараметрыОбмена.Вставить("СтруктураЭД"      , ПараметрыЭД);
		
		СообщениеОбОшибке  = "";
		РаспакованныеФайлыЭД = ОбменСКонтрагентамиВнутренний.РаспаковкаФайловАрхиваЭД(ПараметрыЭД, УникальныйИдентификатор, СообщениеОбОшибке, Отказ);
		
		Если РаспакованныеФайлыЭД = Неопределено Или Отказ Тогда
			Продолжить;
		КонецЕсли;
		
		ИмяФайлаКартинок   = РаспакованныеФайлыЭД.ИмяФайлаКартинок;
		ФайлДопДанных      = РаспакованныеФайлыЭД.ФайлДопДанных;
		ИмяФайла           = РаспакованныеФайлыЭД.ИмяФайла;
		...
		ПараметрыОбмена.Вставить("ДанныеQRКода", ДанныеQRКода);
		...
		ИмяФайлаПДФ = ОбменСКонтрагентамиВнутренний.ДвоичныеДанныеФайлаПредставленияДляБизнесСети(ПараметрыОбмена,
																								НаименованиеФайла,
																								СтрокаДанных.ВладелецЭД,
																								Организация);

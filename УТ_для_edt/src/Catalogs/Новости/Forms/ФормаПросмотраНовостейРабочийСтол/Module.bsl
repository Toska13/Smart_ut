...
Процедура НачатьПолучениеНовостейВФонеНаКлиенте()
	...
	// 2. Подключить обработчик проверки.
	Если ФоновоеЗадание.Статус = "Выполняется" Тогда // "Выполнено", "Ошибка", "Отменено"
		...
		ЭтотОбъект.ПодключитьОбработчикОжидания("Подключаемый_ПроверитьСостояниеПолученияНовостейВФонеНаКлиенте", 5, Истина);
	КонецЕсли;
...
Функция НачатьПолучениеНовостейВФонеНаСервере()
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(ЭтотОбъект.УникальныйИдентификатор);
		...
		ПараметрыВыполнения.АдресРезультата             = Неопределено;
		//ПараметрыВыполнения.ИдентификаторФормы          = ЭтотОбъект.УникальныйИдентификатор; // Уже установлен.
	...
	ПараметрыПроцедуры = Новый Структура;
		ПараметрыПроцедуры.Вставить("ТаблицаНовостей"           , ЭтотОбъект.Новости.Выгрузить());
		ПараметрыПроцедуры.Вставить("Пользователь"              , ЭтотОбъект.ПараметрыСеанса_ТекущийПользователь);
		ПараметрыПроцедуры.Вставить("ПараметрыПолученияНовостей", ПараметрыПолученияНовостей);
		ПараметрыПроцедуры.Вставить("ИнтерактивныеДействия"     , Новый Массив);
	...
	// 3. Обработчик проверки, что фоновое задание завершено, должен быть подключен на клиенте: или явно (НачатьПолучениеНовостейВФонеНаКлиенте) или ПриОткрытии.
...
Процедура ЗавершитьПолучениеНовостейВФонеНаКлиенте()
	...
	// ЗавершитьПолучениеНовостейВФонеНаСервере уже установило СостояниеФоновогоЗаданияПолученияНовостей в Неопределено.
...
&НаСервере
Процедура ОтменитьПолучениеНовостейВФонеНаСервере(ФоновоеЗадание)
	...
	ДлительныеОперации.ОтменитьВыполнениеЗадания(ФоновоеЗадание.ИдентификаторЗадания);
...
&НаКлиенте
Процедура Подключаемый_ПроверитьСостояниеПолученияНовостейВФонеНаКлиенте()
	...
	ПроверитьСостояниеПолученияНовостейВФонеНаСервере(ФоновоеЗадание);

	Если ФоновоеЗадание.Статус = "Выполняется" Тогда // "Выполнено", "Ошибка", "Отменено"
		// Разрешить подключать обработчик проверки 10 раз и не более чем на 90 секунд.
		Если ((ТекущаяУниверсальнаяДатаВМиллисекундах() - ФоновоеЗадание.ТекущаяУниверсальнаяДатаВМиллисекундах) < 90000)
				И (ФоновоеЗадание.КоличествоПроверокЗавершенияОперации < 10) Тогда
			ЭтотОбъект.СостояниеФоновогоЗаданияПолученияНовостей.КоличествоПроверокЗавершенияОперации =
				ЭтотОбъект.СостояниеФоновогоЗаданияПолученияНовостей.КоличествоПроверокЗавершенияОперации + 1;
			ЭтотОбъект.ПодключитьОбработчикОжидания("Подключаемый_ПроверитьСостояниеПолученияНовостейВФонеНаКлиенте", 5, Истина);
		Иначе
			// Принудительно завершить.
			ОтменитьПолучениеНовостейВФонеНаСервере(ФоновоеЗадание);
		КонецЕсли;
	Иначе
		ЗавершитьПолучениеНовостейВФонеНаСервере(ФоновоеЗадание);
	КонецЕсли;
...
&НаСервере
Процедура ПроверитьСостояниеПолученияНовостейВФонеНаСервере(ФоновоеЗадание) Экспорт
	...
	Результат = ДлительныеОперации.ОперацияВыполнена(ФоновоеЗадание.ИдентификаторЗадания);

	ЗаполнитьЗначенияСвойств(ФоновоеЗадание, Результат, "Статус, КраткоеПредставлениеОшибки, ПодробноеПредставлениеОшибки");

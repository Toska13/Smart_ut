...
Функция ПолучитьМассивДляОтбора()
	...
	Если Объект.ВидЭтапа = ПредопределенноеЗначение("Перечисление.CRM_ВидыЭтаповВоронкиПродаж.ПоДокументам") Тогда
		...
		Для Каждого СтрокаСостава Из Объект.Состав Цикл
			Если ЕстьДокументCRM_СчетНаОплатуПокупателю И СтрокаСостава.ТочкаМаршрута.ТипЗначения = Новый ОписаниеТипов(ИмяДокументаCRM_СчетНаОплатуПокупателю) Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ВидВоронкиПриИзмененииСервер()
	Если Объект.ВидЭтапа = Перечисления.CRM_ВидыЭтаповВоронкиПродаж.ПоСостояниямИнтересов Тогда
		...
		Элементы.СоставПредставлениеТочкиМаршрута.Заголовок = НСтр("ru='Состояние';en='Status'");
	...
	ИначеЕсли Объект.ВидЭтапа = Перечисления.CRM_ВидыЭтаповВоронкиПродаж.ПоЭтапам Тогда
		Элементы.СоставКартаМаршрута.Заголовок = НСтр("ru='Карта маршрута';en='Flow Chart'");

...
"создание новых пользователей.';en='Creation of new users"
" is not supported in the demo mode.'");
	...
	Если CRM_ОбщегоНазначенияПовтИсп.ЭтоCRM() Тогда
		CRM_ОбновитьГруппуУровниДоступа();
		CRM_УстановитьУсловноеОформлениеCRM_УровниДоступа();
	КонецЕсли;
	Если НЕ ПолучитьФункциональнуюОпцию("CRM_ИспользуетсяОбменСiCRM") Тогда
		Элементы.CRM_РазрешеноИзменятьКоординаты.Видимость = Ложь;
	КонецЕсли;	
	// -CRM
...
// +CRM
&НаКлиенте
Процедура CRM_СделатьУровеньДоступаОсновным(Команда)
	
	ТекСтрока = Элементы.CRM_УровниДоступа.ТекущиеДанные;
	
	Если ТекСтрока <> Неопределено Тогда
		Объект.CRM_УровеньДоступа = ТекСтрока.УровеньДоступа;
		CRM_УстановитьУсловноеОформлениеCRM_УровниДоступа();
	КонецЕсли;
...
Процедура ОбщаяНастройкаФормы(ТекущийОбъект, ПриСозданииНаСервере = Ложь, ПараметрыЗаписи = Неопределено)
	...
	// +CRM
	Если CRM_ОбщегоНазначенияПовтИсп.ЭтоCRM() Тогда
		Элементы.CRM_УровниДоступа.ТолькоПросмотр	 = Элементы.Наименование.ТолькоПросмотр;
	КонецЕсли;
...
"%1.'"),
	...
	ИначеЕсли Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		Рекомендация = Символы.ПС
			+ НСтр("ru='Нажмите ""Записать"", чтобы устранить проблему и не выводить это предупреждение.';en='Click ""Save"" to resolve this issue and not to show this message again.'");
	Иначе
		Рекомендация = "";
	КонецЕсли;
...
Процедура CRM_ОбновитьГруппуУровниДоступа()

	Если Объект.CRM_УровниДоступа.Количество() = 0 Тогда
		Элементы.CRM_ГруппаУровниДоступа.Заголовок = НСтр("ru = 'Уровни доступа'; en = 'Access Levels'");
	Иначе	
		Элементы.CRM_ГруппаУровниДоступа.Заголовок = НСтр("ru = 'Уровни доступа'; en = 'Access Levels'") + "("+Объект.CRM_УровниДоступа.Количество()+")";
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура CRM_УстановитьУсловноеОформлениеCRM_УровниДоступа()
	
	ПолеКД = Новый ПолеКомпоновкиДанных(Элементы.CRM_УровниДоступа.Имя);
	...
	ПолеКДОтбора = Новый ПолеКомпоновкиДанных("Объект.CRM_УровниДоступа.УровеньДоступа");
	...
	Если ОтборЭлемента = Неопределено Тогда
		...
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	КонецЕсли;
	ОтборЭлемента.ПравоеЗначение = Объект.CRM_УровеньДоступа;

...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Объект.CRM_ЭтоПроект Тогда
		Заголовок = НСтр("ru='Новый проект...';en='New project...'");
		Элементы.Наименование.ПодсказкаВвода = НСтр("ru='Введите наименование проекта';en='Enter name of project'");
		
	Иначе
		Заголовок = НСтр("ru='Новый процесс...';en='New process...'");
		Элементы.Наименование.ПодсказкаВвода = НСтр("ru='Введите наименование процесса';en='Enter Process Name'");
	КонецЕсли;
	
	ТипПроекта = ?(Объект.CRM_ЭтоПроект, НСтр("ru='Проект';en='Project'"), НСтр("ru='Процесс'"));
...
"Формирование отчета возможно только после записи данных.';en='Data is not recorded yet."
"Report generation is possible only after data record.'"));
		//
		Возврат;
	КонецЕсли;
...
Функция ПолучитьПрикрепленныеФайлы() 
	...
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ПроектыПрисоединенныеФайлы.Ссылка КАК Ссылка,
	               |	ПроектыПрисоединенныеФайлы.Наименование КАК Наименование
	               |ИЗ
	               |	Справочник.ПроектыПрисоединенныеФайлы КАК ПроектыПрисоединенныеФайлы
	               |ГДЕ
	               |	ПроектыПрисоединенныеФайлы.ВладелецФайла = &Ссылка
	               |	И НЕ ПроектыПрисоединенныеФайлы.ЭтоГруппа";
...
Процедура ПлановаяДатаНачалаПриИзменении(Элемент)
	...
	ИначеЕсли Объект.ПлановаяДатаНачала > ПлановаяДатаНачалаПредыдущая Тогда
		...
		Иначе
			Объект.ПлановаяДатаНачала = ПлановаяДатаНачалаПредыдущая;
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='При обновлении дат этапов произошла ошибка:'") + Символы.ПС + ОписаниеОшибки);
		КонецЕсли;

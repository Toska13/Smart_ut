...
Процедура ЗаполнитьДеревоРеквизитовОбъекта(ТипОбъекта)
	...
	МетаОбъект = Справочники.ИдентификаторыОбъектовМетаданных.ОбъектМетаданныхПоИдентификатору(ТипОбъекта, Ложь);
...
Процедура НастроитьКомпоновщикУсловияИЗаполнитьДеревоРеквизитовОбъекта(Знач ТипОбъекта, Настройки = Неопределено)
	...
	СхемаКомпоновкиДанных = смартШаблоныПечатныхФормСервер.ПолучитьСхемуСКДПоОбъекту(ТипОбъекта);	
...
Процедура НастроитьКомпоновщикКомбинацииУсловий(Настройки = Неопределено)
	...
	СхемаКомпоновкиДанных = Справочники.смартАлгоритмыПроверки.ПолучитьМакет("Условия");
...
Функция ПроверитьНаличиеУсловияВДереве(ИщемПустоеЗначение)
	...
	Результат = Ложь;
	Если Объект.СпособЗаданияУсловия = Перечисления.смартСпособыЗаданияУсловия.КомбинацияИзДругихУсловий Тогда
	...
	Для Каждого ЭлементОтбора Из Элементы Цикл
		...
		Иначе			
			...
			ИначеЕсли  ЭлементОтбора.ПравоеЗначение <> Неопределено
				И ЭлементОтбора.ПравоеЗначение.СпособЗаданияУсловия = Перечисления.смартСпособыЗаданияУсловия.КомбинацияИзДругихУсловий
...
Функция ПроверитьНаличиеПустыхСтрокОтбораВДереве()
	...
	Результат = Ложь;
	Если Объект.СпособЗаданияУсловия = Перечисления.смартСпособыЗаданияУсловия.ВРежимеКонструктора Тогда
	...
	Для Каждого ЭлементОтбора Из Элементы Цикл
		...
		ИначеЕсли ЭлементОтбора.ПравоеЗначение <> Неопределено Тогда
			...
			Если ЭлементОтбора.ПравоеЗначение.Наименование = ИмяУсловия Тогда
				ЭлементОтбора.ПравоеЗначение = Справочники.смартАлгоритмыПроверки.ПустаяСсылка();
			ИначеЕсли СпособЗаданияУсловия = Перечисления.смартСпособыЗаданияУсловия.КомбинацияИзДругихУсловий Тогда
...
Функция ПроверитьУникальностьУсловия()
	...
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	               |	Условия.Ссылка          
				   |ИЗ
	               |	Справочник.смартАлгоритмыПроверки КАК Условия
	               |ГДЕ
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая()
		...
		КонецЕсли;
		Объект.СпособЗаданияУсловия = Перечисления.смартСпособыЗаданияУсловия.ВРежимеКонструктора;
	...
	Если Объект.СпособЗаданияУсловия = Перечисления.смартСпособыЗаданияУсловия.НаВстроенномЯзыке Тогда
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.СпособЗаданияУсловия = Перечисления.смартСпособыЗаданияУсловия.КомбинацияИзДругихУсловий
	...
	ИначеЕсли Объект.СпособЗаданияУсловия = Перечисления.смартСпособыЗаданияУсловия.ВРежимеКонструктора
	...
	ИначеЕсли Объект.СпособЗаданияУсловия = Перечисления.смартСпособыЗаданияУсловия.НаВстроенномЯзыке
	...
	Если Объект.СпособЗаданияУсловия = Перечисления.смартСпособыЗаданияУсловия.ВРежимеКонструктора
	...
	ИначеЕсли Объект.СпособЗаданияУсловия = Перечисления.смартСпособыЗаданияУсловия.ВРежимеКонструктора Тогда
		ТекущийОбъект.ПредставлениеОтбора = Строка(Компоновщик.Настройки.Отбор);
	ИначеЕсли Объект.СпособЗаданияУсловия = Перечисления.смартСпособыЗаданияУсловия.НаВстроенномЯзыке Тогда
...
Процедура СпособЗаданияУсловияПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.СпособЗаданияУсловия) Тогда
		Если Объект.СпособЗаданияУсловия = ПредопределенноеЗначение("Перечисление.смартСпособыЗаданияУсловия.ВРежимеКонструктора") Тогда
			Элементы.Страницы.ТекущаяСтраница = Элементы["СтраницаВРежимеКонструктора"];
		ИначеЕсли Объект.СпособЗаданияУсловия = ПредопределенноеЗначение("Перечисление.смартСпособыЗаданияУсловия.НаВстроенномЯзыке") Тогда
			Элементы.Страницы.ТекущаяСтраница = Элементы["СтраницаНаВстроенномЯзыке"];
		ИначеЕсли Объект.СпособЗаданияУсловия = ПредопределенноеЗначение("Перечисление.смартСпособыЗаданияУсловия.КомбинацияИзДругихУсловий") Тогда
		...
		Элементы.ГруппаОтветственныйИКомментарий.Видимость = 
			(Объект.СпособЗаданияУсловия = ПредопределенноеЗначение("Перечисление.смартСпособыЗаданияУсловия.НаВстроенномЯзыке"));
	КонецЕсли;
...
Процедура ПроверитьДоступКТипуДокументаИЗагрузитьНастройки(ВыбранноеЗначение, СтандартнаяОбработка)
	...
	МетаОбъект = Справочники.ИдентификаторыОбъектовМетаданных.ОбъектМетаданныхПоИдентификатору(ВыбранноеЗначение, Ложь);
...
Процедура ПроверитьВыражениеСервер()
	...
	Попытка
		...
		УстановитьБезопасныйРежим(Истина);
		Выполнить(Справочники.смартАлгоритмыПроверки.ПолноеВыражениеУсловия(Объект));
...
Процедура ПредметНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ТипВыбранный = смартШаблоныПечатныхФормСервер.ТипПоМетаданнымОбъекта(Объект.ТипОбъекта);
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	ТолькоПросмотр = Не ПравоДоступа("Изменение", Метаданные.Справочники.смартАлгоритмыПроверки)
		Или Объект.Предопределенный;
	
	ПоказатьНадписьПометкиУдаления(
		ЭтотОбъект,
...
Процедура ПредставлениеПараметровНажатие(Элемент, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.смартАлгоритмыПроверки.Форма.РедактированиеПараметров",
		ПараметрыФормы,
		ЭтотОбъект,
...
Процедура ПоказатьНадписьПометкиУдаления(Форма, ПометкаУдаления, ИмяГруппы = "") Экспорт
	...
	Если ЭлементыФормы.Найти(ИмяГруппы) = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ЭлементыФормы[ИмяГруппы].Видимость = ПометкаУдаления;

...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ЕстьДублиТегов.ЕстьДубли Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ru='Создаваемый тег уже существует у подразделения';en='Created tag already exists for department'") + " <" + ЕстьДублиТегов.Подразделение + ">.";
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли; 
КонецПроцедуры
...
Функция  ЕстьДублиТегов(Наименование, Подразделение)
	...
	СписокВышестоящихПодразделений = CRM_МетодыМодулейМенеджеровСправочников.ПолучитьВсеПодразделенияРодители(Подразделение, Истина);
	...
	Запрос.УстановитьПараметр("ПодразделенияРодители", СписокВышестоящихПодразделений);

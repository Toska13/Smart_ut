...
Функция ПроверитьНаНаличиеИзменений()
	...
	Для Каждого Реквизит Из РеквизитыОбъекта Цикл
		...
		Если ИмяРеквизита = "ГрафическаяСхема"
			ИЛИ ИмяРеквизита = "Редактируется"  Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	CRM_ЛицензированиеЭкспортныеМетоды.ПолучитьЗащищеннуюОбработку().ПриСозданииНаСервере(ЭтотОбъект, ДоступностьЭлементов, СтандартнаяОбработка);
...
Процедура УстановитьСвойстваЭлементовПоТипу(ПриСоздании = Ложь)
	...
	Элементы.СостояниеДосрочногоЗавершения.Видимость = (Объект.РазрешеноДосрочноеЗавершение И Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта);
	Если Объект.РазрешеноДосрочноеЗавершение И ЗначениеЗаполнено(Объект.ВидОбъектаСтрокой) И ЗначениеЗаполнено(Объект.РеквизитОбъектаСтрокой) Тогда
		Если Метаданные.Документы.Найти(Объект.ВидОбъектаСтрокой) <> Неопределено Тогда
			МетаданныеОбъекта = Метаданные.Документы[Объект.ВидОбъектаСтрокой];
		ИначеЕсли Метаданные.Справочники.Найти(Объект.ВидОбъектаСтрокой) <> Неопределено Тогда
			МетаданныеОбъекта = Метаданные.Справочники[Объект.ВидОбъектаСтрокой];
		КонецЕсли;
		Если СтрНайти(Объект.РеквизитОбъектаСтрокой,"%Свойство%") > 0 Тогда
			СсылкаНаСвойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.ПолучитьСсылку(Новый УникальныйИдентификатор(СтрЗаменить(Объект.РеквизитОбъектаСтрокой,"%Свойство%","")));
			Элементы.СостояниеДосрочногоЗавершения.ОграничениеТипа = Новый ОписаниеТипов(СсылкаНаСвойство.ТипЗначения.Типы());
			
			ПараметрыВыбораСостояния = Новый Массив;
			ПараметрыВыбораСостояния.Добавить(Новый ПараметрВыбора("Отбор.Владелец",СсылкаНаСвойство));
			Элементы.СостояниеДосрочногоЗавершения.ПараметрыВыбора = Новый ФиксированныйМассив(ПараметрыВыбораСостояния);
			Элементы.СостояниеДосрочногоЗавершения.Заголовок = СсылкаНаСвойство.Заголовок + " ("+ МетаданныеОбъекта.Синоним +")";
		Иначе
			ПараметрыВыбораСостояния = Новый Массив;
			Если Объект.ВидОбъектаСтрокой = "CRM_Интерес" Тогда
				ПараметрыВыбораСостояния.Добавить(Новый ПараметрВыбора("Отбор.Родитель", Объект.СценарийИнтереса));
			КонецЕсли;
			Элементы.СостояниеДосрочногоЗавершения.ПараметрыВыбора = Новый ФиксированныйМассив(ПараметрыВыбораСостояния);
			Элементы.СостояниеДосрочногоЗавершения.ОграничениеТипа = Новый ОписаниеТипов(МетаданныеОбъекта.Реквизиты[Объект.РеквизитОбъектаСтрокой].Тип.Типы());
			Элементы.СостояниеДосрочногоЗавершения.Заголовок = МетаданныеОбъекта.Реквизиты[Объект.РеквизитОбъектаСтрокой].Синоним;
		КонецЕсли;
		Элементы.СостояниеДосрочногоЗавершения.Доступность = Истина;
	Иначе
		Объект.СостояниеДосрочногоЗавершения = Неопределено;
		Элементы.СостояниеДосрочногоЗавершения.Доступность = Ложь;
		Элементы.СостояниеДосрочногоЗавершения.Заголовок = "";
	КонецЕсли;
	...
	Если Объект.УказыватьПричинуНеудачногоЗавершения И Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта И Объект.ВидОбъектаСтрокой = "CRM_Интерес" Тогда
		Объект.УказыватьПричинуНеудачногоЗавершения = Ложь;
		Модифицированность = Истина;
	КонецЕсли;
	Элементы.УказыватьПричинуНеудачногоЗавершения.Доступность = НЕ (Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта И Объект.ВидОбъектаСтрокой = "CRM_Интерес");
	
	Если Объект.РедактироватьДатуВыполненияЗадач И Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта И Объект.ВидОбъектаСтрокой	= "CRM_Интерес" Тогда
		Объект.РедактироватьДатуВыполненияЗадач = Ложь;
		Модифицированность = Истина;
	КонецЕсли;
	Элементы.РедактироватьДатуВыполненияЗадач.Доступность = НЕ (Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта И Объект.ВидОбъектаСтрокой = "CRM_Интерес");
	
	Если Объект.РедактироватьИсполнителейЭтапов И Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта Тогда
		Объект.РедактироватьИсполнителейЭтапов = Ложь;
		Модифицированность = Истина;
	КонецЕсли;
	Элементы.РедактироватьИсполнителейЭтапов.Доступность = НЕ (Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта);
	
	// Если Объект.ДинамическиРассчитыватьСрокиВыполненияЭтапов И Объект.ТипПроцесса <>
	// Перечисления.bpmТипыПроцессов.ПоУмолчанию Тогда
	//	Объект.ДинамическиРассчитыватьСрокиВыполненияЭтапов = Ложь;
	//	Модифицированность = Истина;
	// КонецЕсли;
	// Элементы.ДинамическиРасчитыватьСрокиВыполненияЭтапов.Доступность = (Объект.ТипПроцесса =
	// Перечисления.bpmТипыПроцессов.ПоУмолчанию ИЛИ Объект.РедактироватьИсполнителейЭтапов);
...
Процедура НастроитьОтборы(Команда)
	...
	ЗаголовокФормыНастройкиСхемыКомпоновкиДанных = НСтр("ru='Настройки отборов';en='Filter settings'");
		...
		Если Выборка.ТипЗначения <> NULL
		   И УправлениеСвойствамиСлужебный.ТипЗначенияСодержитЗначенияСвойств(Выборка.ТипЗначения) Тогда
			...
			Если ПервыеЗначения.Количество() = 0 Тогда
				Если РезультатыЗапроса[1].Пустой() Тогда
					ПредставлениеЗначений = НСтр("ru='Значения еще не введены';en='No values entered'");
				Иначе
...
&НаСервере
Функция ЗагрузитьНастройкиЭтапов()
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |ГДЕ
	                      |	CRM_НастройкиЭтапов.Объект = &Объект");
	Запрос.УстановитьПараметр("Объект", Объект.Ссылка);
	НастройкиМаршрута.Загрузить(Запрос.Выполнить().Выгрузить());
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	Если ЗавершениеРаботы Тогда
		ТекстПредупреждения = "";
		Если Объект.Редактируется Тогда
	        ТекстПредупреждения = ТекстПредупреждения + НСтр("ru='Карта маршрута находится в режиме редактирования. ';en='Route map is in edit mode. '");
		КонецЕсли;
		// Если ПроверитьНаНаличиеИзменений() Тогда 
		//    ТекстПредупреждения = ТекстПредупреждения + НСтр("ru='Форма была изменена. ';en='Form was changed. '");
		// КонецЕсли;
		Если НЕ ПустаяСтрока(ТекстПредупреждения) Тогда
	        Отказ = Истина;
			ТекстПредупреждения = ТекстПредупреждения + НСтр("ru='При закрытии формы все данные будут утеряны. Продолжить?';en='When you close form, all data will be lost. Continue?'");
		КонецЕсли;	
		Возврат;
	КонецЕсли;
...
&НаСервере
Функция СохранитьСхемуИНастройки()
	
	ИмяВременногоФайла = ПолучитьИмяВременногоФайла("xml");
	
	мЗаписьXML = Новый ЗаписьXML;
	мЗаписьXML.ОткрытьФайл(ИмяВременногоФайла, "UTF-8");
...
Процедура СохранитьВФайл(Команда)
	...
	Если Не РасширениеПодключено Тогда
		ТекстПредупреждения = НСтр("ru='В Веб-клиенте без установленного расширения работы с файлами данная функция не поддерживается.';en='In Web client without installed extension to work with files, this function is not supported.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
	
	ФормироватьНаСервере = Истина;
	Если Объект.Редактируется И Не Объект.Ссылка.Пустая() Тогда
		ФормироватьНаСервере = Ложь;
	КонецЕсли;
	
	ДиалогВыбора = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Сохранение);
	ДиалогВыбора.МножественныйВыбор = Ложь;
	ДиалогВыбора.ПолноеИмяФайла = ?(ФормироватьНаСервере, "Схема.xml", "Схема.grs");
	ДиалогВыбора.Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Все файлы (*.%1)|*.%1';en='All files (*.%1)|*.%1'"), ?(ФормироватьНаСервере, "xml", "grs"));
	
	ДополнительныеПараметры = Новый Структура("ФормироватьНаСервере", ФормироватьНаСервере);
	ОписаниеОповещения = Новый ОписаниеОповещения("СохранитьВФайлЗавершение", ЭтотОбъект, ДополнительныеПараметры);
	
	Если ФормироватьНаСервере Тогда
		АдресВХранилище = СохранитьСхемуИНастройки();
		ПолучаемыеФайлы = Новый Массив;
		ПолучаемыеФайлы.Добавить(Новый ОписаниеПередаваемогоФайла("Схема.xml", АдресВХранилище));
		НачатьПолучениеФайлов(ОписаниеОповещения, ПолучаемыеФайлы, ДиалогВыбора, Истина);
	Иначе
...
Процедура СохранитьВФайлЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ФормироватьНаСервере Тогда
		Возврат; // Действий не требуется.
	КонецЕсли;
...
"Завершить редактирование невозможно!';en='Errors found while checking route map."
...
Процедура ПроверитьСхему(Команда)
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Проверка карты маршрута недоступна в мобильном клиенте.'"));
	#Иначе	
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.ВидОбъектаСтрокой) Тогда
			...
			Сообщение.Текст = НСтр("ru='Не указан Вид объекта!'");
		...
		Если Не ЗначениеЗаполнено(Объект.РеквизитОбъектаСтрокой) Тогда
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = НСтр("ru='Не указан Реквизит объекта!'");
		...
		Если Объект.ВидОбъектаСтрокой = "CRM_Интерес" И Не ЗначениеЗаполнено(Объект.СценарийИнтереса) Тогда
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = НСтр("ru='Не указан Сценарий Интереса!'");
			Сообщение.Поле = "Объект.СценарийИнтереса";
			Сообщение.Сообщить();
			
			Отказ = Истина;
		КонецЕсли;
		
		Если Объект.РазрешеноДосрочноеЗавершение И Не ЗначениеЗаполнено(Объект.СостояниеДосрочногоЗавершения) Тогда
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = НСтр("ru='Не указано состояние досрочного завершения!'");
			Сообщение.Поле = "Объект.СостояниеДосрочногоЗавершения";
			Сообщение.Сообщить();
...
Процедура СценарийИнтересаПриИзменении(Элемент)
	...
	ПараметрыВыбораСостояния.Добавить(Новый ПараметрВыбора("Отбор.Родитель", Объект.СценарийИнтереса));
	Элементы.СостояниеДосрочногоЗавершения.ПараметрыВыбора = Новый ФиксированныйМассив(ПараметрыВыбораСостояния);
	Если ЗначениеЗаполнено(Объект.СценарийИнтереса) Тогда
		Объект.СостояниеДосрочногоЗавершения = ПолучитьСостояниеИнтересаИнтересПотерян(Объект.СценарийИнтереса);
	КонецЕсли;
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПолучитьСостояниеИнтересаИнтересПотерян(СценарийИнтереса)
	Возврат Справочники.CRM_СостоянияИнтересов.НеудачноеЗавершениеСценария(СценарийИнтереса);
КонецФункции

&НаКлиенте
Процедура РазрешеноДосрочноеЗавершениеПриИзменении(Элемент)
	УстановитьСвойстваЭлементовПоТипу();
КонецПроцедуры

...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ДеревоШаблонаАнкеты.Строки[0].Строки.Найти("","Формулировка",Истина) <> Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не все формулировки или имена разделов заполнены.';en='Not all wordings or section names are filled in.'"),,"ДеревоАнкеты");
		Отказ = Истина;
	КонецЕсли;
	...
	Если НайденныеСтроки.Количество() <> 0 Тогда
		Для каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			Если НайденнаяСтрока.ТипВопроса <> Перечисления.ТипыВопросовШаблонаАнкеты.Табличный
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не все вопросы заполнены.';en='Not all questions are filled in.'"),,"ДеревоАнкеты");
				Отказ = Истина;
				Прервать;
...
Процедура ФормаДереваАнкетыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	СписокВыбора.ПоказатьВыборЭлемента(ОбработчикОповещенияОЗакрытии, НСтр("ru='Выберите тип добавляемого элемента.';en='Select a type of the item being added.'"),СписокВыбора[0]);
...
"Записать?';en='Template of questionnaire was modified. "
"For correct display of changes, template must be written."
"Write?'"),

...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	Если Объект.Подключение.СерверMSExchange <> Объект.УчетнаяЗапись.СерверMSExchange Тогда
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Реквизит ""Сервер MS Exchange"" в подключении отличается от значения, указанного в учетной записи!';en='Attribute ""server MS Exchange"" in connection differs from the value specified in account!'"),, "Объект.Подключение");
		Возврат;
	КонецЕсли;
...
Процедура ВыборПапкиMSExchange(Элемент)
	...
	Если Не СтруктураПапокЗаполненаДляУчетнойЗаписи() Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Для учетной записи ';en='For account '") + """" + Строка(Объект.УчетнаяЗапись) + """ " + НСтр("ru='еще не была загружена структура папок."

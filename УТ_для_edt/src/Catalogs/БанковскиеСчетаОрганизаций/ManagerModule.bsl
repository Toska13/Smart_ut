...
Функция ПолучитьБанковскийСчетОрганизацииПоУмолчанию(Организация, Валюта = Неопределено, НаправлениеДеятельности = Неопределено) Экспорт
    //++ смарт:ПА #815 27.12.2021 19:50:51 согласовано с Дмитрием Каратеевым по умолчанию Если валюта Неопределено устанавливаем рубли
    Если Валюта = Неопределено Тогда
    
    Валюта=Константы.ВалютаРегламентированногоУчета.Получить();	
    
    КонецЕсли;
    //-- смарт:ПА #815 27.12.2021 19:50:51
    
    //++ смарт:ПА #815 24.12.2021 11:42:15
    Запрос = Новый Запрос;
    ...
    | И БанковскиеСчетаОрганизаций.смартИспользоватьПоУмолчаниюДляВалюты
	...
	//++ #Б-4832 смарт: Фролов 10.11.2025 //Доработка документа Заявка на расход ДС.	
	//++ смарт:ММ #042 222 27.03.2020 Рефакторинг. Перенесено из расширения (отмена &Вместо) 	
	//Каратеев +
	//ИначеЕсли ВыборкаБезНаправления.Количество() > 1 Тогда 
	//	Сообщить("Слишком много значений в выборке по счетам");
	//	Пока ВыборкаБезНаправления.Следующий() Цикл
	//		Сообщить(ВыборкаБезНаправления.БанковскийСчетОрганизации.Ссылка);	
	//	КонецЦикла;
	//ИначеЕсли ВыборкаБезНаправления.Количество() < 1 Тогда
	...
	//-- #Б-4832 смарт: Фролов 10.11.2025

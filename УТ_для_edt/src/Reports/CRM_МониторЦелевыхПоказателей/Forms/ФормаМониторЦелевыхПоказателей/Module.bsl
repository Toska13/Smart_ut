...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПредставлениеПериода  = ПредставлениеПериода(ОтборНачалоПериода, ОтборКонецПериода);
	...
	Если НЕ ПустаяСтрока(Параметры.ИдентификаторВариантаАнализа) Тогда
		...
		Заголовок = НСтр("ru='Анализ взаимосвязанных целевых показателей';en='Analysis of related target indicators'");
...
Процедура СводныйОтчет(Команда)
	...
	//	Предупреждение(НСтр("ru='Сформировать сводный отчет невозможно, т.к. монитор показателей не настроен.';en='Generate a summary report is impossible, because indicators monitor is not configured.'"),, НСтр("ru='Ошибка формирования сводного отчета'"));
	//	
	// ИначеЕсли СоставМонитораПустойПоОтбору Тогда
	//	Предупреждение(НСтр("ru='Сформировать сводный отчет невозможно, т.к. в текущий отбор не входит ни один показатель.';en='Generate a consolidated report is impossible, because no indicator is included in the current selection.'"),, НСтр("ru='Ошибка формирования сводного отчета'"));
	//	
	// Иначе 
	//	
...
Процедура ДобавитьКомандыВариантовАнализа(Показатель, Пользователь, ГруппаЭлементов, ИдентификаторЭлементаФормы, ВариантОтображения)
	...
	Если ВариантОтображения = ВариантыОтображения.Диаграмма 
		...
		// Тип("КнопкаФормы"), ПодменюВсеДействия); КнопкаАнализПоказателя.ИмяКоманды = "ОткрытьАнализПоказателя__" + ИдентификаторЭлементаФормы;
	...
	ИначеЕсли ВариантОтображения = ВариантыОтображения.Кратко Тогда
		...
		// Добавим команды анализа, расшифровки и настройки
		// КнопкаАнализПоказателя =Элементы.Добавить("ФормаОткрытьАнализПоказателя__" + ИдентификаторЭлементаФормы,
		// Тип("КнопкаФормы"), КонтекстноеМенюТекстовогоПоказателя); КнопкаАнализПоказателя.ИмяКоманды =
		// "ОткрытьАнализПоказателя__" + ИдентификаторЭлементаФормы;
...
Процедура НарисоватьДиаграмму(ИсточникДанных, ГруппаЭлементов, ИдентификаторЭлементаФормы)
	...
	Если ОшибкаРасчета Тогда
		...
		Иначе
			ОписаниеОшибки = НСтр("ru='Ошибка расчета показателя';en='Error calculating key indicator'");
		КонецЕсли;
...
//	МассивРеквизитов.Добавить(Новый РеквизитФормы("ФактическоеСостояниеЗаголовок__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Состояние';en='Status'")));
...
//	КонецЕсли;
//	
//	ЭтотОбъект["ФактическоеСостояниеЗаголовок__" + ИдентификаторЭлементаФормы]	= НСтр("ru='Состояние:';en='Status:'");
...
Процедура НарисоватьТаблицу(ИсточникДанных, ГруппаЭлементов, ИдентификаторЭлементаФормы)
	...
	Если НЕ ПустойИсточникДанных И НЕ РасчетыНеактуальны Тогда
		...
		МассивРеквизитов.Добавить(Новый РеквизитФормы("ТаблицаЗначений__" + ИдентификаторЭлементаФормы,
														ОписаниеТипов, "",
														СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Таблица для %1';en='Table for %1'"), ИсточникДанных)));
	...
	Иначе
		...
		МассивРеквизитов.Добавить(Новый РеквизитФормы("Диаграмма__" + ИдентификаторЭлементаФормы,
														ОписаниеТипов, "",
														СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Диаграмма для %1';en='Chart for %1'"), ИсточникДанных)));
	...
	// ЭтаФорма["КартинкаСостояния__" + ИдентификаторЭлементаФормы] =
	// Отчеты.CRM_МониторЦелевыхПоказателей.ПолучитьКартинкуСостоянияДляПоказателя(ВариантАнализа, ИсточникДанных);
	// ЭтаФорма["КартинкаТренда__" + ИдентификаторЭлементаФормы] =
	// Отчеты.CRM_МониторЦелевыхПоказателей.ПолучитьКартинкуТрендаДляПоказателя(ВариантАнализа, ИсточникДанных);
	
	НачалоПериода = Формат(ИсточникДанных.СвойстваДанныхПоПериодам.ДатаПервогоЗначения, "ДФ=dd.MM.yy");
	КонецПериода= Формат(ИсточникДанных.СвойстваДанныхПоПериодам.ДатаПоследнегоЗначения, "ДФ=dd.MM.yy");
	ТекстовоеПредставлениеПериода = НачалоПериода + " - " + КонецПериода;
	// Если ВариантАнализа.КратностьЗначений = ВидыПредставленияЧисел.БезИзменений Тогда
		// ЭтаФорма["НаименованиеПоказателя__" + ИдентификаторЭлементаФормы] =
		// ВариантАнализа.Владелец.КраткоеНаименованиеЦелевогоПоказателя + ": " + ВариантАнализа.Наименование + ?(НЕ
		// ПустаяСтрока(ДинамическиеСвойстваВариантаАнализа.Размерность),", " +
		// ДинамическиеСвойстваВариантаАнализа.Размерность, "") + ", " + ТекстовоеПредставлениеПериода; Иначе
	...
	Если НЕ ПустойИсточникДанных И НЕ РасчетыНеактуальны И НЕ ОшибкаРасчета Тогда
		...
		// Отчеты.CRM_МониторЦелевыхПоказателей.ЗаполнитьТаблицуДляМонитора(ЭтаФорма["ТаблицаЗначений__" +
		// ИдентификаторЭлементаФормы], ИдентификаторЭлементаФормы, ВариантАнализа, ИсточникДанных);
...
//	//	
//	// Иначе
//	//	ИмяВидаПоиска = НСтр("ru='АнализируемыйПоказатель';en='АнализируемыйПоказатель'");
//	//	
//	// КонецЕсли;
//	//
		...
		Если ВариантОтображения = ВариантыОтображения.Кратко Тогда
			...
			Если КоличествоНенарисованныхПоказателей = 0 И СчетчикЭлементовСтроки < СтруктураНастроек.КоличествоКолонокМонитораПоказателей Тогда
				Для СчВыравнивающихЭлементов = 1 По (СтруктураНастроек.КоличествоКолонокМонитораПоказателей - СчетчикЭлементовСтроки) Цикл 
					...
					ГруппаЭлемент.ОтображатьЗаголовок = Ложь;
					ГруппаЭлемент.Заголовок = НСтр("ru='Контейнер выравнивающего элемента';en='Alignment Container'");
...
Процедура НарисоватьЭлементыМонитораСУчетомЗоныВнимания(КоллекцияВариантовАнализа, ЗонаВнимания)
	...
	// ВидыПредставленияЧисел = Перечисления.ркВидыПредставленияЧисел;
	ВариантыОтображения = Перечисления.CRM_ВариантыОтображенияАнализаПоказателей;
	ИмяЗоныВнимания = CRM_ОбщегоНазначенияСервер.ПолучитьИмяЗначенияПеречисления(ЗонаВнимания);
	...
	Иначе
		...
		Если СтруктураНастроек.ВариантГруппировкиПоказателей = "ПоВажности" Тогда
			...
			Если СтатистикаПоСтрокам.КритическоеСостояниеКратко > 0 Тогда
				...
				CRM_ОбщегоНазначенияСервер.ПолучитьИмяЗначенияПеречисления(ЗоныВнимания.КритическоеСостояние));
			...
			Если СтатистикаПоСтрокам.ВажноКратко > 0 Тогда
				...
				CRM_ОбщегоНазначенияСервер.ПолучитьИмяЗначенияПеречисления(ЗоныВнимания.Важно));
			...
			Если СтатистикаПоСтрокам.КСведениюКратко > 0 Тогда
				...
				CRM_ОбщегоНазначенияСервер.ПолучитьИмяЗначенияПеречисления(ЗоныВнимания.КСведению));
			...
			Если СтатистикаПоСтрокам.РассчитанныеСОшибкамиКратко > 0 Тогда
				...
				CRM_ОбщегоНазначенияСервер.ПолучитьИмяЗначенияПеречисления(ЗоныВнимания.РассчитанныеСОшибками));
...
Процедура ЗаполнитьСписокВыбораОтбораПоСтатусу()
	...
	СписокВыбора.Добавить(0, НСтр("ru='Все';en='All'"));
	СписокВыбора.Добавить(1, НСтр("ru='Критические';en='Critical'"));
	СписокВыбора.Добавить(2, НСтр("ru='Важные';en='Important'"));
	СписокВыбора.Добавить(3, НСтр("ru='Критические и важные';en='Critical and Important'"));
	СписокВыбора.Добавить(4, НСтр("ru='К сведению';en='For your information'"));
	СписокВыбора.Добавить(5, НСтр("ru='Рассчитанные с ошибкой';en='Calculated with Error'"));
...
Функция ПолучитьВариантыОтчетовПоказателя(ВариантАнализа)
	...
	ВариантыОтчетовПоказателя.Добавить("ОтчетРасшифровкаПоказателя", НСтр("ru='Расшифровка показателя';en='Decoding of the benchmark'"));

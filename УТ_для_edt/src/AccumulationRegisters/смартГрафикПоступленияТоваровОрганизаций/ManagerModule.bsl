...
Функция ТекстЗапросаОборотов(ИспользоватьКорректировку, Разделы = Неопределено) Экспорт
	...
	ШаблонПоступление =
		...
		|ИЗ
		|	РегистрНакопления.смартГрафикПоступленияТоваровОрганизаций.Остатки(,
	...
	ШаблонОтгрузка =
		...
		|	-Таблица.смартКоличествоИзЗаказовГарантийныеОстаток КАК КоличествоГарантийные
		|ИЗ
		|	РегистрНакопления.смартГрафикОтгрузкиТоваровОрганизаций.Остатки(,
		|		(Организация, Номенклатура, Характеристика, Склад) В(
		|			ВЫБРАТЬ
		|				Отбор.Организация    КАК Организация,
		|				Отбор.Номенклатура   КАК Номенклатура,
...
Функция ТекстЗапросаОборотовОбособленных(ИспользоватьКорректировку, Разделы, УчитыватьГрафикОтгрузки)
	...
	ШаблонПоступление =
		...
		|ИЗ
		|	РегистрНакопления.смартГрафикПоступленияТоваровОрганизаций КАК Таблица
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтТоварыОбособленные КАК Отбор
		|		ПО Таблица.Организация    = Отбор.Организация
		|		 И Таблица.Номенклатура   = Отбор.Номенклатура
	...
	ТекстЗапроса =
		...
		|
		|СГРУППИРОВАТЬ ПО
		|	Набор.Организация, Набор.Номенклатура, Набор.Характеристика, Набор.Склад, Набор.Назначение,
		|	Набор.Период
		|ИМЕЮЩИЕ
		|	СУММА(Набор.Количество) <> 0
		|	ИЛИ СУММА(Набор.КоличествоГарантийные) <> 0
		|ИНДЕКСИРОВАТЬ ПО
		|	Организация, Номенклатура, Характеристика, Склад, Назначение,
		|	Период
		|;
		|
		|///////////////////////////////////////////////////
...
Функция ТекстЗапросаОстатковОбособленных(ИспользоватьКорректировку, Разделы, УчитыватьГрафикОтгрузки)
	...
	ШаблонПоступление =
		...
		|ИЗ
		|	РегистрНакопления.смартГрафикПоступленияТоваровОрганизаций.Остатки(,
		|		(Организация, Номенклатура, Характеристика, Склад, Назначение) В(
		|			ВЫБРАТЬ
		|				Отбор.Организация     КАК Организация,
		|				Отбор.Номенклатура    КАК Номенклатура,
		|				Отбор.Характеристика  КАК Характеристика,
	...
	ТекстЗапроса =
		...
		|
		|СГРУППИРОВАТЬ ПО
		|	НаборДанных.Организация, НаборДанных.Номенклатура, НаборДанных.Характеристика, НаборДанных.Склад, НаборДанных.Назначение
		|ИМЕЮЩИЕ
		|	СУММА(НаборДанных.Количество) <> 0
		|	ИЛИ СУММА(НаборДанных.КоличествоГарантийные) <> 0
		|ИНДЕКСИРОВАТЬ ПО
...
Функция ТекстЗапросаОстатковИОборотов(ИспользоватьКорректировку, Разделы = Неопределено) Экспорт
	...
	ТекстЗапроса =
		РегистрыНакопления.смартСвободныеОстаткиОрганизаций.ТекстЗапросаОстатков(ИспользоватьКорректировку, Разделы)
		...
		|		ПО Таблица.Организация    = ОстаткиСклада.Организация
		|		 И Таблица.Номенклатура   = ОстаткиСклада.Номенклатура
		...
		|		ПО Таблица.Организация    = ОстаткиГрафика.Организация
		|		 И Таблица.Номенклатура   = ОстаткиГрафика.Номенклатура
		...
		|		ПО Таблица.Организация    = ОборотыГрафика.Организация
		|		 И Таблица.Номенклатура   = ОборотыГрафика.Номенклатура
...
Функция ТекстЗапросаОстатковИОборотовОбособленных(ИспользоватьКорректировку, Разделы, УчитыватьГрафикОтгрузки) Экспорт
	...
	// непонятно будет ли вообще где-то использоваться данный запрос, поэтому пока
	// вернем абракадабру, чтобы в случае чего вылезла ошибка
	// если где-нибудь вылезет ошибка то надо будет еще зеркалить регистр Обеспечение заказов
	
	Возврат "Ошибка формирования текста запроса.";
...
//     * Организация     - СправочникСсылка.Организации                - организация из выборки данных.
...
Функция ТаблицаДоступныеОстатки(Выборка, ТолькоПоложительные = Ложь, Обособление = Неопределено) Экспорт
	...
	ДоступныеОстатки.Колонки.Добавить("Организация", Новый ОписаниеТипов("СправочникСсылка.Организации"));
	...
	КлючСтроки.Вставить("Организация");

...
Процедура РассчитатьПоДаннымВзаиморасчетов(ТипРасчетов, МассивДокументов) Экспорт
	...
	Пока ВыборкаПоДокументу.Следующий() Цикл
		...
		Пока ВыборкаПоАналитике.Следующий() Цикл
			...
			Пока ВыборкаПоТипуЗаписи.Следующий() Цикл
				...
				Пока Выборка.Следующий() Цикл
					...
					//++ смарт:rnp  #64 316 15.09.2020 для документов Реализация товаров  при взаиморасчетах в валюте будем использовать алгоритм пересчета как в 1С Бухгалтерии,
					//функции ОбщегоНазначенияБПКлиентСервер.РаспределитьПропорционально() и УчетДоходовРасходов. ПолучитьТаблицуРеализацииПоКурсуАвансов
					Если ТипЗнч(ВыборкаПоДокументу.Ссылка)=ТипЗнч(Документы.РеализацияТоваровУслуг.ПустаяСсылка()) И
						(ВыборкаПоДокументу.ВалютаВзаиморасчетов <> ВыборкаПоДокументу.Валюта 
						ИЛИ Константы.ВалютаРегламентированногоУчета.Получить() <> ВыборкаПоДокументу.ВалютаВзаиморасчетов) Тогда
						ПересчетНужен = Истина;
						Запись.СуммаСНДСРегл = Выборка.БазаНДСРегл + Выборка.СуммаНДСРегл;
						Запись.СуммаСНДСУпр =  Выборка.БазаНДСУпр + Выборка.СуммаНДСУпр;
						Запись.СуммаДляБазыНДС = Запись.СуммаСНДСРегл;
						Продолжить;
					КонецЕсли;
					//-- смарт:rnp  #64 316 15.09.2020
		...
		//++ смарт:rnp  #64 316 15.09.2020 для документов Реализация товаров будем использовать алгоритм как в 1С Бухгалтерии,
		//функции ОбщегоНазначенияБПКлиентСервер.РаспределитьПропорционально() и УчетДоходовРасходов. ПолучитьТаблицуРеализацииПоКурсуАвансов
		Если ПересчетНужен = Истина Тогда
			//1. распределим сумму документа с учетом авансов пропорционально по строкам документа
			МассивСтарыхСумм = ТаблицаСуммыДокумента.ВыгрузитьКолонку("СуммаСНДСРегл");			
			МассивНовыхСумм = смартОбщегоНазначения.смартРаспределитьПропорционально(СуммаДокументаРегл, МассивСтарыхСумм);
			Если МассивНовыхСумм <> Неопределено Тогда
				ТаблицаСуммыДокумента.ЗагрузитьКолонку(МассивНовыхСумм, "СуммаСНДСРегл");
				ТаблицаСуммыДокумента.ЗагрузитьКолонку(МассивНовыхСумм, "СуммаДляБазыНДС");
			КонецЕсли;
			
			//2. рассчитаем по каждой строке документа сумму НДС по ставке и сумму без НДС
			Для Каждого СтрокаТаблицы Из ТаблицаСуммыДокумента Цикл
				ПроцентНДС = УчетНДСУПКлиентСервер.ПолучитьСтавкуНДС(СтрокаТаблицы.СтавкаНДС);
				СтрокаТаблицы.СуммаНДСРегл = Окр(СтрокаТаблицы.СуммаСНДСРегл * ПроцентНДС / (100 + ПроцентНДС), 2);
				СтрокаТаблицы.СуммаБезНДСРегл = СтрокаТаблицы.СуммаСНДСРегл - СтрокаТаблицы.СуммаНДСРегл;
			КонецЦикла;
			МассивНовыхСумм = ТаблицаСуммыДокумента.ВыгрузитьКолонку("СуммаБезНДСРегл");
			ТаблицаСуммыДокумента.ЗагрузитьКолонку(МассивНовыхСумм, "БазаНДСРегл");
		КонецЕсли;
		//-- смарт:rnp  #64 316 15.09.2020
...
Процедура ОбновитьДвиженияДокумента(Ссылка, СоставОбновления)
	...
	//++ смарт:ММ #64 316 25.08.2020 
	ДопСвойства.Вставить("смартЭтоОбновлениеДвиженийДокумента", Истина);
	//-- смарт:ММ 25.08.2020

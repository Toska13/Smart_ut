...
Процедура УстановитьНазваниеПрограммыДляОткрытияМакета()
	...
	Если ТипФайла = "mxl" Тогда
		НазваниеПрограммыДляОткрытияМакета = НСтр("ru='""1С:Предприятие - Работа с файлами""';en='1C:Enterprise - File operations'");
	...
	ИначеЕсли ТипФайла = "docx" Тогда
		НазваниеПрограммыДляОткрытияМакета = НСтр("ru='один из офисных пакетов или редактор документов формата Office Open XML';en='one of office packeges or documents editor Office Open XML format'");
	...
	СведенияДляЗаполнения.Вставить("ОписаниеДействия", ?(Параметры.ТолькоОткрытие, НСтр("ru='открытия';en='openings'"), НСтр("ru='внесения изменений'")));
...
#Если Не ВебКлиент И НЕ МобильныйКлиент Тогда
	...
	Иначе
		...
		НачатьЗапускПриложения(ОписаниеОповещения, ПутьКФайлуМакета);
	КонецЕсли;

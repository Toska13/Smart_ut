...
Процедура ПередЗаписью(Отказ, Замещение)
	ТЗНабор = Выгрузить();
	ТЗНаборСвертка = ТЗНабор.Скопировать(, "ПериодНачало, ПериодОкончание, Пользователь, Объект, Партнер");
	ТЗНаборСвертка.Свернуть("ПериодНачало, ПериодОкончание, Пользователь, Объект, Партнер");
	Если ТЗНаборСвертка.Количество() <> Количество() Тогда
		// Удалим дублирующиеся записи
		Для каждого Запись Из ТЗНаборСвертка Цикл
			ПараметрыОтбора = Новый Структура;
			ПараметрыОтбора.Вставить("ПериодНачало", Запись.ПериодНачало);
			ПараметрыОтбора.Вставить("ПериодОкончание", Запись.ПериодОкончание);
			ПараметрыОтбора.Вставить("Пользователь", Запись.Пользователь);
			ПараметрыОтбора.Вставить("Объект", Запись.Объект);
			ПараметрыОтбора.Вставить("Партнер", Запись.Партнер);
			Строки = ТЗНабор.НайтиСтроки(ПараметрыОтбора);
			КоличествоСтрок = Строки.Количество();
			Если КоличествоСтрок>1 Тогда
				Для Номер = КоличествоСтрок - 1 По 1 Цикл
					Индекс = ТЗНабор.Индекс(Строки[Номер]);
					ТЗНабор.Удалить(Индекс);
				КонецЦикла;
			КонецЕсли;
		КонецЦикла;
		Загрузить(ТЗНабор);
...
Процедура ПеренестиОбъект_Задача_ЗадачаИсполнителя(Ошибки)
	...
	Исключение
		...
		СтрОшибка = НСтр("ru='Не удалось перенести задачу: ';en='Failed to relocate task:'")
			+ """" + Строка(Объект) + """: "

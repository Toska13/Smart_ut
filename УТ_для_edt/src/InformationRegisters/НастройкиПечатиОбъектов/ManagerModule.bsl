		...
		Если ТекущийКомплект.Количество() > 0 И КоллекцияПечатныхФорм.Найти(ИмяМакета, "ИмяМакета") = Неопределено Тогда
			...
			Иначе
				...
				УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
					...
					//++ смарт:ММ #051 252 05.08.2020 - имя файла должно браться из настроек во внешней обработке при печати внешних ПФ
					//ВременнаяКоллекцияДляОднойПечатнойФормы[0].ТабличныйДокумент);
					ВременнаяКоллекцияДляОднойПечатнойФормы[0].ТабличныйДокумент
					,,, 
					ВременнаяКоллекцияДляОднойПечатнойФормы[0].ИмяФайлаПечатнойФормы);
					//-- смарт:ММ 05.08.2020

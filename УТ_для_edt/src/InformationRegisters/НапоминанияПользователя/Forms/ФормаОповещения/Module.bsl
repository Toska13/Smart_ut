...
Процедура ОтложитьАктивныеНапоминания()
	...
	//++ смарт оптимизация инфософт 22.07.2020
	ОбработатьНапоминаниеНаСервере(ОбщегоНазначенияКлиент.ДатаСеанса() + ИнтервалВремени);
	//--
	Для Каждого СтрокаТаблицы Из Напоминания Цикл
		//++ смарт оптимизация инфософт 22.07.2020
...
//смарт оптимизация инфософт 22.07.2020
&НаСервере
Процедура  ОбработатьНапоминаниеНаСервере(СрокНапоминания); 
	Для Каждого СтрокаТаблицы Из Напоминания Цикл
		СтрокаТаблицы.СрокНапоминания = СрокНапоминания;

...
Процедура ЗаписатьТокенЗапроса(Знач Идентификатор, Знач ТокенЗапроса) Экспорт
	...
	НСтр("ru='Значение, переданное в параметре state.id, не является уникальным идентификатором.';en='Value passed in state.id parameter is not UID.'"));
	...
	Если Выборка.Следующий() И Выборка.КоличествоЗаписей > 1000 Тогда
		ВызватьИсключение НСтр("ru='Превышено количество попыток авторизации.';en='You have exceeded the number of login attempts.'");
	КонецЕсли;

...
&ИзменениеИКонтроль("ТаблицаОформлено")
Функция смарт_ТаблицаОформлено(Отбор, Корректировка, ТолькоОрдерныеСклады)
	...
	|	ВЫБОР КОГДА Таблица.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА
	|				Таблица.КОтгрузке + Таблица.смартКОтгрузкеГарантийные
	|				- Таблица.Собирается - Таблица.Собрано - Таблица.смартСобираетсяГарантийные - Таблица.смартСобраноГарантийные
	|			ИНАЧЕ
	|				Таблица.Собирается + Таблица.Собрано + Таблица.смартСобираетсяГарантийные + Таблица.смартСобраноГарантийные
	|		КОНЕЦ                КАК Количество
	...
	|		ИЛИ Таблица.Собирается <> 0
	|		ИЛИ Таблица.Собрано <> 0)
	|	ИЛИ (Таблица.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|			И Таблица.смартКОтгрузкеГарантийные <> 0
	|		ИЛИ Таблица.смартСобираетсяГарантийные <> 0
	|		ИЛИ Таблица.смартСобраноГарантийные <> 0) )
	//-- #Б1376 смарт: Шляпников 19.11.2024
	#КонецВставки
	|	И &ВсеСклады
	|
...
&ИзменениеИКонтроль("ТекстЗапросаОсталосьОформить")
Функция смарт_ТекстЗапросаОсталосьОформить(ОтборПоИзмерениям)
	...
	#Вставка
	//++ #Б1376 смарт: Шляпников 19.11.2024 / Раздельный учет товаров. Гарантийные запчасти. Разработка документов и регистров.
	|	, Таблица.смартКОтгрузкеГарантийныеРасход   КАК смартКОтгрузкеГарантийные,
	|	Таблица.смартКОформлениюГарантийныеРасход КАК смартКОформлениюГарантийные,
	|	Таблица.смартСобраноГарантийныеПриход     КАК смартСобраноГарантийные,
	|	Таблица.смартСобираетсяГарантийныеПриход  КАК смартСобираетсяГарантийные
	//-- #Б1376 смарт: Шляпников 19.11.2024
	#КонецВставки
	...
	|	МАКСИМУМ(Набор.СобираетсяГарантийные) КАК СобираетсяГарантийные
	//-- #Б1376 смарт: Шляпников 19.11.2024
	#КонецВставки
	|ИЗ
	|	(
	...
	#Вставка
	//++ #Б1376 смарт: Шляпников 19.11.2024 / Раздельный учет товаров. Гарантийные запчасти. Разработка документов и регистров.
	|		, Таблица.смартКОтгрузкеГарантийные 
	|			+ Таблица.смартСобраноГарантийные        КАК КоличествоГарантийные,
	|		Таблица.смартСобираетсяГарантийные           КАК СобираетсяГарантийные
	//-- #Б1376 смарт: Шляпников 19.11.2024
	#КонецВставки
	|	ИЗ
	...
	#Вставка
	//++ #Б1376 смарт: Шляпников 19.11.2024 / Раздельный учет товаров. Гарантийные запчасти. Разработка документов и регистров.
	|		ИЛИ Таблица.смартКОтгрузкеГарантийные 
	|			+ Таблица.смартСобраноГарантийные > 0
	//-- #Б1376 смарт: Шляпников 19.11.2024
	#КонецВставки
	|		
	|	ОБЪЕДИНИТЬ ВСЕ
	...
	#Вставка
	//++ #Б1376 смарт: Шляпников 19.11.2024 / Раздельный учет товаров. Гарантийные запчасти. Разработка документов и регистров.
	|		, -Таблица.смартКОформлениюГарантийные         КАК КоличествоГарантийные,
	|		Таблица.смартСобираетсяГарантийные           КАК СобираетсяГарантийные
	//-- #Б1376 смарт: Шляпников 19.11.2024
	#КонецВставки
	|	ИЗ
	...
	#Вставка
	//++ #Б1376 смарт: Шляпников 19.11.2024 / Раздельный учет товаров. Гарантийные запчасти. Разработка документов и регистров.
	|		ИЛИ Таблица.смартКОформлениюГарантийные > 0
	//-- #Б1376 смарт: Шляпников 19.11.2024
	#КонецВставки
	|		
	|	ОБЪЕДИНИТЬ ВСЕ
	...
	#Вставка
	//++ #Б1376 смарт: Шляпников 19.11.2024 / Раздельный учет товаров. Гарантийные запчасти. Разработка документов и регистров.
	|		, СУММА(Таблица.смартКОформлениюГарантийные)   КАК КоличествоГарантийные,
	|		МАКСИМУМ(Таблица.смартСобираетсяГарантийные) КАК СобираетсяГарантийные
	//-- #Б1376 смарт: Шляпников 19.11.2024
	#КонецВставки
	|	ИЗ
	|		РегистрНакопления.ТоварыКОтгрузке КАК Таблица
	...
	#Вставка
	//++ #Б1376 смарт: Шляпников 19.11.2024 / Раздельный учет товаров. Гарантийные запчасти. Разработка документов и регистров.
	|		И ( Таблица.смартКОформлениюГарантийные > 0
	|		И Таблица.смартКОформлениюГарантийные > 0 )
	//-- #Б1376 смарт: Шляпников 19.11.2024
	#КонецВставки

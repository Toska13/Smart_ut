...
&ИзменениеИКонтроль("ПолучитьОплаты")
Функция смарт_ПолучитьОплаты(Запрос, ГлобальныеПеременные)
	...
	#КонецУдаления
	#Вставка
	//++ смарт:ММ #058 325 22.07.2020 - Не виделась оплата, внесенная Корректировкой регистров 
	//|		И НЕ Расчеты.Регистратор ССЫЛКА Документ.КорректировкаРегистров
	|		И ВЫБОР 
	|			КОГДА Расчеты.Регистратор ССЫЛКА Документ.КорректировкаРегистров
	|				ТОГДА ВЫРАЗИТЬ(Расчеты.Регистратор КАК Документ.КорректировкаРегистров).Операция = ЗНАЧЕНИЕ(Перечисление.ОперацииКорректировкиРегистров.РучнаяКорректировка)
	|			ИНАЧЕ 
	|				ИСТИНА
	|			КОНЕЦ
	//-- смарт:ММ 22.07.2020
...
&ИзменениеИКонтроль("ПолучитьОтгрузкиПоставки")
Функция смарт_ПолучитьОтгрузкиПоставки(Запрос, ГлобальныеПеременные)
	...
	#КонецУдаления
	#Вставка
	//++ смарт:rnp #058 325 12.10.2020 - Не виделась оплата, внесенная Корректировкой регистров 
	//|		И НЕ Расчеты.Регистратор ССЫЛКА Документ.КорректировкаРегистров
	|		И ВЫБОР 
	|			КОГДА Расчеты.Регистратор ССЫЛКА Документ.КорректировкаРегистров
	|				ТОГДА ВЫРАЗИТЬ(Расчеты.Регистратор КАК Документ.КорректировкаРегистров).Операция = ЗНАЧЕНИЕ(Перечисление.ОперацииКорректировкиРегистров.РучнаяКорректировка)
...
Процедура смарт_ЗаполнитьОперативныеВзаиморасчеты(ОсновныеПараметры) Экспорт
	...
	Реквизиты = Результаты[0].Выгрузить()[0];
	...
	Пока Индексы.ИндексОплаты + Индексы.ИндексОтгрузкиПоставки < ЗаписейОбработать Цикл
		...
		Если Индексы.ИндексОплаты <= ЗаписейОплаты - 1 И Индексы.ИндексОтгрузкиПоставки <= ЗаписейОтгрузкиПоставки - 1 Тогда 
			...
			Если СтрокаОплаты.ПорядокОперации < СтрокаОтгрузкиПоставки.ПорядокОперации 
				#Вставка
				//++ смарт:ММ #062 334 14.08.2020 - Заказ клиента - не зачитывается аванс при указании договора 
				// Обработка ситуации, когда в день реализации был сделан взаимозачет задолженности
				ИЛИ (ТипЗнч(СтрокаОплаты.Регистратор) = Тип("ДокументСсылка.ВзаимозачетЗадолженности")
				   	И ТипЗнч(СтрокаОтгрузкиПоставки.Регистратор) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
					И Лев(СтрокаОплаты.ПорядокОперации, 8) = Лев(СтрокаОтгрузкиПоставки.ПорядокОперации, 8)
					И Сред(СтрокаОплаты.ПорядокОперации, 10, 6) = "000000")
				//-- смарт:ММ 14.08.2020
				...
				Если СтрокаОтгрузкиПоставки.Сумма > 0 И СтрокаОплаты.Сумма > 0 Тогда
					...
					Если (СтрокаОтгрузкиПоставки.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВзаимозачетЗадолженности
						...
						ПредоплатаРеглПредв = СуммаСписания(СтрокаОтгрузкиПоставки.КурсРегл, СтрокаОтгрузкиПоставки.СуммаРегл, СтрокаОтгрузкиПоставки.Сумма, СуммаСписания);
					...
					Если (СтрокаОтгрузкиПоставки.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВзаимозачетЗадолженности
						...
						ИЛИ СтрокаОтгрузкиПоставки.ЭтоВозвратАванса И СтрокаОтгрузкиПоставки.Регистратор = СтрокаОплаты.Регистратор Тогда
						ПредоплатаУпрПредв = СуммаСписания(СтрокаОтгрузкиПоставки.КурсУпр, СтрокаОтгрузкиПоставки.СуммаУпр, СтрокаОтгрузкиПоставки.Сумма, СуммаСписания);
					Иначе
					...
					#КонецУдаления
					#Вставка
					//++смарт:rnp #64 316 22.09.2020 добавлено условие и формулы в "иначе"
					//СтрокаОтгрузкиПоставки.СуммаРегл = СтрокаОтгрузкиПоставки.СуммаРегл - Окр(ДолгРеглПредв + ДолгРаспределеноРегл - Окр(ДолгРаспределеноРегл,2),2);
					//СтрокаОтгрузкиПоставки.СуммаУпр  = СтрокаОтгрузкиПоставки.СуммаУпр - Окр(ДолгУпрПредв + ДолгРаспределеноУпр - Окр(ДолгРаспределеноУпр,2),2);
					...
					//ДолгРеглПредв - в Бухгалтерии формула СтрокаВзаиморасчетов.СуммаРубПоКурсуАванса = Окр(ЗачитываемыйАванс * КоэффициентКурсаАванса, 2)
					//если ниже расчет нераспределенной по авансам суммы, то в Бухгалтерии формулы
					//СтрокаВзаиморасчетов.СуммаРуб = Окр(НераспределеннаяСумма * КоэффициентРуб, 2)
					//КоэффициентРуб = ?(НЕ СтрокаДокумента.РасчетыВВалюте, 1,?(СтрокаДокумента.СуммаВзаиморасчетов = 0, 0, СтрокаДокумента.СуммаРуб / СтрокаДокумента.СуммаВзаиморасчетов)) - фактически текущий курс на дату документа
                    Если СтрокаОтгрузкиПоставки.РучнойКурсРегл И НЕ ДвойнаяЗаписьНеНужна Тогда
                        СтрокаОтгрузкиПоставки.СуммаРегл = СтрокаОтгрузкиПоставки.СуммаРегл - Окр(ДолгРеглПредв + ДолгРаспределеноРегл - Окр(ДолгРаспределеноРегл,2),2);
                        СтрокаОтгрузкиПоставки.СуммаУпр  = СтрокаОтгрузкиПоставки.СуммаУпр - Окр(ДолгУпрПредв + ДолгРаспределеноУпр - Окр(ДолгРаспределеноУпр,2),2);
                    Иначе
                        СтрокаОтгрузкиПоставки.СуммаУпр  = СтрокаОтгрузкиПоставки.СуммаУпр - Окр(ДолгУпрПредв + ДолгРаспределеноУпр - Окр(ДолгРаспределеноУпр,2),2);
                        //++ смарт:Подкин #1137 19.08.2022 14:54:41
                        //СтрокаОтгрузкиПоставки.СуммаРегл = СтрокаОтгрузкиПоставки.СуммаУпр * СтрокаОтгрузкиПоставки.КурсРегл;
                        СтрокаОтгрузкиПоставки.СуммаРегл = Окр(СтрокаОтгрузкиПоставки.КурсРегл * СтрокаОтгрузкиПоставки.Сумма,2);
                        //-- смарт:Подкин #1137 19.08.2022 14:54:41
                    КонецЕсли;
	...
	Если НЕ ОсновныеПараметры.НачальноеЗаполнение И ОсновныеПараметры.ЗаписыватьИзменения Тогда
		...
		Если ОсновныеПараметры.Свойство("ДополнительныеСвойстваПроведения") И ОсновныеПараметры.ДополнительныеСвойстваПроведения <> Неопределено Тогда
			...
			Если ГлобальныеПеременные.ЭтоРасчетыСКлиентами Тогда
				...
				#Вставка
				//смарт: rnp 392 02.11.2020
				|		И НЕ ТаблицаИзменений.ДокументРегистратор ССЫЛКА Документ.КорректировкаРегистров
				//--
				#КонецВставки
				|
				|СГРУППИРОВАТЬ ПО
				...
				//|	ТаблицаИзмененийРасчетыСКлиентамиПоСрокам КАК ТаблицаИзменений");
				|	ТаблицаИзмененийРасчетыСКлиентамиПоСрокам КАК ТаблицаИзменений
				|		ГДЕ НЕ ТаблицаИзменений.ДокументРегистратор ССЫЛКА Документ.КорректировкаРегистров");
				//--
				#КонецВставки

...
// Заменяет Символ(29) в колонке НомерАкцизнойМарки на ○ — иначе XML EnterpriseData обрезает код маркировки.
//
Процедура смартПодготовитьКодыМаркировкиКВыгрузкеXDTO(АкцизныеМарки)
	
	Если АкцизныеМарки = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если АкцизныеМарки.Колонки.Найти("НомерАкцизнойМарки") = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	РазделительGS = Символ(29);
	
	Для Каждого СтрокаМарки Из АкцизныеМарки Цикл
		
		Если ТипЗнч(СтрокаМарки.НомерАкцизнойМарки) = Тип("Строка")
			И СтрНайти(СтрокаМарки.НомерАкцизнойМарки, РазделительGS) > 0 Тогда
			
			СтрокаМарки.НомерАкцизнойМарки = СтрЗаменить(СтрокаМарки.НомерАкцизнойМарки, РазделительGS, "○");
...
Процедура смарт_ПКО_Документ_РеализацияТоваровУслуг_Отправка_ПриОтправкеДанных(ДанныеИБ, ДанныеXDTO, КомпонентыОбмена, СтекВыгрузки)
	Если СтекВыгрузки.Количество() > 1 Тогда
		...
		//-- #Б2356 смарт: Шляпников 17.02.2025
		// Признак Дилер справочника Партнеры - в БП по AdditionalInfo.Дилер пишется СтатусыКонтрагентовИСМП.
		ПартнерДилер = Ложь;
		Если ЗначениеЗаполнено(ДанныеИБ.Партнер) Тогда
			ПартнерДилер = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеИБ.Партнер, "Дилер");
		КонецЕсли;
		AdditionalInfoВставить(ДанныеXDTO, "Дилер", ПартнерДилер);
		#КонецВставки
	...
	#КонецУдаления
	#Вставка
	//++ #Б7868 смарт: коды маркировки в РТУ. Не отсекать выгрузку ШтрихкодыУпаковок при формате >= 1.7.
	ВыгружатьМаркировкуВОбмен = ВыгружатьШтрихкодыУпаковок(КомпонентыОбмена)
		Или ВерсияФорматаЧислом(КомпонентыОбмена.ВерсияФорматаОбмена) >= ВерсияФорматаЧислом("1.7");
	Запрос.УстановитьПараметр("ВыгружатьШтрихкодыУпаковок", ВыгружатьМаркировкуВОбмен);
	#КонецВставки
	...
	Если ВерсияФорматаЧислом(КомпонентыОбмена.ВерсияФорматаОбмена) >= ВерсияФорматаЧислом("1.7") Тогда
		...
		ЗаполнитьХарактеристикуУпаковкуВТЧ(КомпонентыОбмена, АкцизныеМарки);
		#Вставка
		//++ #Б7868 смарт: Символ(29) в коде маркировки недопустим в XML EnterpriseData.
		смартПодготовитьКодыМаркировкиКВыгрузкеXDTO(АкцизныеМарки);
		#КонецВставки
		ДанныеXDTO.Вставить("АкцизныеМарки", АкцизныеМарки);
	КонецЕсли;

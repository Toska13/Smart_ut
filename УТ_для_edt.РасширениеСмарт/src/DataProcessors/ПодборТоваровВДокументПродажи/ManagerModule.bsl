...
&ИзменениеИКонтроль("ЗаполнитьСтатусУказанияСерииВСтроке")
Процедура смарт_ЗаполнитьСтатусУказанияСерииВСтроке(Строка, ПараметрыУказанияСерий)
	...
	#Вставка
	//++#1547 смарт: Лопато 25.12.2023 //Автоматическое заполнение серий чернил в заказах
	Если Строка.Свойство("ДатаОтгрузки") Тогда
		ТаблицаТоваров.Колонки.Добавить("ДатаОтгрузки", Новый ОписаниеТипов("Дата", Новый КвалификаторыДаты(ЧастиДаты.Дата)));
	КонецЕсли;	
	//--#1547 смарт: Лопато 25.12.2023
	#КонецВставки

...
Функция смарт_СформироватьЗаданиеНаОтборРазмещениеТовара(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Для Каждого СтруктураОбъектов Из СтруктураТипов Цикл
		...
		Пока ВыборкаПоШапке.Следующий() Цикл
			...
			Пока ВыборкаПоСкладам.НайтиСледующий(Новый Структура("Склад",ВыборкаПоШапке.Склад)) Цикл
				...
				#Вставка
				//++ #Б-5406 смарт: Фролов 14.05.2026 //Оприходование запчастей без документов (смарт). Доработка функционала. Добавить выбор номера ЗГР.
				//++ #Б-1426 смарт: Фролов 25.11.2024 //Вывести поле комментарий в печатную форму расходного ордера
				Если ТипЗнч(ВыборкаПоШапке.Ссылка) = Тип("ДокументСсылка.ВводОстатков") Тогда
					
						ОбластьКомментарий = Макет.ПолучитьОбласть("Комментарий");
						КомментарийДляОбласти = ВыборкаПоШапке.Ссылка.Комментарий + Символы.ПС;
						ДокОснование = ВыборкаПоШапке.Ссылка.Основание;
						
						Если ЗначениеЗаполнено(ДокОснование) Тогда
							
							КомментарийДляОбласти = КомментарийДляОбласти + "Основание № " + ДокОснование.Номер + " от " + Формат(ДокОснование.Дата, "ДФ=dd.MM.yyyy");
							
						КонецЕсли;
						
						ОбластьКомментарий.Параметры.Комментарий = КомментарийДляОбласти;
				...
				Иначе	
					...
					Попытка
						ОбластьКомментарий = Макет.ПолучитьОбласть("Комментарий");
						ОбластьКомментарий.Параметры.Комментарий = ВыборкаПоШапке.Комментарий;
						ТабличныйДокумент.Вывести(ОбластьКомментарий);
					Исключение
					КонецПопытки;
				...
				//Исключение
				//КонецПопытки;
				//-- #Б-1426 смарт: Фролов 25.11.2024
				//-- #Б-5406 смарт: Фролов 14.05.2026
				#КонецВставки
